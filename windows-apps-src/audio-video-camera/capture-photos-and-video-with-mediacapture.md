...
若要帮助正确确定预览视频的方向，请创建成员变量来跟踪相机是否在外部以及应用当前是否正在生成预览流镜像。 当你认为视频源正在捕获用户时，你的应用应该生成预览流镜像，因为这是更自然的用户体验。
...
**GetCameraOrientation** 帮助程序方法以当前设备方向为起点，然后根据该设备的本机方向和设备上相机的位置进行旋转。 如果相机安装在设备的前面板上（由本示例中的 **\_mirroringPreview** 变量指示），则应反转该相机方向。
...
当关闭你的应用或者用户离开应用的媒体捕获页面时，请注销该硬件按钮处理程序。
