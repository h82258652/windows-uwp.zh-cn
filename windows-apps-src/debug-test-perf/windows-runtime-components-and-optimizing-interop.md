...
**请注意**  中实现大部分与 Windows 10 UWP 组件提供C++因此在您将它们从跨互操作性边界C#或 Visual Basic。 如往常一样，在更改代码前，请务必监测你的应用，确认使用 UWP 组件是否影响你的应用性能。
