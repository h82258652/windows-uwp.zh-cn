...
在**游戏的事件数据**页上，你可以查看实时图该流中的当前记录在 Xbox One 上的事件跟踪的 Windows (ETW) 游戏事件数量。 如果游戏事件记录在系统上，你还可以查看详细信息 （事件名称、 事件发生和在游戏作品） 描述数据图下面数据表中的每个事件。 表仅记录事件是否可用。
