...
Microsoft Visual Studio 允许在各种 Windows 10 设备上部署和调试通用 Windows 平台（UWP）应用。 Visual Studio 将处理在目标设备上生成和注册应用的过程。
...
- “仿真器”目标会使用名称中指定的配置将应用启动并部署到仿真器。 模拟器仅适用于运行 Windows 8.1 或更高版本的启用 Hyper-v 的计算机。
...
从 Visual Studio 2015 Update 3 中，我们添加了选项，使开发人员能够为其 UWP 应用指定布局路径。 当你生成应用时，这将确定程序包布局将复制到磁盘上的位置。 默认情况下，此属性相对于项目的根目录设置。 如果未修改此属性，行为将与以前版本的 Visual Studio 保持相同。
...
若要返回此对话框，你可以打开项目属性并返回到 "**调试**" 选项卡。在该文件中，选择 "在**远程计算机**上**查找**"：
...
为C++或 JavaScript UWP 应用指定远程计算机目标：
...
要将其更改为 "**无身份验证**"，请在**远程调试器**中，转到 "**工具**" -&gt;**选项**"，然后将其设置为"**无身份验证**"。 在设置远程调试器后，你还必须确保已将主机设备设置为[开发人员模式](https://docs.microsoft.com/windows/uwp/get-started/enable-your-device-for-development)。 在这之后，你可以从部署计算机进行部署。
...
从 Visual Studio 2015 Update 3 版本和 Windows 10 周年更新中，对于某些 Windows 10 设备，有新的高级远程部署选项。 可以在项目属性的“调试”菜单上找到高级远程部署选项。
...
- 使用随 Windows 10 Tools 1.4.1 或更高版本（包括 Windows 10 周年更新 SDK）一起安装的 visual Studio 2015 Update 3 或更高版本的 visual Studio 版本，我们建议你使用最新版本的 Visual Studio 进行更新，以确保你获取所有最新的开发和安全功能。
...
在远程设备上，布局会复制到以下默认位置： `\\MY-DEVKIT\DevelopmentFiles\PACKAGE-REGISTRATION-PATH`
...
在远程设备上，根据设备系列将布局注册到以下默认位置： `Xbox: \\MY-DEVKIT\DevelopmentFiles\XrfsFiles`-这是一个指向**包注册路径**的链接，而 PC 不使用链接符号，而是直接注册**包注册路径**

## <a name="debugging-options"></a>调试选项

在 Windows 10 上，UWP 应用的启动性能通过使用一[种名为](https://docs.microsoft.com/windows/uwp/launch-resume/handle-app-prelaunch)预启动的技术主动启动并挂起应用来改进。 许多应用不需要执行任何特殊操作就即在此模式下工作，但某些应用可能需要调整它们的行为。 若要帮助调试这些代码路径中的任何问题，你可以在 Visual Studio 中使用预启动模式开始调试该应用。

从 Visual Studio 项目（"**调试**" -&gt;**其他调试目标** -&gt;**调试通用 Windows 应用**预启动）和已安装在计算机上的应用程序（通过选中 "**使用预启动激活应用程序**" 复选框），都支持调试 **（调试 -** &gt;**其他调试目标** -&gt;**调试安装的应用程序包**）。 有关详细信息，请参阅[调试 UWP 预启动](https://blogs.msdn.com/b/visualstudioalm/archive/2015/11/30/debug-uwp-prelaunch-with-vs2015.aspx)。

可以在启动项目的“调试”属性页上设置以下部署选项。

- **允许本地网络环回**

  出于安全原因，以标准方式安装的 UWP 应用不允许对安装了它的设备进行网络调用。 默认情况下，Visual Studio 部署针对已部署应用从此规则创建豁免。 此豁免允许你在单台计算机上测试通信过程。 在将应用提交到 Microsoft Store 之前，应在没有例外的情况下测试应用。

  若要从应用中删除网络环回豁免：

  - 在 " C#  **调试**Visual Basic" 属性页上，清除 "**允许本地网络环回**" 复选框。
  - 在 JavaScript 和 C++“调试”属性页上，将“允许本地网络环回”值设置为“否”。

- **不启动，但在启动/启动应用程序时调试我的代码**
