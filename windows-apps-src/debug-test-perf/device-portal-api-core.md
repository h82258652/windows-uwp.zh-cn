...
为清楚起见，若要将接口设置为 DHCP，只需在网络上序列化 `AdapterName`：
...
| 钥   | （**如果 *op* == connect 并且网络需要身份验证，则为必需项**）共享的密钥。 |
...
| file   | （**必需**）要从报告下载的文件名称。 这应采用 base64 编码。 |
...
-  无。  **注意：** 这是一个长时间运行的操作。  当 ETL 完成写入到磁盘时，它将返回。
