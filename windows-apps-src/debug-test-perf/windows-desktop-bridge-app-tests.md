...
**测试详细信息**  
此测试检查应用中的所有 UWP 组件：
* 验证该应用包中的每个托管二进制文件均不依赖于通过检查二进制文件的导入地址表不支持的 UWP 应用开发的 Win32 API。
...
> [!NOTE]
> 内置于该调试配置中的 c + + 应用将无法通过此测试，即使配置仅适用于 UWP 应用使用 Windows SDK 中的 Api。 有关详细信息，请参阅[在 UWP 应用中的 Windows Api 的替代项](https://msdn.microsoft.com/library/windows/apps/hh464945.aspx)。
