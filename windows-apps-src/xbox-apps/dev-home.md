...
在较早的恢复选择右侧的主页屏幕上的开发人员主页磁贴中**特别推荐内容**或在 Xbox One 管理器中查看应用程序列表和启动**开发人员主页**。   
...
开发人员主页的用户界面的标头包含以下重要"一览"信息开发主机：   
...
在开发人员主页中，如网络和多人游戏，多个现有和即将推出的工具旨在用于时贴靠到一侧正在运行你的作品，以便进行测试时，你可以有轻松访问工具。   
...
开发人员主页已设计为可自定义，并且具有个性化。 你可以将应用配置为适合你的工作流，然后保存的用作工作区。 此工作区可导出和导入，允许你将布局复制到其他主机作为需要。 在主菜单下**工作区**中找到这些选项。 导出的文件将位于系统暂存驱动器上`Dev Home\Workspaces`目录。   
 
<a id="ID4EVF"></a>

   

### <a name="resizing-and-reordering-tools"></a>调整大小和对其重新排序工具  
   
  
若要更改的大小或位置的一种工具，在游戏时使用上下文菜单按钮 （控制器上的视图按钮） 具有焦点。 从上下文菜单中选择**移动**或**调整大小**。   
...
如果包中包含多个应用程序，开发人员主页将允许你设置的默认应用程序启动。 突出显示启动程序中的包，然后按**A**按钮以打开可用的应用程序的列表。 突出显示你希望设为默认值并按**视图**按钮，然后从上下文菜单选择**设为默认值**。   
...
然后，你可以为你想要注册的游戏的 appxmanifest.xml 文件到输入网络路径。 开发人员主页将尝试注册任何现有凭据用于该共享，游戏，并且当需要将新的网络凭据提示。 如果你需要访问 （例如用于访问符号链接在单独的服务器上的资源） 的更多的共享你将需要添加它们通过下面的选项。   
   
  
你可以管理这些存储的凭据 （且添加其他域） 通过主菜单的**管理网络凭据**选项在主机上。   
 ![](images/dev_home_9.png)   
  
可以查看主机上当前的凭据、 选择凭据的路径，然后单击**按钮**来编辑凭据和删除凭据，请选择删除链接，然后单击**的**按钮。   
...
&nbsp;&nbsp;开发人员主页的**媒体捕获**页面捕获视频的当前在控制台运行的标题。 
