...
你的应用程序可以使用模板缓冲区将 2D 或 3D 图像合成到 3D 场景中。 模板缓冲区中的掩码用于遮挡渲染目标图面的区域。 然后，可以将已存储 2D 信息（如文本或位图）写入遮挡区域。 或者，你的应用程序也可以将额外的 3D 基元渲染到渲染目标图面的模具遮罩区域。 它甚至可以渲染整个场景。
...
例如，在道路上应用轮胎压痕和黄线时，标记应直接在路面上显示。 但是，标记和道路的 z 值完全相同。 因此，深度缓冲区可能无法明确分离两者。 后部基元中的部分像素可能会渲染到前部基元上，反之亦然。 最后生成的图像每一帧之间似乎闪烁微光。 此效果称为 *z-fighting* 或 *flimmering*。

若要解决此问题，可使用模具遮罩后部基元上显示贴纸的部分。 关闭 z 缓冲，并将前部基元的图像渲染至渲染目标图面的已遮罩区域。
...
阴影卷用于在模板缓冲区中绘制阴影。 应用程序通过遮挡几何图形计算阴影卷大小，方法是计算剪影边缘并将它们从光线中挤压到一组 3D 卷中。 然后，这些卷将两次渲染到模板缓冲区中。

第一次渲染绘制前向多边形，并增加模板缓冲区值。 第二次渲染绘制阴影卷的后向多边形，并减少模板缓冲区值。 通常，所有增加和减少值会相互抵消。 但是，由于阴影卷已被渲染，使用常规几何图形渲染的场景会导致部分像素无法通过 z 缓冲测试。 留在模板缓冲区的值与阴影中的像素相对应。 使用这些保留的模板缓冲区内容作为掩码，以在场景中 alpha 混合大面积、全环绕的黑色间隙。 将模板缓冲区作为掩码，结果是将位于阴影处的像素变暗。

这意味着每个光源的阴影几何图形将绘制两次，因此为 GPU 的顶点吞吐量带来了压力。 双面模板功能旨在缓解此问题。 这种方法使用两组模具状态（如下文所列），一组用于前向三角形，另一组用于后向三角形。 这样，每个阴影卷每个光源只需绘制一次。
