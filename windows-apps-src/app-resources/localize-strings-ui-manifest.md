...
硬编码字符串文本可以出现在强制性代码或 XAML 标记中，例如作为 **TextBlock** 的**文本**属性。 它们还可以显示在你的应用包清单（`Package.appxmanifest` 文件）中，例如，作为 Visual Studio 清单设计器的应用程序选项卡上的显示名称的值。 将这些字符串移到资源文件 (.resw) 中，并参考资源标识符替换应用和清单中的硬编码字符串文本。
...
3.  打开 `Resources.resw` 并添加这些字符串资源。
    ...
    在此示例中，我们有一个名称为“Farewell”的简单字符串资源标识符条目，因此我们不能*同时*具有基于该相同标识符的属性标识符。 因此，添加“Farewell.Text”将导致在生成 `Resources.resw` 时出现重复条目错误。

## <a name="refer-to-a-string-resource-identifier-from-xaml-markup"></a>引用 XAML 标记中的字符串资源标识符

你使用 [x:Uid 指令](../xaml-platform/x-uid-directive.md)将你的标记中的控件或其他元素与字符串资源标识符关联。
...
**注意**对于[附加属性](../xaml-platform/attached-properties-overview.md)，你在 .resw 文件的“名称”列中需要特殊句法。 例如，要设置“Greeting”标识符的 [AutomationProperties.Name](/uwp/api/windows.ui.xaml.automation.automationproperties?branch=live#Windows_UI_Xaml_Automation_AutomationProperties_NameProperty) 附加属性的值，你将在“名称”列中输入该内容。
...
你可以使用来自类库（通用 Windows）或 [Windows 运行时库（通用 Windows）](../winrt-components/index.md)项目内的相同代码。 在运行时，加载托管库的应用的资源。 我们建议库从托管库的应用加载资源，因为应用的本地化程度可能更高。 如果库确实需要提供资源，该库应向其托管应用提供将这些资源替换为输入的选项。
...
1. 打开应用包清单（`Package.appxmanifest` 文件），默认情况下，应用的显示名称表示为字符串参数。
...
为默认显示语言测试应用。 之后，你可以在**设置** > **时间和语言** > **区域和语言**（或**语言**）中更改显示语言并重新测试应用。 在你的 UI 和 shell 中查看字符串（例如，你的标题栏，即你的显示语言，以及磁贴上的短名称）。
...
如果你要将“AppDisplayName”资源从 `Resources.resw` 移动到 `ManifestResources.resw`，在你的应用包清单中，你要将 `ms-resource:AppDisplayName` 更改到 `ms-resource:/ManifestResources/AppDisplayName`。
...
默认 [**ResourceContext**](/uwp/api/windows.applicationmodel.resources.core.resourcecontext?branch=live)（从 [**ResourceContext.GetForCurrentView**](/uwp/api/windows.applicationmodel.resources.core.resourcecontext?branch=live#Windows_ApplicationModel_Resources_Core_ResourceContext_GetForCurrentView) 获取）包含每个限定符名称的限定符值，表示默认运行时上下文（换言之，即当前用户和计算机的设置）。 根据该运行时上下文中的限定符值匹配资源文件 (.resw) - 基于其名称中的限定符。
...
某些限定符有系统数据提供程序。 因此，你无需调用 **SetGlobalQualifierValue**，可以改为通过提供程序自己的 API 调整提供程序。 例如，此代码显示如何设置 [**PrimaryLanguageOverride**](/uwp/api/Windows.Globalization.ApplicationLanguages?branch=live#Windows_Globalization_ApplicationLanguages_PrimaryLanguageOverride)。
...
你运行的应用可以响应影响默认 **ResourceContext** 中的限定符值的系统设置中的更改。 其中任何系统设置在 [**ResourceContext.QualifierValues**](/uwp/api/windows.applicationmodel.resources.core.resourcecontext?branch=live#Windows_ApplicationModel_Resources_Core_ResourceContext_QualifierValues) 上调用 [**MapChanged**](/uwp/api/windows.foundation.collections.iobservablemap_k_v_?branch=live#Windows_Foundation_Collections_IObservableMap_2_MapChanged) 事件。

为了响应此事件，你可以从默认的 **ResourceContext** 重新加载字符串。
