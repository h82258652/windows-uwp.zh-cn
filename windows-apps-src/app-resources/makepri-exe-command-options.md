...
你可以运行`MakePri.exe help`或`MakePri.exe /?`以查看你可以使用 MakePri.exe 命令。 你还可以发出`MakePri.exe <command> /?`若要查看有关某个命令，并在极少数情况下的细节，即使`MakePri.exe <command> <option>`以查看有关选项的详细信息。
