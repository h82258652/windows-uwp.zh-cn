...
-   **漫游**跨设备通过漫游数据，可让用户挑选一任务从他们离开的位置并保留他们最 UX，无论他们正在使用的设备来创建连续的体验。 通过在漫游时保持设置和状态，更易于在任何位置（从厨房中的家庭电脑到其工作电脑再到个人平板电脑以及其他外形规格）使用你的应用。 了解有关[管理应用程序数据](../design/app-settings/store-and-retrieve-app-data.md)的详细信息，并参阅[漫游应用程序数据指南](https://msdn.microsoft.com/library/windows/apps/hh465094)。
-   **用户磁贴**使你的应用更个性化你的用户通过用户加载自己磁贴图像，或让用户将从你的应用的内容设置为整个 Windows 其个人磁贴。

**设备功能**可确保你的应用完全利用当今设备的功能。

-   **邻近感应笔势**让你的用户将设备连接与其他用户都以物理方式邻近，通过以物理方式一起触碰设备 （多人游戏）。 了解有关[邻近感应和点击](https://msdn.microsoft.com/library/windows/apps/hh465229)的详细信息。
...
-   **地理位置**使用地理位置信息从标准 web 数据或地理位置传感器来帮助你的用户获取、 查找，在地图上的其位置或获取有关附近的人脉、 活动和目的地的通知。 了解有关[地理位置](https://msdn.microsoft.com/library/windows/apps/hh465139)的详细信息。
...
-   **始终尝试使用内容。** 尽可能让的用户直接操纵内容在应用的画布上，而不是添加作用于内容的命令。 例如，在旅行应用中，让用户通过在画布的列表中拖放活动来重新安排行程，而不是通过选择活动并使用“上移”或“下移”命令按钮。
...
**确定如何设置每个视图中你的应用的布局。** Windows 支持横向和纵向方向，并支持将应用大小调整为任意宽度，从全屏到最小宽度。 你希望你的应用在任何大小、任何屏幕以及任何方向上都正常显示和运行。 这意味着你需要为不同大小和视图规划 UI 元素的布局。 执行此操作时，应用 UI 会不断更改以满足用户的需求和偏好。
...
在进行更深层次的应用开发之前，应该根据指南、用户印象以及要求验证你的设计或原型，以避免以后返工。 每个功能都有一组的 UX 指南来帮助你波兰语你的应用，以及一组必须满足 Microsoft Store 中发布你的应用的应用商店要求。 你可以使用 [Windows 应用认证工具包](https://developer.microsoft.com/windows/develop/app-certification-kit)测试对应用商店要求的技术合规性。 还可以使用 Microsoft Visual Studio 中的性能工具以确保你在所有情况下都为用户提供极佳体验。
