...
-   **漫游**来漫游数据，可让用户挑选一任务从他们离开的位置并保留他们最 UX，无论他们正在使用的设备创建跨设备的连续的体验。 通过在漫游时保持设置和状态，更易于在任何位置（从厨房中的家庭电脑到其工作电脑再到个人平板电脑以及其他外形规格）使用你的应用。 了解有关[管理应用程序数据](../design/app-settings/store-and-retrieve-app-data.md)的详细信息，并参阅[漫游应用程序数据指南](https://msdn.microsoft.com/library/windows/apps/hh465094)。
...
-   **地理位置**使用地理位置信息从 web 标准数据或地理位置传感器来帮助你解决，查找它们，在地图上的位置或获取有关附近的人脉、 活动和目的地的通知的用户。 了解有关[地理位置](https://msdn.microsoft.com/library/windows/apps/hh465139)的详细信息。
...
在进行更深层次的应用开发之前，应该根据指南、用户印象以及要求验证你的设计或原型，以避免以后返工。 每个功能都有一组的 UX 指南可帮助你优化你的应用，以及一组必须满足 Microsoft Store 中发布你的应用的应用商店要求。 你可以使用 [Windows 应用认证工具包](https://developer.microsoft.com/windows/develop/app-certification-kit)测试对应用商店要求的技术合规性。 还可以使用 Microsoft Visual Studio 中的性能工具以确保你在所有情况下都为用户提供极佳体验。
