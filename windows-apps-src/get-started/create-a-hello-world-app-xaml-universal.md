...
description: 使用 Extensible Application Markup Language (XAML) 与C#若要创建一个简单的 Hello world 应用面向 Windows 10 上的通用 Windows 平台 (UWP)。
ms.date: 03/06/2017
ms.topic: article
...
本教程讲解如何使用 XAML 和C#若要创建一个简单"Hello，world"应用程序的 Windows 10 上的 Windows 通用平台 (UWP)。 使用 Microsoft Visual Studio 中的单个项目，您可以构建在任何 Windows 10 设备运行的应用。
...
3.  从左侧的模板列表中，选择**已安装 > Visual C# > Windows 通用**若要查看的 UWP 项目模板列表。

    （如果未看到任何通用模板，可能是缺少用于创建 UWP 应用的组件。 可以通过在**新建项目**对话框中单击*打开 Visual Studio 安装程序*来重复安装流程并添加 UWP 支持。 请参阅[获取设置](get-set-up.md)。)
...
-   App.xaml.cs 是应用的入口点。
-   App.xaml.cs 还包含方法以处理[激活](../launch-resume/activate-an-app.md)并[挂起](../launch-resume/suspend-an-app.md)的应用程序。
...
-   包括依赖项、 资源和你的应用包含的文件的列表。

*徽标图像的一组*
-   Assets/Square150x150Logo.scale-200.png 和 Wide310x150Logo.scale 200.png 开始菜单中表示您的应用程序 （中等或范围大小）。
-   Assets/Square44x44Logo.png 表示在应用中的开始菜单、 任务栏和任务管理器的应用列表。
-   Assets/StoreLogo.png 表示 Microsoft Store 中的应用。
-   Assets/SplashScreen.scale-200.png 是应用启动时显示的初始屏幕。
-   Assets/LockScreenLogo.scale-200.png 可以用于表示在锁定屏幕上，应用程序，当系统处于锁定状态。

## <a name="step-2-adding-a-button"></a>步骤 2：添加按钮

### <a name="using-the-designer-view"></a>使用设计器视图

让我们向我们的页面中添加一个按钮。 在本教程中，您可以只需几个文件与前面所列：App.xaml、 MainPage.xaml 和 MainPage.xaml.cs。
...
## <a name="step-3-start-the-app"></a>步骤 3:启动应用程序
...
请确保方法签名现在包括**异步**关键字，或你尝试运行应用时，将收到错误。
