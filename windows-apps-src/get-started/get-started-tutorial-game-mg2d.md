...
description: 适用于 Microsoft Store、用 C# 和 MonoGame 编写的简单 UWP 游戏
ms.date: 03/06/2017
ms.topic: article
...
谈及游戏开发环境时，我们并不缺少选择。 从功能齐全的引擎（如 Unity）到全面、复杂的多媒体 API（如 DirectX），但这些都让人很难知道从何处入手。 MonoGame 是一组工具，其复杂度处于游戏引擎与更专业的 API（如 DirectX）之间。 它提供了易于使用的内容管道，以及用于创建在各种平台上运行的轻型游戏所需的所有功能。 最重要的是，MonoGame 应用是用纯 C# 编写，因此，你可以通过 Microsoft Store 或其他类似的分发平台快速分发。

## <a name="get-the-code"></a>获取代码
如果你不想跟着教程逐步学习，只想了解 MonoGame 的实际应用，[请单击此处以获得完成后的应用](https://github.com/Microsoft/Windows-appsample-get-started-mg2d)。

在 Visual Studio 2019 中打开项目，并按 **F5** 以运行示例。 首次进行此操作可能会需要一段时间，因为 Visual Studio 需要提取安装中缺少的所有 NuGet 包。

如果已完成此操作，则请跳至下一节“设置 MonoGame”，以查看代码的分步操作实例。

**注意：** 此示例中创建的游戏并不完整（或完全没有乐趣）。 其唯一目的在于展示用 MonoGame 进行 2D 游戏开发的所有核心概念。 可随意使用此代码并进行改进，或者在掌握基础知识之后从头开始。
...
6. 确保已将“x86”和“本地计算机”设为目标平台，然后按 **F5** 生成并运行空项目   。 如已遵循以上步骤，则你应该会在生成完项目之后看到一个空的蓝色窗口。

## <a name="method-overview"></a>方法概述
创建完项目之后，现在请在“解决方案资源管理器”中打开“Game1.cs”文件   。 这是大多数游戏逻辑的存储位置。 创建新的 MonoGame 项目时，将在此自动生成许多关键方法。 下面我们来快速了解这些内容：
...
## <a name="build-the-spriteclass"></a>生成 SpriteClass
...
现在，我们已经生成了 SpriteClass，接下来我们将用其创建两个新的游戏对象：第一个是玩家可通过箭头键和空格键控制的头像。 第二个是玩家必须避让的对象
...
### <a name="4-construct-spriteclasses"></a>4.生成 SpriteClasses
我们将在 **LoadContent** 方法中生成 SpriteClass 对象。 将此代码添加到已有对象中：
...
第二部分用于确定花椰菜的生长速度，这由当前的分数决定。 玩家每成功躲避 5 个花椰菜，其生长速度就会变得更快。
...
在处理键盘输入之前，我们需要一个方法，以便为我们创建的两个对象设置初始游戏状态。 除了在应用运行时就立即开始游戏外，我们希望用户通过按空格键来手动开始游戏。 添加以下代码，以设置动画对象的初始状态并重置分数：
...
接下来，我们需要添加一个新方法，以通过键盘处理用户输入。 将此方法添加到 **Game1.cs**：
...
使用以上代码，我们可以在场景后跟踪玩家的分数，但我们实际上并未告诉玩家分数。 此外，在应用启动时，我们还会提供一个相当不直观的介绍 - 玩家将会看到一个蓝色和绿色窗口，但是没法知道需要按空格键才能开始滚动。

若要修复这两个问题，我们将使用名为 **SpriteFont** 的新型 MonoGame 对象。

### <a name="1-create-spritefont-description-files"></a>1.创建 SpriteFont 描述文件
在“解决方案资源管理器”中找到“Content”文件夹   。 在此文件夹中，右键单击“Content.mgcb”文件并选择“打开方式”   。 从弹出菜单中，选择“MonoGame 管道”，然后按“确定”   。 在新窗口中，右键单击“内容”项并选择“添加”->“新建项”   。 选择“SpriteFont 说明”，将其命名为“分数”并按“确定”   。 然后，通过相同的步骤添加另一个名为“GameState”的 SpriteFont 描述。
...
如同先前一样，右键单击“内容”并选择“添加”->“现有项”为项目添加纹理  。 将此新项命名为“start-splash.png”。
...
以上代码使用我们创建的子画面描述（Arial 36 号）在屏幕右上角附近绘制玩家的当前分数。
...
尝试运行游戏。 你看到介绍初始屏幕了吗？ 花椰菜每繁殖一次，分数是否就递增了呢？

![介绍初始屏幕](images/monogame-tutorial-3.png)

## <a name="collision-detection"></a>碰撞检测
现在，我们已经让花椰菜围绕在你周围，并且花椰菜繁殖一次分数就会递增，但实际上没有办法输掉游戏。 我们需要使用一种方法来知道恐龙和花椰菜子画面是否碰撞，以宣布游戏结束。

### <a name="1-get-the-textures"></a>1.获取纹理
我们所需的最后一张图像是“游戏结束”的图像。 [单击此处以下载此图像](https://github.com/Microsoft/Windows-appsample-get-started-mg2d/blob/master/MonoGame2D/Content/game-over.png)。

与前面的绿色矩形忍者神猫和花椰菜图像一样，通过 **MonoGame 管道**将此图像添加到 **Content.mgcb**，然后将其命名为“game-over.png”。
...
### <a name="4-implement-game-over-logic"></a>4.实现“游戏结束”逻辑
...
有关更多详细信息，请参阅[发布 UWP 应用](https://developer.microsoft.com/en-us/store/publish-apps)。
