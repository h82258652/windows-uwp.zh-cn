---
title: "入门教程 - 用 MonoGame 2D 编写的 UWP 游戏"
description: "适用于 Windows 应用商店、用 C# 和 MonoGame 编写的简单 UWP 游戏"
author: mukin
ms.author: mukin
ms.date: 03/06/2017
ms.topic: article
...
translationtype: HT
---
# <a name="get-started-tutorial-a-uwp-game-in-monogame-2d"></a>入门教程：用 MonoGame 2D 编写的 UWP 游戏

## <a name="a-simple-2d-uwp-game-for-the-windows-store-written-in-c-and-monogame"></a>适用于 Windows 应用商店、用 C# 和 MonoGame 编写的简单 UWP 2D 游戏
...
+    Windows 10 和 Microsoft Visual Studio 2015。 [单击此处了解如何设置 Visual Studio](https://docs.microsoft.com/en-us/windows/uwp/get-started/get-set-up)。
...
谈及游戏开发环境时，我们并不缺少选择。 从功能齐全的引擎（如 Unity）到全面、复杂的多媒体 API（如 DirectX），但这些都让人很难知道从何处入手。 MonoGame 是一组工具，其复杂度处于游戏引擎与更专业的 API（如 DirectX）之间。 它提供了易于使用的内容管道，以及用于创建在各种平台上运行的轻型游戏所需的所有功能。 最重要的是，MonoGame 应用是用纯 C# 编写，因此，你可以通过 Windows 应用商店或其他类似的分发平台快速分发。

## <a name="get-the-code"></a>获取代码
如果你不想跟着教程逐步学习，只想了解 MonoGame 的实际应用，[请单击此处以获得完成后的应用](https://github.com/Microsoft/Windows-appsample-get-started-mg2d)。

在 Visual Studio 2015 中打开项目，并按 **F5** 以运行样本。 首次进行此操作可能会需要一段时间，因为 Visual Studio 需要提取安装中缺少的所有 NuGet 包。

如果已完成此操作，则请跳至下一节“设置 MonoGame”，以查看代码的分步操作实例。

**注意：**此示例中创建的游戏并不完整（或完全没有乐趣）。 其唯一目的在于展示用 MonoGame 进行 2D 游戏开发的所有核心概念。 你可随意使用此代码并进行改进，或者在掌握基础知识之后从头开始。
...
对于玩家的头像，我们将使用 Microsoft 独有的忍者神猫，它骑在它信赖的霸王龙身上。 [单击此处以下载此图像](https://github.com/muhsinking/VeggieJump/blob/master/MonoGame2D/Content/ninja-cat-dino.png)。
...
7. 处理键盘输入 - 接下来，我们需要添加一个新方法，以通过键盘处理用户输入。 将此方法添加到 **Game1.cs**：
...
## <a name="publish-to-the-windows-store"></a>发布到 Windows 应用商店
此游戏是作为一款 UWP 应用创建的，因此，我们可以将此项目发布到 Windows 应用商店。 此流程包含几个步骤。
...
有关更多详细信息，请参阅[发布 Windows 应用商店应用](https://developer.microsoft.com/en-us/store/publish-apps)。
