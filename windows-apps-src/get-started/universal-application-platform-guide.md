...
如果你正在构建的通用 Windows 平台 (UWP) 应用，你可以获取了大量的实际效果和退出"通用 Windows 平台 (UWP)"和"Windows 运行时 (WinRT)"的条款视为更多或更少同义词方便。 但它** 可能看起来将在背后的技术，并确定只区别是之间想法。 如果你不希望了解它，然后本节最后一个适合你。

Windows 运行时和 WinRT Api，Windows Api 的进化。 最初，Windows 设计通过平面，C 样式 Win32 Api。 这些已添加到 COM Api ([DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274)正在突出显示的示例)。 Windows 窗体、 WPF、.NET 和托管的语言将编写 Windows 应用和 API 技术自己风格的方式。 Windows 运行时，深层下, 一个阶段的 com。 在实际应用程序二进制接口 (ABI) 层，在 COM 中其根变得可见。 但 Windows 运行时已设计为可通过不同的编程语言的调用。 和可调用这些语言的每个非常自然的方式。 为此，访问 Windows 运行时是可通过称为语言投影。 没有 Windows 运行时语言投影到 C#、 Visual Basic 到、 到标准 c + +、 到 JavaScript 中，依次类推。 此外，一次打包相应地 （请参阅[桌面桥](/windows/uwp/porting/desktop-to-uwp-root)），你可以从生成的应用程序模型之一的应用调用 WinRT Api: Win32、.NET、 WinForms 和 WPF。

和当然，你可以从你的 UWP 应用中调用 WinRT Api。 UWP 是 Windows 运行时的基础上生成的应用程序模型。 从技术上讲，UWP 应用程序模型基于[CoreApplication](/uwp/api/windows.applicationmodel.core.coreapplication)，尽管该细节可能从您，具体取决于你选择的编程语言中隐藏。 本主题已所述，价值主张的角度来看，从 UWP 适用于编写单个二进制文件可以如果你选择，在发布到 Microsoft Store 和运行设备外形规格的任何一个。 你的 UWP 应用的设备范围取决于的 UWP Api 子集限制于调用，应用有条件地调用。

希望，本部分已成功地描述基础 Windows 运行时 Api 的机制和通用 Windows 平台的企业值的技术之间的区别。
