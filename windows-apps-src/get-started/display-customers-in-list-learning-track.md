...
如果数据源更改，双向绑定还将更新 UI。 若要实现此功能，必须在源上实现 [**INotifyPropertyChanged**](https://msdn.microsoft.com/library/system.componentmodel.inotifypropertychanged(d=robot).aspx)，并确保其属性设置器发起 **PropertyChanged** 事件。 常见做法是让它们调用 **OnPropertyChanged** 这样的帮助程序方法，如下所示：
