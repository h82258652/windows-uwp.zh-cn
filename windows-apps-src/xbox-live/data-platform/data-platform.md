...
将游戏数据写入 Xbox Live 数据平台让你的游戏可以作为一项服务运行。 此外，Xbox Live 数据平台还使用统计信息、排行榜和成就来推动用户参与你的游戏，并在主机 shell 和 Xbox 应用中呈现特别推荐的统计信息。
...
成就不是由统计信息支持，你的游戏决定何时解锁成就。 例如，你的游戏可能有成就：在一分钟内完成比赛。 你的游戏将跟踪解锁此成就所需要的参数。 在此示例中，将由游戏决定是否测量比赛使用的时间，然后对成就给予奖励。 通常会在成就完成时奖励玩家分数。 每个成就所奖励的玩家分数多少由你决定。
