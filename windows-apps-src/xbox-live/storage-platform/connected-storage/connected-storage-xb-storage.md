...
|---------|---------|
| /?                           |  显示 xbstorage.exe 帮助 |
| /X *:address* [*+accesskey*]  | 指定目标主机的主机名称或地址（显示为主机上的**工具 IP**），但不会更改默认的主机。 如果不使用此选项，则使用默认主机。*Accesskey* 是字符串，可使用它来仅限知道访问密钥的人访问主机。 通过使用命令**xbconfig**设置访问键**accesskey = *** 您密钥*;然后，重启主机以使访问密钥生效。 若要访问配置了访问密钥的主机，必须在主机的 IP 地址或主机名后使用加号 (+)，再跟访问密钥。
> [!NOTE]
