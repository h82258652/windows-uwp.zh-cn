...
| 注意                                                                                |
|--------------------------------------------------------------------------------------------------|
| 此文档最初为 Xbox One 开发人员编写。  Windows 上的 UWP 可以忽略某些特定于 Xbox One 的内容（例如本地和标题存储）。  此文档中的概念内容和 API 仍然相关。  如有任何问题，请联系你的 DAM。 |
...
最后，你的应用负责管理 Xbox One 上所有保存的数据；和 Xbox 360 上一样，没有集中保存管理 UI。 例如，当应用耗尽专用存储空间时，应用必须允许用户删除应用内保存的数据才能释放存储空间。
...
## <a name="when-to-acquire-a-connected-storage-space"></a>何时获取连接存储空间

获取用户的连接存储空间的执行时间可能有所不同。 应用应在主执行期间执行此操作，而不是在响应用户开始注销或在响应接收来自系统的暂停通知时执行。

一般情况下，应用应在用户登录并指示要运行的需求时获取连接存储空间，除非游戏处于确定不需要保存功能的模式下。 你还应该考虑将连接存储空间的获取与数据加载的长序列保持一致，以便操作可以并行执行。

应用获取连接存储空间后，它应该保留此值以供将来保存。 将连接存储空间保留一段时间不会对性能和稳定性产生负面影响。 如果系统处于联机状态，获取连接存储空间会导致对云进行同步检查，因此，在慢速或不稳定的网络条件期间释放和重新获取用户的连接存储空间可能会导致用户看到同步 UI，直到系统超时。

无需以显式方式释放云存储空间以引起云同步。 当 **SubmitUpdatesAsync** 调用中指定的完成处理程序返回 **AsyncStatus::Completed** 后，系统会处理与云之间的同步，无论应用是否释放 ConnectedStorageSpace 对象。
...
如果你的游戏设计使用定期、自动或由用户启动的保存，调用连接存储的次数可以比收到暂停通知时多；这样做是降低因断电或故障导致的风险的好方法。

用户注销时，此用户的 User 对象保持有效，此时，应用可能会使用连接存储执行最终的保存操作。 有关此对象的更多信息，请参阅 *User 类*。
...
-   通过为每个保存槽位使用 1 个 blob，将 5 个槽位全部存储在一个固定名称的容器中。 这表示 5 个槽位全部完全同步且可用，或者，如果同步在中途失败且容器不完整，则槽位全都不可用。 如果用户在两个不同的控制台上脱机运行应用，并将进度保存在第一个控制台的 1 号槽位和第二个控制台的 2 号槽位中，则用户必须在将两个控制台连接到 Xbox Live 时选择要保留的数据；容器的合并逻辑将产生冲突。
...
无论使用哪种方法，应用应该为用户提供 UI，以删除槽位中的个别保存。
...
观察下文介绍的做法有助于在用户在控制台中漫游时，实现应用的保存功能，对用户而言，此功能可响应、稳定且易于理解。

### <a name="warning"></a>警告

 **请勿在容器中存储非独立数据**请勿在多个容器中存储具有依赖项的数据。 容器可能会因为同步不完整、断电或其他错误情况而分隔。 存储在单个容器中的数据必须保持自足和本身一致。

### <a name="good-practices"></a>较好做法

 **请勿禁止用户关闭控制台或导航离开**保存时，游戏不应禁止用户关闭控制台或导航离开应用。 在 Xbox 360 上，如果用户在保存游戏时关闭系统，则不会保存用户数据。 在 Xbox One 上，你的游戏会收到暂停事件，并且有 1 秒钟的时间使用**连接存储 API** 来保存状态。 在完全关闭或进入低功耗状态之前，系统会确保数据已正确提交到硬盘。 如果用户弹出游戏的磁盘并运行其他磁盘，会发生同样的暂停过程。

 **当应用尝试加载数据时告知用户** 当应用等待 **GetForUserAsync** 调用完成时，请在视觉上指示用户，应用正在尝试加载数据。 当应用全屏运行时，虽然系统会在同步过程中提供 UI，但是，如果用户导航到“主页”屏幕，则会隐藏 UI。 如果系统正在为请求的连接存储空间同步大量数据，可能会发生这种情况。

 **保留连接存储空间**保留 **ConnectedStorageSpace** 对象，而不是在每次发生读取或写入事件时尝试获取此对象。 对于由长时间保留 **ConnectedStorageSpace** 导致的性能或稳定性没有负面影响。

 **将数据大小保持为小** 将保存的数据大小保持为小。 当控制台处于联机状态时，连接存储中的所有用户数据都会上传到云中。 优化数据格式，确保将延迟和带宽使用量降到最低。

 **验证用户是否不介意未保存** 检查从 **GetForUserAsync** 和 **SubmitUpdatesAsync** 返回的 OutOfLocalStorage 错误，并询问用户，以了解其是否确实要在不保存的情况下运行。 如果用户指示想要保存游戏，请重试此操作。

 **检查用户的配额并提示清除空间** 检查由 **SubmitUpdatesAsync** 返回的 **QuotaExceeded** 错误。 如果应用收到此消息，请通知用户，在释放部分空间之前，无法保存任意其他数据；显示允许用户执行此操作的 UI。 每个用户都能按应用获取 256 MB 的数据，且每个应用都能按计算机获取 64 MB 的本地存储（对于控制台来说）。

 **保存菜单状态以稍后还原** 除了保存核心游戏数据外，保存菜单状态和其他应用设置。 如果用户运行其他应用，然后返回到你的应用，请将其还原为上下文适当的菜单状态。

 **响应已登录用户的更改** 用户可以在应用暂停时注销。 当应用继续时，它应该确定已登录的用户的设置是否发生更改。 发生这种情况时，请考虑导航到应用中的适当位置（例如菜单）。

### <a name="important-practices"></a>重要做法

 **提供 UI 以管理保存的数据** 与 Xbox 360 不同，Xbox One 没有系统的托管 UI 来管理保存的游戏。 你的应用应提供允许用户管理应用中保存的数据的 UI。 对于拥有自动保存系统的应用，应用应提供重置保存的数据的选项，使用户可以重置为默认运行状态。

 **确保用户始终可以访问 UI 来管理保存的游戏** 确保你的应用可以始终访问保存的游戏的管理 UI，即使出现有问题的保存数据。 如果用户的保存数据由于应用 bug 或数据损坏而无法读取，则应用应该允许用户恢复到尚未崩溃的状态，或阻止应用运行。
