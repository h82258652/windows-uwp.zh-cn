...
---
# <a name="session-template-constants"></a>Session template constants
...
userAuthorizationStyle | Indicates if the session supports calls from platforms without strong title identity. This capability can't be set on large sessions.</br></br>将 `userAuthorizationStyle` 功能设置为 `true` 会默认将会话的 `readRestriction` 和 `joinRestriction` 设置为 `local` 而不是 `none`。 这意味着游戏必须使用搜索句柄或转移手柄来加入游戏会话。| true、false | false
