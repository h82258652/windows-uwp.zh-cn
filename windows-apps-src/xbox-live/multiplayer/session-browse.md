...
**示例：** John 可以根据兴趣玩 hero 战斗竞技场样式多玩家游戏，但想要玩游戏，其中所有玩家都选择其 hero 随机。 他可以检索打开的游戏会话列表，并查找描述中包括“随机角色”的游戏会话，或者，如果游戏 UI 允许的话，他可以选择“随机角色”游戏模式，并仅检索标记为“RandomHero”游戏的会话。
...
>[!NOTE]
> 搜索句柄在显示要呈现给用户的会话列表时使用。 使用搜索句柄，背景匹配无效，并改为考虑使用[SmartMatch](multiplayer-manager/play-multiplayer-with-matchmaking.md)
...
`tags` 标记是用户可以使用进行分类游戏会话，类似于井号标签的字符串描述符。 标记必须以字母开头，不能包含空格，且必须少于 100 个字符。
示例标记："ProRankOnly"，"norocketlaunchers"、"cityMaps"。

`strings` 字符串是文本变量和字符串名称必须以字母开头，不能包含空格，并且必须是少于 100 个字符。

示例字符串元数据："武器"="刀具 + pistols + rifles"、"映射名称"="UrbanCityAssault"，"说明"="有趣的休闲游戏，欢迎使用新建人员"。

`numbers` -数字是数值变量和数字的名称必须以字母开头，不能包含空格，并且必须是少于 100 个字符。 Xbox Live API 可将编号检索为浮点类型。

示例数字元数据："MinLevel" = 25, "MaxRank" = 10.

>**注意：** 字母大小写的标记和字符串值保留在服务中，但在查询标记时，必须使用 tolower （） 函数。 这意味着，标记和字符串值当前都被视为小写，即使它们包含大写字符。
...
### <a name="example-create-a-search-handle-with-metadata"></a>示例：使用元数据创建的搜索句柄
...
### <a name="refreshing-search-results"></a>刷新搜索结果
 ...
 若要减少服务呼叫量，搜索句柄包括可用于存储和查询快速变化的会话属性的自定义会话属性。 不应在搜索属性中存储此类属性。
