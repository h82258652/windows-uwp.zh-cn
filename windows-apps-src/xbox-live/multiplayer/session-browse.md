...
会话浏览不同于传统匹配方案。对于前者，玩家自行选择要加入的游戏会话。而在匹配方案中，玩家通常要单击“查找游戏”按钮，尝试自动将玩家放置在相应的游戏会话中。
...
虽然很多游戏会话类型，例如 SmartMatch 或仅限好友的游戏，不需要所有者，但是，每个会话浏览会话都必须至少有一个所有者。 只有标记为会话所有者的成员，才能为该会话创建搜索句柄。

此外，只有所有者才能从会话中删除其他成员，或更改其他成员的所有权状态。

如果会话所有者已阻止 Xbox Live 成员，则该成员不能加入会话。

如果所有的所有者都离开会话，则服务将根据为该会话定义的 `ownershipPolicy.migration` 策略对该会话执行相应的操作。 如果策略为“oldest”，则加入会话时间最长的玩家将被设置为新的所有者。 如果策略为“endsession”，则服务将结束会话，并从该会话中删除其余所有的玩家。
