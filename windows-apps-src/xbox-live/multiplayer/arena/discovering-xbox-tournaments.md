...
ms.openlocfilehash: 76bb4b1a65ae182d7e73a70c49baf6ec9f7460c9
...
玩家可通过 Xbox 操作面板中的多个选项或通过电脑上的 Xbox 应用来了解有关参与或观看锦标赛的更多信息。 还有越来越多的机会可从你的游戏中发现锦标赛。  
...
人们由于对游戏感兴趣而观看锦标赛。 Xbox 游戏有一个独特的机会可在关键时刻吸引玩家，即在玩家寻找扩展游戏体验的新途径时。
...
Arena API 为你的游戏提供了足够数据，以用于在游戏内创建浏览体验。 如果要生成浏览功能，请在“多人游戏”部分添加**锦标赛**入口点。
...
你的游戏可在发出请求后通过其他属性进行筛选，但不能在查询参数中指定这些属性。 这会使得通过其他属性筛选对于此类型的 UI 而言不可靠。 Arena API 一次可在游戏中检索指定数量的结果，例如，游戏可将 **maxItems** 值指定为 10 次锦标赛。 这样有助于改善你的游戏管理性能，并尽量降低大型列表让用户难以应对的风险。 但是，你的游戏提供的任何自定义筛选器都只会应用于此次请求的 10 个项目。 这可能导致每个 API 调用后的筛选结果数不一致。 例如，返回的第一组 10 个锦标赛中可能有 2 个与自定义筛选器（例如“比赛”）相关。 但是下 10 个锦标赛可能显示 9 个此类项目，等等。 游戏完全填充每个自定义筛选器的唯一可靠的方法是，请求每个活动的锦标赛然后对其全部进行筛选，我们并不推荐这种做法。
