...
7.  现在，如中所述为 MPSD 编写会话**如何：更新多人游戏会话**。
...
4.  在本地完成更改后，将其写入到 MPSD 中所述**如何：更新多人游戏会话**。
...
2.  在作品激活时，开始会话并订阅会话更改通知。 请参阅**如何：MPSD 会话更改通知订阅**。
3.  将用户以活动状态加入会话。 请参阅**如何：加入从标题激活 MPSD 会话**。
4.  将大厅会话设置为活动会话，通过个人资料 UI 显示。 请参阅**如何：设置用户的当前活动**。
...
1.  创建会话对象，并根据需要修改属性。
2.  将用户以活动状态加入会话。 请参阅**如何：加入从标题激活 MPSD 会话**。
3.  确定用户是否已被指定为会话仲裁程序。
...
6.  对于远程玩家，应实现游戏激活代码。 请参阅**如何：管理标题激活**。
...
当用户选择使用 Xbox shell UI 加入好友的活动或接受邀请时，使用指示用户想要加入的会话的参数激活游戏。 作品必须处理此激活并将用户添加到相应的会话中。
...
若要设置用户的当前活动通过标题激活，请参阅**如何：加入从标题激活 MPSD 会话**。
...
2.  如中所述更新 MPSD 中的游戏会话**如何：更新多人游戏会话**。
...
3.  确定票证会话是否已满。
4.  如果会话已满，继续游戏。
5.  如果会话尚不完整，创建匹配票证，如中所述**如何：创建匹配票证**。 请务必创建将 *preserveSession* 参数设置为“始终”的票证。
