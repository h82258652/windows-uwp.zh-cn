...
-   我正在开发新游戏。 我应该使用哪些多人游戏 API 元素？
-   我如何从服务访问新的多人游戏 API？
-   我的游戏可以为多个会话订阅更改么？
-   如果用户丢失了网络连接或关闭了主机，他/她会被立即删除么？
-   如何查找要使用的 SCID、会话模板和沙盒？
-   是否有一个请求正文的示例，我可以将自己游戏的请求正文与其相比？
...
### <a name="i-am-developing-a-new-title-which-multiplayer-api-elements-should-i-use"></a>我正在开发新游戏。 我应该使用哪些多人游戏 API 元素？
...
### <a name="can-my-title-subscribe-to-changes-for-more-than-one-session"></a>我的游戏可以为多个会话订阅更改么？

可以，游戏可以订阅接收每个连接最多 10 个会话的更改。
...
如果你未进行游戏的初始注册，在创建此信息时，你将没有访问此信息的权限。 请联系你的开发者帐户管理员，他可以为你获取此数据。


### <a name="is-there-an-example-of-a-request-body-that-i-can-compare-to-the-one-for-my-title"></a>是否有一个请求正文的示例，我可以将自己游戏的请求正文与其相比？
...
如果你的游戏为响应包含句柄 ID 的加入协议激活通过句柄访问 MPSD 会话，协议激活中的句柄 ID 可能由于以下原因之一过期：

-   游戏从中发起加入的 Xbox shell 中的 UI 视图可能已过期。 某些 UI 视图（例如，用户的个人资料卡）在打开时不会自动刷新加入句柄。 若要避免收到 HTTP/404 代码，游戏应先关闭然后重新打开视图以在加入之前刷新数据。
-   游戏正在加入的用户可能已在游戏从 Xbox shell UI 选择了加入操作后切换了活动会话。 HTTP/404 代码的这个原因应该很少见。

在这两种情况下，游戏代码都应该向加入用户显示一条错误消息，指出加入失败。
...
如果你使用的是 XSAPI，你还可以获取响应正文，如 [Xbox Live 服务 API 疑难解答](../../using-xbox-live/troubleshooting/troubleshooting-the-xbox-live-services-api.md)中所述。 或者，你的代码可以使用 **XboxLiveContextSettings.EnableServiceCallRoutedEvents** 属性将输出发送到你的游戏 UI。


### <a name="what-can-or-should-i-change-in-the-session-templates-for-my-title"></a>可以或应当在会话模板中为我的游戏做哪些修改？
...
你的游戏必须避免轮询 MPSD。 如果游戏必须发现对 MPSD 会话的更改，它应该订阅会话更改事件。 有关详细信息，请参阅[操作方法：订阅 MPSD 会话更改通知](multiplayer-how-tos.md)。
...
是的。 如果你的游戏使用多个会话来跟踪游戏会话内玩家的子组，这尤其有用。 加入用户可以加入表示他/她的组的会话，然后需要加入更大的游戏会话。
...
当你的游戏通过加入将用户添加到会话中时，游戏始终会将 **MultiplayerSessionMember.InitializeRequested** 属性设置为 true。 这会告知 MPSD 在将游戏移出初始化阶段之前等待其余会话成员。 否则，等待用户加入的时间段将非常短，用户可能错过 toast 和会话更改通知。
...
请在 <https://forums.xboxlive.com/spaces/22/index.html> 提交反馈。
版本：2.0.90731.0
