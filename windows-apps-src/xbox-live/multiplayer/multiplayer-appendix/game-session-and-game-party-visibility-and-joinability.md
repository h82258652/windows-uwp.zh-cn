...
> **注意：** 为专用设置可见性上可能需要重试**join （)** 调用由受邀的播放机。 如果邀请通过平台 UI 发送，它可以由另一个玩家接收（在该玩家被保留在会话中之前）。 此争用情况可能会导致受邀请玩家的 **join()** 调用失败，因为私人或可见会话需要为加入玩家保留。
>
...
访问控制的第二个方面是可加入性。 它可以在会话生存期内动态设置，并确定哪类用户可以加入会话。 会话 joinability 的值有：
...
**注意：** 游戏会话 joinability 不反映在参与方应用程序 UI 中。 即使可加入性受限，“派对”应用邀请 UI 和 **ShowSendInvitesAsync** 也会允许发送游戏会话邀请。
...
**注意：** 参与方 joinability 不会影响会话 joinability 但参与方 joinability 反映在参与方应用程序 UI。 玩家可以在游戏之外的“派对”应用中手动更改派对可加入性。
...
**注意：** 这将需要受邀的播放机以启动一个标题，并最终决策之前连接到的主机名或仲裁器。 如果会话已满或邀请被拒绝，可以接受向用户显示一条错误消息。

> **注意：** 若要建立与主机或仲裁器的连接，安全设备的地址是必需的。 会话中的 **HostDeviceToken** 应用于指示哪个会话成员是会话当前的主机或仲裁程序，以及受邀请的玩家应该连接到哪个安全设备地址。
...
-   打开游戏 — 对于打开的游戏，游戏的参与方 joinability 应保留为默认值：可加入的朋友。 这允许好友加入游戏派对（通过扩展游戏会话），无需邀请。

-   私人游戏 - 对于私人或封闭游戏，游戏派对可加入性应设置为“仅邀请”。 这将限制没有邀请的其他玩家加入派对（通过扩展游戏会话）。

**注意**：播放机可以手动更改参与方应用程序通过游戏会话的 joinability。
...
仲裁器或主机还应该维护指示哪些成员的索引，已超时的自定义属性 (例如， **TimedOutPlayers** :“3, 5, 9”). 这让新玩家可以正确识别当前的会话成员。
...
**注意：** 如果标题暂停和/或终止，将不再收到通知，播放机将自动删除从游戏的当事方。
