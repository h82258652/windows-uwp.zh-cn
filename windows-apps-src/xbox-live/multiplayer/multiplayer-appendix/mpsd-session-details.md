...
之多人游戏会话目录 (MPSD) 会话的会话名称，并被标识为会话模板，这是为会话提供了默认设置的 JSON 文档的一个实例。 模板是服务配置标识符 (SCID) 的服务配置的一部分的 guid。 可以在上找到此模板[Xbox 开发人员门户 (XDP)](https://xdp.xboxlive.com)并[合作伙伴中心](https://partner.microsoft.com/dashboard)。 服务配置是用于引入、 管理和安全策略的面向开发人员的资源。 通过 MPSD 访问会话时，主体授权执行针对服务配置根据通过 XDP 或合作伙伴中心开发人员设置的访问策略。 当会话在授予了服务配置的访问权限后加载时，辅助访问检查（如会话成员身份验证）在会话级别执行。
...
会话架构通过会话版本（主要版本）和协议版本（次要版本）版本化。 版本合并到 X Xbl 约定版本标头，因为"100\*主要 + 次要"。 例如，v1.7 标题中包含以下标头上每个 REST 请求，假设 107 的最新模板协定版本：X-Xbl-协定的版本：107.
...
功能是 MPSD 会话中的常量，该会话配置 MPSD 应该对其应用的行为。 您最常使用 XDP 和合作伙伴中心设置功能在会话模板中。 它们在 /constants/system/capabilities 对象中设置。 如果不需要功能，则使用空的功能对象。
...
游戏在 MPSD 会话中存储游戏元数据。 游戏元数据是显示会话数据并使游戏可以查找并加入游戏会话所需的信息。 标题会话成员，例如，播放机颜色的会话，等等的首选的播放器有利于存储在自定义属性部分中的特定于播放机的元数据。会话范围的元数据，例如，当前映射中，存储在 MPSD 会话的自定义的全局属性部分。
...
在创建会话时，客户端指定一个主机作为仲裁程序。 请参阅[如何：为 MPSD 会话设置仲裁器](multiplayer-how-tos.md)。
...
仲裁器查找播放机来填充空游戏会话槽由这些操作之一： 如果你的标题使用大厅会话或另一种机制以允许延迟的联接，查找新的会话成员使用该机制。
-   创建另一个匹配票证会话。

另请参阅[如何：在匹配过程填充打开会话槽](multiplayer-how-tos.md)。


#### <a name="handling-invited-session-members"></a>处理受邀请的会话成员

仲裁程序必须监视受邀请的会话成员，并在对单个用户的邀请之间应用最短间隔。 另请参阅[如何：发送游戏邀请](multiplayer-how-tos.md)。
