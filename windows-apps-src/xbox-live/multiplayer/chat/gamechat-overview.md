...
# <a name="game-chat-overview"></a>Game Chat overview
 ...
 Xbox One 拥有专门的硬件加速语音聊天编码解码器，可用于编码和解码，并通过 `Microsoft.Xbox.GameChat` 命名空间公开。 This codec supports several quality settings for title networking bandwidth flexibility, and its use is required for all voice chat communications transmitted by the console. No other codecs are supported.
