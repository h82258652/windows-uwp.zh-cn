...
**3) 将 Microsoft.Xbox.Services.winmd 文件拖放到 Unity 资源文件夹中**

![](../images/unity/unity2-winmd.png)

为此，请执行以下操作：

1.  从 [http://aka.ms/xboxlivesdkunity](http://aka.ms/xboxlivesdkunity) 中下载 Xbox Live WinMD，并将其解压缩到硬盘上的某个位置。

2.  将 Microsoft.Xbox.Services.winmd 拖放到打开的 Unity 项目资源面板中

当你无法在 Unity 编辑器中运行带有 Xbox Live 调用的脚本时，执行此步骤将使你能够在 Unity 编辑器中编译具有 Xbox Live 调用的脚本。
...
**7) 添加“Microsoft.Xbox.Live.SDK.WinRT.UWP”NuGet 程序包引用**

按照[向 UWP 项目添加 Xbox Live API 二进制文件包](add-xbox-live-apis-binary-to-a-uwp-project.md)中的指示操作

将“Microsoft.Xbox.Live.SDK.WinRT.UWP”NuGet 程序包引用添加到 UWP 应用项目中

**8) 将 Xbox Live 配置添加到 UWP 应用中**

按照名为[将 Xbox Live 添加到新的或现有的 UWP 项目](get-started-with-visual-studio-and-uwp.md)的文档中的指示操作

**9) 将 Xbox Live 代码添加到脚本中**
...
**10) 在 Visual Studio 中编译并运行 UWP 应用**

这将启动类似正常的 UWP 应用的应用，并允许进行 Xbox Live 调用，因为它们需要 UWP 应用容器才能正常工作。

**11) 如果对 Unity 中的任何内容进行更改，请重新生成**
