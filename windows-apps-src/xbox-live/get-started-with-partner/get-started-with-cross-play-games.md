...
ms.openlocfilehash: a1cb8f846736affc67d6867c3327f4486e6e1c23
...
-   强调游戏的 Xbox One 和电脑版本之间的连续性的免费游戏/BOPA游戏
...
2.  **游戏的 XDK 和 UWP 版本均可使用在 XDP 中创建的单个服务配置**。 我们已经向 XDP 添加了新功能，允许游戏在其 XDK 和 UWP 版本之间共享单个服务配置。 仍然需要在开发人员中心为程序包/目录发布 UWP，但所有服务配置均可在 XDP 中发布
...
通过遵循[将 Xbox Live 添加到新的或现有的 UWP 项目](get-started-with-visual-studio-and-uwp.md)中的指南，在 Windows 开发人员中心中创建 UWP 产品

## <a name="setup-your-xdk-product-in-xdp"></a>在 XDP 中设置你的 XDK 产品

既然你已经创建了 UWP，那么应该准备好在 XDP 中设置 XDK 产品了。 如果你还没有 XDK 主题作品，则必须创建你的 XDK 主题作品（下文的步骤 4.2.1）。
...
创建 XDP 产品后，请转到产品的“产品设置”页，输入在步骤 4.1.2 中检索到的 MSA 应用 ID。 可在 XDP 中，通过单击产品任务栏最左侧的“状态框”实现“产品设置”，如下所示。

![](../images/ingesting_crossplay_games_xdp/image11.png)

到达“产品设置”页后，请选择“应用程序 ID 设置”部分。 在此区域中，你可以输入在上文的步骤 4.1.2 的“应用程序 ID”字段检中索到的 MSA 应用 ID，如下所示。
...
### <a name="no-dev-sandbox-catalog-equivalent-for-windows-store"></a>没有适用于 Windows 应用商店的等效的开发人员沙盒目录

尽管 XDP 允许你将目录和二进制文件发布到 Xbox One 目录的开发人员沙盒版本，但 Windows 应用商店目录不支持沙盒。 因此，在开发人员沙盒中测试 UWP 需要你旁加载该 UWP 并直接运行。 这不会影响 Xbox Live 测试，但可能会改变你的标准测试过程。
