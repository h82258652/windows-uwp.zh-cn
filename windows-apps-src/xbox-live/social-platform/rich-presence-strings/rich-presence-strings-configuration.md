...
每个字符串集还可以通过使用参数表示字符串来利用游戏内数据进一步增强这些字符串。 字符串集包含的参数可以与你希望的一样多，只要生成的字符串不超过字符数限制。 请参阅[丰富的状态字符串：策略和限制](rich-presence-strings-policies-and-limitations.md)的更多详细信息。
...
----------- | ----------------- | ------------- | ------ | ----
映射 | CurrentMap | Map_Mountains | default | 山脉
 |  |  |  | zh | 山脉
 |  |  |  | en-US | 山脉
 |  |  |  | en-GB | 山脉
 |  |  |  |  de | Gebirge
 | |  |  | 等 |
 | |  | Map_Desert | default | 沙漠
 |  ||  |  zh | 沙漠
 |  ||  |  en-US | 沙漠
 |  ||  |   en-GB | 沙漠
 |  ||  |  de | Wuste
 |  ||  |  等 |
| |  |  Map_Beach | default | 海滩
 ||    |  | zh | 海滩
 ||    |  | en-US | 海滩
 ||    |  | en-GB | 海滩
 ||    |  | de | Strand
 | |   |  | 等 |
启动 | CurrentWeapon | Boot_Light | default | 浅色
 |  ||  |  zh | 浅色
 |  ||  |   en-US | 浅色
 |  ||  |   en-GB | 浅色
 |  ||  |   de | Leicht
 |  ||  |   等  |
 |  | |  Boot_Medium | default | 中等
 |  |  |  | zh | 中等
 |  |  |  | en-US | 中等
 |  |  |  | en-GB | 中等
 |  |  |  | de | Mittel
 |  |  |  | 等 |
 |  | |  Boot_Strong | default | 坚固
 |  |  |  | zh | 坚固
 |  |  |  | en-US | 坚固
 |  |  |  | en-GB | 坚固
 |  |  |  | de | Stark
