...
> [!WARNING]
> 如果客户端使用 RTA 为多人游戏会话，并且断开连接时 30 秒，[多人游戏会话 Directory(MPSD)](../multiplayer/multiplayer-appendix/multiplayer-session-directory.md)检测到 RTA 会话已关闭，并且开始用户退出会话。 它是对 RTA 客户端，以检测时关闭连接并启动重新连接并重新订阅之前 MPSD 结束会话。
