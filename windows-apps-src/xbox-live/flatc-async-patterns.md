...
你应该在每个异步调用的 API 的堆上创建新 AsyncBlock。  AsyncBlock 必须位于之前称为 AsyncBlock 的完成回调，然后可以将其删除。
...
当你完成使用**AsyncQueue**时，通常在即将结束游戏，就可以关闭它与**CloseAsyncQueue**:
