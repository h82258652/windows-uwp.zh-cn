...
如果设置了支持标记的事件和统计信息，为上下文搜索配置游戏就很容易了。  有关设置上下文搜索，如果您尚不熟悉，请参阅其他现有的 XDP 或合作伙伴中心文档。
...
在此页上，每个统计信息是按以下方式进行表示：优先级-显示名称 （Stat 实例名称）
...
1. 选择统计信息实例。
  ...
  请注意，仅支持统计信息实例 - 不接受统计信息模板。  此外应注意的可见性为 stat 实例 （配置 XDP 或合作伙伴中心的统计信息部分中） 已设置。  仅标记为**开放**的统计信息将显示在第三方体验中。
...
目前，你可以在上下文搜索文档中轻松更改统计信息，因为我们现在仅支持广播（这是易失数据）。  在我们开始支持游戏剪辑标记后（2015 年 9 月），如果你决定添加或删除统计信息，你将影响已建立索引的剪辑。  如果你添加新的统计信息，只有在更改生效后创建的游戏剪辑和广播将在使用新统计信息的查询中标记和呈现。旧游戏剪辑无法通过此统计信息查询。如果你从上下文搜索文档中删除了统计信息，附加到游戏剪辑中的旧统计信息将变为无用，且不会返回。
