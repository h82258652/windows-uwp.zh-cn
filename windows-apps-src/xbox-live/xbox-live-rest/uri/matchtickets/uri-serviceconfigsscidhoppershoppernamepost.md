...
| preserveSession| 枚举| 指示会话重复使用作为以匹配到会话的值。 可能的值是"始终"和"从不"。 |
...
可以创建匹配票证，并将该会话必须包含玩家匹配，以及其特定于玩家的属性之前，必须创建由**ticketSessionRef**对象引用会话。 每个玩家必须创建或加入针对将相关的匹配属性添加到会话的 MPSD 会话。 匹配属性均放置在每个玩家上调用 matchAttrs 的自定义属性字段。
