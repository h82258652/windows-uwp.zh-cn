---
title: PUT (/sessions/ {sessionId} {scid} /scids/ /data/ {pathAndFileName} {类型})
...
author: KevinAsgari
description: " PUT (/sessions/ {sessionId} {scid} /scids/ /data/ {pathAndFileName} {类型})"
ms.author: kevinasg
ms.date: 20-12-2017
ms.topic: article
ms.prod: windows
...
ms.locfileid: "3880663"
---
# <a name="put-sessionssessionidscidssciddatapathandfilenametype"></a>PUT (/sessions/ {sessionId} {scid} /scids/ /data/ {pathAndFileName} {类型})
将文件上传。 可以在其中的数据和元数据发送一条消息，或作为其中的数据和元数据发送一系列的较小的块多块上载完整上载上载数据。 可以在一个消息发送小于四个兆字节的文件。 数据类型 json 不支持多块上传。 这些 Uri 的域是`titlestorage.xboxlive.com`。
...
请求正文中包含上载文件的内容。 对于单个消息上载，正文是文件的完整内容。 对于多块上载，正文是文件的查询字符串参数中指定的部分。 
...
如果在调用是多块请求成功，该服务将返回 continution 令牌将下一个的块。
