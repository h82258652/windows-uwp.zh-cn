...
将文件上传。 可在其中的数据和元数据发送一条消息，或作为多块上载的数据和元数据以发送一系列较小块完整上载上传数据。 可以为一条消息发送小于 4 兆字节的文件。 多块上载不受支持的类型 json 数据。 这些 Uri 的域是`titlestorage.xboxlive.com`。
...
| finalBlock| Bool| 设置为 true 的文件的最后一个块。 设置为 false，所有其他时间段。| 
