...
将文件上传。 可在其中的数据和元数据发送一条消息，或在其中的数据和元数据发送一系列的较小的块多块上载的完整上载上传数据。 可以为一条消息发送小于 4 兆字节的文件。 数据类型 json 不支持多块上载。 这些 Uri 的域是`titlestorage.xboxlive.com`。
...
请求正文中包含正在上传文件的内容。 对于单个邮件上载，正文是文件的完整内容。 对于多块上载，正文是文件的查询字符串参数中指定的部分。 
