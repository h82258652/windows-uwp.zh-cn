---
title: 获取 (/ 全局/scid / {scid} /data/ {pathAndFileName} {类型})
...
author: KevinAsgari
description: " 获取 (/ 全局/scid / {scid} /data/ {pathAndFileName} {类型})"
ms.author: kevinasg
ms.date: 20-12-2017
ms.topic: article
ms.prod: windows
...
ms.locfileid: "3880427"
---
# <a name="get-globalscidssciddatapathandfilenametype"></a>获取 (/ 全局/scid / {scid} /data/ {pathAndFileName} {类型})
...
| 参数| 类型| 说明| 
| --- | --- | --- | --- | --- | --- | 
| 选择| 字符串| 仅 json 类型时才可用。 指定当响应由此参数，应仅包含某个属性/值的 JSON。 使用"点"（.） 来指定子属性和放在方括号 ([和]) 来指定数组索引。 例如，"array1 [4].prop2"指定"array1"数组索引 4 的"prop2"属性。| 
