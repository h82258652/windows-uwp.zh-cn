...
ms.locfileid: "57623292"
---
# <a name="post-jsonusersbatchscidssciddatapathandfilenamejson"></a>POST (/json/users/batch/scids/{scid}/data/{pathAndFileName},json)
下载多个文件从多个用户具有相同的文件名。 要下载的文件取决于请求的 URI。 请求的正文包含 XUIDs 的用户列表下载的文件。 响应正文将多部分 MIME 消息，每个部分表示具有其自己的标头集的特定用户的文件。 它是响应的可能是响应的成功和失败的组合的部件。 这些 Uri 的域是`titlestorage.xboxlive.com`。
