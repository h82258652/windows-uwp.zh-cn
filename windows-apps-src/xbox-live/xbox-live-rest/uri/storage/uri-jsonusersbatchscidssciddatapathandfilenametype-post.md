---
title: POST (/ json/用户/批处理/scid / {scid} /data/ {pathAndFileName} json)
...
author: KevinAsgari
description: " POST (/ json/用户/批处理/scid / {scid} /data/ {pathAndFileName} json)"
ms.author: kevinasg
ms.date: 20-12-2017
ms.topic: article
ms.prod: windows
...
ms.locfileid: "3881311"
---
# <a name="post-jsonusersbatchscidssciddatapathandfilenamejson"></a>POST (/ json/用户/批处理/scid / {scid} /data/ {pathAndFileName} json)
将多个文件下载从多个用户具有相同的文件名。 请求 URI 由确定要下载的文件。 请求正文中的包含的用户的 Xuid 列表下载的文件。 该响应正文将多个部分的 MIME 消息中，然后使用表示与自己的标头的一组特定用户的文件的每个部分。 很可能是成功和失败的组合的响应的部分。 这些 Uri 的域是`titlestorage.xboxlive.com`。
