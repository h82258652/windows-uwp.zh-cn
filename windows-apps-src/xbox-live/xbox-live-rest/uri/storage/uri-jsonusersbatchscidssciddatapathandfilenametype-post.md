...
ms.locfileid: "6044647"
---
# <a name="post-jsonusersbatchscidssciddatapathandfilenamejson"></a>POST (/json/users/batch/scids/{scid}/data/{pathAndFileName},json)
将多个文件下载从多个用户具有相同的文件名。 请求 URI 由确定要下载的文件。 请求正文中的包含的用户的 Xuid 列表以下载的文件。 该响应正文将的多部分的 MIME 邮件，包含表示的特定用户的组其自己的标头文件的每个部分。 很可能是组合的成功和失败的响应的部分。 这些 Uri 的域是`titlestorage.xboxlive.com`。
...
| HttpStatusCode| 与此特定的文件中检索相关 HTTP 状态代码。| 
...
| 标题| 说明| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| ETag| ETag 是由对某一 URL 找到资源的特定版本的 web 服务器分配一个不透明的标识符。 如果位于该 URL 的资源内容发生改变，新功能和不同的 ETag 被分配。| 
| Content-Type| 如果已成功检索文件，这是文件的内容类型。| 
