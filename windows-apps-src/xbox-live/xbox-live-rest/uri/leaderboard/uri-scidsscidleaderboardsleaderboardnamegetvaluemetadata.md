...
？ 包括 = valuemetadata 查询参数允许该响应包含任何关联的排行榜值元数据。 值元数据包含客户端指定的值，如模型和用于实现跑道的最佳时间汽车的颜色与关联的数据。
 
排行榜，基于不在排行榜本身上的用户统计数据，定义值元数据。
...
| 值| 字符串| 必需。 用户的统计数据排行榜所依据的值。| 
| valueMetadata| 字符串| 必需。 字符串的以逗号分隔的字符串对，以格式"\"name\": \"value\"，..."
