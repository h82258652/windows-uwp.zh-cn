...
？ 包括 = valuemetadata 查询参数允许该响应包含任何关联的排行榜值元数据。 值元数据包含客户端指定的值，例如模型和用来实现跑道的最佳时间汽车的颜色与相关联的数据。
 
排行榜，基于不适用于排行榜本身上用户统计数据，定义值元数据。
 
排行榜 Api 都是只读的因此仅支持获取动词命令。 它们反映排名和排序"页"派生通过数据平台编写的单个用户统计数据的编制了索引的玩家统计数据。 完整的排行榜索引可能会相当大，并将永远不会要求调用方看到一个完整，因此此 URI 支持多个允许调用方是哪种类型的视图其想要针对的排行榜，请参阅有关特定的查询字符串参数。
...
没有针对内容隔离和访问控制方案实现的授权逻辑。
   ...
   * 可以从任何平台上的客户端读取排行榜和用户的统计数据，前提是调用方提交请求有效的 XSTS 令牌。 写入进行显然限于受数据平台的客户端。
   * 游戏开发人员可以标记为打开或使用 XDP 或合作伙伴中心限制的统计信息。 排行榜是开放的统计信息。 打开统计信息可以访问 Smartglass，以及 iOS、 Android、 Windows、 Windows Phone 和 web 应用程序，只要用户有权访问沙盒。 通过 XDP 或合作伙伴中心管理到沙盒的用户身份验证。
...
| 标题| 描述| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| If-None-Match| 字符串。 如果客户端支持缓存所使用的实体标记。 示例值: <b>"686897696a7c876b7e"</b>|  | 
...
| 标头| 类型| 描述| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| Content-Type| 字符串| 必需。 该响应正文的 MIME 类型。 示例： <b>application/json</b>。| 
...
pagingInfo | pagingInfo| 部分| 可选。 仅显示在请求中指定 maxItems 时。| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
