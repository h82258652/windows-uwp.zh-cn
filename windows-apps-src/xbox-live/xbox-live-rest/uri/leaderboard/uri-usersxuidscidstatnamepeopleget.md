...
若要最大化的任何调用方的能力，以了解其重新获取正确显示的数据，每位用户每个统计数据值将返回为在其中它应显示，和格式化以匹配接受的语言中指定的区域设置格式的字符串在请求中的标头。 为该排行榜 statname 将还返回本地化"显示名称"。
...
| ContinuationToken| 字符串| 必需。 指定要返回到"continuationToken"查询参数，以获取该 URI 的下一页结果，如果所需订阅源的值。 如果没有 pagingInfo 返回则没有"下一页"要获取的数据。|
...
| 标头| 类型| 描述|
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Content-Type| 字符串| 响应正文 mime 类型。 示例值:"application/json"。|
