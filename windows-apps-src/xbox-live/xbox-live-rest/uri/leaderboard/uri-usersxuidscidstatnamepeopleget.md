...
排行榜 Api 都是只读的因此 （通过 HTTPS) 仅支持 GET 动词命令。 它们反映排名和排序"页面"的派生通过数据平台编写的单个用户统计数据的编制了索引的玩家统计数据。 完整的排行榜索引可能会很大，并将永远不会要求调用方看到一个完整，因此此 URI 支持多个允许调用方是哪种类型的视图其想要针对该排行榜，请参阅有关特定的查询字符串参数。
...
| --- | --- | --- |
| xuid| 字符串| 用户的标识符。|
| scid| 字符串| 包含要访问的资源的服务配置标识符。|
| statname| 字符串| 正在访问的用户统计数据资源的唯一标识符。|
| 全部|收藏夹| 枚举| 是否排名统计数据值 （分数） 为当前用户的所有已知的联系人或仅通过该用户指定为常用联系人的联系人。|
...
| skipToUser| 字符串| 返回排行榜结果周围的指定玩家 xuid，无论该用户的排名或分数的页面。 页面将按百分点等级与指定的用户或统计数据的排行榜视图中间中预定义视图的页面的最后一个位置进行排序。 没有任何<b>continuationToken</b>提供对于此类型的查询。 |
| ContinuationToken| 字符串| 如果上一个调用返回<b>continuationToken</b>，然后调用方可以传递回该令牌"按原样"查询字符串，以获取下一页结果。 |
...
可以从任何平台上的客户端读取排行榜和用户的统计数据，前提是调用方提交他们的请求的有效 XSTS 令牌。 写入的 （显然） 个数据平台支持的客户端。
...
| Content-Type| 字符串。 请求正文 MIME 类型。 示例值:"应用程序/json"。|
...
| 标题| 说明|
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| If-None-Match| 字符串。 实体标记-使用如果客户端支持缓存。 示例值:"686897696a7c876b7e"。|
...
若要最大化任何调用方能够了解它重新获取正确显示的数据，每个用户每个统计数据值将返回为格式，它应显示，和格式以匹配接受的语言中指定的区域设置的字符串在请求中的标头。 为该排行榜 statname 将还返回本地化"显示名称"。
...
| 成员| 说明|
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| <b>pagingInfo</b>| 部分| 可选。 在页面中的最后一项的排名较低 totalItems 时返回。 本部分还不会返回时请求中指定 skipToUser。|
| ContinuationToken| 字符串| 必需。 指定要返回到"continuationToken"查询参数，可获取该 URI 的下一页结果，如果所需订阅源的值。 如果没有 pagingInfo 返回则没有"下一页"要获取的数据。|
| totalItems| 64 位无符号的整数| 必需。 排行榜中的条目的总数。|
| <b>leaderboardInfo</b>| 部分| 必需。 始终返回。 包含有关请求排行榜的元数据。|
| 显示名称| 字符串| 必需。 预定义的排行榜的本地化的显示名称。 示例值:"总标志捕获"。|
| totalCount| 字符串| 必需。 排行榜中的条目的总数。|
| 列| array| 必需。|
| 显示名称| 字符串| 必需。 对应于排行榜中的列。|
| statName| 字符串| 必需。 对应于排行榜中的列。|
| type| 字符串| 必需。 对应于排行榜中的列。|
| <b>用户列表</b>| 部分| 必需。 始终返回。 包含请求排行榜的实际用户分数。|
| 玩家代号| 字符串| 必需。 对应于用户在排行榜条目。|
| xuid| 32 位有符号整数| 必需。 对应于用户在排行榜条目。|
...
| 标头| 类型| 说明|
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Content-Type| 字符串| 该响应正文 mime 类型。 示例值:"应用程序/json"。|
