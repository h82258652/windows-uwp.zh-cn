...
排行榜 Api 都是只读的因此 （通过 HTTPS) 仅支持 GET 动词命令。 它们反映排名和排序"页"的派生通过数据平台编写的单个用户统计数据的编制了索引的玩家统计数据。 完整的排行榜索引可能会相当大，并将永远不会要求调用方看到一个完整，因此此 URI 支持多个允许调用方是哪种类型的视图其想要针对该排行榜，请参阅有关特定的查询字符串参数。
...
| skipToUser| 字符串| 返回排行榜结果周围的指定玩家 xuid，无论该用户的排名或分数的页面。 页面将按百分点等级与指定的用户或中间统计数据的排行榜视图中预定义视图的页面的最后一个位置进行排序。 没有任何<b>continuationToken</b>提供对于此类型的查询。 |
...
若要最大化的任何调用方的能力，以了解其重新获取正确显示的数据，对于每位用户每个统计数据值将返回为在其中它应显示，和格式化以匹配接受的语言中指定的区域设置格式的字符串在请求中的标头。 为该排行榜 statname 将还返回本地化"显示名称"。
