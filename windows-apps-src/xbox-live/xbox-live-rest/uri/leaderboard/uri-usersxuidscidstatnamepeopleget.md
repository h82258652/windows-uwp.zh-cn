...
排行榜 Api 都是只读的因此 （通过 HTTPS) 仅支持获取动词命令。 它们反映排名和排序"页"派生通过数据平台编写的单个用户统计数据的编制了索引的玩家统计数据。 完整的排行榜索引可能会相当大，并将永远不会要求调用方看到一个完整，因此此 URI 支持多个允许调用方是哪种类型的视图其想要针对的排行榜，请参阅有关特定的查询字符串参数。
...
| 全部|收藏夹| 枚举| 是否要排名统计数据值 （分数） 为当前用户的所有已知的联系人或仅由用户指定为常用联系人的联系人。|
...
若要最大化的任何调用方的功能，以了解其重新获取正确显示的数据，对于每位用户每个统计数据值将返回为顺序它应该会显示，并且格式以匹配接受的语言中指定的区域设置格式的字符串在请求中的标头。 为该排行榜 statname 将还返回本地化"显示名称"。
...
| <b>userList</b>| 部分| 必需。 始终返回。 包含请求排行榜实际的用户评分。|
