...
ms.locfileid: "7146107"
---
# <a name="get-usersmegroupsmoniker-"></a>GET (/users/me/groups/{moniker} )
我的用户组获取 presencerecord，他的。 这些 Uri 的域是`userpresence.xboxlive.com`。
...
该服务将始终返回 200 确定如果请求本身是标准格式。 但是，它将过滤掉响应中的信息时不能将隐私检查。
...
| 接受| 字符串| 内容类型可接受。 只有一个存在支持<b>application/json</b>，但它仍然必须指定在标头中。| 
...
| 406| 不允许| 不支持资源版本。| 
| 500| 请求超时| 服务可能不理解格式不正确的请求。 通常参数无效。| 
| 503| 请求超时| 服务可能不理解格式不正确的请求。 通常参数无效。| 
...
| 标头| 类型| 说明| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| x xbl 协定版本| 字符串| 名称/的内部版本号此请求应定向到 Xbox LIVE 的服务。 请求将仅可路由到的服务验证该标头，身份验证令牌中的声明的有效性后，依此类推。 示例值： 1，vnext。| 
| Content-Type| 字符串| 请求正文的 mime 类型。 示例值： <b>application/json</b>。| 
| 缓存控制| 字符串| 若要指定缓存行为的礼貌用语请求。 示例值:"非缓存"。| 
...
| 标头| 类型| 说明| 
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | 
| 重试后| 字符串| 返回 503 HTTP 错误。 允许知道如何长重试调用之前要等待的客户端。 示例值:"120"。| 
