...
通过任何客户端、 服务或想要了解一批用户的状态信息的游戏应使用此方法。

为此批处理请求的响应可以由深度和路径的筛选器。 消费者可以使用此找出并显示有关一组用户的状态。 此 API 上的筛选器跨 Or 作为一个属性，但 And 属性。
...
| level| 可能值： <ul><li>用户-获取用户节点</li><li>设备-获取用户和设备节点</li><li>标题-获取基本标题级别信息</li><li>所有-获取完整状态信息和媒体信息</li></ul><br> 默认值为"标题"。|
