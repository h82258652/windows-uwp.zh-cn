...
| 参数| 类型| 描述| 
| --- | --- | --- | --- | --- | --- | 
| level| 字符串| 返回所指定的此查询字符串的详细的级别。 有效选项包括"用户"、"设备"、"title"和"全部"。"用户"级别是没有 DeviceRecord 嵌套对象的 presencerecord，他的对象。 级别"设备"是 presencerecord，他的和 DeviceRecord 的对象，而无需 TitleRecord 嵌套对象。 级别"title"就是没有 ActivityRecord 嵌套对象的 PresenceRecord，DeviceRecord，TitleRecord 对象。 "全部"级别是完整的记录，包括所有嵌套的对象。如果未提供此参数，该服务将默认为标题级别 （即，它将返回到标题的详细信息的该用户的状态）。| 
...
该服务将始终返回 200 确定是否良好请求本身。 但是，它将过滤掉响应中的信息时不能将隐私检查。
...
| 406| 不允许| 资源版本不受支持。| 
| 500| 请求超时| 服务可能不理解的格式不正确的请求。 通常参数无效。| 
| 503| 请求超时| 服务可能不理解的格式不正确的请求。 通常参数无效。| 
...
| 缓存控制| 字符串| 若要指定缓存行为的礼貌用语请求。 示例值:"无缓存"。| 
...
此 API 检索与在 URI 中出现的 XUID 相关的组名字对象由指定的广播用户的状态记录。
