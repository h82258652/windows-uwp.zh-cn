...
| 参数| 类型| 描述|
| --- | --- | --- | --- | --- | --- |
| id| string]| 必需。 所有将为其返回详细信息 （最多 10) 的 Id。 注意任何 ID，包含非法放置在 URL 中的字符 （ProviderContentId 类型 Id 通常是完整的 Url 本身，从而包含非法字符）<b>必须</b>使用 URL 编码，以便正确发送到娱乐发现服务 (EDS)。 另请注意，这仅可单个值 ID 类型是否 ProviderContentId。 如果需要多个 ProviderContentId，则必须对 EDS 进行多次调用。|
...
| ConditionSets| 字符串| <b>可选</b>。 客户端可以请求<b>可用性</b>修剪具体取决于条件集，它们是通过此查询字符串指定的键 / 值对。 它们用于匹配条件集的可用性。 可以用于匹配条件集的键列表如下所示。 <ul><li><b>平台</b>： 产品位置生成且可播放。</li><li><b>订阅</b>： 受支持的订阅 （金牌或银牌） 此可用性的列表。</li><li><b>EntitlementIds</b>： 跟踪后用户购买游戏。</li></ul> | 
