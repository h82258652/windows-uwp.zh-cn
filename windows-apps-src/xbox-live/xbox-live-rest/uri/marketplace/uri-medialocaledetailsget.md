...
| DesiredMediaItemTypes| 字符串| <b>所需如果 MediaGroup 不会传递。 都应该不会传递。</b> 媒体项从的 Id。 所提供的所有 Id 必须具有相同的类型。 如果需要多个类型，将所有可能类型 IdType 上面所述。 此值默认为"未知"如果它不存在，这可能不会为所有 ID 类型的 valied。 |
| MediaGroup| 字符串| <b>所需如果 DesiredMediaItemTypes 不会传递。 都应该不会传递。</b>|
| ConditionSets| 字符串| <b>可选</b>。 客户端可以请求<b>可用性</b>修剪，具体取决于条件集，它们是通过此查询字符串指定的键 / 值对。 它们用于匹配条件集的可用性。 可以用于匹配条件集的键列表如下所示。 <ul><li><b>平台</b>： 产品位置生成且可播放。</li><li><b>订阅</b>： 的受支持 （金牌或银牌） 此可用性的订阅列表。</li><li><b>EntitlementIds</b>： 跟踪后用户购买的游戏。</li></ul> | 
