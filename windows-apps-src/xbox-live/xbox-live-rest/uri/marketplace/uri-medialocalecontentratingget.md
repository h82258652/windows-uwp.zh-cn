...
尽管此 API 返回的确切值可能会最终更改，它们应很少更改 （例如版本的娱乐发现服务 (EDS)） 并因此可以为长时间缓存。 接受**combinedContentRating**参数将提供有意义的错误消息，如果传入的值是无效的这是指示任何 API 调用方只需要调用此 API 再次以获取更新的值。 如果 API 接受**combinedContentRating**参数，但其中一个未提供，任何筛选的内容将基于家长控制。 

> [!NOTE] 
> 这并不意味着，仅"安全"内容会返回-这意味着，返回所有内容，包括潜在显式的内容。 
...
| filterUnrated| 布尔值| 可选。 确定或不，是否应在响应中包含未分级的内容。| 
| maxGameRating| 32 位有符号的整数| 可选。 筛选游戏。| 
| maxMovieRating| 32 位有符号的整数| 可选。 筛选上面特定级别的电影。| 
