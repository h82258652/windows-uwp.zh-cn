...
尽管此 api 返回的确切值最终可能会更改，它们应很少更改 （例如版本的娱乐发现服务 (EDS)） 并因此可能很长一段时间为缓存。 接受**combinedContentRating**参数将提供有意义的错误消息，如果传入的值是无效的这是指示任何 API 调用方只需要调用此 API 再次以获取更新的值。 如果 API 接受**combinedContentRating**参数，但其中一个未提供，任何筛选的内容将基于家长控制。 
