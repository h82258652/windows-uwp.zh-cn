...
## <a name="remarks"></a>备注
   ...
   * 调用方要求占用的数量必须大于 0 正整数。 使用值为 0 或较低的调用将被拒绝。
   * 如果调用方提供了一个空的事务 ID，将拒绝该请求。
   * 如果可用，以便它可以确定哪些游戏报告消耗，将记录的主题作品声明。
   * 使用相同的 transactionId 的其他文章将忽略某个时间段。
...
POST，假设传递身份验证并分配适当授权上下文响应是使用相同的 transactionId 收据 acknolodgement 传递给中 POST 请求、 易耗型项的 URL 和项目的新的服务数量的值。
