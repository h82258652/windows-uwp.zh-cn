...
删除数量字段允许调用方指示的易耗品他们想要删除该消耗品的剩余数量的数量。 交易 ID 字段提供了一种方法重试限制两次计数相同的使用情况的风险时使用易耗型内容的操作与调用方。
...
POST，假设传递身份验证并分配适当授权上下文的响应是具有相同的 transactionId 收据 acknolodgement 传递给中 POST 请求、 易耗型项的 URL 和项目的新的服务数量值。
