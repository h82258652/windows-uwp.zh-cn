...
用于配置的其会话和模板，MPSD 使用大量符合会话 JSON 对象，该目录会强制执行和解释的固定架构。 在调用期间为支持各种会话目录 Uri 的方法，这些对象是验证，并且合并，具体取决于受支持的架构。 与多人游戏配置相关联的主要 JSON 对象是：
...
&nbsp;&nbsp;支持 POST 操作来设置用户的当前活动显示在 Xbox One 仪表板用户体验，并邀请会话成员，如果所需的会话。
