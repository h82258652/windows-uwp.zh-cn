...
在多个 Xbox 用户 Id，在服务配置 ID (SCID) 级别上，此 HTTP/REST 方法创建批处理查询。 此方法可以通过**Microsoft.Xbox.Services.Multiplayer.MultiplayerService.GetSessionsForUsersFilterAsync**换行显示。

为 2015年多人游戏，你可以合并多个查询为单次调用所有查询都均相同，但如果*xuid*查询字符串参数表示不同的 Xbox 用户 Id (Xuid)、 处理。 请注意，查询字符串参数，并响应，相同的常规查询和批处理查询。

对于批处理查询，如请求中提供的*xuid*参数属于每个 XUID 的会话以相同顺序响应中编写。 很可能在同一个会话在响应中，它可以匹配每个*xuid*的一次出现多次。
...
| 可见性| 字符串| 会话的可见性状态。 由<b>MultiplayerSessionVisibility</b>定义可能的值。 此参数设置为"打开"，如果查询应包括仅打开的会话。 如果它被设置为"private"，<i>专用</i>的参数必须设置为 true。 |
