---
title: POST (/serviceconfigs/ {scid} /sessiontemplates/ {sessionTemplateName} / 批次)
...
author: KevinAsgari
description: " POST (/serviceconfigs/ {scid} /sessiontemplates/ {sessionTemplateName} / 批次)"
ms.author: kevinasg
ms.date: 20-12-2017
ms.topic: article
ms.prod: windows
...
在多个 Xbox 用户 Id 在会话模板级别上，此 HTTP/REST 方法创建批查询。 此方法可以由**Microsoft.Xbox.Services.Multiplayer.MultiplayerService.GetSessionsForUsersFilterAsync**包装。

为 2015年多人游戏，你可以将合并许多查询为单次调用所有查询都均相同，但如果*xuid*查询字符串参数表示不同的 Xbox 用户 Id (Xuid)、 处理。 请注意，查询字符串参数，并响应，相同的常规查询和批处理查询。

对于批处理查询，属于每个 XUID 的会话写入顺序响应请求中提供的*xuid*参数。 很可能在同一个会话多次出现在响应中，一次为每个匹配的*xuid* 。
...
| --- | --- | --- | --- | --- | --- | --- |
| 关键字| 字符串| 一个关键字，例如，"foo"，如果要检索必须在会话或模板中找到的。 |
| xuid| 64 位无符号的整数| Xbox 用户 ID，例如，"123"，要包含在查询中的会话。 默认情况下，用户必须为其包括在会话中处于活动状态。 |
| 预订| 布尔型| 若要包括的会话用户设置为保留玩家，但并未加入成为活动玩家。 在查询你自己的会话，或查询特定用户的会话服务器到服务器时，仅使用此参数。 |
| 处于非活动状态| 布尔型| 若要包括用户已接受但不是会主动玩游戏的会话，则为 true。 用户可"准备好"但不是"活动"的会话计数为非活动状态。 |
...
| version| 32 位有符号整数| 应包含最大会话版本。 例如，值为 2 指定该会话与主要的会话版本的 2 或更少应包含。 版本号必须小于或等于请求的协定版本 mod 100。 |
...
[/serviceconfigs/ {scid} /sessiontemplates/ {sessionTemplateName} / 批处理](uri-serviceconfigscidsessiontemplatessessiontemplatenamebatch.md)
