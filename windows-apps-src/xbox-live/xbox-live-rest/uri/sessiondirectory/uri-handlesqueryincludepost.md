...
此 HTTP/REST 方法创建具有"包括"的查询字符串中指定的会话信息的句柄数据的查询。 查询字符串值旨在作为扩展以支持将来的查询选项，支持以逗号分隔的列表，例如，"？ 包括 = relatedInfo，会话"。 可以通过**Microsoft.Xbox.Services.Multiplayer.MultiplayerService.GetActivitiesForUsersAsync**包装 POST 方法。
...
| --- | --- | --- | --- |
| 关键字| 字符串| 一个关键字，例如，"foo"，它们是否检索必须在会话或模板中找到的。 |
...
| 处于非活动状态| 布尔型| 若要包括用户已接受但不是会主动玩游戏的会话，则为 true。 用户可"就绪"，但不是"活动"的会话进行计数为非活动状态。 |
...
| 参加| 32 位有符号整数| 若要检索的会话的最大数量。 此数字必须介于 0 到 100 之间。 |
...
结果将返回为一个句柄结构数组，使用嵌入在每个句柄的唯一 ID。 **RelatedInfo**对象中返回的特定会话信息。 请注意，此信息不会对返回的常规的 POST 方法在此 URI。
