...
此 HTTP/REST 方法创建句柄数据查询使用"包括"查询字符串中指定的会话信息。 查询字符串值旨在作为扩展以支持将来的查询选项，支持以逗号分隔的列表，例如，"？ 包括 = relatedInfo，会话"。 可以通过**Microsoft.Xbox.Services.Multiplayer.MultiplayerService.GetActivitiesForUsersAsync**包装 POST 方法。
