...
此 HTTP/REST 方法创建句柄数据查询使用"包含"查询字符串中指定的会话信息。 查询字符串值旨在为扩展以支持将来的查询选项，支持的以逗号分隔列表，例如，"？ 包括 = relatedInfo，会话"。 可以通过**Microsoft.Xbox.Services.Multiplayer.MultiplayerService.GetActivitiesForUsersAsync**包装 POST 方法。
...
若要获取所有活动的用户的"收藏夹"社交图片，请发布正文如下所示：
...
结果将返回为一个句柄结构数组具有嵌入在每个句柄的唯一 ID。 **RelatedInfo**对象中返回的特定会话信息。 请注意，此信息不会对返回常规的 POST 方法在此 URI。
