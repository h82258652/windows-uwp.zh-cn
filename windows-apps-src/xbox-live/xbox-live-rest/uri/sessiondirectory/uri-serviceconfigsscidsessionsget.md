...
此 HTTP/REST 方法检索所提供的筛选器的会话信息。 此方法可以通过**Microsoft.Xbox.Services.Multiplayer.MultiplayerService.GetSessionsAsync**换行显示。
...
> [!NOTE] 
> 每次调用此方法必须包括关键字、 Xbox 用户 ID 筛选器或两者。 如果调用方不具有正确的<i>私钥</i>和<i>预订</i>参数的权限，方法将返回的错误代码为 403 禁止访问，无论确实存在任何此类会话。  
