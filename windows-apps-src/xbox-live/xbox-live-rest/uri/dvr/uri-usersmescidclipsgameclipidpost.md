...
ms.openlocfilehash: 6cd0972554c1beecce270d306c8a7ea94214cee6
ms.sourcegitcommit: 6cc275f2151f78db40c11ace381ee2d35f0155f9
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/26/2018
ms.locfileid: "5554870"
---
...
用于更新游戏剪辑元数据的 API 在更新元数据的自己的游戏剪辑辅助功能和标题，如和更新的公共属性 （如应用进行评分或递增的视图计数） 分为两类的任何其他游戏剪辑。 如果对 URI 中的 XUID 不匹配的 XUID 声明中，可以编辑仅公开的数据，并将拒绝任何编辑任何其他数据的请求。 在这种情况中多个字段尝试进行编辑，其中一个无效请求的整个请求将会失败。
...
以下 Uri 更新元数据中的公共字段。 没有任何所需的这些请求的正文。 将返回 HTTP 状态代码为 200 的元数据的成功更新时。 否则将相应的 HTTP 状态代码返回 JSON 格式的 ServiceErrorResponse 对象。
 
   * **POST /users/ {ownerId} {scid} /scids/ /clips/ {gameClipId} /ratings/ {评分值}** -适用于指定剪辑指定的评分。 评分值应为 1 到 5 之间的整数。
   * **发布 /users/ {ownerId} {scid} /scids/ /clips/ {gameClipId} / 标志**-标志内容可能可疑检查通过强制执行该剪辑。
   * **POST /users/ {ownerId} {scid} /scids/ /clips/ {gameClipId} / 查看**-递增指定游戏剪辑的视图计数。 建议，这称为不正确播放启动时，但当 75%的 80%的播放完毕。
