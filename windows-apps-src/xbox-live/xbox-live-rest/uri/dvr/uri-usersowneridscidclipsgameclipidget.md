...
该剪辑的查询的所有数据都在从任何元数据查询返回的**GameClip**对象中可用。 **XUID**、 **ServiceConfigId**、 **GameClipId**和**SandboxId**请求的声明中需要获取此 API 通过单个游戏剪辑。 如果游戏剪辑被标记为强制执行，或内容隔离或隐私检查确定用户没有权限，以获取特定的游戏剪辑，该 API 将返回 HTTP 状态代码的 404 （未找到）。
...
> [!NOTE] 
> URI 刷新可能需要最多 30-40 秒后完成此请求。 如果 URI 已过期，请尝试使用它立即进行流式处理的操作将获得从 IIS 平滑流式处理服务器的 HTTP 500 状态代码。 我们正在运行的方法，从而缩短了此操作，请并将该工作，随着相应地更新此说明。 
...
| SandboxId| 十六进制的二进制文件| 是|  | 指示系统用于查找正确的区域，并且用于<b>内容隔离</b>的检查。| 
...
| 接受| 字符串| 内容类型的可接受的值。 示例： <b>application/json</b>。| 
| 缓存控制| 字符串| 若要指定缓存行为的礼貌请求。| 
| 重试后| 字符串| 指示客户端在不可用的服务器的情况下我们深表歉意。 示例： <b>application/json</b>。| 
| 不同| 字符串| 指示下游代理如何缓存响应。 示例： <b>application/json</b>。| 
