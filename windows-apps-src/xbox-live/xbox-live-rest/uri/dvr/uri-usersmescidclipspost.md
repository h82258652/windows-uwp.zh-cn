...
这是 GameClip 上载过程的第一部分。 后的视频捕获，建议调用 GameClips 服务立即获取有关的位上, 传的 ID 和 URI，即使上传未计划立即开始。 用户配额检查和内容隔离，隐私，依此类推，若要查看是否视频应该甚至计划用于上传客户端通过其他检查，将执行此调用。 肯定响应来自此调用指示服务愿意接受视频剪辑的上传。 上传的所有剪辑必须与特定标题 （通过 SCID) 都关联，以接受系统中。
 
此调用不是幂等;后续调用将导致不同 Id 和要颁发的 Uri。 重试失败时应遵循标准客户端后关闭行为。
