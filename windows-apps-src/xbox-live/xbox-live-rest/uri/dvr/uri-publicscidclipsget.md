...
此 API 允许是公共的列表剪辑的各种方法。 在隐私检查和防止请求 XUID 的内容隔离检查返回的剪辑列表。
 
查询每个服务配置标识符 (SCID) 进行了优化。 指定进一步筛选器或下面列出的默认值以外的排序顺序可以在某些情况下耗时更长返回。 这会更明显的较大的视频集。 查询不能指定升序排序顺序。
