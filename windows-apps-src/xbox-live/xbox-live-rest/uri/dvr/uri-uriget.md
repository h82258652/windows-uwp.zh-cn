...
客户端可以下载任何剪辑或已达到已发布状态且**GameClipUri**对象中指定的可下载类型的缩略图。 检索用户或公共剪辑的列表时，响应正文中包含请求文件的 URI。
...
| 参数| 类型| 说明| 
| --- | --- | --- | 
| <b>uri</b>| 字符串| <b>GameClipUri</b>对象中的<b>uri</b>字段。| 
...
如果成功，服务器将返回的视频剪辑，可能会截断根据范围请求标头。 被截断剪辑，响应将为部分内容 (206)。 如果服务器返回整个文件，它将响应确定 (200)。 在发生错误， **GameClipsServiceErrorResponse**对象可能会返回以及相应的 HTTP 状态代码 (例如，416，请求范围无法满足)。
