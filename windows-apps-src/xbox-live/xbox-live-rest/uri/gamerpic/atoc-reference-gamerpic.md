...
玩家图片服务旨在通过授予游戏的功能，以允许用户以生成其游戏的字符的玩家图片向用户提供更多个性化选项，（在此方案中的游戏字符指的是游戏内 protagonist; 它可以是一个人汽车、 宇宙飞船或在游戏中的用户控件的任何其他实体)。
...
能够设置游戏玩家图片受仅强制执行特权 (211)。 如果强制执行吊销权限，将阻止用户保存的游戏的玩家图片，并且该服务将返回 403。 游戏应调用 CheckPrivilege 以验证允许用户要共享内容 （专用 211）。
