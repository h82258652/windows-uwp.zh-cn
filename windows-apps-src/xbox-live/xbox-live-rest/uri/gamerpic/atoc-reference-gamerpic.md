...
玩家图片服务旨在通过授予的能力，以允许用户生成的其游戏角色的玩家图片的标题为用户提供更多个性化选项，（在此方案中的游戏字符指的是游戏内 protagonist; 它可以是一个人汽车、 宇宙飞船或用户控件在游戏中的任何其他实体)。
...
能够设置的游戏玩家图片受仅强制执行特权 (211)。 如果强制执行吊销权限，用户将无法从保存的游戏的玩家图片，并且该服务将返回 403。 游戏应调用 CheckPrivilege 以验证允许用户要共享的内容 （专用 211）。
 
目前，才能使用此服务，你的作品必须列入白名单。 若要请求审批，发送电子邮件至`slsgamerpics@microsoft.com`。
