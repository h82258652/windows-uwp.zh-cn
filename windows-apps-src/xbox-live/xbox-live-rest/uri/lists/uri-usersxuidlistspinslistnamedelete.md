...
若要删除的项由其列表中的索引和中查询字符串参数**的索引**标识。 索引列表必须以逗号分隔，并且可以将 100 个索引传递单次调用。 但是，如果没有索引传递 （空查询字符串参数） 则整个列表的内容将被删除 （空列表将保留，和版本号将继续增加）。 一旦项都被删除，该列表"压缩"，以便任何洞保留在排序的索引。 因此，此调用不幂等。
