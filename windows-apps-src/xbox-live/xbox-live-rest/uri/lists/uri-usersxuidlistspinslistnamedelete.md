...
若要删除的项由其列表中的索引和中查询字符串参数**的索引**标识。 必须以逗号分隔的索引列表，并且可以进行 100 个索引传递单次调用。 但是，如果没有索引传递 （空查询字符串参数） 然后将整个列表的内容将被删除 （空列表将保留，和版本号将继续增加）。 一旦项都被删除，列表"压缩"，以便任何洞处于排序的索引。 因此，此调用不是幂等。
 
此调用要求**如果-匹配： versionNumber**标头包含在请求中， **versionNumber**所在的文件的当前版本号。 如果它不是包含在内，或者不匹配当前的列表版本编号，然后 HTTP 412 （前置条件失败） 将返回状态代码，并且该响应正文将包含最新的元数据的列表，其中包括当前版本号。 这是为了防止从不同的客户端上彼此造成破坏的更新。
