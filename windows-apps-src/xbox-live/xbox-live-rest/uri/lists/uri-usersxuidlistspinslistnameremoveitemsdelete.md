...
从列表中删除项目。 要删除项由其列表中的索引和查询字符串参数"索引"标识。 索引列表必须用逗号分隔，并且可以将 100 个索引传递单次调用。 但是，如果没有索引传递 （空查询字符串参数） 然后内容整个列表将被删除 （列表将保留但为空，版本号将继续增加）。 一旦项都被删除，列表"压缩"，以便任何洞处于排序的索引。 
