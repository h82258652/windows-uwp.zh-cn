---
title: 删除/用户/xuid (xuid) / 列出/PIN / {listname} / RemoveItems
...
author: KevinAsgari
description: " 删除/用户/xuid (xuid) / 列出/PIN / {listname} / RemoveItems"
ms.author: kevinasg
ms.date: 20-12-2017
ms.topic: article
ms.prod: windows
...
ms.locfileid: "3963674"
---
# <a name="delete-usersxuidxuidlistspinslistnameremoveitems"></a>删除/用户/xuid (xuid) / 列出/PIN / {listname} / RemoveItems
...
从列表中删除项目。 要删除项由其索引列表中，并标识查询字符串参数"索引"。 索引列表必须用逗号分隔，并且可以将 100 个索引传递单次调用。 但是，如果没有索引传递 （空查询字符串参数） 则内容整个列表将被删除 （列表将保留但为空，版本号将继续增加）。 一旦项目都被删除，列表"压缩"，以便没有漏洞剩顺序的索引。 
