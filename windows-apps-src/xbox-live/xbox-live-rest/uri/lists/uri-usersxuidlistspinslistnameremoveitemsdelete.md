...
从列表中删除项目。 要删除的项目列表中的索引中所示，并查询字符串参数"索引"标识。 索引列表必须用逗号分隔，并且可以将 100 个索引传递单次调用。 但是，如果没有索引传递 （空查询字符串参数） 则内容整个列表将被删除 （列表将保留但为空，版本号将继续增加）。 一旦项都被删除，列表"压缩"，以便没有漏洞剩顺序的索引。 
 
此调用需要"如果-匹配： versionNumber"标头包含在请求中，versionNumber 所在的文件的当前版本号。 如果它不是包含在内，或者不匹配当前将返回号，则 HTTP 412 – 前置条件失败的状态代码的列表版本，并且该响应正文将包含最新的元数据的列表，其中包括的当前版本号。 这是为了防止从不同的客户端上彼此造成破坏的更新。 
