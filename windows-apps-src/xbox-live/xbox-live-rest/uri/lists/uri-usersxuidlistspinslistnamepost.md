...
要插入的项必须具有必填的字段中的功能规范; 指示否则，将返回 HTTP 400。 同样，如果插入的结果将超过最大大小 （每个列表类型定义） 的列表将返回 HTTP 400 然后将插入执行任何操作。
 
如果该项是** 插入开头或结尾的列表中，则**如果-匹配： versionNumber**所需请求中包括的标头。 标头是可选的如果插入的开头或结尾。 如果存在，标头将经过无论插入的位置。 在标头中**VersionNumber**是列表的当前版本号。 如果它不包含并且是必需的或不匹配当前的列表版本编号，然后 HTTP 412 （前置条件失败） 将返回状态代码和响应正文将包含最新的元数据的列表，包括当前版本号。 这是为了防止从不同的客户端上彼此造成破坏的更新。
