...
要插入的项必须具有必填的字段中的功能规范; 指示否则，将返回 HTTP 400。 同样，如果插入的结果将超过 （每个列表类型定义） 的列表的最大大小然后将返回 HTTP 400 并且将插入执行任何操作。
 
如果该项是** 插入开头或结尾的列表中，则**如果-匹配： versionNumber**所需请求中包括的标头。 标头是可选的如果插入的开头或结尾。 如果存在，该标头将经过无论插入位置。 在标头中**VersionNumber**是列表的当前版本号。 如果它不包含并是必需的或不匹配当前的列表版本编号，然后 HTTP 412 （前置条件失败） 将返回状态代码，并且该响应正文将包含最新的元数据的列表，其中包括的当前版本号。 这是为了防止从不同的客户端上彼此造成破坏的更新。
...
| 参数| 类型| 说明| 
| --- | --- | --- | --- | --- | --- | 
| insertIndex| 字符串| 可选。 定义要插入项目的位置。 受支持的值： 0，正整数和"结束"。 任何大于列表项目数的索引值将列表中，在底部添加新项目，并且不会将"空白"空间插入列表中。 默认值： 0。| 
