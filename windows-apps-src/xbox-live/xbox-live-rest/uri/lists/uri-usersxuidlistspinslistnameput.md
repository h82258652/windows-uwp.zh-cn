...
此调用将更新根据指定每个项目，请求正文中的索引列表中的项。 此调用将插入项列表中，并且如果项不存在指定索引处，通过然后，调用将返回 400 错误请求状态。 可以在单个调用中，更新多个项目，但所有必须存在当前列表中。 也就是说，所有获取更新或 none 获取更新。
 
此调用将允许该项目以进行更新，以便指定的**itemId**而不是**索引**。 若要执行此操作，只需发送给服务的**IndexedItems**结构中的索引使用"-1"。 显然在此情况下， **itemId**不能更改一部分的更新，以便它仅适用于对其他元数据字段的更改。 **提供程序**/**providerId**组合可用于而不是**itemId**标识该项目。 在内部，该服务搜索这些项目和找出更新的正确索引的列表。 如果无法找到或多个项目将返回 400 错误请求状态，并将更新任何项。
 
此调用需要**如果-比赛： versionNumber**标头以包括在请求中，如果使用的索引标识的项。 如果使用项目 Id，以标识项 （并且该列表不允许重复项），然后**If-match**标头是可选的。 如果存在，将始终验证**If-match**标头。 在标头， **versionNumber**是列表的当前版本号。 如果它不包含 （和所需），或执行不匹配将返回当前的列表版本编号，然后 HTTP 412 前置条件失败状态代码和响应正文将包含最新的元数据的列表，包括当前版本号。 这是为了防止从不同的客户端上彼此造成破坏的更新。
...
| listname| 字符串| 列表的名称 （给定 listtype 执行哪些列表）。 始终为"XBLPins"引脚中的项。| 
