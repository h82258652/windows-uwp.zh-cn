...
此终结点允许调用方确定是否一个或多项存在于指定的列表中未实际获取列表和搜索本身。 通过 itemId （或提供程序/providerId 组合） 进行标识的项目集，并且返回数据列表是否包含每个项目使用布尔值 true 或 false ndicating 传递的数据。 
...
如果在调用成功，返回的项目列表，以及每个项目指定该项目是否可以在列表中的一个布尔值。 
