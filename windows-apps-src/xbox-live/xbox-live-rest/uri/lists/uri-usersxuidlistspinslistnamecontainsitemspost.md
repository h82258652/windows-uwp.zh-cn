...
此终结点允许调用方确定是否一个或多个项目存在于指定的列表中未实际获取列表和搜索本身。 通过 itemId （或提供商/providerId 组合） 进行标识的项目集并返回的数据是列表是否包含每个项目使用布尔值 true 或 false ndicating 传递的数据。 
