...
[/ 用户/xuid (xuid) / 列出了/PIN / {listname} / ContainsItems](uri-usersxuidlistspinslistnamecontainsitems.md)

&nbsp;&nbsp;确定是否项 （由 itemId 指定） 的一组包含在列表中而不检索整个列表。

[/ 用户/xuid (xuid) / 列出了/PIN / {listname} / 索引 （{索引}）？ insertIndex = {insertIndex}](uri-usersxuidlistspinslistnameindex.md)

&nbsp;&nbsp;将移动列表中的一项。

[/ 用户/xuid (xuid) / 列出了/PIN / {listname} / RemoveItems](uri-usersxuidlistspinslistnameremoveitems.md)
