...
中返回的数据的**listCount**字段表示多少项是在维护服务-此类的总列表中，它可以用于确定其中列表末尾，并且这可能是从多少特定项已 retu 不同数量该请求 rned。
...
| filterDeviceType| 字符串| 可选。 指定要返回的设备类型的以逗号分隔列表 （将不会返回类型不在列表中）。 筛选返回的设置为仅返回已插入从一组特定的设备类型的项。 设备类型的以逗号分隔列表用于此筛选器 （多个设备类型可查询中调用一次）。 可能的值： xbox One、 MCapensis、 WindowsPhone、 WindowsPhone7、 Web、 PC、 MoLive。 默认值： null （所有内容类型）。| 
