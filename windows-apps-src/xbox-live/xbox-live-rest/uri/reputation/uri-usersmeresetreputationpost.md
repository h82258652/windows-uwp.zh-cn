...
ms.locfileid: "3881346"
---
# <a name="post-usersmeresetreputation"></a>POST （/用户/me/resetreputation）
允许执行团队后 （例如） 帐户劫持将当前用户的信誉评分设置为某些任意值。 这些 Uri 的域是`reputation.xboxlive.com`。
...
通过任何其他合作伙伴的所有除零售沙盒和 Retail，除非出于测试目的的所有沙盒中的用户，也可能会调用此方法。 请注意，此请求用于设置用户的"基本"信誉评分，他正面的反馈正面权重将全部归。在执行此调用后用户的实际信誉将这些基本评分以及他大使奖金他关注者奖金。
...
如果成功，响应正文为空。 失败时，将返回[ServiceError (JSON)](../../json/json-serviceerror.md)文档。
