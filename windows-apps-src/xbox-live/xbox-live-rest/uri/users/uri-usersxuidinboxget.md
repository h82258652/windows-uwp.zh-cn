...
用户的消息摘要包含仅邮件主题。 对于用户生成的消息，这是当前消息文本的前 20 个字符。 系统消息可能会提供另一个使用者，如"LIVE System"。

发送; 顺序相反返回消息即，较新消息首先返回。
