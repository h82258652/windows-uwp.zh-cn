...
没有针对内容隔离和访问控制方案实现的授权逻辑。
   ...
   * 假设调用方提交与请求有效的 XSTS 令牌，可以从任何平台上的客户端读取排行榜和用户统计信息。 写入进行显然限于数据平台支持的客户端。
