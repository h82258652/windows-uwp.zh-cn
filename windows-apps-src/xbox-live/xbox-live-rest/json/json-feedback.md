...
| CommsTextMessage| 用户| 收件箱服务调用信誉服务，该更新发件人的信誉，报告给执行团队的反馈。 **注意：** 收件箱 UI 应该有一个按钮，以允许用户标志一条消息。 |
  ...
  | FairPlayConsoleBanRequest | 合作伙伴 | 合作伙伴将作为建议禁止从 Xbox Live 控制台发送此反馈。  |
  | FairPlayIdler | 用户合作伙伴 | 如果用户代表处于空闲状态目的在游戏中，通常圆形后一轮，确定的游戏可以发送此反馈无需用户干预。  |
  ...
  | PositiveHighQualityUGC | 用户合作伙伴 | 用户和合作伙伴发送此反馈以指示游戏应允许用户提交的游戏内从共享 UGC 上的正面反馈例如，调整 Forza 中的设置。  |
