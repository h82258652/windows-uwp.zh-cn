...
以下是调用方可以使用它来检查是否可以执行特定的操作的值。 与上述的设置，这些封装定义服务的策略，并不能直接更改由用户，但在大多数情况下，基于其值由用户定义的一个或多个设置将构建策略。 这些是针对多个上面定义的设置通常复合检查。 示例： <b>ViewProfile</b>权限执行操作目标的<b>ShareProfile</b>隐私设置和请求者的<b>AllowProfileViewing</b>权限的检查。
