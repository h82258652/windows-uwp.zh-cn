...
以下参数可以用于优化到更有针对性的项目集娱乐发现服务 (EDS) 查询。 所有这些参数必需的在任何 API 中，但他们要接受接受查询优化器任何 API 中。
...
下面是这可能中的工作方式做法：
   ...
   * 由第二个查询。 它等同于第一个，不同之处在于，"流派 = 射击游戏"添加。
...
| <b>xboxHasChildMediaItemTypes</b>| 字符串的数组| 使用游戏的媒体组的所有返回的项目必须具有子元素的媒体项目类型是一个提供的值。| 
