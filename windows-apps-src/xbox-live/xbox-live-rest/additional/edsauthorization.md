...
娱乐发现服务 (EDS) 3.1 不会支持匿名流量。 所有请求 EDS 需要身份验证。 EDS 将需要通过正确进行身份验证的客户端的调用方 XToken。 这些令牌由 XSTS 生成，并且可以通过各种 Xbox 身份验证服务 (XAS) 中获取。 有单独的身份验证服务的设备、 用户和游戏将所有定义的令牌的标识。
...
用户并想要使用我们的服务的事项。 并且，我们希望大部分这些操作和用户能够使用我们的服务。 但如何执行我们确保操作不假装个人，而且用户实际上是他们所说的人？ 我们将向他们提供他们可以用它来确定自己给其他人的令牌。
 
这些令牌由 XSTS 生成，并且通常称为 XTokens。 XToken 是一个广泛的术语，用于覆盖令牌，它包含各种不同的内容，并且可以有很多不同的形式，但是所有创建、 已签名，而且由 XSTS 服务器 （可选） 加密。 在内部，XSTS 使用 MXAN 来创建并格式化令牌。 MXAN 是不断从 XToken 中提取信息是唯一的组件。 使用令牌的服务传递到 MXAN 以破解的请求标头。 处理和验证令牌和表示令牌中的声明将返回到该服务。 服务然后，可以使用这些声明值来标识调用用户或设备，并执行操作，具体取决于该信息。
 
基本的标识标记中的用户、 设备和游戏-对于提供通过 Xbox 身份验证服务 (XAS)。 每个 XAS 负责生成为他们将负责的各种声明指定的值的标识令牌。
 
   * XASD (适用于设备的 XAS): 创建 DToken 可提供设备标识
   * XASU (XAS 用户): 创建 UToken 可提供用户身份
...
根据&lt;哈希 >，该令牌的处理方式是不同：
   ...
   * 如果&lt;哈希 > 等于 * （星号），则没有特定的用户正在执行请求的令牌中的所有用户存在和反序列化的主体中。 这是真正的多用户表单。
   * 如果&lt;哈希 > 等于-(dash)，则没有用户正在执行的请求。 反序列化的主体中的任何用户都将去除。
   * 如果&lt;哈希 > 不等于 * 或-则指示在令牌中的用户发出请求的标识符。 仅指示的用户将会出现在反序列化主体。 所有其他用户都将去除。这是单用户 3.0 令牌。
   
<a id="ID4EYC"></a>

 
## <a name="does-eds-support-multi-users"></a>EDS 是否支持多用户？
 * 答案是没有。 中所述的情况下，控制台将始终发送单个用户令牌。 即使有多个登录的用户，必须指定"调用方"，其他所有身份均都放置的位置。
