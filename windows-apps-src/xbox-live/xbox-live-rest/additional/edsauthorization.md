...
人脉和内容，想要使用我们的服务。 并且，我们希望大部分这些操作，用户能够使用我们的服务。 但如何执行我们确保内容不假装个人，而且用户实际上是谁他们所说？ 我们将向他们提供他们可以用它来标识自己给其他人的令牌。
 
这些标记通过 XSTS 生成和通常被称为 XTokens。 XToken 是一个广泛的术语，用于覆盖标记包含各种不同的内容，并可以采用多种不同的形式，但它们全部创建、 签名，并由 XSTS 服务器 （可选） 加密。 在内部，XSTS 使用 MXAN 来创建并格式化令牌。 MXAN 是不断提取 XToken 信息是唯一的组件。 服务使用令牌传递给 MXAN 以破解的请求标头。 处理和验证令牌和表示令牌中的声明将返回到该服务。 服务然后可以使用这些声明值来标识调用用户或设备，并执行操作，具体取决于该信息。
...
## <a name="authorization-process"></a>授权流程
   ...
   * 创建 XSTS 请求。
    ...
    * 执行 XSTS 请求并缓存结果 XToken。 返回的 XToken （最） 包含所有标识标记与任何其他声明 （当前订阅状态、 用户组等） 的设备、 用户和游戏标识信息。
...
根据&lt;哈希 >，该令牌的处理方式是不同：
   ...
   * 如果&lt;哈希 > 不等于 * 或-则用于指示在令牌中的用户发出请求标识符。 只有指定的用户将在反序列化的主体中存在。 所有其他用户都将去除。这是单用户 3.0 令牌。
