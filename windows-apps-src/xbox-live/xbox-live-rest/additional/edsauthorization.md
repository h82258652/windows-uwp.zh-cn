...
娱乐发现服务 (EDS) 3.1 不会支持匿名流量。 所有请求 EDS 需要身份验证。 EDS 需要调用方进行正确身份验证客户端从 XToken。 这些标记由 XSTS 和可以获得通过各种 Xbox 身份验证服务 (XAS)。 有单独的身份验证服务的设备、 用户和游戏将所有定义的令牌的标识。
 
XSTS 是 Xbox LIVE 的网关守卫。 它是防御以确定是否用户或设备有权将连接到任何 Xbox LIVE 服务的第一行。 在对用户进行身份验证之后, XSTS 生成 XToken 它们可用于安全地将自己标识为在服务上的任何组件。 此 XToken 是生存你 passport。
 
用户和事情，想要使用我们的服务。 和我们希望大部分这些操作和用户能够使用我们的服务。 但我们不如何进行确保内容不假装用户，并且用户实际上是他们所说的？ 我们将向他们提供它们可用于标识自己给其他人的令牌。
 
这些标记由 XSTS 和通常称为 XTokens。 XToken 是一个广泛的术语，用于覆盖标记包含各种不同的内容，并可以有很多不同的形式，但是所有创建、 签名，而且由 XSTS 服务器 （可选） 加密。 在内部，XSTS 使用 MXAN 来创建并格式化令牌。 MXAN 是不断提取 XToken 信息的唯一组件。 使用令牌的服务传递到 MXAN 以破解的请求标头。 处理和验证令牌和表示令牌中的声明将返回到该服务。 服务然后，可以使用这些声明值来标识调用用户或设备，并根据该信息执行操作。
...
## <a name="authorization-process"></a>授权流程
   ...
   * 获取一个或多个标识标记。 你可以请求与最每个 D、 U 和 T 令牌之一 XToken。 你必须提供至少一个 D 或 u。 
 ...
 * 答案是没有。 中所述的情况下，控制台将始终发送单个用户令牌。 即使有多个用户登录，必须指定"调用方"，所有其他标识放置的位置。
