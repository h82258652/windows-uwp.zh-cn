...
**功能更新**已在运行 Windows 10 的设备上安装最新功能、 体验和功能。 因为功能更新包含整个 Windows 副本，因此客户也将它们用于在运行 Windows 7 或 Windows 8.1 的现有设备和未安装操作系统的新设备上安装 Windows 10。 Microsoft 预期每半年一次更新发布。 
...
在 Windows 10 开发期间，Microsoft 简化了 Windows 产品工程设计和发布周期，以便我们能够比以往更快地交付客户需要的特性、体验以及功能。 同时我们创造新的方法来交付并安装功能更新和质量更新，从而简化部署和持续的管理、扩大保持使用最新 Windows 功能和体验的员工基础，并降低企业的总成本。 因此，我们实现了新服务的选项 – 称为半年频道和长期服务频道 (LTSC) – 提供实用的解决方案，让更多的设备在企业环境中比以前可能较新的。
...
| 半年频道 | 大约在 Microsoft 第一次发布之后的 4 个月 | 当从 18 个月第一次发布 | 在部署之前提供更多时间测试 新功能升级 | 专业版、教育版、企业版、移动企业版、IoT 核心专业版 |
...
在 Windows 7 时间范围中，兼容性的实现方法非常被动。 在 Windows 8 中，我们已开始以不同的方式看待这个，以确保该兼容性的 Windows 中的工作是设计而不是事后才进行。 到目前为止，Windows 10 是兼容性按设计实现的最佳版本的操作系统。 下面提供了我们实现兼容性的几个关键方法：
-   **应用程序遥测**:这有助于我们了解 Windows 生态系统，以通知兼容性测试中的应用受欢迎程度。
-   **ISV 合作关系**:直接与外部合作伙伴通力合作，向他们提供数据和帮助解决用户遇到的问题的合作。
-   **设计评审、 上游检测**:合作伙伴与功能团队以减少在 Windows 中的重大更改的数量。 兼容性审查是我们的功能团队必须通过的门槛。
-   **通信**:对 API 更改和改进的通信的更紧密控制。
-   **外部测试和反馈循环**:Windows 预览体验成员接收到外部测试版的版本，可帮助提高我们在最终生成发布给客户之前找到兼容性问题的能力。 此反馈过程不仅可以显露 Bug，还可确保我们交付用户所需的功能。
...
OS 版本已根据 Windows 10 递增。 这意味着内部版本号已更改为 10.0。 同过去一样，在 OS 版本更改后我们将尽力维护应用程序和设备兼容性。 对于大多数应用程序类别 （而无需任何内核依赖项），更改不会产生负面影响应用功能和现有应用程序将继续在 Windows 10 上正常工作。
...
我们鼓励所有 Win32 应用 ISV 开发[通用 Windows 平台 (UWP)](https://go.microsoft.com/fwlink/?LinkID=780560)，特别是 [Centennial](https://go.microsoft.com/fwlink/?LinkID=780562) 应用要与时俱进。 开发这些应用包有许多好处，而使用传统 Win32 安装程序则没有。 也支持 UWP 应用[Microsoft Store](https://go.microsoft.com/fwlink/?LinkID=780563)，因此它是更轻松地自动更新为一致的版本的用户还可降低支持成本。
...
### <a name="step-1-become-a-windows-insider-and-participate-in-flighting"></a>第 1 步：成为 Windows 预览体验和参与外部测试
...
因为您必须访问以预览版本的 Windows 10、 Windows 10 移动版和最新的 Windows SDK 和仿真程序，你必须所有工具任您选用开发优秀的应用程序和了解什么是通用 Windows 平台和 Microsoft Store 中的新增功能。
...
**OS\\设备功能测试：** 如果应用依赖操作系统中的特定功能，请确保应用按预期运行。 用于测试的常见区域如下所示（为确保覆盖范围，通常选择常用电脑机型）：
-   Audio
...
### <a name="step-4-register-on-ready-for-windows"></a>步骤 4：在准备好进行 Windows 注册
...
[Windows 10 维护服务更新和升级选项](https://technet.microsoft.com/itpro/windows/manage/introduction-to-windows-10-servicing)
