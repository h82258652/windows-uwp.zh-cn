...
**注意** **Em** 是字体的度量单位。 字体中的 1 Em 等于分辨率为 72ppi 时 100% 的特定点值。 例如，16pt 等于分辨率为 72ppi 时的 16px (aka 100% Plateau)。 设计了新的 MDL2 字体，以便占用的图标区域为方形 Em。 因此，如果将 16px 的宽度和高度加入代码中，你将获得一个 16x16px 的图标占用。 这并不总是意味着该图标即为全尺寸占用。
...
![使用零宽度字形](images/segoe-ui-symbol-layering.png)

**Segoe MDL2 Assets** 中的所有字形的固定宽度均相同，且高度和左原点均保持一致，从而可通过直接在其上绘制字形来实现分层和着色效果。

大部分图标还具有镜像样式，以供在使用从右到左文本方向的语言（如阿拉伯语、波斯语和希伯来语）中使用。

如果你使用 C#/VB/C++ 和 XAML 开发应用，则可以选择使用 [**Symbol enumeration**](https://msdn.microsoft.com/library/windows/apps/dn252842)（而不是 Unicode ID）从 Segoe MDL2 Assets 字体指定字形。 如果你使用 JavaScript 和 HTML 开发应用，则可以选择使用 [**AppBarIcon enumeration**](https://msdn.microsoft.com/library/windows/apps/hh770557)（而不是 Unicode ID）从 **Segoe MDL2 Assets** 字体指定字形。
...
<td align="left"><p>断裂的心形（零宽度）</p></td>
</tr>
</tbody>
</table>
...
<td align="left">实心星形（填充色较浅，适合在 HTML 中使用）</td>
</tr>
<tr class="odd">
...
|--------|--------------------------------------------------------------------------------|---------------------------------|-------------------------|
| 代码   | 符号                                                                         | 枚举                            | 说明             |
| U+E001 | ![checkmarklegacy](images/checkmarklegacy.png)                                 | CheckMarkLegacy                 | 对勾标志              |
| U+E002 | ![checkboxfilllegacy](images/checkboxfilllegacy.png)                           | CheckboxFillLegacy              | 已勾选的复选框         |
| U+E003 | ![checkboxlegacy](images/checkboxlegacy.png)                                   | CheckboxLegacy                  | 复选框                |
| U+E004 | ![checkboxindeterminatelegacy](images/checkboxindeterminatelegacy.png)         | CheckboxIndeterminateLegacy     | 不确定状态     |
| U+E005 | ![checkboxcompositereversedlegacy](images/checkboxcompositereversedlegacy.png) | CheckboxCompositeReversedLegacy | 已反转                |
| U+E008 | ![checkmarkzerowidthlegacy](images/checkmarkzerowidthlegacy.png)               | CheckMarkZeroWidthLegacy        | 对勾标志（零宽度） |
| U+E009 | ![checkboxfillzerowidthlegacy](images/checkboxfillzerowidthlegacy.png)         | CheckboxFillZeroWidthLegacy     | 已勾选（零宽度）       |
| U+E0A2 | ![checkboxcompositelegacy](images/checkboxcompositelegacy.png)                 | CheckboxCompositeLegacy         | 复合               |
| U+E739 | ![checkbox](images/checkbox.png)                                               | Checkbox                        | 复选框                |
| U+E73A | ![checkboxcomposite](images/checkboxcomposite.png)                             | CheckboxComposite               | 复合复选框      |
| U+E73B | ![checkboxfill](images/checkboxfill.png)                                       | CheckboxFill                    | 已勾选的复选框         |
| U+E73C | ![checkboxindeterminate](images/checkboxindeterminate.png)                     | CheckboxIndeterminate           | 不确定状态     |
| U+E73D | ![checkboxcompositereversed](images/checkboxcompositereversed.png)             | CheckboxCompositeReversed       | 反转复合      |
| U+E73E | ![checkmark](images/checkmark.png)                                             | CheckMark                       | 对勾标志              |
...
| 代码   | 符号                                                                     | 枚举                          | 说明                               |
|--------|----------------------------------------------------------------------------|-------------------------------|-------------------------------------------|
| U+E0C4 | ![backbttnarrow20legacy](images/backbttnarrow20legacy.png)                 | BackBttnArrow20Legacy         | 后退按钮箭头, 20pt                   |
| U+E0A6 | ![backbttnarrow42legacy](images/backbttnarrow42legacy.png)                 | BackBttnArrow42Legacy         | 后退按钮箭头, 42pt                   |
| U+E0AD | ![backbttnmirroredarrow20legacy](images/backbttnmirroredarrow20legacy.png) | BackBttnMirroredArrow20Legacy | 后退按钮镜像箭头反转, 20pt |
| U+E0AB | ![backbttnmirroredarrow42legacy](images/backbttnmirroredarrow42legacy.png) | BackBttnMirroredArrow42Legacy | 后退按钮镜像箭头反转, 42pt |
| U+E830 | ![chromeback](images/chromeback.png)                                       | ChromeBack                    | Chrome 后退按钮                        |
| U+EA47 | ![chromebackmirrored](images/chromebackmirrored.png)                       | ChromeBackMirrored            | Chrome 后退镜像按钮               |
...
| 代码   | 符号                                                         | 枚举                    | 说明                |
|--------|----------------------------------------------------------------|-------------------------|----------------------------|
| U+E0D5 | ![arrowhtmllegacy](images/arrowhtmllegacy.png)                 | ArrowHTMLLegacy         | 适用于 HTML 的箭头         |
| U+E0AE | ![arrowhtmlmirroredlegacy](images/arrowhtmlmirroredlegacy.png) | ArrowHTMLMirroredLegacy | U+E0D5 镜像版 |
...
**对于设计人员**
* [字体指南](fonts.md)
* [在 W3C 上，哪些语言从右到左 (RTL) 编写？](http://www.i18nguy.com/temp/rtl.mdl)
**对于开发人员 (XAML)**
