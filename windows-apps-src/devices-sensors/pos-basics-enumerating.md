...
<br/>
显示设备选取器 UI，并让用户选择是连接的设备。 此方法处理设备连接并删除，时更新列表和简单且更安全比其他方法。
...
此示例使用上方定义拍摄快照使用**FindAllAsync**的设备选择器然后通过集合返回的项目的每个枚举，设备名称和 ID 写入调试输出。 
