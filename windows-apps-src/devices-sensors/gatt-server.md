...
在客户端角色通常运行 Windows。 不过，许多情况下会出现这需要充当蓝牙 LE GATT 服务器以及 Windows。 适用于 IoT 设备，以及大多数跨平台 BLE 通信的几乎所有方案都需要 Windows GATT 服务器。 此外，将通知发送到附近可穿戴设备已成为要求以及此技术常见方案。  
...
> 所有 Windows Api 都使用的术语 GUID，但蓝牙标准定义这些后台任务视为 Uuid。 对我们来说，这些两个术语彼此互换，因此我们将继续使用术语 UUID。 

如果该属性是标准和定义蓝牙 SIG 定义，还会有一个相应的 16 位短 ID (例如电池级别 UUID 是 0000**2A19**-0000-1000年-8000-00805F9B34FB 短 ID 且 0x2A19)。 可以看到这些标准 Uuid [GattServiceUuids](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.bluetooth.genericattributeprofile.gattserviceuuids.aspx)和[GattCharacteristicUuids](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.bluetooth.genericattributeprofile.gattcharacteristicuuids.aspx)中。

如果你的应用实现它自己的自定义服务，将需要生成自定义 UUID。 这是轻松地完成在 Visual Studio 中通过工具-> CreateGuid （使用选项 5 获取它采用"xxxxxxxx-xxxx-...xxxx"格式）。 此 uuid 现在可用于声明新的本地服务、 特征和描述符。
...
### <a name="build-up-the-heirarchy-of-services-and-characteristics"></a>建立层次结构的服务和特性
...
```
如上所示，这也是不错的声明每个特性支持的操作的事件处理程序。  若要正确响应请求，应用必须定义，并为该属性支持每个请求类型设置事件处理程序。  未能注册一个处理程序会被立即完成与*UnlikelyError*由系统在请求中。

### <a name="constant-characteristics"></a>常量的特征
有时，将不会更改的应用的生命周期期间的特性值。 在此情况下，最好声明常量的特征，以防止不必要的应用激活： 
...
### <a name="read"></a>已阅读
在远程设备将尝试读取特性值 （并且不常数值） 时，调用 ReadRequested 事件。 对以及参数 （包含有关远程设备的信息） 调用读取的特点传递给该委托： 
...
### <a name="write"></a>写入
当在远程设备将尝试值写入特性时，有关详细信息在远程设备，以写入的特征和值本身调用 WriteRequested 事件： 
...
最常见的 GATT 服务器操作，通知执行的关键功能将数据推送到远程设备。 有时，你会想要通知所有订阅的客户端，但 othertimes 你可能想要选取的设备发送给新值： 
...
```
> 请注意，应用程序可以获取的最大通知大小 MaxNotificationSize 属性的特定客户端。  任何大于最大大小的数据将被截断由系统。
