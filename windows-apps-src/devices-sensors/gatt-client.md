...
author: msatranjr
title: 蓝牙 GATT 客户端
description: 本文概述了适用于通用 Windows 平台 (UWP) 应用，以及针对常见的用例的示例代码的蓝牙通用属性配置文件 (GATT) 客户端。
ms.author: misatran
ms.date: 02/08/2017
ms.topic: article
...
蓝牙 LE GATT Api 公开对象和函数，而不是对原始传输访问。 GATT Api 还使开发人员可以使用蓝牙 LE 设备能够执行以下任务：

-   执行属性发现
-   读取和写入属性值
-   为 Characteristic ValueChanged 事件注册回调

若要创建有用的实现，开发人员必须具有特定特性值，以便由 API 提供的二进制数据转换为的前期知识的 GATT 服务和应用程序应使用的特性并且过程呈现给用户之前有用的数据。 蓝牙 GATT API 仅公开与蓝牙 LE 设备进行通信所需的基本基元。 要解释数据，应用程序配置文件必须经蓝牙 SIG 标准配置文件定义，或经由设备供应商实现的自定义配置文件定义。 配置文件创建应用程序和设备之间的绑定合约，此合约关于交换的数据所代表的内容以及如何解释它。
...
现在，回 DeviceWatcher 方法。 蓝牙 LE 设备在 Windows 中的任何其他设备一样，且可使用[枚举 Api](https://msdn.microsoft.com/library/windows/apps/BR225459)的查询。 使用[DeviceWatcher](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.enumeration.devicewatcher)类，并传递查询字符串指定要查找的设备： 
...
deviceWatcher.Start();
```
一旦你开始 DeviceWatcher，你将收到[DeviceInformation](https://msdn.microsoft.com/library/windows/apps/br225393)为满足问题的设备[添加](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.enumeration.devicewatcher.added)事件处理程序中的查询每台设备。 有关 DeviceWatcher 的更多详细信息，请参阅示例[Github 上](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/DeviceEnumerationAndPairing)的完成。 
...
```
另一方面，释放对 BluetoothLEDevice 的所有引用设备对象 （和系统上没有其他的应用是否具有对该设备的引用） 将会触发自动在较小的超时时间后断开连接。 
...
```  
操作系统返回 GattCharacteristic 的只读列表对象，你可以执行操作。

## <a name="perform-readwrite-operations-on-a-characteristic"></a>读/写上执行操作特性

特点是 GATT 的基本单元基于通信。 它包含一个值，表示一个不同的设备上的数据。 例如，在电池级别特性具有一个值，表示该设备的电池剩余电量。
...
请确保在特性支持标识或通知 （检查以确保特性属性）。 

> **预留**： 指示被视为更可靠，因为每个值已更改事件在要与客户端设备确认。 通知是更流行，因为大多数 GATT 交易记录将而节省电源而不是非常可靠。 在任何情况下，所有的处理在控制器层因此应用不会不获取会涉及到。 我们将汇集起来称为它们只需"notifications"，但现在你已知道。 

有要负责获取通知前两个事项：
- 写入到客户端特性配置描述符 (CCCD)
- 处理 Characteristic.ValueChanged 事件

写入到 CCCD 告知此客户端想知道该特定特征值更改的每次服务器设备。 若要实现此目的，请执行以下操作： 
