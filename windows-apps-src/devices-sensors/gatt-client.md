...
本文介绍了通用 Windows 平台 (UWP) 应用，以及有关常见的 GATT 客户端任务的示例代码的蓝牙通用属性 (GATT) 客户端 Api 的使用情况：
- 查询附近设备
- 连接到设备
- 枚举设备的特性与受支持的服务
- 读取和写入特性
...
```  
操作系统会返回 GattCharacteristic 的只读列表对象，你可以执行的操作。
...
```
> **提示**： 获取熟悉使用[DataReader](https://msdn.microsoft.com/en-us/library/windows/apps/windows.storage.streams.datareader.aspx)和[DataWriter](https://msdn.microsoft.com/en-us/library/windows/apps/windows.storage.streams.datawriter.aspx)。 使用你获得许多蓝牙 Api 的原始缓冲区时，其功能将是不可或缺。 
## <a name="subscribing-for-notifications"></a>订阅通知

请确保在特性支持标识或通知 （检查是否为特征的属性，以确保）。 

> **预留**： 指示被视为更可靠，因为每个值更改事件结合确认从客户端设备。 通知是普及，因为大多数 GATT 交易将而节电而不是非常可靠。 在任何情况下，所有的处理在控制器层因此应用不会不获取会涉及到。 我们将汇集起来称为它们只是"notifications"，但现在你已知道。 
...
```
现在，GattCharacteristic ValueChanged 事件将获取每次调用的值获取在远程设备上发生更改。 只是实现的处理程序： 
