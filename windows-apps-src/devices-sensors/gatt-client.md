...
服务定义 LE 设备的功能合约，并且包含定义该服务的特性的集合。 这些特性反过来包括描述特性的描述符。 这些 3 术语通常被称为设备的属性。
...
若要创建有用的实现，开发人员必须具有特定特性值，以便由 API 提供的二进制数据转换为的前期知识的 GATT 服务和应用程序应使用的特性并且过程有用的数据，然后再向用户呈现。 蓝牙 GATT API 仅公开与蓝牙 LE 设备进行通信所需的基本基元。 要解释数据，应用程序配置文件必须经蓝牙 SIG 标准配置文件定义，或经由设备供应商实现的自定义配置文件定义。 配置文件创建应用程序和设备之间的绑定合约，此合约关于交换的数据所代表的内容以及如何解释它。
...
现在，返回到 DeviceWatcher 方法。 蓝牙 LE 设备在 Windows 中的任何其他设备一样，且可使用[枚举 Api](https://msdn.microsoft.com/library/windows/apps/BR225459)的查询。 使用[DeviceWatcher](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.enumeration.devicewatcher)类，并传递查询字符串指定要查找的设备： 
...
deviceWatcher.Start();
```
一旦你开始 DeviceWatcher，你将收到[DeviceInformation](https://msdn.microsoft.com/library/windows/apps/br225393)为满足问题的设备[添加](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.enumeration.devicewatcher.added)事件处理程序中的查询每台设备。 有关 DeviceWatcher 的更多详细信息，请参阅完整[Github 上](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/DeviceEnumerationAndPairing)的示例。 

## <a name="connecting-to-the-device"></a>连接到设备
一旦发现所需的设备，则使用[DeviceInformation.Id](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.enumeration.deviceinformation.id)获取设备的蓝牙 LE 设备对象中的问题： 
...
```
另一方面，释放对 BluetoothLEDevice 的所有引用设备对象 （和系统上没有其他的应用是否具有对该设备的引用） 将会触发自动在较小的超时后断开连接。 

```csharp
bluetoothLeDevice.Dispose();
```
如果应用需要再次访问该设备，则只需重新创建设备对象并访问特性 （在下一节中讨论） 将触发操作系统在必要时重新连接。 如果设备在附近，你将收到访问设备否则为它将返回带有 DeviceUnreachable 错误。  

## <a name="enumerating-supported-services-and-characteristics"></a>枚举受支持的服务和特性
既然你已拥有 BluetoothLEDevice 对象下, 一步是发现设备公开的数据。 若要执行此操作的第一步是查询服务： 
...
```  
操作系统返回 GattCharacteristic 的只读列表对象，然后可以上执行操作。

## <a name="perform-readwrite-operations-on-a-characteristic"></a>读/写上执行操作的特征

特点是 GATT 的基本单元基于通信。 它包含一个值，表示一个不同的设备上的数据。 例如，电池级别特征都有一个值，表示设备的电池剩余电量。
...
> **预留**： 指示被视为更可靠，因为每个值更改事件结合确认从客户端设备。 通知是更普遍，因为大多数 GATT 交易记录将而节省电源而不是非常可靠。 在任何情况下，所有的处理在控制器层因此应用不会不获取会涉及到。 我们将共同称为它们只需"notifications"，但现在你已知道。 

有两个需要获取通知之前负责的事项：
- 写入到客户端特征配置描述符 (CCCD)
- 处理 Characteristic.ValueChanged 事件

写入 CCCD 告知此客户端想知道该特定特征值更改的每次的服务器设备。 若要实现此目的，请执行以下操作： 
