...
> [!TIP]
> 本主题中的示例基于[通用 Windows 平台（uwp）打印示例](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/Printing)，它是 GitHub 上[通用 Windows 平台（uwp）应用示例](https://github.com/Microsoft/Windows-universal-samples)存储库的一部分。
...
> [!TIP]
> If 你需要支持在应用中的多个页面上打印，你可以将此打印代码放在一个公共帮助器类中，并让你的应用页重用它。 有关具体做法的示例，请参阅 [UWP 打印示例](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/Printing)中的 `PrintHelper` 类。
...
在此示例中，事件处理程序在 `UnregisterForPrinting` 方法中被注销。
...
> [!IMPORTANT]
> 如果用户更改打印设置，将再次调用分页事件处理程序，以允许您重排内容。 为了获得最佳的用户体验，我们建议在重新排列内容之前检查设置，以避免在不必要时重新初始化分页内容。
...
> [!NOTE]
> 这些选项将按附加的顺序显示在打印预览 UI 中，第一个选项显示在窗口顶部。
...
> [!TIP]
> 有关如何解析用户在“范围”文本框中输入的页面范围的详细信息，请参阅 `GetPagesInRange`[UWP 打印示例](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/Printing)中的方法。
...
* [//生成2015视频：开发在 Windows 10](https://channel9.msdn.com/Events/Build/2015/2-94) 中打印的应用
