...
* **范围**： 解码**DecodeLength1**和**DecodeLength2**单字节字符之间的长度。 **DecodeLength1**和**DecodeLength2** do 无关紧要 （无论是可以是更高版本或低于其他） 的顺序。
...
你还可以设置是否将经过条形码检查数字。 将此设置之前，请确保标志支持检查[IsCheckDigitValidationSupported](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.barcodesymbologyattributes.ischeckdigitvalidationsupported)数字验证。 然后，设置是否与[IsCheckDigitValidationEnabled](https://docs.microsoft.com/uwp/api/windows.devices.pointofservice.barcodesymbologyattributes.ischeckdigitvalidationenabled)启用检查数字验证。
