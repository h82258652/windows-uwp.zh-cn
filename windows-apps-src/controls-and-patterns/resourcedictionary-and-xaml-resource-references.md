...
资源不需要作为字符串；它们可以作为任何可共享的对象，例如样式、目标、画笔和颜色。 但是，控件、形状和其他 [FrameworkElement](https://msdn.microsoft.com/library/windows/apps/br208706) 不可共享，因此不能将其声明为可重复使用的资源。 有关共享的详细信息，请参阅本主题后面的 [XAML 资源必须可共享](#xaml-resources-must-be-sharable)部分。
