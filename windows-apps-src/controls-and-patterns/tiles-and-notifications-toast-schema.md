...
如果你希望使用原始 XML 而非[通知库](https://www.nuget.org/packages/Microsoft.Toolkit.Uwp.Notifications/)，请参阅 [XML 架构](tiles-and-notifications-toast-xml-schema.md)。
...
| **BaseUri** | Uri | 否 | 与图像源属性中的相对 URL 组合的默认基本 URL。 |
| **AddImageQuery** | bool? | 否 | 设置为“是”将使 Windows 能够将查询字符串附加到在 toast 通知中提供的图像 URL。 如果你的服务器托管图像并且可以处理查询字符串，则可通过基于查询字符串搜索图像变量，或通过忽略查询字符串并且不带查询字符串按指定方式返回图像，来使用此特性。 此查询字符串指定缩放倍数、对比度设置和语言；例如，通知中提供的“www.website.com/images/hello.png”值将变为“www.website.com/images/hello.png?ms-scale=100&ms-contrast=standard&ms-lang=en-us” |
...
| 属性 | 类型 | 必需 |描述 |
|---|---|---|---|
| **Text** | 字符串或 BindableString | 否 | 要显示的文本。 在创意者更新中添加了数据绑定支持，但只适用于顶级文本元素。 |
...
| 属性 | 类型 | 必需 | 描述 |
|---|---|---|---|
| **url** | uri | 否 | 播放来代替默认声音的媒体文件。 仅支持 ms-appx 和 ms-appdata。 |
| **loop** | 布尔值 | 否 | 如果声音应在 Toast 显示时不断重复，则设置为 true；设置为 false 将仅播放一次（默认设置）。 |
| **silent** | 布尔值 | 否 | 设置为 true 将声音静音；设置为 false 允许播放 toast 声音（默认设置）。 |


## <a name="toastheader"></a>ToastHeader
在操作中心内将多个通知分组在一起的自定义标头。

| 属性 | 类型 | 必需 | 描述 |
|---|---|---|---|
| **id** | 字符串 | 是 | 开发人员创建的标识符，用以唯一标识此标头。 如果两个通知具有相同的标头 id，它们将显示在操作中心中的同一标头下。 |
| **title** | 字符串 | 是 | 标头的标题。 |
| **action**| 操作 | 否 | 声明用户单击 toast 时要执行哪些操作。 |
