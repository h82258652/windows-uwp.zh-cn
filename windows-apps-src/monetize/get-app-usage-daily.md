...
description: 在 Microsoft Store 分析 API 中使用此方法以获取每日应用使用情况数据的给定的日期范围和其他可选的筛选器。
title: 获取每日应用使用情况
ms.date: 08/15/2018
ms.topic: article
...
下面的示例演示了用于获取每日应用使用情况数据请求。 将 *applicationId* 值替换为你的应用的存储 ID。
...
| dailySessionCount         | 长整型    | 该天的用户会话数。                                  |
| engagementDurationMinutes | Double  | 在其中用户会主动使用您的应用程序的非重复的一段时间，在应用启动时启动 （进程开始） 和结束时终止 （进程结束） 或处于非活动状态一段时间后的分钟。             |
| dailyActiveUsers          | 长整型    | 使用这一天的应用程序的客户数。                           |
| dailyActiveDevices        | 长整型    | 与您的应用程序进行交互的所有用户使用的每日设备数。  |
| dailyNewUsers             | 长整型    | 第一次该天使用您的应用程序的客户数。    |
