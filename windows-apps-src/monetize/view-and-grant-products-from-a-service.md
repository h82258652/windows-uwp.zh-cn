...
4.  在客户端 Windows 应用中，[创建 Microsoft Store ID 密钥](#step-4)表示当前用户和传递此密钥的身份返回到你的服务。
5.  在你具有所需的 Azure AD 访问令牌和 Microsoft Store ID 密钥后，[从你的服务调用 Microsoft Store 收集 API 或购买 API](#step-5)。

此端到端过程涉及到执行不同的任务的两个软件组件：

* **你的服务**。 这是你的业务环境的上下文中安全地运行的应用程序，可以使用你选择的任何开发平台实现它。 创建 Azure AD 访问令牌方案和需要调用 REST Uri 的 Microsoft 应用商店收集 API 和购买 API，你的服务负责有效。
...
你可以使用 Microsoft Store 收集 API 或购买 API 之前，必须创建 Azure AD Web 应用程序、 检索租户 ID 和应用程序的应用程序 ID 和生成一个密钥。 Azure AD Web 应用程序是指你想要调用 Microsoft Store 收集 API 或购买 API 的服务。 你需要生成需要调用该 API 的 Azure AD 访问令牌的租户 ID、 应用程序 ID 和密钥。

> [!NOTE]
> 你只需执行一次本部分中任务。 更新你的 Azure AD 应用程序清单并获取租户 ID、 应用程序 ID 和客户端密钥后，你可以重复使用这些值你需要创建一个新的任何时间 Azure AD 访问令牌。

1.  如果你没有这样做，请按照[与 Azure Active Directory 集成应用程序](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications)中的说明来注册**Web 应用 / API**与 Azure AD 应用程序。
    > [!NOTE]
    > 当你注册你的应用程序时，你必须选择**Web 应用 / API**为应用程序类型，以便你可以为你的应用程序检索密钥 （也称为*客户端密码*）。 若要调用 Microsoft Store 收集 API 或购买 API，必须在稍后步骤从 Azure AD 中请求访问令牌时提供客户端密码。

2.  在[Azure 管理门户](https://portal.azure.com/)中，导航到**Azure Active Directory**。 选择你的目录，在左侧的导航窗格中，单击**应用注册**，然后选择你的应用程序。
3.  你将转到应用程序的主注册页。 在此页面上，将复制以供稍后使用的**应用程序 ID**值。
4.  创建一个密钥，你将需要更高版本 （这调用所有*客户端密码*）。 在左侧窗格中，单击**设置**，然后选择**密钥**。 在此页面上，完成[创建密钥](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications#to-add-application-credentials-or-permissions-to-access-web-apis)的步骤。 复制以供以后使用此密钥。
5.  将多个所需的受众 Uri 添加到你的[应用程序清单](https://docs.microsoft.com/azure/active-directory/develop/active-directory-application-manifest)中。 在左侧窗格中，单击**清单**。 单击**编辑**，替换`"identifierUris"`带有以下文本部分，然后再单击**保存**。
...
你可以使用 Microsoft Store 收集 API 或购买 API 以配置的所有权和购买应用或加载项之前，必须在开发人员中心仪表板来将你的 Azure AD 应用程序 ID 与应用 （或者包含加载项的应用） 相关联。
...
* *Client\_id*和*client\_secret*参数，指定应用程序 ID 和你从[Azure 管理门户](http://manage.windowsazure.com)中检索的应用程序客户端密码。 若要创建带有 Microsoft Store 收集 API 或购买 API 所需的身份验证级别的访问令牌，这两个参数都是必需的。
...
2.  在你的应用代码中，调用以下方法之一以检索 Microsoft Store ID 密钥：
  ...
  * 如果你的应用使用 [Windows.ApplicationModel.Store](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store) 命名空间中的 [CurrentApp](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Store.CurrentApp) 类来管理应用内购买，请使用 [CurrentApp.GetCustomerCollectionsIdAsync](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store.currentapp.getcustomercollectionsidasync) 方法。
    ...
    将 Azure AD 访问令牌传递给该方法的 *serviceTicket* 参数。 如果你维护匿名用户 Id 作为当前应用的发布者管理的服务的上下文中，还传递给*publisherUserId*参数以将当前用户与新的 Microsoft 应用商店 ID 密钥 （用户 ID 将 em 相关联的用户 IDbedded 项中)。 否则，如果你不需要用户 ID 相关联的 Microsoft 应用商店 ID 密钥，你可以向*publisherUserId*参数传递任何字符串值。
...
下图描述的 Microsoft 应用商店收集 API 或购买 API 中调用的方法，从你的服务的过程。
