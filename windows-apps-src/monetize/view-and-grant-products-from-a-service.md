...
* **你的服务**。 这是你的业务环境的上下文中安全地运行的应用程序，可以使用你选择任何开发平台实现它。 创建 Azure AD 访问令牌所需的方案和用于调用 REST Uri 的 Microsoft 应用商店收集 API 和购买 API，你的服务负责有效。
...
你可以使用 Microsoft Store 收集 API 或购买 API 之前，必须创建 Azure AD Web 应用程序、 检索租户 ID 和应用程序的应用程序 ID 并生成一个密钥。 Azure AD Web 应用程序是指要从中调用 Microsoft Store 收集 API 或购买 API 的服务。 你需要生成你需要调用该 API 的 Azure AD 访问令牌的租户 ID、 应用程序 ID 和密钥。

> [!NOTE]
> 你只需执行一次本部分中任务。 在更新 Azure AD 应用程序清单，并且你有租户 ID、 应用程序 ID 和客户端密钥后，你可以重复使用这些值的每当你需要创建一个新 Azure AD 访问令牌。
...
4.  创建一个密钥，你将需要更高版本 （这调用所有*客户端密钥*）。 在左侧窗格中，单击**设置**，然后选择**密钥**。 在此页上，完成[创建密钥](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications#to-add-application-credentials-or-permissions-to-access-web-apis)的步骤。 复制以供以后使用此密钥。
5.  将多个所需的受众 Uri 添加到你的[应用程序清单](https://docs.microsoft.com/azure/active-directory/develop/active-directory-application-manifest)。 在左侧窗格中，单击**清单**。 单击**编辑**，替换`"identifierUris"`部分具有以下文本，然后依次**保存**。
...
你可以使用 Microsoft Store 收集 API 或购买 API 以配置的所有权和购买你的应用或加载项之前，必须在合作伙伴中心中将 Azure AD 应用程序 ID 关联与应用 （或者包含加载项的应用）。
...
* 对于*client\_id*和*client\_secret*参数，指定应用程序 ID 和你从[Azure 管理门户](https://manage.windowsazure.com)中检索的应用程序客户端密码。 若要创建带有 Microsoft Store 收集 API 或购买 API 所需的身份验证级别的访问令牌，这两个参数都是必需的。

* 对于*资源*参数，请指定[上一节](#access-tokens)中列出的受众 URI 之一，具体取决于要创建的访问令牌的类型。

在你的访问令牌到期后，可以按照[此处](https://azure.microsoft.com/documentation/articles/active-directory-protocols-oauth-code/#refreshing-the-access-tokens)的说明刷新令牌。 有关访问令牌的结构的更多详细信息，请参阅[支持的令牌和声明类型](https://go.microsoft.com/fwlink/?LinkId=722501)。
...
2.  在你的应用代码中，调用以下方法之一以检索 Microsoft Store ID 密钥：
  ...
  * 如果你的应用使用 [Windows.ApplicationModel.Store](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store) 命名空间中的 [CurrentApp](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Store.CurrentApp) 类来管理应用内购买，请使用 [CurrentApp.GetCustomerCollectionsIdAsync](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store.currentapp.getcustomercollectionsidasync) 方法。
    ...
    将 Azure AD 访问令牌传递给该方法的 *serviceTicket* 参数。 如果你维护匿名用户 Id 作为当前应用的发布者管理的服务的上下文中，你还可以传递的用户 ID 向*publisherUserId*参数当前用户关联 （该用户 ID 将是 em 新的 Microsoft 应用商店 ID 密钥bedded 项中)。 否则，如果你不需要用户 ID 相关联的 Microsoft 应用商店 ID 密钥，你可以向*publisherUserId*参数传递任何字符串值。
...
下图描述的 Microsoft 应用商店收集 API 或购买 API 中调用方法，从你的服务的过程。
