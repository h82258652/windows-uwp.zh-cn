...
你可以使用 Microsoft Store 收集 API 或购买 API 之前，你必须创建 Azure AD Web 应用程序、 检索租户 ID 和应用程序的应用程序 ID 和生成一个密钥。 Azure AD Web 应用程序是指要从中调用 Microsoft Store 收集 API 或购买 API 的服务。 你需要的租户 ID、 应用程序 ID 和密钥，以生成你需要调用该 API 的 Azure AD 访问令牌。

> [!NOTE]
> 你只需执行一次本部分中任务。 在更新 Azure AD 应用程序清单，并且已准备你的租户 ID、 应用程序 ID 和客户端密钥后，你可以重复使用这些值你需要创建一个新的任何时间 Azure AD 访问令牌。
...
1.  如果你尚未操作，请按照[与 Azure Active Directory 集成应用程序](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications)中的说明来注册**Web 应用 / API**与 Azure AD 应用程序。
    > [!NOTE]
    > 当你注册你的应用程序时，你必须选择**Web 应用 / API**为应用程序类型，以便你可以为你的应用程序检索密钥 （也称为一个*客户端密码*）。 若要调用 Microsoft Store 收集 API 或购买 API，必须在稍后步骤从 Azure AD 中请求访问令牌时提供客户端密码。
...
4.  创建密钥，你将需要更高版本 （这调用所有*客户端密码*）。 在左侧窗格中，单击**设置**，然后选择**密钥**。 在此页上，完成[创建密钥](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications#to-add-application-credentials-or-permissions-to-access-web-apis)的步骤。 复制以供以后使用此密钥。
5.  将多个所需的受众 Uri 添加到你的[应用程序清单](https://docs.microsoft.com/azure/active-directory/develop/active-directory-application-manifest)中。 在左侧窗格中，单击**清单**。 单击**编辑**，替换`"identifierUris"`部分与以下文本，并单击**保存**。
...
你可以使用 Microsoft Store 收集 API 或购买 API 以配置的所有权和购买你的应用或加载项之前，必须在开发人员中心仪表板来将你的 Azure AD 应用程序 ID 与此应用 （或者包含加载项的应用） 相关联。
...
* *Client\_id*和*client\_secret*参数，指定应用程序 ID 和你从[Azure 管理门户](http://manage.windowsazure.com)中检索的应用程序客户端密钥。 若要创建带有 Microsoft Store 收集 API 或购买 API 所需的身份验证级别的访问令牌，这两个参数都是必需的。
