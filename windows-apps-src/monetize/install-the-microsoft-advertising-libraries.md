...
要在 Windows 应用中显示广告，请安装以下 SDK 之一：

* 对于 Windows10 上的 UWP 应用，请安装 [Microsoft 广告 SDK](http://aka.ms/ads-sdk-uwp)。 此 SDK 是 Visual Studio 2015 和更高版本的扩展。
* 对于 Windows 8.1 和 Windows Phone 8.x 应用，请安装[适用于 Windows 和 Windows Phone 8.x 的 Microsoft 广告 SDK](http://aka.ms/store-8-sdk)。 此 SDK 是 Visual Studio 2015 和 Visual Studio 2013 的扩展。

> [!NOTE]
> 如果开发的是 JavaScript/HTML UWP 应用，则除了安装 Windows10 SDK (14393) 或更高版本以外，还必须安装 WinJS 库。 此库过去包含在以前版本的 Windows10 SDK 中，但从 Windows10 SDK (14393) 开始，此库必须单独安装。 若要安装 WinJS，请参阅[获取 WinJS](http://try.buildwinjs.com/download/GetWinJS/)。
...
3.  下载和安装 [Microsoft 广告 SDK](http://aka.ms/ads-sdk-uwp)（对于 Windows 10 上的 UWP 应用）或[适用于 Windows 和 Windows Phone 8.x 的 Microsoft 广告 SDK](http://aka.ms/store-8-sdk)（对于 Windows 8.1 和 Windows Phone 8.x 上的 XAML 及 JavaScript/HTML 应用）。 安装它可能需要几分钟。 请确信并等待，直到进程结束。
...
## <a name="install-via-nuget-uwp-only"></a>通过 NuGet 安装（仅限 UWP）
...
3. 在**引用管理器**中，根据你的项目类型选择以下引用之一：

    -   对于通用 Windows 平台 (UWP) 项目：请展开**通用 Windows**，单击**扩展**，然后选择**适用于 XAML 的 Microsoft 广告 SDK** 或**适用于 JavaScript 的 Microsoft 广告 SDK**（适用于使用 JavaScript 和 HTML 构建的应用）旁边的复选框。

    -   对于 Windows 8.1 的项目：请展开**Windows 8.1**，单击**扩展**，然后选中**适用于 Windows 8.1 XAML 的 Ad Mediator SDK**（适用于 XAML 应用）或**适用于 Windows 8.1 本机 (JS) 的 Microsoft 广告 SDK**（适用于使用 JavaScript 和 HTML 构建的应用）旁边的复选框。

    -   对于 Windows Phone 8.1 的项目：请展开**Windows Phone 8.1**，单击**扩展**，然后选中**适用于 Windows Phone 8.1 XAML 的 Ad Mediator SDK**（适用于 XAML 应用）或**适用于 Windows Phone 8.1 本机 (JS) 的 Microsoft 广告 SDK**（适用于使用 JavaScript 和 HTML 构建的应用）旁边的复选框。
...
<span id="framework" />
## <a name="understanding-framework-packages-in-the-microsoft-advertising-sdk-uwp-only"></a>了解 Microsoft 广告 SDK (UWP only) 中的框架包
