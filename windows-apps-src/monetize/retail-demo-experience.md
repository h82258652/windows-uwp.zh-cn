...
ms.locfileid: "9117667"
---
# <a name="add-retail-demo-rdx-features-to-your-app"></a>零售演示 (RDX) 功能添加到你的应用

在你的 Windows 应用中包括零售演示模式，以便客户试用电脑和设备上销售场地可以直接跳转中。

当客户在零售商店中时，他们希望能够试用的电脑和设备的演示。 他们通常会花费相当大块的他们玩[零售演示体验 (RDX)](https://docs.microsoft.com/windows-hardware/customize/desktop/retail-demo-experience)应用的时间。

你可以设置你的应用提供不同体验在_正常_或_零售_模式。 例如，如果你的应用启动与安装过程中，你可以在零售模式下，跳过它，预填充应用的示例数据和默认设置，以便他们可以直接跳转中。

从客户的角度来看，没有只能在一个应用。 为了帮助客户区分两种模式，我们建议应用在零售模式下时，它显示单词"零售"突出的标题栏中或在合适的位置。
...
* **付费应用必须具有试用模式**。 你的应用也需要免费或包括[试用模式](https://msdn.microsoft.com/windows/uwp/monetize/exclude-or-limit-features-in-a-trial-version-of-your-app)。 客户不希望为零售商店中的体验付费。
...
* **已更新内容体验**。 你的应用应永远不会提示更新联机时。 如果需要更新，应以静默方式执行它们。

* **无匿名通信**。 使用零售演示设备的客户是匿名用户，因为它们不应能够从设备消息或共享内容。

* **提供一致的体验，使用清理过程**。 当客户走到零售演示设备前时，每个客户都应具有相同的体验。 你的应用应使用[清理过程](#cleanup-process)在每次使用后返回到相同的默认状态。 我们不希望以查看哪些最后一个客户留下的下一个客户。 这包括计分牌、成就和解锁。
...
* **能够在广泛的设备上成功运行**。 应用必须也在所有设备，包括带有低端规范的设备上运行。 如果不满足最低要求的设备上安装应用，则应用将需要清楚地告知用户这。 必须公布最低设备要求，以便应用可以始终高性能地运行。
...
[**RetailInfo**](https://docs.microsoft.com/uwp/api/Windows.System.Profile.RetailInfo)实用程序类，这是 Windows 10 SDK 中的[Windows.System.Profile](https://docs.microsoft.com/uwp/api/windows.system.profile)命名空间的一部分，在[**IsDemoModeEnabled**](https://docs.microsoft.com/uwp/api/windows.system.profile.retailinfo.isdemomodeenabled)属性用作的布尔值指示器用来指定哪个代码路径的运行你的应用_法线_模式或_零售_模式。
...
清理开始两分钟后购物者停止与设备交互。 零售演示播放，并且 Windows 开始重置联系人、 照片和其他应用中的任何示例数据。 具体取决于设备，这可能需要 1-5 分钟内以完全所有内容都将重置为正常。 这将确保零售商店中的每个客户可以走到设备，与设备交互时获得相同的体验。
