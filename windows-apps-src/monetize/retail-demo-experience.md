...
从客户的角度，没有只能在一个应用。 为了帮助客户区分两种模式，我们建议应用在零售模式下时，它显示单词"零售"突出的标题栏中或在合适的位置。
