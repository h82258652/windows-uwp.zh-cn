---
author: mcleanbyron
description: "使用此部分中的 Java 代码示例了解有关使用 Windows 应用商店提交 API 提交游戏选项和预报片的详细信息。"
...
ms.prod: windows
ms.technology: uwp
keywords: "windows 10, uwp, Windows 应用商店提交 API, 代码示例, 游戏选项, 预告片, 高级应用一览, java"
...
```CreateAndSubmitSubmissionExample``` 类实现 ```main``` 程序，该程序调用其他示例方法，以使用 Windows 应用商店提交 API 来创建并确认包含游戏选项和预告片的应用提交。 要调整此代码供自己使用，请执行以下操作：
...
```DevCenterAccessTokenClient``` 类定义一个帮助程序方法，该方法使用 ```tenantId```、```clientId``` 和 ```clientSecret``` 值来创建要用于 Windows 应用商店提交 API 的 Azure AD 访问令牌。
...
```DevCenterClient``` 类定义帮助程序方法，这些方法在 Windows 应用商店提交 API 中调用多种方法并上传一个 ZIP 文件，其中包含应用提交的程序包、应用一览图像和预告片文件。
