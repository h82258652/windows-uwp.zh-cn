...
ms.locfileid: "57636742"
---
# <a name="code-your-app-for-experimentation"></a>针对实验为你的应用编码

检查完[创建项目并在合作伙伴中心定义远程变量](create-a-project-and-define-remote-variables-in-the-dev-center-dashboard.md)，已准备好更新到通用 Windows 平台 (UWP) 应用程序中的代码：
* 从合作伙伴中心收到远程变量值。
* 使用远程变量为你的用户配置应用体验。
* 将事件记录到合作伙伴中心，指示当用户有查看你的试验，并且执行了所需的操作 (也称为*转换*)。

若要向你的应用添加此行为，请使用 Microsoft Store Services SDK 所提供的 API。

以下部分介绍获取变体的实验和日志事件记录到合作伙伴中心的一般的过程。 编码的试验应用后，你可以[在合作伙伴中心定义试验](define-your-experiment-in-the-dev-center-dashboard.md)。 有关演示如何创建并运行实验的端到端过程的演练，请参阅[通过 A/B 测试来创建并运行你的第一个实验](create-and-run-your-first-experiment-with-a-b-testing.md)。

> [!NOTE]
> 一些试验中 Microsoft Store Services SDK 的 Api 使用[异步模式](../threading-async/asynchronous-programming-universal-windows-platform-apps.md)从合作伙伴中心中检索数据。 这意味着，这些方法的部分执行可能会在调用这些方法后发生，以便你的应用的 UI 可以一边保持响应，一边完成操作。 异步模式要求你的应用在调用该 API 时使用 **async** 关键字和 **await** 运算符，如本文中的代码示例所示。 按照惯例，异步方法以 **Async** 结尾。
...
在你的项目中，找到你想要在实验中修改的功能代码。 添加检索数据的一种变体的代码、 使用此数据来修改测试时，该功能的行为，然后将你的试验的视图事件记录到 A / B 测试在合作伙伴中心服务。
...
1. 声明[StoreServicesExperimentVariation](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicesexperimentvariation)对象，表示当前的变体分配和一个[StoreServicesCustomEventLogger](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicescustomeventlogger)对象，将使用日志视图和转换合作伙伴中心的事件。

    [!code-cs[ExperimentExamples](./code/StoreSDKSamples/cs/ExperimentExamples.cs#Snippet1)]

2. 声明将分配给要检索的实验的[项目 ID](run-app-experiments-with-a-b-testing.md#terms) 的字符串变量。
    > [!NOTE]
    > 获取项目 ID 时您[在合作伙伴中心创建一个项目](create-a-project-and-define-remote-variables-in-the-dev-center-dashboard.md)。 以下所示的项目 ID 仅用作示例。
...
7. 最后，记录[视图事件](run-app-experiments-with-a-b-testing.md#terms)实验到 A / B 测试在合作伙伴中心服务。 将 ```logger``` 字段初始化为 [StoreServicesCustomEventLogger](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicescustomeventlogger) 对象，然后调用 [LogForVariation](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicescustomeventlogger.logforvariation) 方法。 传递[StoreServicesExperimentVariation](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicesexperimentvariation)对象，表示当前的变体分配 （此对象到合作伙伴中心提供有关事件的上下文） 和试验的视图事件的名称。 它必须匹配在合作伙伴中心实验输入的视图事件名称。 你的代码应在用户开始查看实验的一部分变体时记录视图事件。
...
## <a name="log-conversion-events-to-partner-center"></a>转换将事件记录到合作伙伴中心

接下来，添加代码，以记录[转换事件](run-app-experiments-with-a-b-testing.md#terms)到 A / B 测试在合作伙伴中心服务。 你的代码应在用户达到实验目标时记录转换事件。 你需要的具体代码将依应用而定，但在此处是一般步骤。 有关完整的代码示例，请参阅[利用 A/B 测试创建和运行你的第一个试验](create-and-run-your-first-experiment-with-a-b-testing.md)。

1. 在用户达到实验其中一个目标时运行的代码中，重新调用 [LogForVariation](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicescustomeventlogger.logforvariation) 方法，并传递 [StoreServicesExperimentVariation](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicesexperimentvariation) 对象和实验的转换事件名称。 它必须匹配在合作伙伴中心实验输入的转换事件名称之一。
...
2. [运行和管理在合作伙伴中心实验](manage-your-experiment.md)。
