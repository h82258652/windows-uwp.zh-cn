...
| 方法        | URI    | 描述                                                                 |
|---------------|--------|-----------------------------------------------------------------------------|
| GET | ```https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions/{submissionId}``` | 获取现有应用提交的数据。 有关详细信息，请参阅[获取应用提交](get-an-app-submission.md)。 |
| GET | ```https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions/{submissionId}/status``` | 获取现有应用提交的状态。 有关详细信息，请参阅[获取应用提交的状态](get-status-for-an-app-submission.md)。 |
| POST | ```https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions``` | 为已注册到 Windows 开发人员中心帐户的应用创建新提交。 有关详细信息，请参阅[创建应用提交](create-an-app-submission.md)。 |
| POST | ```https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions/{submissionId}/commit``` | 将新的或更新的应用提交到 Windows 开发人员中心。 有关详细信息，请参阅[确认应用提交](commit-an-app-submission.md)。 |
| PUT | ```https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions/{submissionId}``` | 更新现有的应用提交。 有关详细信息，请参阅[更新应用提交](update-an-app-submission.md)。 |
| DELETE | ```https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions/{submissionId}``` | 删除应用提交。 有关详细信息，请参阅[删除应用提交](delete-an-app-submission.md)。 |
...
3. [获取 Azure AD 访问令牌](create-and-manage-submissions-using-windows-store-services.md#obtain-an-azure-ad-access-token)。 在 Windows 应用商店提交 API 中，必须将此访问令牌传递给相关方法。 获取访问令牌后，在它到期前，你有 60 分钟的使用时间。 该令牌到期后，可以获取新的令牌。

4. 在 Windows 应用商店提交 API 中执行以下方法。 此方法会创建新的正在进行的提交，这是你上一发布的提交副本。 有关详细信息，请参阅[创建应用提交](create-an-app-submission.md)。

  ```
  POST https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions
  ```

  响应正文包含三个项目：新的提交 ID、新的提交数据（包括所有列表和定价信息），以及用于上载提交的任何应用包和列表图像的共享访问签名 (SAS) URI。 有关 SAS 的详细信息，请参阅[共享访问签名，第 1 部分：了解 SAS 模型](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)。

3. 如果要为提交添加新的程序包或图像，请[准备应用包](https://msdn.microsoft.com/windows/uwp/publish/app-package-requirements)并[准备应用屏幕截图和图像](https://msdn.microsoft.com/windows/uwp/publish/app-screenshots-and-images)。 将所有这些文件添加到 ZIP 存档。

4. 使用新提交所需的任何更改来更新提交数据，并执行以下方法来更新提交。 有关详细信息，请参阅[更新应用提交](update-an-app-submission.md)。

  ```
  PUT https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions/{submissionId}
  ```

  >**注意**&nbsp;&nbsp;如果要为提交添加新的程序包或图像，请确保更新提交数据以便在 ZIP 存档中引用这些文件的名称和相对路径。
...
5. 通过执行以下方法确认提交。 这将向开发人员中心发出警报：已完成提交，更新现在应该应用到了帐户。 有关详细信息，请参阅[确认应用提交](commit-an-app-submission.md)。

  ```
  POST https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions/{submissionId}/commit
  ```

6. 通过执行以下方法来检查提交状态。 有关详细信息，请参阅[获取应用提交的状态](get-status-for-an-app-submission.md)。
...
| applicationPackages           |   数组  | 包含提供提交中关于每个程序包详细信息的对象。 有关详细信息，请参阅下面的[应用程序包](#application-package-object)部分。 |    
...
|  priceId               |   字符串      |  用于指定应用[基价](https://msdn.microsoft.com/windows/uwp/publish/define-pricing-and-market-selection#base-price)的[价格段](#price-tier)。   |


<span id="sale-object" />
### 销售

此资源包含应用的销售信息。 此资源具有以下值。
