...
> [!IMPORTANT]
> 如果你使用 Windows 应用商店提交 API 创建应用提交，请务必只使用此 API 而非开发人员中心仪表板对提交进行进一步更改。 如果你使用仪表板更改你最初使用此 API 创建的提交，则将无法再使用此 API 更改或提交该提交。 在某些情况下，在提交过程中无法继续进行时，提交可能会处于错误状态。 如果发生这种情况，你必须删除提交并创建新的提交。
...
4. 如果要为提交添加新的程序包、应用一览图像或预告片文件，请[准备应用包](https://msdn.microsoft.com/windows/uwp/publish/app-package-requirements)并[准备应用屏幕截图、图像和预告片](https://msdn.microsoft.com/windows/uwp/publish/app-screenshots-and-images)。 将所有这些文件添加到 ZIP 存档。
    > [!NOTE]
    > 部分开发者帐户目前还不能使用此 API 提交应用提交的预告片。 如果你的帐户无法访问此资源，当你获取或创建提交时，[应用提交资源](#app-submission-object)中的 *trailers* 数组为 null。
...
某些提交可能无法使用以下两组高级应用一览选项：[游戏选项](#gaming-options-object)和[预告片](#trailer-object)。

这些应用一览选项是在向开发人员首次发布 Windows 应用商店提交 API 后添加的。 如果你在这些应用一览选项推出前通过提交 API 为应用创建了提交并且此提交仍在进行中，则在成功确认提交或删除提交前，你将无法访问这些应用一览选项。

在使用提交 API 时，要确定能否为给定应用访问上述应用一览选项，请使用[获取应用](get-an-app.md)方法并检查[应用程序资源](get-app-data.md#application_object)的 *hasAdvancedListingPermission* 字段是否为 true。 如果无法为某个应用访问上述应用一览选项，则在[创建应用提交](create-an-app-submission.md)时，你的[应用提交资源](#app-submission-object)中的 *gamingOptions* 和 *trailers* 值为 null。
...
| gamingOptions           |  数组 |   一个包含[游戏选项资源](#gaming-options-object)的数组，用于为应用定义游戏相关设置。<br/><br/>**注意：**&nbsp;&nbsp;部分开发者帐户目前还不能使用此 API 配置游戏选项。 如果你的帐户无法访问此资源，则 *gamingOptions* 值为 null。 要确定能否为应用提交配置 *gamingOptions*，请使用[获取应用](get-an-app.md)方法并检查[应用程序资源](get-app-data.md#application_object)的 *hasAdvancedListingPermission* 字段是否为 true。      |   
...
| trailers           |  数组 |   一个包含最多 15 个[预告片资源](#trailer-object)的数组，用于表示应用一览的视频预告片。<br/><br/>**注意：**&nbsp;&nbsp;部分开发者帐户目前还不能使用此 API 提交应用提交的预告片。 如果你的帐户无法访问此资源，则 *trailers* 值为 null。 要确定能否为应用提交配置 *trailers*，请使用[获取应用](get-an-app.md)方法并检查[应用程序资源](get-app-data.md#application_object)的 *hasAdvancedListingPermission* 字段是否为 true。   |  
...
> [!IMPORTANT]
> **销售**资源不再受支持，并且当前不能使用 Windows 应用商店提交 API 获取或修改应用提交的销售数据。 将来，我们将更新 Windows 应用商店提交 API，以引入以编程方式访问应用提交的销售信息的新方法。
...
|  title               |     字符串    |   应用一览的标题。   |  
|  shortDescription               |     字符串    |  此值仅适用于 Xbox 应用一览。 用于为应用一览指定简短描述，这有助于用户了解你的游戏内容、游戏类型以及哪些类型的玩家可能会喜欢此款游戏。 条目长度不得超过 500 个字符。   |  
|  shortTitle               |     字符串    |  此值仅适用于 Xbox 应用一览。 用于为常规描述太长的上下文指定简短版本的产品描述。 简短标题的所有版本（包括标准、本地化和替代标题）的长度都不得超过 50 个字符。 替代版本在使用替代语言的单一区域显示。    |  
|  sortTitle               |     字符串    |   此值仅适用于 Xbox 应用一览。 用于为应用一览指定排序标题。 条目长度不得超过 255 个字符。   |  
|  voiceTitle               |     字符串    |   此值仅适用于 Xbox 应用一览。 用于为应用一览指定语音标题。 条目长度不得超过 255 个字符。    |  
...
> [!NOTE]
> 此资源[可能无法用于某些提交](#advanced-listings)。

如果你在这些应用一览选项推出前通过提交 API 为应用创建了提交并且此提交仍在进行中，则在成功确认提交或删除提交前，你将无法为此应用的任何提交访问这些应用一览选项。 在使用提交 API 时，要确定能否为给定应用访问上述应用一览选项，请使用[获取应用](get-an-app.md)方法并检查[应用程序资源](get-app-data.md#application_object)的 *hasAdvancedListingPermission* 字段是否为 true。
...
|  kinectDataForExternal               |   字符串      |  指定以下字符串值之一，表示游戏是否可以收集 Kinect 数据并将其发送到外部服务： <ul><li>NotSet</li><li>Unknown</li><li>Enabled</li><li>Disabled</li></ul>   |
...
### <a name="trailer-resource"></a>预告片资源
