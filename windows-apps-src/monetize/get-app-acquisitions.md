...
| 参数        | 类型   |  说明      |  必需  
|---------------|--------|---------------|------|
| applicationId | 字符串 | 要检索购置数据的应用的存储 ID。 存储 ID 在开发人员中心仪表板的[应用标识页](../publish/view-app-identity-details.md)上提供。 存储 ID 的一个示例是 9WZDNCRFJ3Q8。 |  是  |
...
| orderName | 指定促销充值码（用于获取应用）订单名称的字符串（仅当用户通过兑换促销充值码获取应用后才可用）。 |
