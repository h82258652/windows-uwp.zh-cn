...
| 名称         | 类型  | 说明   |  必填  |
|----------------|--------|-------------|-----------|
| recurrenceId | 字符串 | 想要更改的订阅 ID。 要获取此 ID，请调用[获取用户的订阅](get-subscriptions-for-a-user.md)方法和标识响应正文项值，该值代表您想要更改的订阅加载项的项中使用的**id**字段值。     | 是      |
...
| expirationTimeWithGrace | 字符串 | 过期日期和时间订阅将宽限期，包括以 ISO 8601 格式。 此值指示当该用户将失去对订阅的访问之后未能自动续订订阅。    |
