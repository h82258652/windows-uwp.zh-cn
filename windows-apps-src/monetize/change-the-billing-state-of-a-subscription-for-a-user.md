...
| 名称         | 类型  | 说明   |  必填  |
|----------------|--------|-------------|-----------|
| recurrenceId | 字符串 | 想要更改的订阅 ID。 若要获取此 ID，调用[获取用户订阅](get-subscriptions-for-a-user.md)方法、 标识表示你想要更改的订阅加载项的响应正文条目并使用该条目的**id**字段的值。     | 是      |
...
| expirationTimeWithGrace | 字符串 | 过期日期和时间订阅将包括在宽限期，采用 ISO 8601 格式。 此值指示当该用户将失去对订阅的访问后已失败订阅自动续订。    |
