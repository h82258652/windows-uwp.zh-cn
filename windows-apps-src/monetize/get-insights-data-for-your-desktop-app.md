...
|---------------|--------|---------------|------|
| applicationId | string | 你想要获取见解数据在桌面应用程序的产品 ID。 若要获取的桌面应用程序的产品 ID，请打开任意[分析报告，以便您在合作伙伴中心中的桌面应用程序](https://docs.microsoft.com/windows/desktop/appxpkg/windows-desktop-application-program)(如**运行状况报告**)，并从 URL 检索产品 ID。 如果不指定此参数，响应正文将包含已注册到你的帐户的所有应用的 insights 数据。  |  否  |
| startDate | date | 开始 insights 数据的日期范围内要检索的日期。 默认值为当前日期之前 30 天。 |  否  |
| endDate | date | 最终的 insights 数据的日期范围内要检索的日期。 默认值为当前日期。 |  否  |
| filter | string  | 在响应中筛选行的一条或多条语句。 每条语句包含的响应正文中的字段名称和值使用 **eq** 或 **ne** 运算符进行关联，并且语句可以使用 **and** 或 **or** 进行组合。 *filter* 参数中的字符串值必须使用单引号括起来。 例如，*筛选器 = 数据类型 eq 获取*。 <p/><p/>此方法目前只支持的筛选器**运行状况**。  | 否   |
...
| ReplTest1               | 在任务栏的搜索框中键入   | 描述                           |
|---------------------|--------|-------------------------------------------|
| applicationId       | string | 为其检索 insights 数据的桌面应用程序的产品 ID。     |
| insightDate                | string | 我们发现特定度量值中的更改的日期。 此日期表示在其中我们检测到大量增加的一周结束，或减小相比前的一周的指标。 |
| dataType     | string | 一个字符串，指定此信息通知的常规分析区域。 目前，此方法仅支持**运行状况**。    |
...
| ReplTest1               | 在任务栏的搜索框中键入   | 描述                           |
|---------------------|--------|-------------------------------------------|
| FactName           | string | 一个字符串，指示当前的见解或当前维度描述的指标。 目前，此方法仅支持值**点击次数**。  |
| SubDimensions         | 数组 |  描述单个指标以深入了解的一个或多个对象。   |
| PercentChange            | string |  在您的整个客户群之间更改度量值所占百分比。  |
| DimensionName           | string |  当前维度中所述的指标的名称。 示例包括**EventType**，**市场**， **DeviceType**，以及**PackageVersion**。   |
| DimensionValue              | string | 描述当前维度中的指标值。 例如，如果**DimensionName**是**EventType**， **DimensionValue**可能**崩溃**或**挂起**.   |
| FactValue     | string | 检测到见解的日期的跃点数的绝对值。  |
| Direction | string |  更改的方向 (**正**或**负**)。   |
| Date              | string |  我们发现与当前的见解或当前维度相关的更改的日期。   |
