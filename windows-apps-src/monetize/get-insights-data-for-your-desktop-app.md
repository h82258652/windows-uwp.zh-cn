...
| 参数        | 类型   |  说明      |  必需  
|---------------|--------|---------------|------|
| applicationId | 字符串 | 想要获取的见解数据的桌面应用程序的产品 ID。 若要获取桌面应用程序的产品 ID，请打开 （例如，**运行状况报告**） 的任何[桌面应用程序在合作伙伴中心分析报告](https://msdn.microsoft.com/library/windows/desktop/mt826504)，并从 URL 检索产品 ID。 如果不指定此参数，响应正文将包含注册到帐户的所有应用的见解数据。  |  否  |
...
| 具有           | 字符串 |  指标当前维度中所述的名称。 示例包括**EventType**、**市场**、 **DeviceType**，和**PackageVersion**。   |
