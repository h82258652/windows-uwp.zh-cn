...
ms.locfileid: "6851734"
---
# <a name="get-insights-data-for-your-desktop-application"></a>获取桌面应用程序的见解数据

在 Microsoft Store 分析 API 中使用此方法来获取数据相关的桌面应用程序已添加到[Windows 桌面应用程序](https://docs.microsoft.com/windows/desktop/appxpkg/windows-desktop-application-program)的运行状况指标的见解。 此数据也是在合作伙伴中心中的桌面应用程序[运行状况报告](https://docs.microsoft.com/windows/desktop/appxpkg/windows-desktop-application-program#health-report)中可用。
...
| insightDate                | 字符串 | 我们标识特定指标的更改的日期。 此日期表示的末尾一周中我们检测到了显著增加或减少相比于前的一周的指标。 |
| 数据类型     | 字符串 | 一个字符串，指定此相关的见解通知的常规分析区域。 目前，此方法仅支持**运行状况**。    |
...
| 具有           | 字符串 |  度量当前维度中所述的名称。 示例包括**事件类型**、**市场**、 **DeviceType**，以及**PackageVersion**。   |
| DimensionValue              | 字符串 | 的度量值当前维度中所述。 例如，如果**维度**， **EventType** **DimensionValue**可能**崩溃**或**挂起**。   |
