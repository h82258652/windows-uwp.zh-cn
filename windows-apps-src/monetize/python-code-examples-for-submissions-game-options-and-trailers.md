---
author: mcleanbyron
description: "使用此部分中的 Python 代码示例了解有关使用 Windows 应用商店提交 API 提交游戏选项和预告片的详细信息。"
...
ms.prod: windows
ms.technology: uwp
keywords: "windows 10, uwp, Windows 应用商店提交 API, 代码示例, 游戏选项, 预告片, 高级应用一览, python"
...
此代码调用其他示例类和函数，以使用 Windows 应用商店提交 API 来创建并确认包含游戏选项和预告片的应用提交。 要调整此代码以供自己使用，请执行以下操作：
...
* 将 ```application_id``` 变量指定为要为其创建提交的应用的[应用商店 ID](in-app-purchases-and-trials.md#store_ids)。
...
* ```DevCenterAccessTokenClient``` 类定义一个帮助程序方法，该方法会使用你的 ```tenantId```、```clientId``` 和 ```clientSecret``` 值来创建要用于 Windows 应用商店提交 API 的 Azure AD 访问令牌。
