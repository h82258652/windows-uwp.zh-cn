...
| 参数         | 类型         | 描述                                                                                                                                                                                                                                                          | 必需 |
|-------------------|--------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|
| 受益人     | UserIdentity | 表示要为产品查询的用户的 UserIdentity 对象。                                                                                                                                                                                           | 是      |
...
| ValidityType      | 字符串       | 当设置为 **All** 时，将返回用户的所有产品，包括已过期的项目。 当设置为 **Valid** 时，仅返回在此时有效的产品（即它们处于活动状态，开始日期 < 现在，结束日期 > 现在）。 | 否       |
...
| localTicketReference | 字符串             | 之前在请求正文中提供的 `localTicketReference` 的 ID。                                                                      | 是      |
...
| productId            | 字符串             | Windows 应用商店目录中的产品 ID。 产品 ID 的一个示例为“9WZDNCRFJ3Q8”。                                                            | 是      |
