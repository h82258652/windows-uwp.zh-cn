...
-   *筛选器 = 市场 eq 美国和 deviceType eq 电话*
...
| deviceType    | 以下字符串之一：<strong>PC/平板电脑</strong>或<strong>Phone</strong>。 |
| adUnitId    | 指定要应用到筛选器的广告单元 ID 的字符串。 |
| pubCenterAppName    | 指定要应用到筛选器的当前应用的 pubCenter 名称的字符串。 |
| adProvider    | 指定要应用到筛选器的广告提供商名称的字符串。 |
| 日期    | 指定要应用到筛选器的日期（格式为 YYYY/MM/DD）的字符串。 |


### <a name="request-example"></a>请求示例

以下示例演示用于获取广告性能数据的多个请求。 将 *applicationId* 值替换为你的应用的存储 ID。
