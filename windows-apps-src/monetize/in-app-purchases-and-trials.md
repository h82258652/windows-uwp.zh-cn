...
| 加载项类型 |  说明  |
|---------|-------------------|
| 耐用品  |  加载项的持续时间为生命周期，你[在合作伙伴中心中指定](../publish/enter-iap-properties.md)。 <p/><p/>默认情况下，耐用型加载项永远不会过期，在此情况下只能购买它们一次。 如果你为加载项指定特定的持续时间，则用户可以在它过期后重新购买该加载项。 |
...
1. 如果你的应用提供可供客户购买，[创建加载项提交合作伙伴中心中为应用](https://msdn.microsoft.com/windows/uwp/publish/add-on-submissions)的加载。
...
2. 接下来，确保已完成以下操作：
    ...
    * 如果你的应用提供的加载项可供客户购买，[创建加载项提交合作伙伴中心中为应用](https://msdn.microsoft.com/windows/uwp/publish/add-on-submissions)。
...
* 对于加载项，你可以添加的概述页面上的合作伙伴中心中获取应用商店 ID。
...
如果你想要你的应用的上下文中向客户提供加载项，则必须[输入唯一的产品 ID](../publish/set-your-add-on-product-id.md#product-id)的加载项时[创建加载项提交](../publish/add-on-submissions.md)合作伙伴中心中。 你可以使用此产品 ID 在代码中引用加载项，但可以使用产品 ID 的特定方案取决于你在应用中实现应用内购买所用的命名空间。
