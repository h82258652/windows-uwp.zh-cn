...
> [!IMPORTANT]
> **Windows.ApplicationModel.Store** 命名空间不再更新新功能，如果可能，建议你的应用改用 **Windows.Services.Store** 命名空间。 **Windows.ApplicationModel.Store**命名空间不支持在使用[桌面桥](https://developer.microsoft.com/windows/bridges/desktop)的 Windows 桌面应用程序或应用或游戏，在合作伙伴中心中使用的开发沙盒 （例如，这是这种情况的任何游戏的与 Xbox Live 集成）。
...
若要在应用中向客户提供加载项，你必须[定义为你在合作伙伴中心中的应用的加载项](../publish/add-on-submissions.md)以便使应用商店知道它。 然后，应用可以使用 **Windows.Services.Store** 或 **Windows.ApplicationModel.Store** 命名空间中的 API 提供加载项，作为应用内购买向用户销售。

UWP 应用可提供以下类型的加载项。

| 加载项类型 |  说明  |
|---------|-------------------|
| 耐用品  |  加载项的生存期内持续该你[在合作伙伴中心中指定](../publish/enter-iap-properties.md)。 <p/><p/>默认情况下，耐用型加载项永远不会过期，在此情况下只能购买它们一次。 如果你为加载项指定特定的持续时间，则用户可以在它过期后重新购买该加载项。 |
...
1. 如果你的应用提供可供客户购买，[创建适用于合作伙伴中心中应用的加载项提交](https://msdn.microsoft.com/windows/uwp/publish/add-on-submissions)的加载。
...
1. [配置为在合作伙伴中心中的免费试用版应用](../publish/set-app-pricing-and-availability.md#free-trial)。
...
1. 如果你的应用尚未发布并可在应用商店中，请确保你的应用满足最低的[Windows 应用认证工具包](https://developer.microsoft.com/windows/develop/app-certification-kit)要求，在合作伙伴中心，[提交你的应用](https://msdn.microsoft.com/windows/uwp/publish/app-submissions)，并确保你的应用通过认证过程。 在测试应用期间，你可以[将应用配置为在 Microsoft Store 中隐藏](https://msdn.microsoft.com/windows/uwp/publish/set-app-pricing-and-availability)。 请注意[软件包外部测试版](../publish/package-flights.md)的正确配置。 未正确配置的软件包外部测试版可能无法下载。
...
2. 接下来，确保已完成以下操作：
    ...
    * 如果你的应用提供的加载项可供客户购买，并[创建合作伙伴中心中的应用的加载项提交](https://msdn.microsoft.com/windows/uwp/publish/add-on-submissions)。
    * 如果你想要排除或限制试用版的应用，[应用配置为在合作伙伴中心中的免费试用版](../publish/set-app-pricing-and-availability.md#free-trial)中的某些功能。

3. 在项目在 Visual Studio 中打开的情况下，单击**项目菜单**、指向** Microsoft Store**，然后单击**将应用与 Microsoft Store 关联**。 完成向导中的说明以将应用项目与你想要用于测试的合作伙伴中心帐户中的应用相关联。
    > [!NOTE]
...
Microsoft Store 中的每个产品都至少有一个 *SKU*，而每个 SKU 都至少有一个*可用性*。 这些概念抽象合作伙伴中心中的大多数开发人员，并且大多数开发人员将永远不会定义 Sku 或可用性为其应用或加载项。 但是，由于 **Windows.Services.Store** 命名空间中的 Microsoft Store 产品的对象模型包括 SKU 和可用性，因此大致了解这些概念对于某些情况可能很有帮助。
...
* 对于应用，可以在合作伙伴中心中[的应用标识页面](../publish/view-app-identity-details.md)上获取应用商店 ID。
...
如果你想要你的应用的上下文中向客户提供加载项，则必须[输入唯一的产品 ID](../publish/set-your-add-on-product-id.md#product-id)的加载项时你合作伙伴中心中的[创建加载项提交](../publish/add-on-submissions.md)。 你可以使用此产品 ID 在代码中引用加载项，但可以使用产品 ID 的特定方案取决于你在应用中实现应用内购买所用的命名空间。
...
如果你的应用使用**Windows.ApplicationModel.Store**命名空间，你将需要使用分配给合作伙伴中心中的大多数操作的加载项的产品 ID。 例如：
