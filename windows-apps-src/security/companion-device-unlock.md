...
> **注意** 配套设备框架是不向所有应用开发人员提供的特定功能。 若要使用此框架，应用必须由 Microsoft 专门配置，并且在它的清单中列出受限制的 *secondaryAuthenticatorFactor* 功能。 若要获得批准，请联系 [cdfonboard@microsoft.com](mailto:cdfonboard@microsoft.com)。 
...
如果配套设备支持生物识别，则配套设备框架仅在设备满足以下条件之一时才能使用：

- 具有 UI 和可以引导用户注册和管理生物识别的非 Windows 操作系统（例如具有生物识别的手机）。 如果配套设备在 Windows 上运行，并且支持生物识别（人脸和指纹），则它必须遵守 [Windows 生物识别框架](https://msdn.microsoft.com/en-us/library/windows/hardware/mt608302(v=vs.85).aspx)。 
- 没有 UI（如 USB 硬件保护装置），也不使用人脸、虹膜或指纹。 

如果这些条件无一适用，并且配套设备支持生物识别，则必须遵守 [Windows Hello OEM 和 IHV 应用方案](https://msdn.microsoft.com/en-us/library/windows/hardware/mt608302(v=vs.85).aspx)指南。 
...
- 从 Windows 应用商店下载的配套设备的[通用 Windows 平台 (UWP)](https://msdn.microsoft.com/en-us/windows/uwp/get-started/universal-application-platform-guide) 配套设备应用。 
...
在配套设备可插入配套设备框架前，它需要先向该框架注册。 注册体验完全归配套设备应用所有。 配套设备框架不会给此用户体验施加任何限制。 
...
- 有关登录说明，请参阅 *device name（设备名）*
- 将*设备名*插入 USB 端口以登录
- 打开蓝牙以使用*设备名*登录
- 打开 NFC 以使用*设备名*登录
- 连接到 WLAN 网络以使用*设备名*登录
- 出现了一些问题。 请使用 PIN 或密码登录，然后重新设置*“设备名”*。
- 再次点击*“设备名”*
- 你的企业可阻止使用*设备名*登录。 使用其他登录选项。
- 点击*“设备名”*以登录
- 将你的手指放在*“设备名”*名上以登录
- 手指在*“设备名”*名上轻扫以登录
- 无法使用*设备名*登录。 使用其他登录选项。 
- 再试一次
- 向*设备名*说出密码
- 准备好使用*设备名*登录
