...
一般情况下，当你的代码在 C + + CX 组件使用常规 c + + 库和内置类型，除了抽象二进制接口 (ABI) 边界处将数据与代码传递在另一个.winmd 程序包中的位置。 因此，请使用 Windows 运行时类型和的特殊语法的 C + + CX 支持用于创建和处理这些类型。 此外，在 C + + CX 编码，使用诸如委派和事件在 JavaScript、 Visual Basic、 c + + 或 C# 中实现可从组件引发并处理事件的类型。 有关 C + + /CX 语法，请参阅[Visual c + + 语言参考 (C + + CX)](https://msdn.microsoft.com/library/windows/apps/xaml/hh699871.aspx)。
...
在.NET 语言中，你只需创建类型的变量定义在 C + + CX 组件。
...
## <a name="net"></a>.NET
.NET 语言中识别重载 C + + CX ref 类任意.NET Framework 类中一样。
...
枚举值传递之间 C + + CX 和 JavaScript 为整数。 你可以选择性地声明的 JavaScript 对象，包含相同的命名的值的 C + + CX 枚举，并使用它以遵循。
