...
一般情况下，你的代码在 C + + /cli CX 组件使用常规 c + + 库和内置类型，除非在抽象二进制接口 (ABI) 边界处将数据传入和传出代码传递另一个.winmd 包中的位置。 存在，请使用 Windows 运行时类型和特殊语法的 C + + /CX 支持用于创建和操作这些类型。 此外，在将 C + + /CX 代码，如在 JavaScript、 Visual Basic、 c + + 中实现，可以从组件引发和处理事件的委托和事件的使用类型或C#。 详细了解 C + + /cli CX 语法，请参阅[Visual c + + 语言参考 (C + + /cli CX)](https://msdn.microsoft.com/library/windows/apps/xaml/hh699871.aspx)。
...
仅 Windows 运行时类型可以跨 ABI 边界传递。 如果组件在公共方法中具有作为返回类型或参数的类型（例如 std::wstring），编译器将引发错误。 Visual C++ 组件扩展 (C++/CX) 内置类型包括诸如整型和双精度型的常用标量及其 typedef 等效项 int32、float64 等。 有关详细信息，请参阅[类型系统 (C++/CX)](https://msdn.microsoft.com/library/windows/apps/hh755822.aspx)。
...
## <a name="ccx-built-in-types-library-types-and-windows-runtime-types"></a>C + + /cli CX 内置类型、 库类型和 Windows 运行时类型
可激活类（也称为 ref 类）是可通过其他诸如 JavaScript、C# 或 Visual Basic 语言实例化的类。 若要能够通过其他语言使用，组件必须包含至少一项可激活类。

Windows 运行时组件可以包含多个公共的可激活类以及其他仅为组件内部所知的类。 将应用[WebHostHidden](https://msdn.microsoft.com/library/windows/apps/windows.foundation.metadata.webhosthiddenattribute.aspx)归于 C + + /CX 不应以 JavaScript 可见的类型。
...
若要跨 ABI 传递用户定义值结构，定义一个 JavaScript 对象具有相同的成员的值结构的定义在 C + + /cli CX。 你可以然后该对象作为参数传递到 C + + /cli CX 方法，以便该对象隐式转换为 C + + /CX 类型。
...
在.NET 语言中，您只需创建一个变量的类型的定义中 C + + /cli CX 组件。
...
C + + /cli CX 公共 ref 类可以包含重载的方法，但 JavaScript 只有有限的能力区分重载的方法。 例如，它可以区分以下签名之间的区别：
...
## <a name="net"></a>.NET
.NET 语言识别重载在 C + + /CX ref 类的任何.NET Framework 类中一样。
...
DateTime 值传递从 C + + /CX 到 JavaScript 中的，JavaScript 接受作为日期对象并将其显示为长型日期字符串的默认情况下。
...
一种.NET 语言将 System.DateTime 中传递到 C + + /cli CX 组件，该方法接受它作为 Windows::Foundation::DateTime。 在组件将 Windows::Foundation::DateTime 传递到 .NET Framework 方法时，Framework 方法将其作为 DateTimeOffset 接受。
...
集合始终作为 Windows 运行时类型（例如 Windows::Foundation::Collections::IVector^ 和 Windows::Foundation::Collections::IMap^）的句柄在 ABI 边界上传递。 例如，如果将句柄返回到 Platform::Collections::Map，它会隐式转换为 Windows::Foundation::Collections::IMap^。 集合接口定义中的命名空间不同于 C + + /cli CX 类提供具体的实现。 JavaScript 和 .NET 语言使用接口。 有关详细信息，请参阅[集合 (C++/CX)](https://msdn.microsoft.com//library/windows/apps/hh700103.aspx) 和[数组和 WriteOnlyArray (C++/CX)](https://msdn.microsoft.com/library/windows/apps/hh700131.aspx)。
...
公共 ref 类在 C + + /cli CX 组件扩展公共数据成员作为属性公开，方法是使用属性关键字。 此概念与 .NET Framework 属性一致。 普通属性类似于数据成员，因为其功能是隐式的。 特殊属性具有显式获取和设置的访问器和作为值的“备份存储”的已命名私有变量。 在此示例中，私有成员变量\_propertyAValue 是 PropertyA 的后备存储区。 属性可以在它的值更改时引发某个事件，并且客户端应用可注册为接收该事件。
...
.NET 语言访问属性上的本机 C + + /cli CX 对象就像.NET Framework 对象上一样。
...
## <a name="enums"></a>枚举
Windows 运行时枚举在 C + + /cli CX 声明使用公共类枚举;它类似于标准 c + + 中的范围的枚举。
...
枚举值传递之间 C + + /cli CX 和 JavaScript 为整数。 您可以选择声明一个 JavaScript 对象，包含相同的命名的值作为 C + + /CX 枚举和使用它作为遵循。
...
若要实现异步方法，在 C + + /CX 中，使用[创建\_异步](https://msdn.microsoft.com/library/hh750102.aspx)在 ppltasks.h 中定义的函数。 有关详细信息，请参阅[创建的异步操作在 C + + /cli CX 适用于 UWP 应用](https://msdn.microsoft.com/library/vstudio/hh750082.aspx)。 有关示例，请参阅[演练：创建一个基本的 Windows 运行时组件中 C + + /cli CX 并从 JavaScript 调用它或C# ](walkthrough-creating-a-basic-windows-runtime-component-in-cpp-and-calling-it-from-javascript-or-csharp.md)。 .NET 语言中使用 C + + /cli CX 异步方法就像在.NET Framework 中定义的任何异步方法。
...
如果 JavaScript 代码似乎无法识别组件中的公共属性或方法，请确保在 JavaScript 中使用的是 Camel 大小写格式。 例如，LogCalc c + + /cli CX 方法必须作为 logCalc 在 JavaScript 中进行引用。

如果删除 C + + /cli CX Windows 运行时组件项目从解决方案，您还必须手动删除从 JavaScript 项目的项目引用。 如果此操作无法完成，将阻止后续调试或生成操作。 如有必要，你可以稍后向 DLL 添加程序集引用。

## <a name="related-topics"></a>相关主题
* [演练：创建一个基本的 Windows 运行时组件中 C + + /cli CX 并从 JavaScript 调用它或C#](walkthrough-creating-a-basic-windows-runtime-component-in-cpp-and-calling-it-from-javascript-or-csharp.md)
