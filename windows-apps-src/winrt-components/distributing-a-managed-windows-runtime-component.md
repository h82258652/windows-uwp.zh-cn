...
你的二进制文件可能会和其他开发人员的二进制文件一起安装在应用包中。 请参阅中的"扩展 Sdk"[如何：创建软件开发工具包](https://msdn.microsoft.com/library/hh768146.aspx)，MSDN 库中。
...
> **请注意**  对于复杂的组件，NuGet 程序包管理系统提供了扩展 Sdk 的开源替代方法。 像扩展 SDK 一样，NuGet 支持创建可简化复杂组件安装的程序包。 有关 NuGet 程序包和 Visual Studio 扩展 SDK 的比较，请参阅 MSDN 库中的[使用 NuGet 与扩展 SDK 添加引用](https://msdn.microsoft.com/library/jj161096.aspx)。
...
2.  使用 VSIX 项目模板创建新项目。 你可以在“扩展性”类别的 Visual C# 或 Visual Basic 下找到该模板。 此模板作为 Visual Studio SDK 的一部分进行安装。 ([演练：创建 SDK 使用C#或 Visual Basic](https://msdn.microsoft.com/library/jj127119.aspx)或[演练：使用 c + + 创建 SDK](https://msdn.microsoft.com/library/jj127117.aspx)，演示如何使用此模板中一个非常简单的方案。 )
3.  确定 SDK 的文件夹结构。 该文件夹结构和“引用”、“Redist”以及DesignTime文件夹开始于 VSIX 项目的根级别。
    ...
    在每个配置文件夹中，你可以为特定于平台的文件提供体系结构文件夹。 如果将相同文件用于所有平台，可以提供一个名为“neutral”的文件夹。 您可以找到详细信息的文件夹结构，包括中的其他体系结构的文件夹名称，[如何：创建软件开发工具包](https://msdn.microsoft.com/library/hh768146.aspx)，MSDN 库中。 （该文讨论平台 SDK 和扩展 SDK。 若要折叠有关平台 SDK 的部分以避免混淆，你会发现该文很有用。 )

4.  创建 SDK 清单文件。 该清单指定名称和版本休息、SDK 支持的体系结构、.NET Framework 版本和其他有关 Visual Studio 使用 SDK 的方式的信息。 您可以找到详细信息和示例，请参见[如何：创建软件开发工具包](https://msdn.microsoft.com/library/hh768146.aspx)。
