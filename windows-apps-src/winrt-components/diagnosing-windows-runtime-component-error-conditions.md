...
> **请注意**，@no__t 引用 Windows 运行时使用旧术语的1The 错误消息。 这现在称为通用 Windows 平台 (UWP)。 例如，Windows 运行时类型现在称为 UWP 类型。

## <a name="missing-references-to-mscorlibdll-or-systemruntimedll"></a>缺少对 mscorlib.dll 或 System.Runtime.dll 的引用

此问题仅在从命令行使用 Winmdexp.exe 时发生。 建议使用/reference 选项同时包含对 mscorlib.dll 和 ProgramFiles 的引用，.NET Framework 该程序集位于 "% （x86）% \\Reference 程序集 @ no__t-1Microsoft @ no__t-2Framework @ no__t-3. NETCore @ no__t-4v 4.5" （"% ProgramFiles% \\ ..."在32位计算机上）。
...
> **请注意** In 错误消息，运算符由其元数据名称标识，如 op @ No__t-2Addition、op @ No__t-3Multiply、op @ No__t-4ExclusiveOr、op @ No__t-5Implicit （隐式转换）等。
...
| 错误编号 | 消息文本                                                                                                                                                                      |
|--------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1059      | 多 @no__t 旁 1/-0-"{1}. {2}" 的参数重载用 Windows.foundation.metadata.defaultoverloadattribute 修饰。                                                            |
| WME1085      | @No__t 的 @no__t 旁1/-0 重载。 {2} 必须通过使用 Windows.foundation.metadata.defaultoverloadattribute 修饰它来明确指定为默认重载的一个方法。 |
...
| WME1044      | 公共类型具有不与其他命名空间（"{0}"）共享公共前缀的命名空间（"@no__t"）。 Windows 元数据文件中的所有类型必须存在于由文件名暗示的命名空间的子命名空间中。                                                                                                                              |
...
<td align="left">方法 "{0}" 具有类型为 "{2}" 的参数 "{1}"。 “{2}”不是有效的 Windows 运行时参数类型。</td>
</tr>
<tr class="even">
...
> **请注意** This 限制不适用于在实现内部使用的数组。

 

| 错误编号 | 消息文本                                                                                                                                                     |
|--------------|--------------------|
| WME1034      | 方法 "{0}" 的签名中具有类型为 "{1}" 的数组，下限为非零。 Windows 运行时方法签名中数组的下限必须为零。 |
| WME1035      | 方法 "{0}" 在其签名中具有类型为 "{1}" 的多维数组。 Windows 运行时方法签名中的数组必须是一维数组。                  |
| WME1036      | 方法 "{0}" 在其签名中具有类型为 "{1}" 的嵌套数组。 Windows 运行时方法签名中的数组无法嵌套。                                    |
...
| 错误编号 | 消息文本         |
|--------------|----------------------|
| WME1101      | 方法 "{0}" 具有作为数组的参数 "{1}"，该参数同时具有 {2} 和 @no__t。 在 Windows 运行时中，数组参数内容必须是可读或可写。 请从“{1}”中删除其中一个属性。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| WME1102      | 方法 "{0}" 具有输出参数 "{1}"，该参数是一个数组，但其 @no__t 为2。 在 Windows 运行时中，可写入输出数组的内容。 请从“{1}”中删除该属性。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| WME1104      | 方法 "{0}" 具有参数 "{1}"，该参数不是数组，并且具有 @no__t 2 或 @no__t 3。 Windows 运行时不支持使用 {2} 或 {3} 标记非数组参数。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
