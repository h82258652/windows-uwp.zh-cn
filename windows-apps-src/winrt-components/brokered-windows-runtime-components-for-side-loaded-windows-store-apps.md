...
>**注意**  可以为 [Visual Studio 2015 & 2017](https://aka.ms/brokeredsample) 下载本文附带的示例代码。 用于生成中转 Windows 运行时组件的 Microsoft Visual Studio 模板可以在此处下载：[面向 Windows 10 通用 Windows 应用的 Visual Studio 2015 模板](https://visualstudiogallery.msdn.microsoft.com/10be07b3-67ef-4e02-9243-01b78cd27935)
...
如“定义合约”部分中所提到的，必须对桌面组件的引用 winmd 进行项目引用。 Visual Studio 项目系统通常使用相同的名称创建一个两级目录结构。 在本例中，它是 EnterpriseIPCApplication\\EnterpriseIPCApplication。 引用 **winmd** 手动复制到此二级目录，然后“项目引用”对话框用于（单击“浏览...”**** 按钮）定位和引用此 **winmd**。 在此之后，桌面组件的顶级命名空间（例如 Fabrikam）应当作为顶级节点出现在该项目的“引用”部分中。
...
正常情况下，仅进行其他项目间引用。 但是，桌面组件项目有一组非常特殊的引用。 它起初是“Classic Desktop\\Class Library”项目，因此是桌面项目。 因此，必须显式引用 Windows 运行时 API（通过引用 **winmd**）。 添加适当的引用，如下所示。
...
正确配置引用后，下一个任务是实现服务器的功能。 请参阅 MSDN 主题 [Windows 运行时组件的互操作性最佳做法（使用 C\#/VB/C++ 和 XAML 的 UWP 应用）](https://msdn.microsoft.com/library/windows/apps/hh750311.aspx)。
...
[在 Windows 运行时组件中引发事件](https://msdn.microsoft.com/library/windows/apps/dn169426.aspx)主题介绍了用于创建并注册代理和存根，以在常规 UWP 应用包内部使用的过程。
