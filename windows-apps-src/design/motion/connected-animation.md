...
| - |
| 当用户在应用中 (从 B 到 A) 向后导航，动画是更直接。 连接的元素线性转换从 B 到使用减速三次方贝塞尔缓动函数。 向后可视化提示使用户返回到其之前的状态尽快同时仍维护上下文的导航流程。 |
...
对于后退导航 (Page_B 到 Page_A)，请遵循相同的步骤中，但是，源和目标页面的相反。
...
- 使用[SuppressNavigationTransitionInfo](/uwp/api/windows.ui.xaml.media.animation.suppressnavigationtransitioninfo)阻止**帧**中的转换动画，如果你使用**ConnectedAnimationService**，因为连贯的动画不打算同时使用的默认导航转换。 请参阅 [NavigationThemeTransition](/uwp/api/Windows.UI.Xaml.Media.Animation.NavigationThemeTransition) 详细了解如何使用导航转换。
