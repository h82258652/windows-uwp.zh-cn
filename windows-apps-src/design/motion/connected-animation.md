...
在连贯动画，某一元素似乎以期间更改 UI 内容，在屏幕上从源视图中的位置掠过到其在新视图中的目标中的两个视图之间"继续"。 这强调了不同视图之间的共同内容，并创建了转换过程中美观且动态的效果。
...
> [!IMPORTANT]
> 此功能需要你的应用的目标版本为 Windows 10 版本 1809 ([SDK 17763](https://developer.microsoft.com/windows/downloads/windows-10-sdk)) 或更高版本。 配置属性不是早期的 Sdk 中提供的。 你可以低于 SDK 17763 的最低版本为目标使用自适应代码或条件 XAML。 有关详细信息，请参阅[版本自适应应用](/debug-test-perf/version-adaptive-apps)。
...
| - |
| 这是默认配置，并建议对向前导航。 |
用户前进 (A 到 B) 在应用中，会出现连接的元素以物理方式"拉入页面关闭"。 在执行此操作，元素似乎在 z 空间向前移动，并作为参加暂停引力效果有点丢弃。 若要克服引力的影响，元素获得速度并加快了到其最终位置。 结果是一个英寸缩放和 dip 英寸的动画。 |
...
若要实现不同效果，一些配置忽略这些属性上 ConnectedAnimationService 以及相反，此表中所述使用他们自己的值。
...
对于后退导航 (Page_B 到 Page_A)，请遵循相同的步骤，但反转，源和目标页面。

当用户导航回来时，他们希望尽快返回到之前的状态的应用。 因此，建议的配置是[DirectConnectedAnimationConfiguration]()。 此动画是更快、 更直接，并使用减速缓动。
...
- 使用[SuppressNavigationTransitionInfo](/uwp/api/windows.ui.xaml.media.animation.suppressnavigationtransitioninfo)阻止**帧**中的转换动画，如果你使用的**ConnectedAnimationService**，因为连贯的动画不应与默认导航同时使用转换。 请参阅 [NavigationThemeTransition](/uwp/api/Windows.UI.Xaml.Media.Animation.NavigationThemeTransition) 详细了解如何使用导航转换。
