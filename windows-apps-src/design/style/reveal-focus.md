...
如你所见，它是应用检测到它正在 Xbox 上运行时打开展示焦点相当简单。 那么，系统为何不为你打开它？ 显示焦点会增加焦点视觉对象的大小，因为这可能导致你的 UI 布局问题。 在某些情况下，你会想要自定义显示焦点效果来优化你的应用。
