---
author: cphilippona
description: 显示焦点是一种灯光效果，当用户将游戏板或键盘焦点移至它们时，可聚焦元素的边框进行动画处理。
title: 显示焦点
template: detail.hbs
ms.author: mijacobs
ms.date: 03/1/2018
...
（展示突出显示效果，突出显示交互性元素的灯光看到[展示突出显示文章](/windows/uwp/design/style/reveal)。）
...
<td>
    <p>如果你有安装的<strong style="font-weight: semi-bold">XAML 控件库</strong>应用，单击此处<a href="xamlcontrolsgallery:/item/RevealFocus">打开该应用，请参阅展示的实际的焦点</a>。</p>
    <ul>
...
设置**FocusVisualKind**属性后，系统会自动显示焦点效果适用于其[UseSystemFocusVisuals](/uwp/api/Windows.UI.Xaml.Controls.Control.UseSystemFocusVisuals)属性设置为**True** （大多数控件的默认值） 的所有控件。 
