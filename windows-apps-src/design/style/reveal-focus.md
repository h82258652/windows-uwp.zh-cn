...
（展示突出显示效果，照明影响，突出显示交互性元素，请参阅[展示突出显示文章](/windows/uwp/design/style/reveal)。）
...
## <a name="why-isnt-reveal-focus-on-by-default"></a>为何焦点不是显示在默认情况下？ 
如你所见，很容易启用展示焦点时在应用检测到在 Xbox 上运行。 那么，系统为何不为你打开它？ 显示焦点会增加焦点视觉对象的大小，因为这可能导致你的 UI 布局问题。 在某些情况下，你将希望自定义显示焦点效果优化为你的应用。
...
但在开始对构成之前，最好先更多了解构成显示焦点的组件。

有三个部分的默认显示焦点视觉对象： 主边框、 辅助边框和显示明亮辉光。 主边框为 **2px** 粗，在辅助边框的*外部*周围运行。 辅助边框为 **1px** 粗，在主边框的*内部*周围运行。 显示焦点明亮具有粗细与主边框的粗细成比例，并在*外部*的主边框周围运行。

除了静态元素，显示焦点视觉功能动画的灯光，跳动时移动焦点时，焦点方向移动。
...
若要自定义显示焦点的另一个方法是通过使用视觉状态绘制你自己选择退出系统提供的焦点视觉对象。 若要了解详细信息，请参阅[焦点视觉示例](http://go.microsoft.com/fwlink/p/?LinkID=619895)。
