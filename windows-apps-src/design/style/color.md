...
除了在应用中定义自己的颜色，你可以使用**ColorSchemeResources**标记范围我们 systematized 的颜色以所需的区域设置整个应用。 此 API 允许你不仅着色和主题大的同时通过设置一些属性，但还提供你许多其他系统优势的控件组通常不会获取与手动定义你自己的自定义颜色：

- 使用**ColorSchemeResources**设置任何颜色不会影响高对比度
  * 这意味着你的应用可以访问更多的用户，而无需任何额外的设计或开发人员成本
- 可以轻松地设置颜色的浅色、 深色或普遍跨两种主题通过 api 设置一个属性
- **ColorSchemeResources**上设置的颜色将向所有类似的控件，还使用该系统颜色级联
...
限定作用域通过**ColorSchemeResources** API 在 XAML 中的资源可以采用任何系统颜色或在我们[的主题资源](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/xaml-theme-resources)库，并且重页面或容器的范围内定义它们的画笔。

例如，如果你定义两个系统颜色- **SystemBaseLowColor**和**SystemBaseMediumLowColor**内 grid 中，并将两个按钮放在页面上： 一个在该网格，以及一个外部：
...
但是，由于我们的所有系统颜色太都层叠到其他控件，则设置**SystemBaseLowColor**和**SystemBaseMediumLowColor**将影响不仅仅按钮。 在此情况下，控件像**切换按钮**、**单选按钮**和**滑块**也会影响通过这些系统的颜色更改，这些控件应当上方 exampl 网格的作用域。
...
嵌套系统颜色也有可能，并已完成此操作通过将**ColorSchemeResources**放在你的应用的布局的标记内的嵌套的元素的资源：
...
在此示例中，继承**Button_A** **Grid_A**资源中定义的颜色和**嵌套按钮**继承**Grid_B**资源的颜色。 通过扩展，这意味着，任何其他控件放置在**Grid_B**将检查或之前检查或应用**Grid_A**资源，首先，应用**Grid_B**资源和最后应用我们的默认颜色，如果没有在定义页面或应用级别。
