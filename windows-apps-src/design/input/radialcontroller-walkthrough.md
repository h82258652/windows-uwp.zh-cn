...
9. 如果菜单未显示 （Dial 还应出现振动） 屏幕上，转到返回蓝牙设置，删除该设备，然后尝试重新连接设备。
...
在 [windows-appsample-get-started-radialcontroller 示例](https://aka.ms/appsample-radialcontroller)从 [GitHub](https://github.com/) 下载此 Visual Studio 示例和源代码：
...
当应用在前台运行时，长按 Surface Dial 显示 **RadialController** 菜单。
