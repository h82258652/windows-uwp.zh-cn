---
Description: 可视化工具是可帮助开发人员设计自适应的存储区中的新通用 Windows 平台 (UWP) 应用的通知适用于 Windows 10 动态磁贴。
title: 通知可视化工具
...
通知可视化工具是一个新的通用 Windows 平台 (UWP) 应用[存储区中](https://www.microsoft.com/store/apps/notifications-visualizer/9nblggh5xsl1)，可帮助开发人员设计自适应动态磁贴和适用于 Windows 10 的交互式 toast 通知。
...
**请注意**  通知可视化工具模拟的 Windows 开始菜单和 toast 通知并不总是完全准确，并且它不支持某些高级的有效负载属性。 当拥有所需的磁贴和 Toast 时，可通过固定磁贴或弹出 Toast 对其进行测试来验证它是否会按预期显示。
...
编辑器可提供实时错误和警告。 例如，如果有效负载大于 5 KB（平台限制），通知可视化工具会在负载超出该限制时发出警告。 如果存在不正确的属性名或值，它将向你发出警告，这有助于你调试视觉问题。

可以控制磁贴属性，如显示名称、颜色、徽标、ShowName 和锁屏提醒值。 这些选项可帮助你立即了解磁贴属性和磁贴通知负载的交互方式以及它们产生的结果。
...
* [获取存储中的通知可视化工具](https://www.microsoft.com/store/apps/notifications-visualizer/9nblggh5xsl1)
