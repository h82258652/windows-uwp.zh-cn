...
| 属性 | 类型 | 必需 | 说明 |
|---|---|---|---|
| **开始**| string | false | 当应用程序由 toast 激活时向其传递的字符串。 此字符串的格式和内容由应用根据其自身用途定义。 当用户点击或单击 Toast 来启动其关联应用时，启动字符串会向应用提供上下文，以允许该应用向用户显示与 Toast 内容相关的视图，而不是以其默认方式启动。 |
| **C** | [ToastVisual](#toastvisual) | true | 描述 toast 通知的可视部分。 |
...
|---|---|
| **默认设置** | 正常 toast 行为。 |
| **一下** | 提醒通知。 这类通知将以预先展开的状态显示在用户屏幕上并一直显示，直至消失。 |
...
| 值 | 含义 |
|---|---|
| **默认设置** | 默认值。 由渲染器确定的对齐行为。 |
| **加宽** | 图像拉伸以填充可用宽度（根据图像的位置，可能还要填充可用高度）。 |
| **左中** | 向左对齐图像，并用其原始分辨率显示图像。 |
| **Center** | 在水平方向上中心对齐图像，并用其原始分辨率显示图像。 |
| **然后** | 向右对齐图像，并用其原始分辨率显示图像。 |
...
| **值** | Double 或 [AdaptiveProgressBarValue](#adaptiveprogressbarvalue) 或 [BindableProgressBarValue](#bindableprogressbarvalue) | false | 获取或设置进度栏的值。 支持数据绑定。 默认为 0。 |
...
| **ContextMenuItems** | IList<[ToastContextMenuItem](#toastcontextmenuitem)> | false | 周年更新中的新增功能：自定义上下文菜单项，如果用户右键单击通知，则会提供更多操作。 最多只能有 5 个按钮和上下文菜单项*组合*。 |
...
| 属性 | 类型 | 必需 | 说明 |
|---|---|---|---|
| **识别** | string | true | Id 是必需的，并用于将用户输入的文本映射到你的应用以后使用的 id/值的键值对。 |
...
| 属性 | 类型 | 必需 | 说明 |
|---|---|---|---|
| **识别** | string | true | 此 Id 是必需的。 如果用户选择此项，则此 Id 将传递回应用的代码，表示他们所做的选择。 |
| 内容 | string | true | Content 是必需的，是显示在选择项上的字符串。 |
...
| 属性 | 类型 | 必需 | 说明 |
|---|---|---|---|
| 内容 | string | true | 必需。 要显示在按钮上的文本。 |
| **形参** | string | true | 必需。 如果用户单击此按钮，应用将在稍后接收到的应用定义的参数字符串。 |
...
| 值 | 含义 |
|---|---|
| **默认设置** | 默认行为。 当用户在 Toast 上执行操作时将消除 Toast。 |
...
| 属性 | 类型 | 必需 | 说明 |
|---|---|---|---|
| 内容 | string | true | 必需。 要显示的文本。 |
| **形参** | string | true | 必需。 用户单击菜单项后，应用一旦激活便可以在稍后检索的应用定义的参数字符串。 |
...
| **自行** | boolean | false | 设置为 true 将声音静音；设置为 false 允许播放 toast 声音（默认设置）。 |
...
| 属性 | 类型 | 必需 | 说明 |
|---|---|---|---|
| **识别** | string | true | 开发人员创建的标识符，用以唯一标识此标头。 如果两个通知具有相同的标头 id，它们将显示在操作中心中的同一标头下。 |
| **Title** | string | true | 标头的标题。 |
| **形参**| string | true | 获取或设置当用户单击此标头时返回给应用的、开发人员定义的参数字符串。 不能为 Null。 |
...
* [快速入门：发送本地 toast 并处理激活](https://blogs.msdn.microsoft.com/tiles_and_toasts/2015/07/08/quickstart-sending-a-local-toast-notification-and-handling-activations-from-it-windows-10/)
