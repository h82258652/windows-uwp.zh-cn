...
另一方面，格式模式是区域性特定的。 让我们来访问我们格式模板的格式模式。
...
日期部分对应于“month day”格式模板，而时间部分对应于“hour minute”格式模板。 因此，可以为相关的日期和时间格式模板构造格式化程序，然后将其输出一起使用的可本地化的格式字符串相连接。
