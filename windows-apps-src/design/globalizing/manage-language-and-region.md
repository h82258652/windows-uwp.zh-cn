...
使用语言资源限定符命名资源文件或其文件夹。 若要了解有关资源限定符的详细信息，请参阅[定制语言、比例、高对比度和其他限定符的资源](../../app-resources/tailor-resources-lang-scale-contrast.md)。 资源文件可以是一个图像 （或其他资产），也可以是资源容器文件，如 *.resw* ，其中包含文本字符串。
...
### <a name="not-all-resources-need-to-be-localized"></a>并非所有资源都需要进行本地化
