...
[拆分按钮](/uwp/api/windows.ui.xaml.controls.splitbutton) | 一个具有两条边按钮。 一侧启动某项操作，并且另一侧打开的菜单。
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | 使用两个边切换按钮。 一侧切换开/关，并且另一侧打开的菜单。
...
使用**DropDownButton**时该按钮具有浮出控件，其中包含更多选项。 默认 v 形图标提供了一个按钮，包含一个浮出控件的视觉显示。
...
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton)是显示一个可视指示器，它具有一个附加的浮出控件，包含更多选项 v 形图标的按钮。 它具有相同的行为与在浮出控件; 一个标准按钮不同的外观。

下拉列表按钮继承 Click 事件，但你通常不使用它。 相反，你可以使用浮出控件属性将附加浮出控件，以及调用操作在浮出控件中使用的菜单选项。 单击该按钮时，此弹出窗口将自动打开。
...
> [!NOTE]
> 当调用借助触摸，拆分按钮的行为与一个下拉列表按钮;按钮的两个部分调用浮出控件。 使用其他方法的输入，用户可以单独调用该按钮任一半部分。

拆分按钮的典型行为是：

- 当用户单击按钮部件时，处理 Click 事件来调用当前在下拉列表中选择的选项。
...
> [!NOTE]
> 当调用借助触摸，拆分按钮的行为与一个下拉列表按钮。 使用其他方法的输入，用户可以单独调用该按钮任一半部分。 借助触摸，按钮的两个部分调用浮出控件。 因此，你必须在浮出控件内容来打开或关闭切换按钮中包含一个选项。
...
- [ToggleSplitButton.IsChecked](/uwp/api/windows.ui.xaml.controls.togglesplitbutton.ischecked)属性是只是一个**布尔值**，不**为空的布尔值**。
