...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | （预览版）一个具有两条边的切换按钮。 一侧切换开/关，并且另一侧打开的菜单。
...
> [!NOTE]
> 当调用借助触摸，拆分按钮的行为作为一个下拉列表按钮;按钮的两个部分调用浮出控件。 用于输入其他方法，用户可以分别调用该按钮任一半部分。

拆分按钮的典型行为是：

- 当用户单击按钮部件时，处理 Click 事件来调用当前在下拉列表中选择的选项。
- 打开下拉列表时，这两个更改下拉哪个选项的项目的句柄调用处于选中状态，然后调用它。 务必调用浮出控件项，因为按钮单击事件不会发生在使用触摸时。
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton)有两个部分，可以单独调用。 另一部分行为类似于可以打开或关闭切换按钮。 其他部分调用浮出控件包含用户可以选择的其他选项。

切换拆分按钮通常用于启用或禁用时该功能有多个选项可供用户选择的一项功能。 例如，在文档编辑器中，它可以用于打开列表，打开或关闭时的下拉框用于选择列表的样式。

> [!NOTE]
> 当调用借助触摸，拆分按钮的行为作为一个下拉列表按钮。 用于输入其他方法，用户可以分别调用该按钮任一半部分。 借助触摸，按钮的两个部分调用浮出控件。 因此，你必须在浮出控件内容来打开或关闭切换按钮中包含一个选项。
...
- [ToggleSplitButton.IsChecked](/uwp/api/windows.ui.xaml.controls.togglesplitbutton.ischecked)属性是只是一个**布尔值**，不**可以为 null bool**。
