...
ms.locfileid: "5989531"
---
# <a name="buttons"></a>按钮

按钮为用户提供了触发即时操作的方法。 某些按钮都专用于特定的任务，如导航、 重复的操作，或显示菜单。
...
控件 | 描述
------- | -----------
[按钮](/uwp/api/windows.ui.xaml.controls.button) | 启动即时操作。 可与命令绑定的 Click 事件。
[RepeatButton](/uwp/api/windows.ui.xaml.controls.primitives.repeatbutton) | 引发 Click 事件连续地同时按下按钮。
[HyperlinkButton](/uwp/api/windows.ui.xaml.controls.hyperlinkbutton) | 一个按钮的样式像超链接，用于导航。 有关详细信息，请参阅[超链接](hyperlinks.md)。
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) | V 形图标以打开一个附加的浮出控件与一个按钮。
[拆分按钮](/uwp/api/windows.ui.xaml.controls.splitbutton) | 一个具有两个边按钮。 一侧启动某项操作，并且另一侧打开的菜单。
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | 一个具有两个边的切换按钮。 一侧切换开/关，并且另一侧打开的菜单。

| **获取 Windows UI 库** |
| - |
| 作为 Windows UI 库，包含新的控件和适用于 UWP 应用的 UI 功能的 NuGet 包的一部分包含 DropDownButton、 拆分按钮和 ToggleSplitButton。 有关详细信息，包括安装说明，请参阅[Windows UI 库概述](https://docs.microsoft.com/uwp/toolkits/winui/)。 |
...
在该按钮具有浮出控件，其中包含更多选项时，请使用**DropDownButton** 。 默认 v 形图标提供按钮包括浮出控件的视觉指示。
...
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton)是显示一个可视指示器，它具有一个附加的浮出控件，包含更多选项 v 形图标的按钮。 它具有相同的行为与浮出控件; 一个标准按钮只是不同的外观。

下拉列表按钮继承 Click 事件，但你通常不使用它。 相反，你可以使用浮出控件属性以将附加浮出控件，并调用使用浮出控件中的菜单选项的操作。 单击该按钮时，此弹出窗口将自动打开。
...
此示例显示了如何使用浮出控件，其中包含段落对齐方式 RichEditBox 中的命令创建一个下拉列表按钮。 （有关详细信息和代码，请参阅[富编辑框](rich-edit-box.md)）。
...
[拆分按钮](/uwp/api/windows.ui.xaml.controls.splitbutton)具有两个部分，可以单独调用。 另一部分行为类似于标准按钮，并调用即时操作。 其他部分调用浮出控件，其中包含用户可以选择的其他选项。
...
- 打开下拉列表时，哪个选项中放置到这两个更改项的句柄调用已选中，然后再调用它。 务必要调用的浮出控件项，因为该按钮的单击事件不会发生在使用触摸时。

> [!TIP]
> 有多种方法，向下放置在下拉菜单中的项目并处理其调用。 如果你使用 ListView 或 GridView，一种方法是处理 SelectionChanged 事件。 如果你这样做，请将[SingleSelectionFollowsFocus](/uwp/api/windows.ui.xaml.controls.listviewbase.singleselectionfollowsfocus)设置为**false**。 这将允许用户导航而无需调用每次更改上的项使用键盘的选项。
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton)有两个部分，可以单独调用。 另一部分的行为就像可以打开或关闭切换按钮。 其他部分调用浮出控件，其中包含用户可以选择的其他选项。

切换拆分按钮通常用于启用或禁用该功能有多个选项可供用户选择的一项功能。 例如，在文档编辑器中，它可以用于打开列表，打开或关闭时的下拉框用于选择列表的样式。

> [!NOTE]
> 当通过触摸进行调用，拆分按钮的行为作为一个下拉列表按钮。 使用其他方法的输入，用户可以单独调用该按钮任一半部分。 借助触摸，按钮的两个部分调用浮出控件。 因此，你必须在浮出控件的内容以打开或关闭切换按钮包括一个选项。

### <a name="differences-with-togglebutton"></a>与 ToggleButton 之间的区别

与[ToggleButton](/uwp/api/windows.ui.xaml.controls.primitives.togglebutton)，不同 ToggleSplitButton 不具有不确定状态。 因此，你应记住这些差异：
...
下面的示例演示如何拆分按钮切换可用于打开格式打开或关闭的列表和更改的列表，RichEditBox 中的样式。 （有关详细信息和代码，请参阅[富编辑框](rich-edit-box.md)）。
