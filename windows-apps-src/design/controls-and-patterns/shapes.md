...
**提示**  位置而不是使用有的 UI 定义的一些方案[**矩形**](/uwp/api/Windows.UI.Xaml.Shapes.Rectangle)即[**边框**](https://msdn.microsoft.com/library/windows/apps/BR209250)可能会更合适。 如果你打算在其他内容周围创建一个矩形形状，最好使用 **Border**，因为它可能会有子内容而且将自动在子内容周围调整大小，而不是像 **Rectangle** 那样使用固定的高度和宽度尺寸。 **Border** 在你设置了 [**CornerRadius**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.controls.border.cornerradius) 属性的情况下还提供了创建圆角的选项。
...
**提示**  A [**点**](https://msdn.microsoft.com/library/windows/apps/BR225870)值通常用于为 XAML 中的类型声明的形状顶点以外的方案。 例如，**Point** 属于触摸事件的事件数据，因此你可以精确了解触摸操作发生在坐标空间的哪个位置。 有关 **Point** 以及如何将其用于 XAML 或代码的详细信息，请参阅 [**Point**](https://msdn.microsoft.com/library/windows/apps/BR225870) 的 API 参考主题。
...
**请注意**  无法显式具有完全相同的起始点并在终结点[**点**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.shapes.polyline.points.aspx)设置为[**折线**](/uwp/api/Windows.UI.Xaml.Shapes.Polyline)，但在这种情况下您可能也可以使用[**多边形**](/uwp/api/Windows.UI.Xaml.Shapes.Polygon)相反。
...
下一示例将介绍我们讨论过的其他技术的用法：具有 [**PathGeometry**](https://msdn.microsoft.com/library/windows/apps/BR210168) 的 [**GeometryGroup**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.media.geometrygroup)。 此示例中运用某种可作为的一部分的参与 geometry 类型**PathGeometry**:[**PathFigure** ](https://msdn.microsoft.com/library/windows/apps/BR210143)并可以是中的某个段的各种元素[ **PathFigure.Segments**](https://msdn.microsoft.com/library/windows/apps/BR210164)。
