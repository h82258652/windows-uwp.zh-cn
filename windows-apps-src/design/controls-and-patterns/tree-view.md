...
<td>
    <p>如果你安装了该<strong style="font-weight: semi-bold">XAML 控件库</strong>应用，单击此处<a href="xamlcontrolsgallery:/item/TreeView">打开此应用，请参阅的实际树视图</a>。</p>
    <ul>
...
你可以通过将[ItemsSource](/uwp/api/windows.ui.xaml.controls.treeview.itemssource)绑定到分层数据源，创建树视图或者你可以创建并自行管理 TreeViewNode 对象。
...
从 Windows Insider Preview 开始，你可以绑定分层数据源到[ItemsSource](/uwp/api/windows.ui.xaml.controls.treeview.itemssource)属性来提供树视图内容，就像使用列表视图的 ItemsSource。 同样，使用[ItemTemplate](/uwp/api/windows.ui.xaml.controls.treeview.itemtemplate) （和可选[ItemTemplateSelector](/uwp/api/windows.ui.xaml.controls.treeview.itemtemplate)） 提供一个 datatemplate，呈现项目它。

> [!IMPORTANT]
> ItemsSource 是替代机制到 TreeView.RootNodes，将内容放到树视图控件。 不能同时设置 ItemsSource 和 RootNodes。 当你使用 ItemsSource 时，为你创建的节点和可以从 TreeView.RootNodes 属性访问它们。
...
在大多数情况下，树视图显示源的数据，因此通常声明根 TreeView 控件在 XAML 中，但在代码中或使用数据绑定添加 TreeViewNode 对象。

### <a name="bind-to-a-hierarchical-data-source"></a>绑定到分层数据源

若要创建树视图使用数据绑定，将设置为 TreeView.ItemsSource 属性的分层集合。 然后在 ItemTemplate，设置子项目集合的 TreeViewItem.ItemsSource 属性。
...
| [TreeView.ContainerFromItem](/uwp/api/windows.ui.xaml.controls.treeview.containerfromitem) | 获取指定的数据项目的 TreeViewItem 容器。 |
...
选择或取消选择一个父节点将选择或取消选择该节点下的所有子元素。 如果某些，但不是全部会选择子级的父节点下，父节点的复选框显示为不确定 （填充黑色框）。
...
此示例显示了如何创建与前面示例相同的树视图。 但是，而不是在 XAML 中创建的数据层次结构，并将数据在代码中创建绑定到树视图的 ItemsSource 属性。 （在上一示例中显示的按钮事件处理程序适用于此示例还。）
