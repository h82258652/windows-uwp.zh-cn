...
- 在扩展模式中，这两个主要和辅助命令显示。 （如果控件有仅辅助项目，它们所示的 MenuFlyout 控件类似的方法。）
...
此示例显示了如何创建命令栏浮出控件，并使用它主动和被动。 当点击该图像时，其折叠模式中显示浮出控件。 显示为上下文菜单，浮出控件是显示在其扩展模式。 在任一情况下，用户可以展开或折叠浮出控件后打开它。
...
主动显示上下文命令时，仅主要命令应显示默认情况下 （应折叠命令栏浮出控件）。 将最重要的命令放置在主要命令集合中，将从传统上讲进入上下文菜单中的辅助命令集合的其他命令。
...
默认情况下，添加到命令栏中的项目也会添加到 **PrimaryCommands** 集合中。 这些命令，命令栏中显示，并在折叠和扩展模式中可见。 CommandBar，与主要命令不自动溢出辅助命令，以及可能会被截断。
...
- 带有仅其图标; 在命令栏中显示应用栏按钮用作主要命令不显示的文本标签。 我们建议你使用工具提示显示的命令，一个文本描述如下所示。
    ```xaml
    <AppBarButton Icon="Copy" ToolTipService.ToolTip="Copy"/>
    ```
...
它们包装在 AppBarElementContainer 中，可以将其他控件添加到命令栏浮出控件。 这允许你添加控件如[DropDownButton]()或[拆分按钮]()，或添加类似[StackPanel]()创建更复杂的 UI 的容器。
...
- 要替换默认的 TextCommandBarFlyout 文本选择上显示的可以创建自定义 CommandBarFlyout （或其他浮出控件类型），并将其分配给**SelectionFlyout**属性。 将 SelectionFlyout 设置为**null**，如果没有命令将显示在选择。
