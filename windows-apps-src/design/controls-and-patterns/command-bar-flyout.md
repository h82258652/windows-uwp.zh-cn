...
[CommandBar](app-bars.md)，如 CommandBarFlyout 具有可用于添加命令的**PrimaryCommands**和**SecondaryCommands**属性。 你可以将命令放置在集合，或者两者。 何时以及如何显示主要和辅助命令取决于显示模式。

命令栏浮出控件中有两个显示模式：*折叠*和*扩展*。

- 在折叠模式中，显示仅主要命令。 如果命令栏浮出控件具有主要和辅助命令，"查看更多"按钮，该按钮由省略号 \ [• • • \] 中，将显示。 这使用户可以通过过渡到扩展模式来获取对辅助命令的访问。
- 在扩展模式中，这两个主要和辅助命令显示。 （如果控件有仅辅助项，它们是显示的方式类似于 MenuFlyout 控件。）

## <a name="is-this-the-right-control"></a>这是正确的控件吗？

使用 CommandBarFlyout 控件来显示给用户，如按钮和菜单项，应用画布上的某个元素的上下文中的命令的集合。

TextCommandBarFlyout TextBox、 TextBlock、 RichEditBox、 RichTextBlock、 和 PasswordBox 的控件中显示文本的命令。 命令自动正确配置到当前所选文本。 使用 CommandBarFlyout 替换为文本控件上的默认文本命令。
...
> 相关的信息，请参阅[浮出控件](../controls-and-patterns/dialogs-and-flyouts/flyouts.md)、[菜单和上下文菜单](menus.md)和[命令栏](app-bars.md)。
...
## <a name="proactive-vs-reactive-invocation"></a>主动与被动的调用

通常有两种方法来调用浮出控件或具有与 UI 画布上的某个元素关联的菜单：_主动调用_和_被动的调用_。

在主动调用中，命令时自动显示在用户与命令关联的项目交互。 例如，文本格式化命令可能会弹出当用户在文本框中选择文本。 在此情况下，命令栏浮出控件不会焦点。 相反，它提供了相关接近与用户交互的项的命令。 如果用户不会使用命令交互，它们可以消除。
...
此示例显示了如何创建命令栏浮出控件，并使用它，主动和被动。 当点击该图像时，其折叠模式中显示浮出控件。 当显示为上下文菜单，其扩展模式中显示浮出控件。 在任一情况下，用户可以展开或折叠浮出控件之后它已打开。
...
主动显示上下文命令时，仅主要命令应显示默认情况下 （命令栏浮出控件折叠）。 将最重要的命令放置在主要命令集合，并将从传统上讲进入上下文菜单中的辅助命令集合的其他命令。

若要主动显示命令，通常处理[单击](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click)或[Tapped](/uwp/api/windows.ui.xaml.uielement.tapped)事件，以显示命令栏浮出控件。 设置浮出控件的[ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode)为**瞬态**或**TransientWithDismissOnPointerMoveAway**以在其折叠模式中打开浮出控件，而无需获得焦点。
...
被动，为上下文菜单显示上下文命令时，辅助命令显示默认情况下 （应扩展在命令栏浮出控件）。 在此情况下，主要和辅助命令或仅辅助命令，可能有命令栏浮出控件。
...
> [!TIP]
> 有关显示浮出控件以及如何控制放置的浮出控件时的选项的详细信息，请参阅[浮出控件](../controls-and-patterns/dialogs-and-flyouts/flyouts.md)。

## <a name="commands-and-content"></a>命令和内容

CommandBarFlyout 控件具有 2 个可用于添加命令和内容的属性： [PrimaryCommands](/uwp/api/windows.ui.xaml.controls.commandbarflyout.primarycommands)和[SecondaryCommands](/uwp/api/windows.ui.xaml.controls.commandbarflyout.secondarycommands)。
...
它们包装在 AppBarElementContainer，可以将其他控件添加到命令栏浮出控件。 这允许你添加控件如[DropDownButton]()或[拆分按钮]()，或添加[StackPanel]()来创建更复杂的 UI，如容器。

若要添加到命令栏浮出控件的主要或辅助命令集合中，元素必须实现[ICommandBarElement](/uwp/api/windows.ui.xaml.controls.icommandbarelement)接口。 AppBarElementContainer 是实现此接口，以便你可以将元素添加到命令栏，即使它不会实现接口本身的包装。

在这里，AppBarElementContainer 用于将额外的元素添加到命令栏浮出控件。 拆分按钮添加到主要命令，以允许选择颜色。 StackPanel 将添加到辅助命令，以允许的缩放控件具有更复杂的布局。

> [!TIP]
> 默认情况下，用于在应用画布的元素可能看起来不正确命令栏中。 当你添加使用 AppBarElementContainer 元素时，有一些进行匹配其他命令栏元素的元素应采取的步骤：
>
...
> [!NOTE]
> 此示例显示仅在命令栏浮出控件的 UI，它并不实现的任何命令所示。 有关实现这些命令的详细信息，请参阅[按钮](buttons.md)和[命令设计基础知识](../basics/commanding-basics.md)。
...
## <a name="create-a-context-menu-with-secondary-commands-only"></a>创建辅助命令仅使用上下文菜单

你可以使用 CommandBarFlyout 使用辅助命令仅作为[上下文菜单](menus.md)，MenuFlyout 代替。
...
- 若要替换默认的文本选择显示的 TextCommandBarFlyout，可以创建自定义 CommandBarFlyout （或其他浮出控件类型），并将其分配给**SelectionFlyout**属性。 你将 SelectionFlyout 设置为**null**，如果没有命令显示所选内容。
- 若要替换默认显示为上下文菜单的 TextCommandBarFlyout，分配给**ContextFlyout**属性在文本控件上自定义 CommandBarFlyout （或其他浮出控件类型）。 如果你将 ContextFlyout 设置为**null**，而不是 TextCommandBarFlyout 显示菜单浮出控件显示在以前版本的文本控件。
