...
命令栏浮出控件，可以通过与 UI 画布上的元素相关的浮动工具栏中显示的命令将用户提供轻松访问常见任务。
...
- 在扩展模式中，显示主要和辅助命令。 （如果控件有仅辅助项目，它们所示的 MenuFlyout 控件类似的方法。）
...
TextCommandBarFlyout TextBox、 TextBlock、 RichEditBox、 RichTextBlock 和 PasswordBox 控件中显示文本的命令。 为当前选定文本，命令会自动适当配置。 使用 CommandBarFlyout 替换文本控件的默认文本命令。

要显示上下文命令列表项上的，请按照[命令用于集合和列表的上下文](collection-commanding.md)中的指南。

### <a name="commandbarflyout-vs-menuflyout"></a>CommandBarFlyout vs MenuFlyout

若要在上下文菜单中显示的命令，你可以使用 CommandBarFlyout 或 MenuFlyout。 我们建议 CommandBarFlyout，因为它提供更多的功能比 MenuFlyout。 你可以使用 CommandBarFlyout 仅辅助命令来获取的行为和外观的 MenuFlyout，或使用主要和辅助命令的完整命令栏浮出控件。
...
<td>
    <p>如果你安装了该<strong style="font-weight: semi-bold">XAML 控件库</strong>应用，单击此处<a href="xamlcontrolsgallery:/item/CommandBarFlyout">打开该应用并查看操作中的 CommandBarFlyout</a>。</p>
    <ul>
...
在被动调用中，命令都显示在响应显式用户操作请求命令;例如，右键单击。 这对应于传统的[上下文菜单](menus.md)概念。
...
此示例显示了如何创建命令栏浮出控件，并使用它主动和被动。 点击该图像时, 处于折叠模式下显示浮出控件。 当显示为上下文菜单，其扩展模式中显示浮出控件。 在任一情况下，用户可以展开或折叠浮出控件后它已打开。
...
主动显示上下文命令时，仅主要命令应显示默认情况下 （命令栏浮出控件应折叠）。 将最重要的命令放置在主要命令集合中，将从传统上讲进入上下文菜单中的辅助命令集合的其他命令。

若要主动显示命令，通常处理[单击](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click)或[Tapped](/uwp/api/windows.ui.xaml.uielement.tapped)事件以显示命令栏浮出控件。 设置浮出控件的[ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode)为**瞬态**或**TransientWithDismissOnPointerMoveAway**处于折叠模式下打开浮出控件，而无需获得焦点。
...
若要在上下文菜单中显示的命令，你通常将浮出控件分配给 UI 元素的[ContextFlyout](/uwp/api/windows.ui.xaml.uielement.contextflyout)属性中。 这样一来，打开浮出控件由该元素，并且无需执行任何其他操作。
...
[TextCommandBarFlyout](/uwp/api/microsoft.ui.xaml.controls.textcommandbarflyout)是专用的命令栏浮出控件，其中包含用于编辑文本的命令。 每个文本控件显示 TextCommandBarFlyout 自动为上下文菜单 （右键单击），或选择文本时。 文本命令栏浮出控件适应文本选择，以仅显示相关的命令。
...
- 要替换默认在文本选择时显示的 TextCommandBarFlyout，可以创建自定义 CommandBarFlyout （或其他浮出控件类型），并将其分配给**SelectionFlyout**属性。 将 SelectionFlyout 设置为**null**，如果没有命令将显示在所选内容。
