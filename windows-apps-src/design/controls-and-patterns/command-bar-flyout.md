...
TextCommandBarFlyout TextBox、 TextBlock、 RichEditBox、 RichTextBlock、 和 PasswordBox 控件中显示文本的命令。 命令会自动向当前选定文本相应地配置。 使用 CommandBarFlyout 替换为文本控件上的默认文本命令。
...
在主动调用中，命令时自动显示在用户与命令关联的项目交互。 例如，文本格式化命令可能会弹出当用户在文本框中选择文本。 在此情况下，命令栏浮出控件不会焦点。 相反，它提供了相关靠近用户与交互的项的命令。 如果用户不会与命令进行交互，它们可以消除。
...
此示例显示了如何创建命令栏浮出控件，并使用它主动和被动。 点击该图像时, 处于折叠模式下显示浮出控件。 当显示为上下文菜单，其扩展模式中显示浮出控件。 在任一情况下，用户可以展开或折叠浮出控件后打开它。
...
默认情况下，添加到命令栏中的项目也会添加到 **PrimaryCommands** 集合中。 这些命令，命令栏中显示，在折叠和扩展模式中可见。 CommandBar，与主要命令不会自动溢出辅助命令，以及可能会被截断。
...
通过它们包装在 AppBarElementContainer，可以将其他控件添加到命令栏浮出控件。 这允许你添加控件如[DropDownButton]()或[拆分按钮]()，或添加[StackPanel]()来创建更复杂的 UI，如容器。
...
> [!TIP]
> 默认情况下，用于在应用画布元素可能看起来不正确命令栏中。 当你添加元素使用 AppBarElementContainer 时，有一些进行匹配其他命令栏元素的元素应采取的步骤：
>
...
- 若要替换默认文本选择显示的 TextCommandBarFlyout，可以创建自定义 CommandBarFlyout （或其他浮出控件类型），并将其分配到**SelectionFlyout**属性。 将 SelectionFlyout 设置为**null**，如果没有命令将显示在所选内容。
