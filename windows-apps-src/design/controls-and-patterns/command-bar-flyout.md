...
ms.locfileid: "5885698"
---
# <a name="command-bar-flyout"></a>命令栏浮出控件

在命令栏浮出控件可使你能够轻松访问常见任务的用户提供通过与 UI 画布上的元素相关的浮动工具栏中显示的命令。
...
若要在上下文菜单中显示的命令，你可以使用 CommandBarFlyout 或 MenuFlyout。 我们建议 CommandBarFlyout，因为它提供更多的功能比 MenuFlyout。 你可以使用 CommandBarFlyout 仅辅助命令来获取的行为和外观 MenuFlyout，或使用主要和辅助命令的完整命令栏浮出控件。
...
在主动调用中，命令时自动显示在用户与命令关联的项目交互。 例如，文本格式化命令可能会弹出当用户在文本框中选择文本。 在此情况下，命令栏浮出控件不会焦点。 相反，它提供靠近用户与交互的项相关的命令。 如果用户不会与命令交互，它们可以消除。
...
若要主动显示命令，通常处理[单击](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click)或[Tapped](/uwp/api/windows.ui.xaml.uielement.tapped)事件，以显示在命令栏浮出控件。 设置浮出控件的[ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode)为**瞬态**或**TransientWithDismissOnPointerMoveAway**处于折叠模式下打开浮出控件，而无需获得焦点。
...
它们包装在 AppBarElementContainer 中，可以将其他控件添加到命令栏浮出控件。 这允许你添加控件如[DropDownButton]()或[拆分按钮]()，或添加像[StackPanel]()创建更复杂的 UI 的容器。
...
- 要替换默认的文本选择显示的 TextCommandBarFlyout，可以创建自定义 CommandBarFlyout （或其他浮出控件类型），并将其分配给**SelectionFlyout**属性。 将 SelectionFlyout 设置为**null**，如果没有命令将显示在选择。
