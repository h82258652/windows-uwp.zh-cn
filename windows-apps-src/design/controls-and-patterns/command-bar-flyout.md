...
- 在折叠模式中，显示仅主要命令。 如果命令栏浮出控件具有主要和辅助命令，"查看更多"按钮，该按钮由省略号 \ [• • • \]，会显示。 这使用户可以通过过渡到扩展模式来获取对辅助命令的访问。
- 在扩展模式中，显示主要和辅助命令。 （如果控件有只有辅助项目，它们所示的 MenuFlyout 控件类似的方法。）

## <a name="is-this-the-right-control"></a>这是正确的控件吗？

使用 CommandBarFlyout 控件来显示给用户，如按钮和菜单项，应用画布上的元素的上下文中的命令的集合。

TextCommandBarFlyout TextBox、 TextBlock、 RichEditBox、 RichTextBlock、 和 PasswordBox 控件中显示的文本命令。 为当前选定文本，命令会自动适当配置。 使用 CommandBarFlyout 替换文本控件的默认文本命令。
...
<td>
    <p>如果你有安装的<strong style="font-weight: semi-bold">XAML 控件库</strong>应用，单击此处<a href="xamlcontrolsgallery:/item/CommandBarFlyout">打开该应用，请参阅的实际 CommandBarFlyout</a>。</p>
    <ul>
...
在主动调用中，命令时自动显示在用户与命令关联的项目交互。 例如，文本格式化命令可能会弹出当用户在文本框中选择文本。 在此情况下，命令栏浮出控件不会焦点。 相反，它提供了相关靠近用户交互使用的项目的命令。 如果用户不会与命令交互，他们可以消除。
...
此示例显示了如何创建命令栏浮出控件，并使用它主动和被动。 当点击该图像时，其折叠模式中显示浮出控件。 当显示为上下文菜单，其扩展模式中显示浮出控件。 在任一情况下，用户可以展开或折叠浮出控件后打开它。
...
若要主动显示命令，你通常处理[单击](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click)或[Tapped](/uwp/api/windows.ui.xaml.uielement.tapped)事件以显示在命令栏浮出控件。 设置浮出控件的[ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode)为**瞬态**或**TransientWithDismissOnPointerMoveAway**在其折叠模式下打开浮出控件，而无需获得焦点。
...
若要显示命令的上下文菜单中，你通常将浮出控件分配给 UI 元素的[ContextFlyout](/uwp/api/windows.ui.xaml.uielement.contextflyout)属性中。 这样一来，打开在浮出控件由元素，并且你无需执行任何其他操作。
...
你还可以将命令添加到**SecondaryCommands**集合。 辅助命令显示的控件的菜单部分中，并且仅在扩展模式中可见。
...
- 仅其图标; 在命令栏中显示应用栏按钮用作主要命令不会显示的文本标签。 我们建议你使用工具提示显示的命令，一个文本描述如下所示。
    ```xaml
    <AppBarButton Icon="Copy" ToolTipService.ToolTip="Copy"/>
    ```
...
此处，AppBarElementContainer 用于将额外的元素添加到命令栏浮出控件。 拆分按钮添加到主要命令以允许选择的颜色。 StackPanel 将添加到允许的缩放控件具有更复杂的布局的辅助命令。
...
[TextCommandBarFlyout](/uwp/api/microsoft.ui.xaml.controls.textcommandbarflyout)是专用的命令栏浮出控件，其中包含用于编辑文本的命令。 每个文本控件显示 TextCommandBarFlyout 自动为上下文菜单 （右键单击），或当文本处于选中状态。 文本命令栏浮出控件适应文本选择，以仅显示相关的命令。
...
- 若要替换默认的文本选择显示的 TextCommandBarFlyout，可以创建自定义 CommandBarFlyout （或其他浮出控件类型），并将其分配给**SelectionFlyout**属性。 将 SelectionFlyout 设置为**null**，如果没有命令将显示在选择。
- 若要替换默认显示为上下文菜单的 TextCommandBarFlyout，分配到**ContextFlyout**属性在文本控件上自定义 CommandBarFlyout （或其他浮出控件类型）。 如果将 ContextFlyout 设置为**null**，而不是 TextCommandBarFlyout 显示菜单浮出控件显示在以前版本的文本控件。
