...
TextCommandBarFlyout TextBox、 TextBlock、 RichEditBox、 RichTextBlock，以及 PasswordBox 控件中显示文本的命令。 命令会自动向当前的文本选择相应地配置。 使用 CommandBarFlyout 替换为文本控件上的默认文本命令。
...
若要主动显示命令，通常可处理[单击](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click)或[Tapped](/uwp/api/windows.ui.xaml.uielement.tapped)事件来显示命令栏浮出控件。 设置浮出控件的[ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode)为**瞬态**或**TransientWithDismissOnPointerMoveAway**打开浮出控件处于折叠模式下无需使焦点。

从 Windows 10 Insider Preview 开始，文本控件具有**SelectionFlyout**属性。 当你将在浮出控件分配给此属性时，它时自动显示文本处于选中状态。
...
默认情况下，添加到命令栏中的项目也会添加到 **PrimaryCommands** 集合中。 这些命令，命令栏中显示，在折叠和扩展模式中可见。 CommandBar，与主要命令不自动溢出辅助命令，以及可能会被截断。
...
若要添加到命令栏浮出控件的主要或辅助命令集合中，元素必须实现[ICommandBarElement](/uwp/api/windows.ui.xaml.controls.icommandbarelement)接口。 AppBarElementContainer 是实现此接口，以便你可以将某个元素添加到命令栏，即使它不会实现接口本身的包装。
...
- 若要替换默认 TextCommandBarFlyout 文本选择上显示的可以创建自定义 CommandBarFlyout （或其他浮出控件类型），并将其分配给**SelectionFlyout**属性。 将 SelectionFlyout 设置为**null**，如果没有命令将显示在选择。
