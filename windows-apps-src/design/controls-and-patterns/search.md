...
**零输入。** 后用户已激活输入的字段，但用户已输入文本之前，你可以显示所谓的"零输入的画布。 零输入画布通常显示在应用画布中，以便[自动建议](auto-suggest-box.md)在用户开始输入查询时替换此内容。 最近搜索历史记录、热门搜索、上下文搜索建议、提示和使用技巧都非常适用于零输入状态。
...
**结果集。** 搜索结果通常直接显示下方的搜索输入字段。 尽管这不是一项要求，但并置的输入和结果可保留上下文，并且向用户提供即时访问以编辑以前的查询或输入新查询。 通过将提示文本替换为创建了结果集的查询，可以进一步传递此连接。
...
**我的资料。** 搜索跨设备、 云、 社交图片和详细信息。 结果各种各样，但受到与用户帐户的连接约束。
