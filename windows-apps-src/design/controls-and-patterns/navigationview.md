...
在中等屏幕，并在较大的屏幕上的左适应小屏幕上的 LeftMinimal、 LeftCompact 之间。 请参阅有关详细信息的[自适应行为](#adaptive-behavior)部分。
...
满足硬件和软件要求时，NavigationView 会自动使用[亚克力材料](../style/acrylic.md)在其窗格中，并仅在其左侧窗格中[显示突出显示](../style/reveal.md)。
...
NavigationView 的默认自适应行为是在较小的窗口宽度上显示较大的窗口宽度展开左侧窗格中，中等大小的窗口宽度上左仅限图标的导航窗格和汉堡菜单按钮。 有关自适应行为的窗口大小的详细信息，请参阅[屏幕大小和断点](../layout/screen-sizes-and-breakpoints-for-responsive-design.md)。
...
第二个常见的自适应模式是较大的窗口宽度和在这两个中等和小窗口宽度汉堡菜单上使用展开的左侧窗格。
...
第三个常见的自适应模式是在较大的窗口宽度，并且这两个中小型窗口宽度左仅限图标的导航窗格上使用展开的左侧窗格。 良好示例是邮件应用。
...
- 这样，如果此设置中的一个类别不适合在屏幕上，你折叠到左侧的导航，以便为他们提供同等重要具有同等重要的顶级导航类别显示在一起，一组。
...
在选项卡模型中，选择和焦点相关联。 通常将焦点移将还转移选择操作。 在以下示例中，右箭头会移动选择指示器显示到放大镜。 你可以通过[SelectionFollowsFocus](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.selectionfollowsfocus)属性设置为启用达到此目的。
...
如果你想要更改默认样式，你可以替代 NavigationView 的[MenuItemContainerStyle](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.menuitemcontainerstyle)属性。 你还可以设置[MenuItemTemplate](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.menuitemtemplate)属性来指定一个不同的数据模板。
