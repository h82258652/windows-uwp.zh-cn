...
中等屏幕和大屏幕上的左边适应小屏幕上都具有 LeftMinimal、 LeftCompact 之间。 请参阅有关详细信息的[自适应行为](#adaptive-behavior)部分。
...
第二个常见的自适应模式是较大窗口宽度和两个中等和小窗口宽度汉堡菜单上使用展开的左侧窗格。
...
第三个常见的自适应模式是展开的左侧窗格中使用较大窗口宽度，这两个中等和小窗口宽度为仅图标的左侧的导航窗格。 良好示例是邮件应用。
...
- 你有同等重要顶级导航类别显示在一起，一组，以便在屏幕上不符合此组中的一个类别，如果你折叠到左侧的导航，以便为他们提供同样重要。
...
在选项卡模型中，选择和焦点相关联。 通常将焦点移将还转移选择操作。 在以下示例中，右箭头会选择指示器从移动显示到放大镜。 你可以通过[SelectionFollowsFocus](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.selectionfollowsfocus)属性设置为启用达到此目的。
...
:::row:::
    ...
    :::column:::
     <b>顶级导航</b><br>
    ![NavigationView 顶级导航上的后退按钮](images/topnav-back.png)
    :::column-end:::
:::row-end:::
...
对于无缝的外观和感觉，如果你的应用具有使用 ScrollViewer 的页面，并且你导航窗格顶部放置，我们建议在顶部导航窗格下方的内容滚动。 这使应用粘滞标头类型的行为。
...
有时，如果用户向下滚动，你可能想要隐藏导航窗格中，通过上 NavigationView 的[IsPaneVisible](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.ContentOverlay)属性设置为 false 来实现。
