...
本教程介绍关于使用 XAML 的自适应和定制布局功能的基础知识，利用这些功能，可以创建外观适用于任何设备的应用。 我们将学习如何创建新的 DataTemplate、添加窗口吸附点，以及使用 VisualStateManager 和 AdaptiveTrigger 元素定制应用的布局。 我们将使用这些工具针对较小的设备屏幕优化图像编辑程序。 
...
## <a name="part-1-run-the-mobile-emulator"></a>第 1 部分：运行移动版模拟器

在 Visual Studio 工具栏中，确保将解决方案平台设置为 x86 或 x64 而不是 ARM，然后将目标设备从本地计算机更改为你已安装的移动版模拟器之一（例如，5 英寸的 1GB Mobile Emulator 10.0.15063 WVGA）。 请尝试在你通过按 **F5** 选择的移动版模拟器中运行“照片库”应用。
...
我们现在已经为我们的数据创建了新布局，但该应用目前无法知道何时优先于默认样式使用此布局。 为了解决此问题，我们需要添加 **VisualStateManager**。 将以下代码添加到 **RelativePanel** 页面根元素中。
...
接下来，我们将使用 **VisualState** 资源库告诉 **VisualStateManager** 在触发状态时要应用哪些属性。 每个资源库都以特定 XAML 元素的一个属性为目标，并将其设置为给定值。 将以下代码添加到刚创建的移动版 **VisualState** 中 **VisualState.StateTriggers** 元素的下面。 
...
现在，请尝试使用移动版模拟器运行应用。 是否会成功显示新布局？ 应该会看到较小缩略图的网格，如下所示。 如果看到的仍然是旧布局，则 **VisualStateManager** 代码中可能有拼写错误。
...
在 Visual Studio 工具栏中，将目标设备设置为“本地计算机”  ，并运行该应用。 加载该应用时，请尝试更改窗口的大小。 将窗口缩小为小尺寸时，应该会看到应用切换为你在第 2 部分中创建的移动布局。
