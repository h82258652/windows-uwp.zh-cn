...
从高层次上看，将应用的部分文件拆分到其他包（非体系结构程序包）后，你将无法直接访问那些相对于你的代码运行位置的文件。 原因在于这些软件包跟你的体系结构程序包安装到了不同的目录。 例如，如果您要做一个游戏，并且您的游戏本地化为法语和德语，为构建 x86 和 x64 计算机，则应具有这些游戏的应用程序包内的应用包文件：
...
每个应用包文件时您的游戏安装到用户的计算机上，将具有其自己的文件夹**WindowsApps**目录。 因此，对于运行 64 位 Windows 的法国用户，你的游戏将如下所示：
