...
| [使用 WinAppDeployCmd.exe 工具安装应用](install-universal-windows-apps-with-the-winappdeploycmd-tool.md) | Windows 应用程序部署 (WinAppDeployCmd.exe) 是可用于将部署到任意 windows 10 移动版设备从 windows 10 计算机的 UWP 应用的命令行工具。 你可以使用此工具时在 windows 10 移动版设备而无需为该应用的 Microsoft Visual Studio 或解决方案是通过 USB 连接还是位于同一子网部署应用包。 本文介绍如何使用此工具安装 UWP 应用。 |
