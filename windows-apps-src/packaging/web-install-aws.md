...
选择**创建桶**，并输入你的网站**存储段名称**。 按照对话框提示用于设置属性和权限。 若要确保你的 UWP 应用，可以从你的网站分配，启用**读取**和**写入**你存储段的权限，并选择**公共读可访问此存储桶**。

![设置 Amazon S3 桶权限](images/aws-permissions.png) 

查看摘要并确认所选的选项将会反映出来。 单击**创建桶**以完成此步骤。 
...
现在，我们将准备要上传应用包和我们想要托管在我们 Amazon S3 存储桶中的 web 页面。 
...
类似于创建 Amazon S3 桶的权限，存储段中的内容必须还有**读取**、**写入**和**授予对此对象的公共读取访问**权限。
...
> [!IMPORTANT]
> 你上传 web 页面之前，确认你的 web 页面中的应用程序包引用正确无误。 

若要获取应用程序包引用，请首先上传应用包并复制的应用包 URL。 编辑 html web 页面，以反映正确的应用程序包路径。 查看更多详细信息的代码示例。 

选择要获取的参考链接到应用包的已上传的应用包文件，它应该类似于以下示例：
...
一旦网页上载到 Amazon S3 桶，通过选择已上传的 html 文件获取到网页的链接。

使用以下链接以打开 web 页面。 由于我们设置的权限，以授予对应用包和网页的公共访问权限，因此附带指向网页的链接的任何人都将能够访问它，并且安装 UWP 应用包使用应用安装程序。 请注意，应用安装程序是 Windows 10 平台的一部分。 作为开发人员，你不需要将任何其他代码或功能添加到你的应用能够使用应用安装程序。 
