...
设置步骤 1 到 4 你程序包依赖关系后，你可以继续开发像往常一样。 如果你想要将代码从可选包加载到主程序包，你将需要生成一组相关。 查看更多详细信息[相关设置](#related_sets)部分。
...
如果你想要将代码从可选包加载到主程序包，你将需要生成一组相关。 若要生成的相关的集，必须紧密耦合你的主要包和可选包。 .Appxbundle 或.msixbundle 主程序包的文件中指定相关集的元数据。 Visual Studio 可帮助你在文件中获取正确的元数据。 若要配置的相关集的应用的解决方案，请使用以下步骤：
...
调试相关的集的可选项目是当前不支持在 Visual Studio 中。 若要解决此问题，你可以部署和启动激活 （Ctrl + F5） 和手动附加到进程的调试程序。 若要附加调试程序，在 Visual Studio 中的"调试"菜单，选择"附加到进程..."，过头来将调试程序附加到**主应用进程**。