...
进程外后台任务是更容易复原，因为如果发生错误，后台进程不能使应用崩溃。 但复原的代价在于管理跨进程通信的应用和后台任务之间的更为复杂的价格。
...
在 Windows10 版本 1607 中，可以在无需创建后台任务的情况下启用后台活动。 你可以改为运行后台代码直接在前台应用程序的过程。
...
在应用启动期间检查你的后台任务注册。 确保你的应用取消的后台任务中 BackgroundTaskBuilder.AllTasks 存在。 重新注册的不存在。 不再需要的任何任务取消注册。 这将确保所有后台任务注册在每次启动应用时都是最新。
