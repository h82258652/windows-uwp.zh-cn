...
你可以通过向应用的 package.appxmanifest 文件添加扩展，将应用注册为“自动播放内容”或“自动播放设备”事件处理程序。 如果使用 Visual Studio，则可以在**声明选项卡**中添加**自动播放内容**或**自动播放设备**。如果要直接编辑应用的 package.appxmanifest 文件，请向将 **windows.autoPlayContent** 或 **windows.autoPlayDevice** 指定为**类别**的程序包清单添加 [**Extension**](https://msdn.microsoft.com/library/windows/apps/br211400) 元素。 例如，程序包清单中的以下条目添加**自动播放内容**扩展，以将应用注册为 **ShowPicturesOnArrival** 事件的处理程序。
