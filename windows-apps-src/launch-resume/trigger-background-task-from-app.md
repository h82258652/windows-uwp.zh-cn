...
如果你正在考虑使用应用程序触发器来扩展你的前台进程生命周期，请考虑改为使用[扩展执行](run-minimized-with-extended-execution.md)。 应用程序触发器用于创建单独宿主的进程中执行工作。 下面的代码段注册进程外后台触发器。
...
- CPU 时间： 后台任务受基于触发器类型时，他们获取的时钟时间内使用时间量。 通过应用程序触发器触发后台任务并局限于大约 10 分钟。
