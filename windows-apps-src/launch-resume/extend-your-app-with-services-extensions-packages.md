...
有多种技术在 Windows 10 中得到扩展和组件化你的应用。 此表应可以帮助你确定应使用根据要求的技术。 后面是方案和技术的简要说明。
...
在进程内与应用一起运行的代码。 仅支持 C++。 还可能包括内容。 由于代码在进程内运行，所以假定更高级别的信任。 你可以选择不公开此类的第三方扩展。
...
**资产包**资产包是以供你的应用的可执行文件，或非可执行文件的公用集中源。 它们通常是非处理器或语言特定的文件。 例如，可以在一个资产包中包含一系列图片，在另一个资产包中包含视频，两种资产都由同一个应用使用。 如果你的应用支持多个体系结构和多种语言，这些资源可以包含在体系结构包或资源包中，但这也意味着资产将重复多次跨不同的体系结构程序包中，执行磁盘空间。 如果使用资产包，则只需将其包含在整个应用包中一次。 有关详细信息，请参阅[资产包简介](../packaging/asset-packages.md)。

**可选包**

可选包用于补充或扩展应用包的原始功能。 可先发布应用，晚些时候再发布可选包，或同时发布应用和可选包。 通过可选包来扩展应用，你将拥有将内容作为单独的应用包来分发和盈利的优势。 可选包通常由原始应用开发人员来开发，因为它们用主应用的标识来运行（与应用扩展不同）。 根据定义可选包的方式，你可以从可选包向主应用中加载代码、资产或代码和资产。 如果你需要增强你的应用包含的内容，可以盈利、 许可和单独分布式，那么可选包可能会为你的正确选择。 有关实现详细信息，请参阅[可选包和相关集创作](https://docs.microsoft.com/windows/uwp/packaging/optional-packages)。
...
应用服务是可向其他 UWP 应用提供服务的 UWP 应用。 它们是类似于在设备上的 web 服务。 应用服务作为后台任务在主机应用中运行，并可向其他应用提供其服务。 例如，应用服务可能会提供其他应用可能使用的条形码扫描仪服务。 应用的企业套件中可能有一个通用的拼写检查应用服务，该服务可供套件中的其他应用使用。
