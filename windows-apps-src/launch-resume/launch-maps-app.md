...
了解如何从你的应用启动 Windows 地图应用。 本主题介绍 **bing maps:， *ms-到驱动器:、 ms-到遍历：** 和**ms 设置：** 统一资源标识符 (URI) 方案。 使用这些 URI 方案来将 Windows 地图应用启动为特定的地图、路线和搜索结果或从“设置”应用下载 Windows 地图离线地图。
...
-   **Bing maps:** URI 提供地图位置、 搜索结果、 方向和流量。
-   **Ms-到驱动器：** URI 提供打开通过启用从你当前的位置的行车路线。
-   **Ms-到审核：** URI 提供打开通过打开步行方案从你的当前位置。
...
若要启动该 Windows 映射应用程序从您的应用程序，请调用[ **LaunchUriAsync** ](https://msdn.microsoft.com/library/windows/apps/hh701476)方法替换**bing maps:**， **ms-到驱动器：**，或**ms-到审核：** URI。 以下示例启动前一个示例中相同的 URI。 有关通过 URI 启动应用的详细信息，请参阅[启动 URI 的默认应用](launch-default-app.md)。
...
| bingmaps:?collection=point.40.726966\_-74.006076\_一些 %255fbusiness        | 使用名为一些的点显示地图\_业务 （在拉斯维加斯举行）。                                                                                                                               |
...
| bingmaps:？ bb = 39.719\_-74.52 ~ 41.71\_-73.5 & q = 比萨              | 在指定的边框（即在纽约市）搜索比萨。      |
...
| bingmaps:?collection=point.36.116584\_-115.176753\_Caesars%20Palace~point.36.113126\_-115.175188\_%20bellagio & lvl = 16 & cp = 36.114902 ~-115.176669                   | 显示名为恺撒王宫酒店和百乐宫酒店（均位于拉斯维加斯）的两枚图钉，缩放级别为 16。              |
| bingmaps:?collection=point.40.726966\_-74.006076\_虚设 %255fbusiness%255Fwith %255funderscore                                                                        | 使用名为虚设图钉显示纽约\_业务\_与\_下划线。                                                  |
| bingmaps:？ 集合 = 名称。Hotel%20List~point.36.116584\_-115.176753\_Caesars%20Palace~point.36.113126\_-115.175188\_%20bellagio & lvl = 16 & cp = 36.114902 ~-115.176669 | 显示名为酒店列表的列表，以及拉斯维加斯的恺撒王宫酒店和百乐宫酒店的两枚图钉，缩放级别为 16。 |
...
| 示例 URI                                                                                                              | 结果                                                                                                                                                         |
|-------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| bingmaps:?rtp=pos.44.9160\_-110.4158 ~ pos.45。 0475年\_-109.4187                                                             | 显示带有点对点路线的地图。 因为未指定 *mode*，所以将使用交通首选项的用户的模式提供路线。 |
| bingmaps:?cp=43.0332~-87.9167&amp;trfc=1                                                                                    | 显示以威斯康辛州的密尔沃基市为中心的地图以及路况。                                                                                                        |
| bingmaps:？ rtp = adr。一个 Microsoft 的方式，Redmond，WA 98052~pos.39.0731\_-108.7238                                           | 显示带有从指定的地址到指定位置的路线的地图。                                                                            |
| bingmaps:?rtp=adr.1%20Microsoft%20Way,%20Redmond,%20WA,%2098052~pos.36.1223\_-111.9495\_总计 %20canyon%20northern %20rim | 显示从 1 Microsoft Way, Redmond, WA, 98052 到 Grand Canyon's northern rim 的路线。                                                                |
...
**Ms-到驱动器：** 和**ms-到审核：** URI 方案，可以启动直接置于启用通过打开视图中的路由。 这些 URI 方案只能提供自用户的当前位置开始的路线。 如果您必须提供不包括用户的当前位置的点之间的方向，使用**bing maps:** 在上一部分中所述的 URI 方案。 有关这些 URI 方案的详细信息，请参阅 [ms-drive-to:](#ms-drive-to-param-reference) 和 [ms-walk-to:](#ms-walk-to-param-reference) 参数引用。

> **重要**时**ms-到驱动器：** 或**ms-到审核：** 启动 URI 方案，地图应用程序将检查以查看是否设备已执行的修复的 GPS 位置。 如果有，则“地图”应用将继续进行路线规划。 如果没有，应用将显示路线概述，如[显示路线和路况](#display-directions-and-traffic)中所述。
...
**Ms 设置：** URI 方案，可以直接在设置应用中的特定页启动。 虽然**ms 设置：** URI 方案不会启动到地图应用程序，它允许您直接到设置应用中的脱机映射页启动并显示确认对话框中，若要下载脱机映射映射应用使用。 URI 方案接受由纬度和经度指定的点，并自动确定是否存在可用于包含该点的离线地图。  如果经过的纬度和经度恰好落在多个下载区域内，确认对话框将让用户选取要下载其中哪一个区域。 如果离线地图不适用于包含该点的区域，则“设置”应用中的“离线地图”页面将显示一个错误对话框。
...
<li>:显示映射的鸟瞰图。</li>
<li>**R**:显示道路地图的视图。</li>
<li>**3d**:显示 3D 地图的视图。 可与 **cp** 参数结合使用，也可以与 **rad** 参数结合使用。</li>
</ul>
<p>在 Windows 10 中，鸟瞰图和 3D 视图样式相同。</p>
<div class="alert">
**请注意**  Omitting **sty**参数生成相同的结果作为 sty = r。
</div>
<div>
 
</div></td>
...
<div class="alert">
**请注意**  标题可以使用某一位置，如果指定的位置**pos**参数值。 将显示标题，而不是显示纬度和经度。
</div>
<div>
 
</div></td>
...
语法映射应用程序特定参数**ms 设置：** URI 方案定义如下： **映射 downloadmaps**与指定**ms 设置：** URI 中的窗体**ms-设置： 映射-downloadmaps？** 来指示脱机映射设置页。 
