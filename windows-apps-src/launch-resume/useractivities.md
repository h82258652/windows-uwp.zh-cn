...
此代码用于检测应用是否是通过协议激活的。 如果是，它会设法确定应用应执行什么操作来继续之前的任务（应用因此任务被激活）。 作为一个简单的应用，此应用程序恢复的唯一活动是在页面上放置辅助当应用启动后。
...
* 详细了解如何[ **UserActivity** API](https://docs.microsoft.com/uwp/api/windows.applicationmodel.useractivities)
