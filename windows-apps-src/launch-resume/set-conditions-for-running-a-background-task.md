...
有时，后台任务需要后台任务成功满足某些条件。 你可以在注册后台任务时指定由 [**SystemConditionType**](https://msdn.microsoft.com/library/windows/apps/br224835) 指定的一个或多个条件。 引发触发器之后，将检查条件。 然后将排队的后台任务，但满足所有所需的条件之前不会运行。
...
添加条件之前，创建[**SystemCondition**](https://msdn.microsoft.com/library/windows/apps/br224834)对象来表示必须实际用于运行后台任务的条件。 在构造函数中，指定[**SystemConditionType**](https://msdn.microsoft.com/library/windows/apps/br224835)枚举值，必须满足的条件。
