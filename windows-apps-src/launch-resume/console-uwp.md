...
- UWP 控制台应用可能无法创建一个窗口。 它们无法使用 messagebox （） 或 Location() 或出于任何原因，可能会创建一个窗口的任何其他 API，如用户同意提示。
