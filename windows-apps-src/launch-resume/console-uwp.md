...
ms.locfileid: "2815489"
---
# <a name="create-a-universal-windows-platform-console-app"></a>创建通用 Windows 平台控制台应用

本主题介绍如何创建[C + + / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt)或 C + + / CX 通用 Windows 平台 (UWP) 控制台应用程序。

使用 Windows 10，版本 1803、 启动您可以编写 C + + / WinRT 或 C + + / CX UWP 的控制台窗口中，如 DOS 或 PowerShell 控制台窗口中运行的控制台应用程序。 控制台应用程序使用控制台窗口的输入和输出，并且可以使用[通用 C 运行时](/cpp/c-runtime-library/reference/crt-alphabetical-function-reference)功能，如**printf**和**getchar**。 UWP 控制台应用可以发布到 Microsoft Store。 它们在应用列表中有对应条目，并有可以固定到“开始”菜单的主要磁贴。 可以从开始菜单中，启动 UWP 控制台应用程序，但通常将从命令行启动它们。

若要查看一个在操作，下面是有关创建 UWP 控制台应用程序的视频。

> [!VIDEO https://www.youtube.com/embed/bwvfrguY20s]

## <a name="use-a-uwp-console-app-template"></a>使用 UWP 控制台应用模板 

若要创建 UWP 控制台应用，请首先安装 [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=AndrewWhitechapelMSFT.ConsoleAppUniversal)中提供的**控制台应用（通用）项目模板**。 已安装的模板，可在**新建项目**下 > **Installed** > **其他语言** > **Visual c + +** > **Windows 通用**作为**控制台应用程序使用 + / WinRT (通用 Windows)** 和**控制台应用程序 C + + / CX (通用 Windows)**。
...
通过添加下面的示例的**Program.cpp** **控制台应用程序使用 + / WinRT**模板：
...
- 仅 C + + / WinRT 和 C + + / CX UWP 应用程序可能是控制台应用程序。
- UWP 控制台应用必须针对桌面或 IoT 项目类型。
- UWP 控制台应用程序可能不会创建一个窗口。 他们无法使用 MessageBox()，Location()，或者可以出于任何原因，创建一个窗口的任何其他 API，如用户同意提示。
