...
-   在系统请求为项目选择**目标版本**时，请至少选择 **10.0.14393**。 如果你想要使用新的 `SupportsMultipleInstances` 属性，则必须使用 Visual Studio 2017 并面向 **10.0.15063**（**Windows 10 创意者更新**）或更高版本。
...
**SupportsMultipleInstances** 属性指示每次调用应用服务时，它应在新进程中运行。 虽然此属性不是必需的，但是如果需要该功能，并且目标是 `10.0.15063`SDK（**Windows 10 创意者更新**）或更高版本，则可以使用此属性。 还应该对其加上 `uap4` 命名空间前缀。
...
2.  在 AppServiceProvider 项目中，添加对新 MyAppService 项目的项目间引用。 此步骤至关重要，因为虽然在不添加引用的情况下看不到任何编译错误，但应用服务将不会在运行时进行连接。
...
- 验证 `<Extensions>` 项及其子元素是否已添加至 Package.appxmanifest 文件，该文件属于[将应用服务扩展添加到 package.appxmanifest](#add-an-app-service-extension-to-package-appxmanifest) 中上面指定的应用服务提供程序项目。
...
- 确保 `AppServiceConnection.PackageFamilyName` 与[将应用服务扩展添加到 package.appxmanifest](#add-an-app-service-extension-to-package-appxmanifest) 中上面指定的应用服务提供程序组件的包系列名称匹配
