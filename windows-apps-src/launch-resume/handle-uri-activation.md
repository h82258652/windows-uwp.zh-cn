---
处理 URI 激活
了解如何将应用注册为统一资源标识符 (URI) 方案名称的默认处理程序。
...
> **注意** 在 UWP 应用中，保留某些 URI 和文件扩展名以供内置应用和操作系统使用。 使用保留的 URI 或文件扩展名注册应用的尝试将被忽略。 参阅[保留 URI 方案名称和文件类型](reserved-uri-scheme-names.md)以获取无法注册为 UWP 应用的 Uri 方案（因为它们为保留或禁止的文件类型）的字母顺序列表。
...
1.  在**“解决方案资源管理器”**中，双击 package.appxmanifest 打开清单设计器。 选择**“声明”**选项卡，并在**“可用声明”**下拉列表中，选择**“协议”**，然后单击**“添加”**。
