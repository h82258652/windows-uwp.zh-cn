---
author: stevewhims
description: Windows 运行时是引用计数系统;请务必要了解有关的重要性和区别，系统中，并强和弱引用。
title: C++/WinRT 中的弱引用
ms.author: stwhi
ms.date: 10/03/2018
ms.topic: article
...
ms.locfileid: "4535137"
---
# <a name="strong-and-weak-references-in-cwinrt"></a>强引用和弱引用在 C + + WinRT

Windows 运行时是引用计数系统;和在这样的系统很重要强了解的重要性和区别，弱引用 （和既不，如隐式*此*指针的引用）。 你将看到在本主题中，了解如何管理这些引用正确意味着可靠的系统运行顺畅和居中崩溃的一个之间的区别。 通过提供具有语言投影，深入支持的帮助程序函数[C + + WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt)满足中间中生成更复杂的系统，只需并正常工作。
...
通过这项更改的代码，我们会遇到问题在步骤 4，因为此类实例已被删除，并且*此*不再有效。 只要异步对象会尝试访问内部类实例的变量，它将崩溃 （或执行完全未定义）。
...
模式是事件接收者在其*此*指针具有依赖关系的 lambda 事件处理程序。 每当事件接收者的生存时间超过事件源，其生存时间超过这些依赖项。 和在这些情况下，通用的该模式运行良好。 这些情况中有一部分很明显，例如当 UI 页面处理由页面上的控件引发的事件时。 在页面的生存时间超过该按钮&mdash;因此，在处理程序还生存时间超过该按钮。 每当接收者拥有源（例如作为数据成员）时，或者每当接收者和源是同级且直接由其他某个对象拥有时，就会出现这种情况。 如果你不确定是否遇到处理程序的生存时间不会超过它依赖的 *this* 的情况，则通常可以捕获 *this* 而不考虑强或弱生存时间。

但仍有*此*不生存时间超过 （包括完成和进度由异步操作和运算引发的事件处理程序），在处理程序中使用，并且务必知道如何处理这些情况。
...
事件接收者将被销毁，但**事件**仍订阅中的 lambda 事件处理程序。 引发该事件后，lambda 尝试取消引用*此*指针，此时无效。 因此，从代码中处理程序 （或在协同程序的延续中） 产生访问冲突尝试使用它。

> [!IMPORTANT]
> 如果你遇到这种情况，然后你将需要考虑*此*对象; 的生命周期并且，指示捕获*此*对象的生存时间超过捕获。 如果没有，则使用捕获它的强引用或弱引用，因为我们将展示下方。
>
