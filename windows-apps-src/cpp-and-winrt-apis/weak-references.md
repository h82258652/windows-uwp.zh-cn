---
author: stevewhims
description: Windows 运行时是一种引用计数系统;和在系统务必要了解有关的重要性和之间，区别强和弱引用。
title: C++/WinRT 中的弱引用
ms.author: stwhi
ms.date: 10/03/2018
ms.topic: article
...
ms.locfileid: "5404108"
---
# <a name="strong-and-weak-references-in-cwinrt"></a>强引用和弱引用在 C + + WinRT

Windows 运行时是一种引用计数系统;并且，此类系统中请务必了解的重要性和之间，区别强弱引用 （和既不，如隐式*此*指针的引用）。 你将看到在本主题中，了解如何管理这些引用正确可能意味着可靠的系统运行顺畅和居中崩溃的一个之间的区别。 通过提供语言投影中, 具有深入支持的帮助程序函数[C + + WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt)符合你在你的工作生成更复杂的系统，只需并正确的中间。
...
通过这项更改到代码，我们会遇到问题在步骤 4，因为类实例已被删除，并且*此*不再有效。 只要异步对象会尝试访问内类实例的变量，它将崩溃 （或执行完全未定义）。

解决方案是为了提供异步操作&mdash;协同程序&mdash;类实例自己强引用。 为当前编写的代码，该协同程序有效地保留原始*此*指针的类实例中;但这是不足够使类实例保持活动状态。

若要使类实例保持活动状态，更改到，如下所示的**RetrieveValueAsync**实现。
...
因为 C + + /winrt 对象直接或间接派生[**winrt:: implements**](/uwp/cpp-ref-for-winrt/implements)模板的 C + + /winrt 对象可以调用其[**implements.get_strong**](/uwp/cpp-ref-for-winrt/implements#implementsgetstrong-function)受保护成员函数以检索其*此*指针的强引用。 请注意，如果没有无需实际上使用`strong_this`变量;只需调用**get_strong**增加你引用计数，并使你隐式*此*指针保持有效。
...
在上述示例中，弱引用不会保留从时没有强引用保持被销毁类实例。 但是，它会为你提供一种方法检查是否可以访问成员变量之前获取的强引用。
...
在上一节突出显示的协同程序和并发区域中的潜在生命周期问题。 但是，如果你处理事件的对象的成员函数，或需要考虑事件接收者 （处理事件的对象） 和事件源 （的对象的相对生存期对象的成员函数，则你的 lambda 函数引发事件）。 让我们看一下一些代码示例。
...
模式是事件接收者在其*此*指针有依赖项的 lambda 事件处理程序。 每当事件接收者的生存时间超过事件源，其生存时间超过这些依赖项。 和在这些情况下，通用的该模式运行良好。 这些情况中有一部分很明显，例如当 UI 页面处理由页面上的控件引发的事件时。 在页面的生存时间超过该按钮&mdash;因此，在处理程序还的生存时间超过该按钮。 每当接收者拥有源（例如作为数据成员）时，或者每当接收者和源是同级且直接由其他某个对象拥有时，就会出现这种情况。 如果你不确定是否遇到处理程序的生存时间不会超过它依赖的 *this* 的情况，则通常可以捕获 *this* 而不考虑强或弱生存时间。

但仍有*此*不的生存时间超过 （包括完成和进度由异步操作和运算引发的事件处理程序），在处理程序中使用，并且务必知道如何处理这些情况。
...
事件接收者将被销毁，但在其中的 lambda 事件处理程序仍订阅**事件**。 当引发该事件时，lambda 尝试取消引用*此*指针，此时无效。 因此，从代码中处理程序 （或在协同程序的延续中） 产生访问冲突尝试使用它。

> [!IMPORTANT]
> 如果你遇到这种情况，然后你将需要考虑*此*对象; 的生命周期和捕获*此*对象的生存时间超过捕获。 如果没有，则使用捕获它的强引用或弱引用，如我们将如下所示。
>
...
解决方案是捕获的强引用。 *执行*强引用增加引用计数，并且它*执行*保持当前对象保持活动状态。 你只需声明捕获变量 (称为`strong_this`在此示例中)，并将其初始化通过调用[**implements.get_strong**](/uwp/cpp-ref-for-winrt/implements#implementsgetstrong-function)，检索我们*此*指针的强引用。
...
这是指对象和其成员函数的标准的传统方法。 若要使其成为安全，你可以&mdash;从 Windows sdk 版本 10.0.17763.0 (Windows 10 版本 1809年)&mdash;建立的强引用或弱引用在其中注册处理程序的点。 此时，事件收件人对象被认为是仍处于活动状态。
...
对于弱引用，调用[**get_weak**](/uwp/cpp-ref-for-winrt/implements#implementsgetweak-function)。 C + + WinRT 可确保结果委托保留的弱引用。 在最后一分钟，在后台，该委托会尝试解析到一个强大的弱引用并仅调用成员函数，如果成功。
...
上方，我们可以看到正在使用的弱引用。 一般情况下，它们适合于破坏循环引用。 例如，对于基于 XAML 的 UI 框架的本机实现&mdash;由于框架的历史设计&mdash;的弱引用在 C + + /winrt 是来处理循环引用。 在 XAML 之外，不过，你可能不需要使用弱引用 (没有任何内容不本质上是特定于 XAML 的有关它们)。 相反，通常会，应该能够设计你自己的 C + + 避免需要进行循环引用和弱引用的方式的 WinRT Api。 
