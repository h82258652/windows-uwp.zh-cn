---
author: stevewhims
description: Windows 运行时是一种引用计数系统;请务必要了解有关的、 重要性和区别，系统中，并强和较弱引用。
title: C++/WinRT 中的弱引用
ms.author: stwhi
ms.date: 10/03/2018
ms.topic: article
...
ms.locfileid: "5599635"
---
# <a name="strong-and-weak-references-in-cwinrt"></a>强引用和弱引用在 C + + WinRT

Windows 运行时是一种引用计数系统;并且，此类系统中请务必了解的重要性和之间的区别强弱引用 （和既不，如隐式*此*指针的引用）。 你将看到在本主题中，了解如何管理这些引用正确意思可能是运行流畅、 可靠的系统和居中崩溃的一个之间的区别。 通过提供具有语言投影，深入支持的帮助程序函数[C + + WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt)符合中间中生成更复杂的系统，只需并正常工作。
...
但是，在异步操作完成之前类实例将被破坏怎么办？ 有各种类实例可以离开作用域之前完成异步方法的方法。 但是，我们可以模拟它将类实例设置为`nullptr`。
...
通过这项更改为代码，我们会遇到问题在步骤 4，因为类实例已被删除，并且*此*不再有效。 只要异步对象会尝试访问内部类实例的变量，它将故障 （或执行完全未定义）。

解决方案是为了提供异步操作&mdash;协同程序&mdash;类实例自己强引用。 为当前编写的代码，该协同程序有效地指针原始*此*对类实例;但是，不足以使类实例保持活动状态。
...
这解决此问题，我们之前已当我们向第 4 步。 即使对类实例的所有其他引用消失，该协同程序已经保证其依赖项稳定的预防措施。
...
在上面的示例中，将弱引用不会保留被销毁时将不保留任何强引用类实例。 但是，它会为你提供一种检查是否可以访问成员变量之前获取的强引用。

## <a name="safely-accessing-the-this-pointer-with-an-event-handling-delegate"></a>安全地访问*此*指针事件处理委托

### <a name="the-scenario"></a>方案

有关事件处理的常规信息，请参阅[事件处理委托通过 C + + WinRT](handle-events.md)。

在上一节突出显示的协同程序和并发区域中的潜在生命周期问题。 但是，如果你处理事件的对象的成员函数，或需要考虑事件接收者 （处理事件的对象） 和事件源 （该对象的相对生存期对象的成员函数，那么你的 lambda 函数引发事件）。 让我们来看看一些代码示例。

下面的代码列表首先定义一个简单的**EventSource**类，该类引发一般事件，由任何已添加到它的委托。 此示例将会发生事件使用[**Windows::Foundation::EventHandler**](/uwp/api/windows.foundation.eventhandler)委托类型，但问题和补救方法此处适用于所有的委托类型。
...
事件接收者被销毁，但在其中的 lambda 事件处理程序仍订阅**事件**。 引发该事件后，lambda 尝试取消引用*此*指针，此时无效。 因此，在处理程序中 （或在协同程序的延续中） 的代码中产生访问冲突尝试使用它。
...
在这两种情况下，我们只需正在捕获原始*此*指针。 且不会影响引用计数，因此任何阻止当前对象被销毁。
...
你甚至可以省略自动捕获的当前的对象，并通过捕获变量而不是通过隐式*此*访问的数据成员。
...
以及 lambda 函数，这些原则也适用于成员函数用作代理。 语法不同，让我们看一下一些代码。 首先，下面是可能不安全的成员函数的事件处理程序，使用原始*此*指针。
...
上方，我们可以看到正在使用的弱引用。 一般情况下，它们适合破坏循环引用。 例如，对于基于 XAML 的 UI 框架的本机实现&mdash;由于框架的历史设计&mdash;的弱引用在 C + + /winrt 是来处理循环引用。 在 XAML 之外，不过，你可能不需要使用弱引用 (没有任何不本质上特定于 XAML 的有关它们)。 相反，通常会，应该能够设计你自己的 C + + 的方式避免需要进行循环引用和弱引用的 WinRT Api。 
