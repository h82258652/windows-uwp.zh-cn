...
- 强化的智能指针。 以前，失败时撤销事件 revokers 移动-分配一个新值。 这有助于发现智能指针类不可靠地处理自我分配; 问题[**winrt:: com_ptr 结构模板**](/uwp/cpp-ref-for-winrt/com-ptr)中获得 root 权限。 **winrt:: com_ptr**已修复，并固定来处理事件 revokers 移动语义正确，以便它们分配时撤销。
