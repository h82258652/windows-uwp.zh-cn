...
如果忽略的包含`BookstoreViewModel.idl`(的列表，请参阅`MainPage.idl`上方)，则你将看到错误**预期 \ < 附近"MainViewModel"**。 另一条提示是确保保留相同的命名空间中的所有类型： 命名空间，如下所示的代码清单。
...
打开 `MainPage.xaml`，其中包含主 UI 页面的 XAML 标记。 下面的列表中所示，从按钮中，删除名称，并将其**内容**的属性值从文字更改为绑定表达式。 记下绑定表达式上的 `Mode=OneWay` 属性（单向从视图模型到 UI）。 没有该属性，UI 将不会响应属性更改事件。
