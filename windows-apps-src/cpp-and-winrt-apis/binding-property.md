...
>
> 在应用程序中声明的任何运行时类*未*从基本类被称为*可组合*类。 并且可组合类周围的约束。 要通过使用 Visual studio 和 Microsoft store 来验证提交的[Windows 应用认证工具包](../debug-test-perf/windows-app-certification-kit.md)测试应用程序 (并因此能够成功地引入 Microsoft Store 的应用程序)，可组合类必须最终派生自 Windows 基类。 这意味着非常根目录继承层次结构的类必须源自 windows.* 命名空间类型。 如果你需要从基类派生的运行时类&mdash;例如，若要实现一个**BindableBase**类，用于从派生视图模型的所有&mdash;，然后你可以从[**Windows.UI.Xaml.DependencyObject**](/uwp/api/windows.ui.xaml.dependencyobject)派生。
>
...
在**标题**转变器函数中，我们会检查是否正在设置值不同于当前值。 并且，如果是这样，我们更新标题引发[**inotifypropertychanged:: Propertychanged**](/uwp/api/windows.ui.xaml.data.inotifypropertychanged.PropertyChanged)事件，其中包含一个等于已更改的属性名称的参数。 这样，用户界面 (UI) 将知道要重新查询的属性的值。
...
保存文件。 该项目将不会生成到时刻，完成，但现在生成很有用，它以执行操作，因为它会重新生成源代码文件在其中实现**MainPage**运行时类 (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h`和`MainPage.cpp`)。 因此继续并立即生成。 你会看到此阶段生成错误是**MainViewModel': 不是 winrt::Bookstore::implementation::MainPage 的成员**。

如果忽略的包含`BookstoreViewModel.idl`(的列表，请参阅`MainPage.idl`上方)，则你将看到错误**预期 \ < 附近"MainViewModel"**。 另一条提示是确保将所有类型都保留在相同的命名空间： 代码一览中显示的命名空间。
...
在`\Bookstore\Bookstore\MainPage.h`，包括`BookstoreViewModel.h`，这将声明该实现类型 (**winrt::Bookstore::implementation::BookstoreViewModel**)。 添加私有成员以存储视图模型。 请注意，属性访问器函数（添加成员 m_mainViewModel）是根据 **Bookstore::BookstoreViewModel** 实现的，后者是投影类型。 该实现类型是同一项目 （编译单元） 中为应用程序，因此我们构造 m_mainViewModel 通过构造函数重载`nullptr_t`。 此外删除**MyProperty**属性。
