...
> [!NOTE]
> 你的视图模型类&mdash;事实上，在你的应用程序中声明的任何运行时类&mdash;不需要从基类派生。 以上声明的**BookSku**类是这样的示例。 它实现接口时，但它不会从任何基类派生。
>
> 应用程序中声明的任何运行时类*未*从基类派生类被称为*可组合*类。 并且可组合类周围的约束。 要通过使用 Visual studio 和 Microsoft store 来验证提交的[Windows 应用认证工具包](../debug-test-perf/windows-app-certification-kit.md)测试应用程序 (并因此应用程序无法成功地引入 Microsoft Store)，可组合类必须最终派生自 Windows 基类。 这意味着非常根目录继承层次结构的类必须源自 windows.* 命名空间类型。 如果你需要从基类派生的运行时类&mdash;例如，若要实现一个**BindableBase**类，用于从派生视图模型的所有&mdash;，然后你可以从[**Windows.UI.Xaml.DependencyObject**](/uwp/api/windows.ui.xaml.dependencyobject)派生。
>
> 视图模型是视图的一种抽象，因此它直接绑定到视图 （在 XAML 标记）。 数据模型是一种抽象的数据，时，它具有消耗仅从视图模型，并且不直接绑定到 XAML。 因此，你可以将数据模型声明不是作为运行时类，但为 c + + 结构或类。 它们无需在 MIDL 中, 声明，你可以自由地使用你喜欢的任何继承层次结构。
...
在**标题**转变器函数中，我们检查是否正在设置值不同于当前值。 并且，如果是这样，我们更新标题引发[**inotifypropertychanged:: Propertychanged**](/uwp/api/windows.ui.xaml.data.inotifypropertychanged.PropertyChanged)事件包含一个等于已更改的属性名称的参数。 这样，用户界面 (UI) 将知道要重新查询的属性的值。
...
保存并生成。 将 `BookstoreViewModel.h` 和 `BookstoreViewModel.cpp` 从 `Generated Files` 文件夹复制到项目文件夹中，然后将其包含在项目中。 打开这些文件并实现运行时类，如下所示。 注意如何，请在`BookstoreViewModel.h`，我们要包括`BookSku.h`，这将声明该实现类型 (**winrt::Bookstore::implementation::BookSku**)。 我们正在通过删除恢复的默认构造函数和`= delete`。
...
> [!NOTE]
> 类型`m_bookSku`是投影的类型 (**winrt::Bookstore::BookSku**)，和使用[**winrt:: make**](/uwp/cpp-ref-for-winrt/make)的模板参数是实现类型 (**winrt::Bookstore::implementation::BookSku**)。 即使如此，**make** 也会返回投影类型的实例。
...
保存文件。 该项目将不会生成到时刻，完成，但现在生成很有用，它以执行操作，因为它会重新生成源代码文件在其中实现**MainPage**运行时类 (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h`和`MainPage.cpp`)。 因此继续操作并立即生成。 你会看到此阶段生成错误是**MainViewModel': 不是 winrt::Bookstore::implementation::MainPage 的成员**。

如果忽略的包括`BookstoreViewModel.idl`(看到的一览`MainPage.idl`上方)，然后你将看到错误**预期 \ < 附近"MainViewModel"**。 另一条提示是确保将所有类型都保留在相同的命名空间： 代码一览中显示的命名空间。

若要解决我们希望看到的错误，你现在需要复制访问器存根针对**MainViewModel**属性从生成的文件 (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h`和`MainPage.cpp`) 到`\Bookstore\Bookstore\MainPage.h`和`MainPage.cpp`。

在`\Bookstore\Bookstore\MainPage.h`，包括`BookstoreViewModel.h`，这将声明该实现类型 (**winrt::Bookstore::implementation::BookstoreViewModel**)。 添加私有成员以存储视图模型。 请注意，属性访问器函数（添加成员 m_mainViewModel）是根据 **Bookstore::BookstoreViewModel** 实现的，后者是投影类型。 该实现类型是同一项目 （编译单元） 中为应用程序，因此我们构造 m_mainViewModel 通过采用的构造函数重载`nullptr_t`。 此外删除**MyProperty**属性。
...
打开 `MainPage.xaml`，其中包含主 UI 页面的 XAML 标记。 下面的列表中所示，从按钮中，删除名称，然后将其**内容**的属性值从文字更改为绑定表达式。 记下绑定表达式上的 `Mode=OneWay` 属性（单向从视图模型到 UI）。 没有该属性，UI 将不会响应属性更改事件。
...
为当前已发布版本的 C + + /winrt 中，以便能够使用你将需要实现的[ICustomPropertyProvider](/uwp/api/windows.ui.xaml.data.icustompropertyprovider)和[ICustomProperty](/uwp/api/windows.ui.xaml.data.icustomproperty)接口 {Binding} 标记扩展。
