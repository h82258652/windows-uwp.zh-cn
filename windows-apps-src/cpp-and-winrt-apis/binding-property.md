...
保存文件。 该项目将不会生成到时刻，完成，但现在生成很有用，它以执行操作，因为它会重新生成源代码文件中实现**MainPage**运行时类 (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h`和`MainPage.cpp`)。 因此继续操作并立即生成。 你会看到此阶段生成错误的原因在于 **'MainViewModel': 不是 winrt::Bookstore::implementation::MainPage 的成员**。
...
在`\Bookstore\Bookstore\MainPage.h`，包括`BookstoreViewModel.h`，这将声明该实现类型 (**winrt::Bookstore::implementation::BookstoreViewModel**)。 添加私有成员以存储视图模型。 请注意，属性访问器函数（添加成员 m_mainViewModel）是根据 **Bookstore::BookstoreViewModel** 实现的，后者是投影类型。 实现类型位于同一项目 （编译单元） 与应用程序，因此我们构造 m_mainViewModel 通过构造函数重载`nullptr_t`。 此外删除**MyProperty**属性。
