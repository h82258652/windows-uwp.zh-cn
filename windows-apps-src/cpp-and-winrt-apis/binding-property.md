...
打开 `MainPage.idl`，这将声明表示主 UI 页面的运行时类。 添加导入语句以导入 `BookstoreViewModel.idl`，然后添加名为类型 **BookstoreViewModel** 的 MainViewModel 的只读属性。 此外可以删除**MyProperty**属性。 另请注意`import`指令在以下列表中。
...
保存文件。 该项目将不会生成到时刻，完成，但现在生成很有用，它以执行操作，因为它会重新生成源代码文件中实现**MainPage**运行时类 (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h`和`MainPage.cpp`)。 因此继续并立即生成。 你会看到此阶段生成错误**MainViewModel': 不是 winrt::Bookstore::implementation::MainPage 的成员**。
...
在`\Bookstore\Bookstore\MainPage.h`，包括`BookstoreViewModel.h`，这将声明该实现类型 (**winrt::Bookstore::implementation::BookstoreViewModel**)。 添加私有成员以存储视图模型。 请注意，属性访问器函数（添加成员 m_mainViewModel）是根据 **Bookstore::BookstoreViewModel** 实现的，后者是投影类型。 该实现类型是同一项目 （编译单元） 中为应用程序，因此我们构造 m_mainViewModel 通过构造函数重载`nullptr_t`。 此外可以删除**MyProperty**属性。
...
在`\Bookstore\Bookstore\MainPage.cpp`，调用[**winrt:: make**](/uwp/cpp-ref-for-winrt/make) （与该实现类型） 以将投影类型的新实例分配到 m_mainViewModel。 为书籍的标题分配一个初始值。 针对 MainViewModel 属性实现访问器。 最后，在按钮的事件处理程序中更新书籍的标题。 此外可以删除**MyProperty**属性。
