...
>
> 视图模型是视图的一种抽象，因此它直接绑定到视图 （在 XAML 标记）。 数据模型是一种抽象的数据，时，它具有消耗仅从视图模型，并且不直接绑定到 XAML。 因此，你可以将数据模型声明不是作为运行时类，但作为 c + + 结构或类。 它们无需在 MIDL 中, 声明，你可以自由地使用你喜欢的任何继承层次结构。
...
如果忽略的 include `BookstoreViewModel.idl` (的列表，请参阅`MainPage.idl`上方)，则你将看到错误**预期 \ < 附近"MainViewModel"**。 另一条提示是确保将所有类型都保留在相同的命名空间： 代码一览中显示的命名空间。
