...
这些函数名称中的“单线程”表示它们不支持任何并发性&mdash;换言之，它们不是线程安全的。 此处提到的线程与单元无关，因为这些函数所返回的对象都是敏捷的（请参阅 ](agile-objects.md)C++/WinRT 中的敏捷对象[）。 但是指对象是单线程的。 如果只想跨应用程序二进制接口 (ABI) 以某种方式传递数据，那么这完全合适。
