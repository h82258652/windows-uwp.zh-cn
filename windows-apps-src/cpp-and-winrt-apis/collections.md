...
"单线程"这些函数的名称中指示他们不提供任何并发&mdash;换句话说，它们不是线程安全。 提及的线程的无关而言，因为从这些函数返回的对象是所有敏捷 (请参阅[敏捷对象在 C + + WinRT](agile-objects.md))。 它只是对象是单线程。 并且这是完全合适，如果你只是想要跨应用程序二进制接口 (ABI) 传递或其他数据的一种方法。
