...
[**IVector**](/uwp/api/windows.foundation.collections.ivector_t_)是任何随机访问元素的集合，由实现的 Windows 运行时接口。 如果你要自行实现**IVector** ，还需要实现[**IIterable**](/uwp/api/windows.foundation.collections.iiterable_t_)、 [**IVectorView**](/uwp/api/windows.foundation.collections.ivectorview_t_)和[**IIterator**](/uwp/api/windows.foundation.collections.iiterator_t_)。 即使你*需要*自定义集合类型，这是大量工作。 但是，如果你已将**std:: vector** （或**std:: map**或**std::unordered_map**） 中的数据，并且只想要执行操作传递给 Windows 运行时 API，然后你想要避免尽可能进行工作，该级别。 和避免它*是*可能的因为 C + + WinRT 将帮助你高效地轻松地创建集合。

另请参阅[XAML 项目控件; 绑定到 C + + /winrt 集合](binding-collection.md)。

> [!NOTE]
> 如果你尚未安装了 Windows SDK 版本 10.0.17763.0 (Windows 10 版本 1809年)，或更高版本，然后你将不会有权访问的函数和本主题中介绍的基类。 相反，请参阅[是否你有较早版本的 Windows SDK](/uwp/cpp-ref-for-winrt/single-threaded-observable-vector#if-you-have-an-older-version-of-the-windows-sdk)的可以改为使用可观测矢量模板列表。

## <a name="helper-functions-for-collections"></a>对于集合的帮助程序函数

### <a name="general-purpose-collection-empty"></a>常规用途的集合空

本部分介绍了想要创建初始为空; 一个集合的方案然后再填充*后*创建。

若要检索实现通用集合的类型的新对象，你可以调用[**winrt::single_threaded_vector**](/uwp/cpp-ref-for-winrt/single-threaded-vector)函数模板。 作为[**IVector**](/uwp/api/windows.foundation.collections.ivector_t_)，返回的对象，它通过该调用返回的对象的函数和属性的接口。
...
你可以看到在上面的代码示例中，创建集合后可以追加元素，循环访问它们，并通常认为该对象，就像你可能已从 API 收到任何 Windows 运行时集合对象。 如果你需要对集合的不可变的视图，然后你可以调用[**IVector::GetView**](/uwp/api/windows.foundation.collections.ivector-1.getview)，所示。 如上所示的模式&mdash;创建和使用集合的&mdash;适用于想要将数据传入，或获取数据，API 的简单方案。 你可以传递**IVector**或**IVectorView**， [**IIterable**](/uwp/api/windows.foundation.collections.iiterable_t_)预期的任意位置。
...
本部分介绍了想要创建集合，并在同一时间填充它的方案。

你可以避免**追加**到前面的示例代码中调用的开销。 你可能已具有源数据，或者你可能想要填充之前创建的 Windows 运行时集合对象的源数据。 操作方法如下。
...
你可以传递一个包含到**winrt::single_threaded_vector**，你的数据的临时对象与`coll1`上面。 也可以移动 （假设你将不会访问它再次） **std:: vector**注入函数。 在这两种情况下，你在传递到函数的*rvalue* 。 这样可以让编译器有效，并避免复制数据。 如果你想要了解有关*rvalues*的详细信息，请参阅[值类别和对它们的引用](cpp-value-categories.md)。

如果你想要将 XAML 项目控件绑定到你的集合，然后就可以。 但请注意，若要正确设置[**ItemsControl.ItemsSource**](/uwp/api/windows.ui.xaml.controls.itemscontrol.itemssource)属性，你需要将其设置为类型**IVector** **IInspectable** （或互操作性类型，如[**IBindableObservableVector**](/uwp/api/windows.ui.xaml.interop.ibindableobservablevector)） 的值。 下面是一个代码示例，生成的集合类型，适合用于绑定，并将元素追加到它。
...
你可以从数据，创建 Windows 运行时集合，并准备要传递至 API 时，无需复制任何所有对它的视图。
...
在上述示例中，我们创建*可以*的集合可绑定到 XAML 项目控件;但集合不是可供观察。
...
有关更多详细信息和代码示例，有关绑定你的用户界面 (UI) 控件到一个可观测集合，请参阅[XAML 项目控件; 绑定到 C + + /winrt 集合](binding-collection.md)。
...
（可选） 可以通过将类型**std:: map**或**std::unordered_map** *rvalue*传递给函数来优化数据与这些集合。
...
"单线程"这些函数的名称中指示他们不提供任何并发&mdash;在换而言之，它们不是线程安全。 提及的线程的无关而言，因为从这些函数返回的对象是所有敏捷 (请参阅[敏捷对象在 C + + WinRT](agile-objects.md))。 它只是对象是单线程。 这也完全相应，如果你只是想要跨应用程序二进制接口 (ABI) 传递或其他数据的一种方法。

## <a name="base-classes-for-collections"></a>对于集合的基类

如果要为完整的灵活性，实现你自己的自定义集合，然后你会想要避免执行此操作的方法。 例如，这是一个自定义的矢量视图将如下所示*不借助 C + + /winrt 的基类*。
...
返回**get_container**容器必须提供的**开始**和**结束**界面该**winrt::vector_view_base**预期。 如上面的示例中所示， **std:: vector**提供的。 但是，则可以返回满足相同合约，包括你自己的自定义容器任何容器。
