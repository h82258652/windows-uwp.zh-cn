...
如果你想要将 XAML 项目控件绑定到集合，然后就可以。 但请注意，要正确设置[**ItemsControl.ItemsSource**](/uwp/api/windows.ui.xaml.controls.itemscontrol.itemssource)属性，你需要将其设置为类型**IVector** **IInspectable** （或互操作性类型，如[**IBindableObservableVector**](/uwp/api/windows.ui.xaml.interop.ibindableobservablevector)） 的值。 下面是代码示例，生成的集合类型适合绑定，并将某个元素追加到它。
...
相反，它是更易于派生[**winrt::vector_view_base**](/uwp/cpp-ref-for-winrt/vector-view-base)结构模板中，你的自定义的矢量视图和只实现**get_container**函数来公开容器保存你的数据。
