...
ms.locfileid: "6453478"
---
# <a name="value-categories-and-references-to-them"></a>值类别和对它们的引用
本主题介绍了各种类别的值 （和参考值） 存在于 c + + 中。 你将肯定听到的*左值*和*rvalues*，但你可能不将其视为它们在本主题提供的条款。 还有其他类型的值，太。
...
*左值*具有标识。 现在是分配的一种仅历史感兴趣"左值"中的"l"是分配的"左"（如，左左） 的缩写。 在 c + +，左值可以出现在的左侧*或*右侧的分配。 "L"中"左值"，然后，不会实际可帮助你理解，也不会定义是什么。 你只需要了解，我们调用左值是具有标识的值。
...
但不是 glvalues 的值。 因此，存在你*不能*获得的内存地址 （或不能依靠它来有效） 的值。 我们之前所见上面的代码示例中的某些此类值。 这听起来像一个缺点。 但实际上值利用像就是，你可以从它 （这是通常成本低廉），*移动*，而不是从它 （这是通常成本高昂） 的复制。 从一个值意味着它不会再在它所使用的位置。 因此，尝试在它所使用的位置中访问它是一款用于避免。 从讨论了何时以及*如何*移动值已超出本主题的范围。 本主题中，我们只需要知道的值是可移动被称为*rvalue* （或*传统 rvalue*）。

在"rvalue"中的"r"是分配的"权限"（如，右键左） 的缩写。 但你可以使用 rvalues，并对 rvalues，之外分配的引用。 在"rvalues"，"r"，则不需要关注的件事。 你只需要了解，我们调用 rvalue 是可移动的值。
...
本部分介绍对 rvalue 引用的语法。 我们需要等待另一个主题，以转到移动和转发，大量处理，但这些 rvalue 引用解决的问题。 我们看一下 rvalue 引用之前，不过，我们首先需要会清晰有关`T&`&mdash;件事我们已经以前已调用只是"引用"。 它实际上是"左值 (非 const) 引用"，这是指引用的用户可写入的值。
...
Rvalue 引用将绑定到 rvalue。 事实上，根据重载分辨率，rvalue*首选*绑定到 rvalue 引用比向左值 const 引用。 但 rvalue 引用无法绑定到左值，因为我们已经说过，rvalue 引用引用的值将假设我们无需保留 （比如，移动构造函数的参数） 其内容。
...
在此阶段，我们知道哪些具有标识。 并且，我们已经知道什么是可移动及其内容不是。 但我们尚未名为一组值*不*具有标识。 这组称为*prvalue*或*纯 rvalue*。
...
在上面的代码示例中，我们尚未移动任何尚未。 我们只需创建了 xvalue 通过强制转换为未命名的 rvalue 引用左值。 它仍然可以标识通过其左值名称;但是，xvalue，它是现在*能够*在移动。 执行此操作的原因，哪些移动实际上如下所示，需要等待另一个主题。 但是，你可以将其视为"xvalue"作为含义"专家仅"如果，可帮助中的"x"。 通过强制左值转换到 xvalue （一种 rvalue），则该值将成为能够被绑定到 rvalue 引用。
...
但是，是 rvalue 引用本身 rvalue 吗？ *未命名*rvalue 引用 （如上面的 xvalue 代码示例所示的） 是 xvalue 因此，它是的 rvalue。 它倾向于绑定到 rvalue 引用函数参数，如移动构造函数。 相反，或许 counter-intuitively） 如果 rvalue 引用具有一个名称，那么该名称组成表达式是左值。 因此它*无法*绑定到 rvalue 引用参数。 很容易使其执行此操作，但是&mdash;再次只需将其转换到未命名的 rvalue 引用 (xvalue)。
...
### <a name="im"></a>\!i\ 和 \!m
值，不具有标识，不是可移动的类型是一个组合，我们尚未尚未所述。 但是，我们可以忽略它，因为该类别不是在 c + + 语言中非常有用的想法。
...
- 传递左值将导致转发参考成为`_Ty& &&`，这向左值引用折叠`_Ty&`。
