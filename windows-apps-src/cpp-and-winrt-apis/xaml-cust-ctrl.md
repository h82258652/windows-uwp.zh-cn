...
上面的一览演示声明一个依赖属性 (DP) 时遵循的模式。 有两个部分每个 DP。 首先，你声明类型[**DependencyProperty**](/uwp/api/windows.ui.xaml.dependencyproperty)的只读的静态属性。 它具有 DP 再加上*属性*的名称。 你将在实现中使用此静态属性。 第二，你使用的类型和你 DP 名称声明读写实例属性。

> [!NOTE]
> 如果你想 DP 与浮点类型，然后将其`double`(`Double` [MIDL 3.0](/uwp/midl-3/)中)。 声明和实现类型的 DP `float` (`Single` MIDL 中)，然后为该 DP 在 XAML 标记中设置一个值将导致错误并*无法创建 'Windows.Foundation.Single 从文本<NUMBER>*。
...
在本演练中，我们将不会使用**介绍 OnLabelChanged**。 但它的目的是，这样你可以查看如何使用属性已更改的回调中注册依赖属性。 **介绍 OnLabelChanged**的实现还显示了如何从基本的投影类型 （基本的投影的类型是**DependencyObject**，在此情况下） 获取派生的投影的类型。 并且，它显示了如何获取，然后实现的投影的类型的类型的指针。 第二个操作自然地将仅可能在项目中实现的投影的类型 （即，实现运行时类的项目）。
...
在此情况下，默认样式设置的唯一属性是控件模板。 该模板由 （其背景绑定到 XAML[**控件**](/uwp/api/windows.ui.xaml.controls.control)类型的所有实例具有**背景**属性） 的正方形和文本元素 （其文本绑定到**BgLabelControl::Label**依赖属性） 组成。
...
派生自定义控件从[**控件**](/uwp/api/windows.ui.xaml.controls.control)运行时类，该类本身进一步派生基本的运行时类。 并且可重写方法的**控件**、 [**FrameworkElement**](/uwp/api/windows.ui.xaml.frameworkelement)和你的派生类，可以覆盖的[**UIElement**](/uwp/api/windows.ui.xaml.uielement) 。 下面是代码示例向你显示如何执行该操作。
