...
| Windows 应用认证工具包测试将产生一个错误，表示一个运行时类“*不是派生自 Windows 基类。所有可组合类必须最终派生自 Windows 命名空间中的类型*”。|（即您的应用程序中声明） 从一个基类派生的任何运行时类称为*可组合*类。 必须源自 Windows.* 命名空间; 的类型的最终可组合类的基类。例如， [**Windows.UI.Xaml.DependencyObject**](/uwp/api/windows.ui.xaml.dependencyobject)。 请参阅[XAML 控制; 绑定到 C + + / WinRT 属性](binding-property.md)的详细信息。|
...
| C + + 链接器生成"*LNK2019 错误： 无法解析外部符号*"|请参阅[链接器是否为什么发出我"LNK2019： 无法解析外部符号"错误？](faq.md#why-is-the-linker-giving-me-a-lnk2019-unresolved-external-symbol-error)|
| LLVM 和 Clang toolchain 将产生错误时用于 C + + / WinRT。|我们不支持 LLVM 和 Clang toolchain for C + + / WinRT，但如果您希望模拟如何我们其内部使用，则您可以尝试证实类中所述为[我可以使用 LLVM/Clang 编译与 C + + / WinRT？](faq.md#can-i-use-llvmclang-to-compile-with-cwinrt)。|
| C + + 编译器生成的计划类型的"*可用没有适当的默认构造函数*"。 | 如果您尝试运行时类对象的初始化延迟或使用和运行时类实现在同一个项目中，则您需要呼叫`nullptr_t`构造函数。 有关详细信息，请参阅[通过 C++/WinRT 使用 API](consume-apis.md)。 |
| C + + 编译器生成"*C3861 错误: from_abi： 找不到标识符*"，和源自*base.h*其他错误。 您可能会看到此错误，如果您使用 Visual Studio 2017 (版本 15.8.0 或更高版本)，和面向 Windows SDK 版本 10.0.17134.0 (Windows 10，版本 1803年)。 | 以下任意目标更高版本 （一致性更强） 版本的 Windows SDK 或设置项目属性**C/c + +** > **语言** > **一致性声明模式： 否**(此外，如果 **/ permissive-** 出现在项目属性**C/c + +**  > **语言** > **命令行**在**其他选项**，然后将其删除)。 |
