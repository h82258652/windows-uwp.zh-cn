...
本主题的其余部分演示了如何创建的最小控制台应用程序项目中使用 C + + /winrt 来实现基本 coclass （COM 组件或 COM 类） 和类工厂。 示例应用程序显示了如何，传递 toast 通知回调按钮和 coclass （可实现**INotificationActivationCallback** COM 接口） 允许应用程序启动和名为后退时用户单击该 toast 上的按钮。

有关 toast 通知功能区域的更多背景位于[发送本地 toast 通知](/windows/uwp/design/shell/tiles-and-notifications/send-local-toast)。 没有任何文档的此部分中的代码示例使用 C + + /winrt 中，但，因此我们建议你想在本主题中所示的代码。
...
但是，不允许异常转义你 COM 方法实现。 你可以确保使用`noexcept`COM 方法上的说明符。 只要你处理这些方法退出之前是方法的确定为你的调用图中任意位置引发异常。 如果你使用`noexcept`，但你然后允许异常转义你方法中，则将终止你的应用程序。
...
项目模板生成`main`为你的函数。 删除`main`函数，并在其位置粘贴该代码列表，其中包括代码可以注册你 coclass，然后提供能够调用返回你的应用程序的 toast。
...
生成应用程序，并运行它至少一次以管理员身份会导致在注册时，和其他设置，用于运行代码。 无论你正在运行它作为管理员，然后按 \ 以使一个 toast 以显示。 然后可以单击直接从 toast 通知，将启动 pop，或从操作中心，并且你的应用程序、 实例化，coclass 和 INotificationActivationCallback****回调 ToastAndCallback**按钮:: 激活**执行方法。

## <a name="in-process-com-server"></a>进程内 COM 服务器

上述*ToastAndCallback*示例应用作为本地 （或进程外） COM 服务器。 这指示由[LocalServer32](/windows/desktop/com/localserver32) Windows 注册表项，你使用注册其 coclass CLSID。 本地 COM 服务器承载其 coclass(es) 可执行的二进制文件 ( `.exe`)。
