...
本主题的其余部分演示了如何创建的最小控制台应用程序项目中使用 C + + WinRT 实现基本 coclass （COM 组件或 COM 类） 和类工厂。 示例应用程序显示了如何提供与回调按钮的 toast 通知，并 coclass （可实现**INotificationActivationCallback** COM 接口） 允许应用程序启动并调用当用户单击 toast 上的按钮。
...
实现上述 coclass 遵循中所示的相同模式[创作 Api 通过 C + + WinRT](/windows/uwp/cpp-and-winrt-apis/author-apis#if-youre-not-authoring-a-runtime-class)。 因此，你可以使用的相同技术实现 COM 接口，以及 Windows 运行时接口。 COM 组件以及 Windows 运行时类公开其功能通过接口。 每个 COM 接口最终派生[**IUnknown**](https://msdn.microsoft.com/library/windows/desktop/ms680509)接口。 Windows 运行时基于 COM&mdash;一个区别是，Windows 运行时接口最终派生[**IInspectable 接口**](/windows/desktop/api/inspectable/nn-inspectable-iinspectable)（和**IInspectable**派生自**IUnknown**）。
...
我们只需实现 coclass 称为通知， *COM 激活器*，它具有其类 id (CLSID) 的形式`callback_guid`标识符 （ **GUID**类型），请参阅上述内容。 我们将使用该标识符更高版本，在开始菜单快捷方式和 Windows 注册表项的形式。 COM 激活器 CLSID，并且其关联的 COM 服务器 （这是我们在此处生成的可执行文件的路径） 的路径是一种的机制的 toast 通知知道哪些类创建其回调按钮时的实例 (是否通知单击操作中心中或不）。

## <a name="best-practices-for-implementing-com-methods"></a>实现 COM 方法的最佳做法

错误处理和资源管理技术可以转手中手。 它是更加方便和实际使用比错误代码的异常。 并且，如果你使用的资源的购置-即-初始化 (RAII) 用法，然后你可以避免显式检查错误代码，然后显式释放资源。 此类显式检查使代码更复杂超出必要，并提供 bug 很多地方来隐藏。 相反，使用 RAII，并引发/catch 异常。 这样一来，资源分配为异常安全的并且你的代码为简单。

但是，不允许异常转义 COM 方法实现。 你可以确保通过使用`noexcept`COM 方法上的说明符。 只要你处理这些方法退出之前已成功的任意位置的方法中，调用图中引发异常。 如果你使用`noexcept`，但你然后允许异常转义你方法中，则将终止你的应用程序。

## <a name="add-helper-types-and-functions"></a>添加帮助程序类型和函数

在此步骤中，我们将添加一些帮助程序类型和函数可以使其余的代码使用。 因此，然后再`main`，添加以下内容。
...
## <a name="implement-the-remaining-functions-and-the-wmain-entry-point-function"></a>实现其余的函数，并 wmain 入口点函数
...
生成应用程序，然后再在以管理员身份会导致在注册时，和其他设置，若要运行的代码至少一次运行它。 无论你正在运行它作为管理员，然后按 \ 会导致显示 toast。 然后，你可以单击该**回调 ToastAndCallback**按钮可以直接从 toast 通知，将启动 pop，或从操作中心，并且你的应用程序、 实例化，coclass 和 INotificationActivationCallback **:: 激活**执行方法。

## <a name="in-process-com-server"></a>进程内 COM 服务器

上述*ToastAndCallback*示例应用作为本地 （或进程外） COM 服务器。 这是你用于注册的其 coclass CLSID 指示[LocalServer32](/windows/desktop/com/localserver32) Windows 注册表项。 本地 COM 服务器承载其 coclass(es) 可执行的二进制文件 ( `.exe`)。

或者，可以说更有可能你可以选择托管你 coclass(es) 内动态链接库 ( `.dll`)。 形式的 DLL 的 COM 服务器被称为进程内 COM 服务器，并由注册使用[InprocServer32](/windows/desktop/com/inprocserver32) Windows 注册表项的 Clsid。

### <a name="create-a-dynamic-link-library-dll-project"></a>创建动态链接库 (DLL) 项目

你可以通过在 Microsoft Visual Studio 中创建新项目中创建的进程内 COM 服务器的任务。 创建**Visual c + +** > **Windows 桌面版** > **动态链接库 (DLL)** 项目。
...
打开`dllmain.cpp`，并向其添加代码列表如下所示。

如果你已经拥有一个 DLL，实现 C + + WinRT Windows 运行时类，则你将已经**DllCanUnloadNow**函数如下所示。 如果你想要添加到此 DLL 组件类，你可以添加**DllGetClassObject**函数。

如果没有现有的[Windows 运行时 c + + 模板库 (WRL)](/cpp/windows/windows-runtime-cpp-template-library-wrl)代码你想要保持兼容，则可以从显示的代码中删除的 WRL 部分。
