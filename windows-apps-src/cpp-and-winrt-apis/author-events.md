...
你可以看到上方，该事件实现方面[**winrt::event**](/uwp/cpp-ref-for-winrt/event)结构模板中，通过特定委托类型参数化。
...
添加一个引用，然后浏览到`\BankAccountWRC\Debug\BankAccountWRC\BankAccountWRC.winmd`（或如果两个项目在同一个解决方案中添加项目到项目的引用，）。 单击**添加**，然后单击**确定**。 立即生成 BankAccountCoreApp。 在你看到的错误的可能事件的负载文件`readme.txt`不存在，从 Windows 运行时组件项目中排除该文件，重新生成它，然后重新生成 BankAccountCoreApp。
...
下面的示例中首先显示委托签名不带任何参数 （本质上是简单信号），，然后另一个接受一个字符串。
...
注意如何可以添加到事件与你希望的多个订阅委托。 但是，没有与事件关联一些开销。 如果你只需要是仅的单个订阅的委托的简单回调，则你可以使用[**winrt:: delegate&lt;...T&gt;**](/uwp/cpp-ref-for-winrt/delegate)自己。
