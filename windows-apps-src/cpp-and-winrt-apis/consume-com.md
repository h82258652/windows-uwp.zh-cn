...
当你计划使用 COM 时，你直接与接口，而不是与对象 （这的还在幕后适用于 Windows 运行时 Api，它们是 COM 的进化 true） 起作用。 COM 类上调用的函数，例如，你激活的类，获取回来，接口，然后在该接口上调用函数。 若要访问对象的状态，不要直接调用访问其数据成员相反，你可以在接口上调用访问器和转变器函数。
...
## <a name="com-functions-that-return-an-interface-pointer-as-void"></a>返回为**void**的接口指针的 COM 函数
...
[**DWriteCreateFactory**](/windows/desktop/api/dwrite/nf-dwrite-dwritecreatefactory)函数返回其最后一个参数，其中包含[**IUnknown**](https://msdn.microsoft.com/library/windows/desktop/ms680509)类型通过 DirectWrite 工厂接口指针。 对于此类函数，使用[**com_ptr:: put**](/uwp/cpp-ref-for-winrt/com-ptr#comptrput-function)，但将重新解释强制转换，为**IUnknown**。
