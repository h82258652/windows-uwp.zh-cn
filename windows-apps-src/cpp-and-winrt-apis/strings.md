...
C++ 中有很多字符串类型。 除了 C++ 标准库中的 **std::basic_string** 之外，变体还存在于很多库中。 C++17 具有字符串转换实用程序和 **std::basic_string_view**，用来消除所有字符串类型之间的差别。 **hstring** 利用 **std::wstring_view** 提供了可转换性，以实现 **std::basic_string_view** 应有的互操作性。
...
但重复一下，由于 **hstring** 的 [**std::wstring_view** 的转换运算符](/uwp/api/windows.foundation.uri#hstringoperator-stdwstringview)，注意该细节是一个可选操作。
...
你可以使用 [hstring::c_str 函数](/uwp/api/windows.foundation.uri#hstringcstr-function)从 **hstring** 获取标准宽字符串（正如你可以从 **std::wstring** 获取一样）。
...
为 **hstring** 实现了大量构造函数、运算符、函数和迭代程序。

**hstring** 是一个范围，因此你可以将其与基于范围的 `for` 或与 `std::for_each` 一起使用。 它还提供了一个比较运算符，用于自然、高效地与它在 C++ 标准库中的对应项进行比较。 它还包含将 **hstring** 用作关联容器的键所需的一切。 我们发现很多 C++ 库使用了 **std::basic_string**，并且仅与 UTF-8 文本配合。 为方便起见，我们提供了用于向后和向前转换的帮助程序。
