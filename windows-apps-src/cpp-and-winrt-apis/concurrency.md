...
异步 Windows 函数的示例是 [**SyndicationClient::RetrieveFeedAsync**](https://docs.microsoft.com/uwp/api/windows.web.syndication.syndicationclient.retrievefeedasync)，其返回类型 [**IAsyncOperationWithProgress&lt;TResult, TProgress&gt;**](/uwp/api/windows.foundation.iasyncoperationwithprogress_tresult_tprogress_) 的异步操作对象。 让我们看一下一些方法&mdash;第一个阻止，然后非阻止性&mdash;使用 C + + /winrt 来调用类似 API 的。
...
协同程序是一个函数与任何其他调用方受到阻止，直到函数将执行返回给它。 用于返回的协同程序的第一个机会是第一个`co_await`， `co_return`，或`co_yield`。
...
一般来说后暂停点协同程序中，, 执行的原始线程可能会消失和恢复可能会发生任何线程上 （换言之，任何线程可能调用**已完成**方法异步操作）。

但是，如果你`co_await`任何的四个 Windows 运行时异步操作类型 (**IAsyncXxx**)，然后 C + + WinRT 捕获调用上下文此时你`co_await`。 并且，它可确保你是仍在该上下文上延续恢复时。 C + + WinRT 执行此通过检查你是否已在调用上下文并，如果不是，切换到它。 如果你已在之前在单线程单元 (STA) 的线程上`co_await`，然后你将在同一个以后;如果你已在之前在多线程的单元 (MTA) 的线程上`co_await`，然后就随后会在一个。
...
你可以依赖于此行为的原因是因为 C + + /winrt 提供代码以适应 （这些代码段都称为等待适配器） 的 c + + 协同程序语言支持这些 Windows 运行时异步操作类型。 其余的可等待类型在 C + + WinRT 只是线程池包装器和/或帮助程序;因此他们完成线程池上。
...
如果你`co_await`某些其他类型&mdash;甚至在 C + + /winrt 协同程序实现&mdash;另一个库提供了对适配器，然后你将需要了解哪些这些适配器方面执行的操作恢复和上下文。

若要使最小的上下文切换，你可以使用的一些技巧，我们已在本主题中看到。 我们来看一些执行此操作的插图。 在下一个伪代码示例，我们展示的事件处理程序调用 Windows 运行时 API 来加载图像、 降到处理该映像，在后台线程上，然后返回到 UI 线程以在 UI 中显示该图像的轮廓。
...
对于此方案中，没有一点 ineffiency 周围**StorageFile::OpenAsync**调用。 没有必要的上下文切换到后台线程 （以便在处理程序可以将执行返回给调用方），在恢复后的 C + + WinRT 还原 UI 线程上下文。 但是，在此情况下，不需要在 UI 线程上进行，直到我们要更新 UI。 我们调用*之前* **winrt:: resume_background**，我们调用的多不必要的和-往返上下文切换，我们会产生更多 Windows 运行时 Api。 解决方案是不调用*任何*Windows 运行时 Api 之前。 所有后**winrt:: resume_background**移动它们。
...
异步编程的 Windows 运行时的功能允许你取消正在进行的异步操作。 下面是示例调用[**StorageFolder::GetFilesAsync**](/uwp/api/windows.storage.storagefolder.getfilesasync)可检索潜在较大集合的文件，并且它将生成的异步操作对象存储中的数据成员。 用户可以选择取消操作。
...
在协同程序暂停时，取消可能，当然，会发生。 仅协同程序恢复时，或点击另一个`co_await`，它将检查取消。 该问题是响应取消可能太粗糙-粒度延迟之一。

因此，另一个选项是显式轮询从协同程序中取消。 使用以下列表中的代码更新上面的示例。 在此新的示例中， **ExplicitCancellationAsync**检索[**winrt::get_cancellation_token**](/uwp/cpp-ref-for-winrt/get-cancellation-token)函数中，返回的对象，并使用它来定期检查是否已取消协同程序。 只要其不被取消后，该协同程序之间无限;一旦其被取消后，该循环并将该函数正常退出。 由于之前示例中，但此处退出事件在发生显式方法，并具有唯一控制权，结果都是相同的。
...
等待**winrt::get_cancellation_token**检索与你的名义**IAsyncAction**协同程序是在生成的知识取消令牌。 你可以使用该令牌函数调用运算符查询取消状态&mdash;本质上轮询取消。 如果你正在执行某些受计算限制的操作，或循环访问较大集合，这将是合理的技术。

### <a name="register-a-cancellation-callback"></a>注册取消回调

在 Windows 运行时取消不会自动流到其他异步对象。 但是&mdash;在 Windows SDK 版本 10.0.17763.0 (Windows 10 版本 1809年) 中引入&mdash;你可以注册取消回调。 这是性挂钩的取消程度可以传播，并使其成为可能与现有的并发库集成。

在下一个代码示例， **NestedCoroutineAsync**执行的工作，但它没有特殊取消逻辑在其中。 **CancellationPropagatorAsync**是实质上是嵌套协同程序; 在包装器包装器将优先转发取消。
...
**CancellationPropagatorAsync**注册其自己的取消回调，lambda 函数，然后等待 （它暂停） 嵌套的工作完成后才。 当或**CancellationPropagatorAsync**被取消后，它会传播到嵌套协同程序取消。 若要轮询取消; 无需也不会被取消阻止无限期。 此机制非常灵活，你可以使用它设置的互操作具有协同程序或并发库知道任何 C + WinRT。

## <a name="reporting-progress"></a>报告进度

如果你的协同程序返回[**IAsyncActionWithProgress**](/uwp/api/windows.foundation.iasyncactionwithprogress_tprogress_)或[**IAsyncOperationWithProgress**](/uwp/api/windows.foundation.iasyncoperationwithprogress_tresult_tprogress_)，然后你可以检索[**winrt::get_progress_token**](/uwp/cpp-ref-for-winrt/get-progress-token)函数中，返回的对象并使用它返回到一个进度报告进度处理程序。 下面是代码示例。
...
有时，你的任务可与其他工作，同时完成并且你无需等待完成该任务 （任何其他工作取决于它），你需要返回一个值，也不会。 在此情况下，你可以关闭该任务触发，并且忘记它。 你可以通过编写协同程序其返回类型是[**winrt::fire_and_forget**](/uwp/cpp-ref-for-winrt/fire-and-forget) （而不是一种 Windows 运行时异步操作类型或**concurrency:: task**） 来执行该操作。
