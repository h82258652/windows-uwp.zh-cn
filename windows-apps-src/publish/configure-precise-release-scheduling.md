...
>
>如果您以前发布的应用程序支持早期的 OS 版本，任何**停止获取**您选择的日期将不适用于这些客户; 它们仍将无法获得应用程序 (除非在提交将更新与在中的新选项[可见性](choose-visibility-options.md#discoverability)部分中，或者如果您选择**使应用程序不可用**从**应用概述**页)。
...
务必要了解此选项必须与选择相同的影响**将此应用程序，可发现，但不可用**中[可见性](choose-visibility-options.md#discoverability)部分并选择**停止获取：任何具有直接链接的客户可以看到该产品的应用商店列出，但如果他们拥有该产品之前，或有促销代码和使用的是 Windows 10 设备，它们可以仅下载它。** 若要完全停止向新客户提供应用，请从“应用概述”页面中单击**禁止提供应用**。 有关详细信息，请参阅[从应用商店中删除应用](guidance-for-app-package-management.md#removing-an-app-from-the-store)。
...
若要添加应用于多个市场的计划，需创建*市场组*。 为此，请选择要包括的市场，然后为该组输入名称。 （此名称是仅供您参考并且不会显示任何客户。）例如，如果你想要创建市场组对于北美地区，您可以选择**加拿大**，**墨西哥**，并**美国**，并将其命名**北美**或所选的另一个名称。 完成创建市场组后，请单击**保存**。 然后将出现上述相同的**发布**和**停止获取**选项，但所做选择将仅应用于该市场组。
