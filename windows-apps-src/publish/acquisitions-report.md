...
ms.locfileid: "2795268"
---
# <a name="acquisitions-report"></a>购置报告


**收购**报表 Windows 开发人员中心仪表板中的，可以查看用户已获取并安装您的应用程序，以及人口统计和平台的详细信息，以及有关如何在 Windows 10 （包括 Xbox） 上的客户已到达您的应用程序的显示信息列表。 您还可以查看近乎实时获取数据的最后一个小时或有 70-两个小时段。 
...
在页面顶部附近，可以选择希望显示数据的时间段。 默认选择为 **30D**（30 天），但你可以选择要显示 3、6 或 12 个月的数据或指定的自定义数据范围的数据。 接近实时数据将显示所有选项 （中**累积的应用程序**数据除外）。 **1 H**和**72 H**时间段仅应用于**收购**图表的**每日的应用程序**选项卡和**市场**图表的**获取**选项卡。 
...
默认情况下，我们将显示**每日的应用程序**视图，其中包括近乎实时数据。 你还可以通过选择**应用累计**看到应用的终生购置数。 这将显示从首次发布应用起所有购置的累计总数。
...
在**每日的应用程序**视图中，选择时间段后**30 D** ，您可能会看到圆形标记。 这些表示显著增加或减少我们认为您需要了解的给定值。 在这个圆将显示该日期表示我们检测显著增大或减小相比于之前的周的一周的末尾。 若要查看有关已更改内容的更多详细信息，请悬停在这个圆。  
