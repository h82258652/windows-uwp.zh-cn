...
在页面顶部附近，可以选择希望显示数据的时间段。 默认选择为 **30D**（30 天），但你可以选择要显示 3、6 或 12 个月的数据或指定的自定义数据范围的数据。 接近实时数据将显示所有选项 （中**应用累积**数据除外）。 **1 H**和**72h**时间期间仅应用于**每日应用**选项卡的**购置**图表和**市场**图的**购置**选项卡。 
...
为你的应用 （从 2016 年 10 月-存在) 的**总销售**还可以在此图表中显示 （以美元） 的应用销售中获得的总金额。 请注意，此时间不考虑任何退款、 撤单、 拒付等。
...
在**每日应用**视图中，选中**30d**时间段后，你可能会看到圆圈标记。 这些表示显著增加或减少我们认为你会想要了解有关为给定值。 出现圆的日期表示一周中，我们检测到了显著增加或减少相比于前的一周结尾。 若要查看有关更改内容的更多详细信息，请悬停在圆圈。  
...
**应用页面查看次数和转换的通道**图表了解 windows 10 上的客户如何找到所选时段内你的应用一览。
...
*页面查看*指客户查看你的应用的应用商店一览页面上，通过基于 web 的应用商店或从 windows 10 上的应用商店应用。 其中包括未使用 Microsoft 帐户登录的用户的查看次数。 一些客户选择退出为 Microsoft 提供此信息。
