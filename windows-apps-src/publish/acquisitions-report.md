...
在**每日应用**视图中，选中**30d**时间段后，你可能会看到圆圈标记。 这些表示显著增加或减少在我们认为你会想要了解有关为给定值。 出现圆的日期是指在其中我们检测到了显著增加或减少相比于之前的周本周结束。 若要查看有关的更改的更多详细信息，请悬停在圆圈。  
...
**应用页面查看和管道转换**图表了解 windows 10 上的客户如何找到所选时段内的应用的一览。
...
*页面查看*意味着某位客户查看你的应用商店一览页，通过基于 web 的应用商店或从 windows 10 上的应用商店应用。 其中包括未使用 Microsoft 帐户登录的用户的查看次数。 一些客户选择退出为 Microsoft 提供此信息。
