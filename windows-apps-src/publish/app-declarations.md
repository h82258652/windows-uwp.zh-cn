...
如果你想要阻止你的应用安装到备用驱动器或可移动存储，并仅允许安装到其设备上的内部硬盘驱动器，请取消选中此框。 （注意，如果没有限制，以便*仅*可以应用的安装选项安装到可移动存储媒体。）
