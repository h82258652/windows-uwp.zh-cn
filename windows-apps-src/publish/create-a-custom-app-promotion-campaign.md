...
如果你为应用举行自定义市场活动，你可以通过为每个自定义市场活动创建不同的 URL（其中每个 URL 包含不同的*市场活动 ID*）来跟踪每个市场活动的相对表现。 当运行 Windows 10 的客户单击包含市场活动 ID 的 URL 时，Microsoft 将单击相应的自定义市场活动与相关联，并将这些数据中向你提供[合作伙伴中心](https://partner.microsoft.com/dashboard)。

> [!IMPORTANT]
> 此数据会仅跟踪客户在 Windows 10 上。 使用其他操作系统的客户仍可以跟踪你的应用一览的链接，但有关这些客户的活动的数据将不包括在内。
...
* 您可以查看页面视图和应用程序或外的接程序与转换有关的数据**应用程序页面视图和通过市场活动 ID 的转换**和**总活动转换**图表中[收购报表](acquisitions-report.md)。
...
* 她将发布到她的 Facebook 主页的 URL 包括自定义市场活动 ID `my-facebook-campaign`

* 她将发布到 Twitter 的 URL 包括自定义市场活动 ID `my-twitter-campaign`
...
自定义市场活动*转换*是由客户单击通过自定义市场活动推广的 URL 所导致的购置。 有不同的转换为符合条件的方案**应用程序页面视图和通过市场活动 ID 的转换**和**总活动转换**图表中[收购报表](acquisitions-report.md)和对符合条件的有关转换[以编程方式检索活动 ID](#programmatically)。

### <a name="qualifying-conversions-in-the-acquisitions-report"></a>符合条件的收购报表中的转换
...
* 运行的设备上**Windows 10，版本 1607，或更高版本**:客户 （无论是已登录到已识别的 Microsoft 帐户） 单击包含自定义市场活动 ID 和重定向到应用商店一览页面的 URL。 客户在通过单击 URL 查看应用商店一览时购置应用。

* 运行的设备上**Windows 10 版本 1511，或更早版本**:客户 （他们必须登录已识别的 Microsoft 帐户） 单击包含自定义市场活动 ID 和重定向到应用商店一览页面的 URL。 客户在通过单击 URL 查看应用商店一览时购置应用。 在这些版本的 Windows 10 上，用户必须使用已识别的 Microsoft 帐户登录，才能在以编程方式检索市场活动 ID 时将购置限定为转换。
...
3.  将以下字符串附加到应用的 URL 的末尾：
    ...
    * 对于 HTML 格式 URL，请附加 **`?cid=*my custom campaign ID*`**。 例如，如果 Skype 引入了一个市场活动 ID 的值**自定义\_市场活动**，包括 ID 将营销活动的新 URL: `https://www.microsoft.com/store/apps/skype/9wzdncrfj364?cid=custom\_campaign`。

    * 对于协议格式 URL，请附加 **`&cid=*my custom campaign ID*`**。 例如，如果 Skype 引入了一个市场活动 ID 的值**自定义\_市场活动**，其中包括市场活动 ID 将新协议 URL: `ms-windows-store://pdp/?PRODUCTID=9wzdncrfj364&cid=custom\_campaign`。
...
* Windows 10，版本 1607，或更高版本：使用[ **StoreContext** ](https://docs.microsoft.com/uwp/api/windows.services.store.storecontext)类**Windows.Services.Store**命名空间。 在使用此 API 时，无论用户是否使用已识别的 Microsoft 帐户进行登录，你都可以检索任何[限定购置](#conversions)的自定义市场活动 ID。

* Windows 10 版本 1511，或更早版本：使用[ **CurrentApp** ](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Store.CurrentApp)类**Windows.ApplicationModel.Store**命名空间。 在使用此 API 时，只有在用户使用已识别的 Microsoft 帐户登录的情况下，才能检索[限定购置](#conversions)的自定义市场活动 ID。
...
4. 确认预期的页面视图和转换是否出现在**应用程序页面视图和通过市场活动 ID 的转换**和**总活动转换**图表中[收购报表](acquisitions-report.md)，和测试应用程序的代码，以确认是否可以成功检索使用上面所述的 Api 的市场活动 ID。
