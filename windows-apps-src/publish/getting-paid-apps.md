...
> [!IMPORTANT]
> 你可以从 Microsoft Store 中的应用销售量接收资金之前，需要[将设置付款帐户，需要税务窗体中填写](setting-up-your-payout-account-and-tax-forms.md)。
...
你可以免费提供你的应用，也可以设定客户获取你的应用所需支付的价格金额。 价格段从 0.99 美元开始，并具有附加增量（1.09 美元、1.19 美元等）。 价格越高，价格段之间的增量就越大。

> [!NOTE] 
> 这些价格段也适用于从应用中提供的任何加载项。

应用商店为每个价格段都提供了多种不同货币对应值。 我们使用这些值来帮助你在世界范围内以相当的价格销售你的应用。 但由于外汇汇率的变化，从一种货币转换到另一种货币的确切销售金额可能会略有不同。
...
你可以访问有关你的付款信息的详细信息和下载中的报表**付款摘要**的[合作伙伴中心](https://partner.microsoft.com/dashboard)。 有关此处显示的信息和我们如何对你的收入进行归类的详细信息，请参阅[付款摘要](payout-summary.md)。
...
> [!NOTE]
> 你**付款保存状态**选项适用于**所有**通过合作伙伴中心 (Microsoft Store，播发，Azure Marketplace，等等。) 中的 Windows 开发人员程序支付的收入来源。 无法为每个收入来源选择不同的暂停状态。
