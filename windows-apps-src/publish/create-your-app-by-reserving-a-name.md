...
ms.locfileid: "2815880"
---
# <a name="create-your-app-by-reserving-a-name"></a>通过保留名称创建应用

在[Windows 开发人员中心仪表板](https://partner.microsoft.com/dashboard)中创建新的应用程序的第一步保留应用程序名称。 每个保留名称（有时被称为应用的*标题*）在整个 Microsoft Store 中必须是唯一的。
...
> [!IMPORTANT]
> 您可以保留应用程序，其他名称，您可以选择使用您的应用程序的已发布版本中的那些之一而不是保留时您先在仪表板中创建您的应用程序的一个。 但请注意，将某些应用程序的[标识详细信息](view-app-identity-details.md)，如**包系列名称 (PFN)** 使用您在此处输入的第一个名称。 这些值可能看到某些用户，并且无法更改，因此请确保您保留的名称是适用于此，请使用。
