...
在页面顶部附近，可以选择希望显示数据的时间段。 默认选择是 **72H**（72 小时），但可以改选 **30D** 以显示最近 30 天的数据。 注意在本地时区**72 H**视图和 UTC **30 D**视图中显示数据。
...
**30 D**选定时间段后，您可能会看到圆形标记。 这些表示显著增加或减少我们认为您需要了解的给定值。 在这个圆将显示该日期表示我们检测显著增大或减小相比于之前的周的一周的末尾。 若要查看有关已更改内容的更多详细信息，请悬停在这个圆。  

> [!TIP]
> 您可以查看与通过最近 30 天的[见解报表](insights-report.md)中的重大更改的详细见解。
...
> [!TIP]
> 仅当故障发生在使用 Windows 预览体验成员版本的计算机中时 CAB 文件才将可用，因此并非所有故障都将包含 CAB 下载选项。 若要显示有 CAB 文件的故障，在内容筛选器选择**下载失败**。 您还可以单击的结果进行排序，以使故障，其中包括 CAB 文件显示在列表的顶部将**失败日志**中的**链接**标头。

在**故障的详细信息**页上，您还会看到**堆栈传播**图表中，按百分比排序顶部堆栈的贡献失败的显示，和**设备配置 (30 D)** 图表中，提供有关详细信息发生故障的设备的配置。 


## <a name="crash-free-sessions-and-devices-30d"></a>崩溃忙会话和设备 (30 D)

**崩溃忙会话和设备**图显示了设备或未在过去 30 天内遇到故障的用户会话的百分比。 此信息可帮助您了解您崩溃广泛如何影响您的用户。 例如，应用程序可以具有一天中的 10,000 崩溃。 如果将影响您的设备的 90%，然后您将可能进行分类的为关键和措施来立即修复。 但是，如果仅表示使用您的应用程序的设备的 5%，可能较低优先级。

此图表具有两个选项卡：
- **崩溃忙设备**： 显示未 （在过去 30 天） 的每一天遇到故障的唯一设备的百分比。
- **无崩溃的会话**： 显示未 （在过去 30 天） 的每一天遇到故障的唯一用户会话的百分比。
