...
4.  在**广告单元类型**下拉列表中，选择广告类型。
    ...
    * 如果您的应用程序中显示横幅广告时，选择**标题**。
    * 如果显示时的间隙视频 ad 或间隙横幅 ad 应用程序中，选择**视频间隙**或**横幅间隙**（请务必选择您想要显示的间隙 ad 类型相应的选项）。
    * 如果您的应用程序中显示本机 ad 时，选择**本机**。
...
* 如果您的应用程序显示[本机 ads 的关系](../monetize/native-ads.md)，则将这些值传递给**NativeAdsManagerV2**构造函数。
  > [!IMPORTANT]
...
> [!NOTE]
> 如果 ad 单位过去六个月有没有活动，我们将标签作为**非活动**，并最终从您的仪表板中删除它。 你可以使用筛选器来仅显示**活动**或**停用**广告单位。 如果看到你认为被不准确地标记为**停用**的任何广告单元，请[联系支持人员](http://aka.ms/storesupport)。

<span id="mediation" />

## <a name="mediation-settings"></a>中介设置

当您[创建一个新的 UWP ad 单位](#create-ad-unit)或[编辑现有的 UWP ad 单位](#available-ad-units)，使用选项本节中的 ad 单位为配置[ad 中介](../monetize/ad-mediation-service.md)。 广告中介显示来自多个广告网络（包括其他付费广告网络）的广告以及 Microsoft 应用促销活动的非盈利性广告，能够最大化广告收益和应用促销能力。 我们负责调停来自所选广告网络的横幅广告请求。 如果已在应用中拥有与横幅广告、间隙广告或本机广告相关联的 UWP 广告单元，则启用广告中介无需在应用中进行任何代码更改。
...
2. 在**应用程序内 ads 的关系**页上，转到**中介设置**部分和配置您的设置。

    * 默认情况下，**让 Microsoft 为你的应用选择最佳中介设置**复选框处于选中状态。 我们建议你使用此选项。 此选项会使用机器学习算法自动为你的应用选择广告中介设置，以帮助最大化你在你的应用所支持的各市场中的广告收益。 使用此选项时，您还可以选择您想要配置中使用的 ad 网络。 取消选中您不想要配置的一部分，我们算法将确保您的应用程序仅从所选的 ad 网络接收 ads 的关系的 ad 网络。
    * 如果您想要选择您自己的广告中介设置，选择**修改默认设置**。

    > [!NOTE]
    > 本节中的其余步骤仅适用于您选择**修改默认设置**。
...
|  广告网络  |  说明  |  支持的广告类型  |
|--------------|---------------|---------------------|
| Oath 和 AppNexus |  这是可通过我们的合作伙伴的广告网络，Oath 和 AppNexus Microsoft 托管 ad 网络。<p/>**注意**： Oath 和 AppNexus 始终排名首先**付费 ad 网络**列表中的横幅 ad 单位，它不能更改为这些类型的广告低排名。 | 横幅、视频间隙 |
| AppNexus（直接） | 选择此选项可从[AppNexus](https://www.appnexus.com)提供广告。 | 视频间隙、本机  |
...
| Undertone | 选择此选项可从[Undertone](https://www.undertone.com/)提供广告。 | 间隙的横幅 |
