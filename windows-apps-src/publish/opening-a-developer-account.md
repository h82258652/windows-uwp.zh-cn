...
title: 开设开发者帐户
description: 下面是概要说明如何为 Microsoft Store 和其他 Microsoft 程序，在合作伙伴中心注册 Winodws 开发人员帐户。
ms.date: 10/31/2018
ms.topic: article
...
ms.locfileid: "57628172"
---
# <a name="opening-a-developer-account"></a>开设开发者帐户

下面是如何进行 Windows 开发人员帐户中注册的概览[合作伙伴中心](https://partner.microsoft.com/dashboard)。
...
5.  输入希望使用的**发布者显示名称**（不超过 50 个字符）。 客户在浏览应用时将看到此名称，并通过此名称记住你的应用，因此请谨慎选择该名称。 对于公司帐户，请确保使用组织的注册公司名称或商标。 请注意，如果输入其他人已选择的名称，或者系统显示其他人有权使用该名称，我们将不允许你使用该名称。 
...
10.  单击**完成**以确认注册。 将处理付款，并且我们将向你的电子邮件地址发送一条确认消息。
...
> [!IMPORTANT]
> 若要允许多个用户访问你的开发人员帐户，我们建议使用 Azure Active Directory 以将角色分配给单个用户 （而不是共享访问 Microsoft 帐户）。 每个用户可以通过使用其个人登录到合作伙伴中心访问开发人员帐户的 Azure AD 凭据。 有关详细信息，请参阅[管理帐户用户](manage-account-users.md)。

如果你将需要具有多个用户访问权限，通过登录使用打开的 Microsoft 帐户 （而不作为单个用户添加到帐户)，以下指导原则的公司帐户可能会有所帮助：

-   创建 Microsoft 帐户使用电子邮件地址不已属于您或其他人，如MyCompany_PartnerCenter@outlook.com。 不在公司的域，使用电子邮件地址，尤其是如果你的公司已使用 Azure AD。 (如上文所述，可以将其他用户添加从公司的 Azure AD 更高版本。)
-   将此 Microsoft 帐户的访问权限限制为尽可能少的用户。
-   设置公司电子邮件通讯组列表，其中包括所有人员需要访问开发人员帐户，并将添加到此电子邮件地址[与 Microsoft 帐户关联的安全信息](https://account.microsoft.com/security)。 这允许所有员工接收安全代码发送到此别名的列表上。 如果设置通讯组列表不可行，可以将个人的电子邮件地址添加到你的安全信息，但该电子邮件地址的所有者将是唯一的人可以访问和共享安全代码 （如新的安全信息添加到 t 时出现提示时该帐户，或从新设备的访问时）。
