...
（除非已在**可见性**部分选择**停止获取**选项），您必须对您的加载项选择基本定价。 是**免费**的默认选择，因此如果您想要收费的加载项，请务必选择其中一个可用的价格层 （从.99 美元开始）。

还可计划价格变更，以指示加载项价格应更改的日期和时间。 此外，还可选择针对特定市场自定义这些更改。 

> [!TIP]
> 订阅加载项，您不能通过选择中更高版本的提交的更高的基本定价还是按计划增加价格的价格更改发布该加载项之后, 提升价格。 您可以选择较低的价格使用上述任一方法，但后降低价格您将无法引发高于该新的价格。 因此，是以确保您选择订阅加载项的适当价格层尤其重要。 
