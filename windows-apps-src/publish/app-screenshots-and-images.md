...
若要为 Windows 10 和 Xbox 上的客户已在应用商店仅使用图像你上传 （适用于使用 Windows 10 客户，包括 Xbox），不使用你的程序包中的任何图像、 选中该框，指出 **，显示上传徽标图像，而不是包的图像**.

选中此框，出现了名为**应用商店显示图像**的新部分。 此处，你可以上传 3 张图像，其中包括**1:1 应用磁贴图标 （300 x 300 像素）** 的大小 （如果选中该框，以提供该图像将移动到此部分的字段）。 如果你使用此选项，我们建议提供所有三种图像大小：300 x 300、150 x 150 和 71 x 71 像素。 但是，只需要 300 x 300 大小的图像。
