...
应用商店还可能会使用你包含在应用程序包中的应用磁贴和其他图像。 提交应用前，请运行 [Windows 应用认证工具包](../debug-test-perf/windows-app-certification-kit.md)来确定你是否缺少任何必需的图像。 有关这些图像的指南和建议，请参阅[磁贴和图标资源](../design/shell/tiles-and-notifications/app-assets.md)。
...
这种图像用作 Windows 10 和 Xbox 设备客户的主要徽标图像，因此**强烈建议**提供此图像以确保正确显示。 如果不包含此图像，一览效果可能不佳，并且不能与客户在浏览 Microsoft Store 时看到的其他一览保持一致。 在搜索结果或编辑精选的集锦中，也可使用此图像。
...
你可以选择防止应用商店向 Windows 10 （包括 Xbox） 上的客户显示你的一览时使用你的应用的软件包中的徽标图像，并改为具有应用商店使用你上传的图像。 这可以使你更好地控制应用在整个 Microsoft Store 上针对 Windows 10（包括 Xbox）客户的各种显示的外观。 （如果你的产品支持较早操作系统版本，这些客户可能仍会看到图像从你的程序包，即使你使用此选项。）

若要 For Windows 10 和 Xbox 上的客户，显示在应用商店仅使用图像你上传 （适用于 Windows 10，包括 Xbox 客户），并且不使用你的程序包中的任何图像，请检查框**上传的徽标图像，而不是包的图像**.

当你选中此框时，将显示一个新的部分，调用**应用商店显示图像**。 在这里，你可以上传 3 张图像，其中包括**1:1 应用磁贴图标 （300 x 300 像素）** 大小 （如果你选中此框，提供该图像将移动到此部分的字段）。 如果你使用此选项，我们建议提供所有三种图像大小：300 x 300、150 x 150 和 71 x 71 像素。 但是，只需要 300 x 300 大小的图像。
...
> [!TIP]
> 如果计划在 Microsoft Store 一览中包括[视频预告片](#trailers)，则特别推荐 **16:9 超级人物插图**；如果不包括，则一览顶部不会显示预告片。
...
**16:9 超级人物插图（1920 x 1080 或 3840 x 2160 像素）** 图像适用于所有 Windows 10（包括 Xbox）设备类型的 Microsoft Store 中的各种布局。 无论你的应用针对哪种操作系统版本或设备类型，我们都建议提供此图像。
...
如果应用支持以前的操作系统版本 (Windows 8.x 和/或 Windows Phone 8.x)，则必须提供这些图像，以便我们在促销布局中考虑特别推荐你的应用（但它们并不能保证你的应用将获得特别推荐）。 如果应用不支持这些以前的操作系统版本，可以跳过此部分。 （此部分以前称为**可选促销图像**。）
...
若要从列表中删除预告片，请单击其文件名旁边的 **X**。 你可以选择仅将其从当前使用的 Microsoft Store 一览中删除，或将其从产品的所有 Microsoft Store 一览（每种语言）中删除。
