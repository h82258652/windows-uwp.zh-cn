...
ms.locfileid: "5934144"
---
# <a name="app-screenshots-images-and-trailers"></a>应用屏幕截图、图像和预告片

设计精良的图像是在应用商店中向潜在客户表示应用的主要途径之一。

你可以提供[屏幕截图](#screenshots)、[徽标](#store-logos)、[预告片](#trailers)和在你的应用的应用商店一览中包含其他插图资源。 这些项目中的一些是必要的，一些是可选的（不过一些可选图像应该包含在内，以获得最佳的应用商店显示）。
...
注意，如果你创建了[多种语言](supported-languages.md)的应用商店一览，你将为每种语言提供一个**应用商店一览**页面。 你将需要为每种语言单独上传图像（即使你使用相同的图像），并提供要用于每种语言的描述文字。 （如果你有大量的语言的应用商店一览，你可能会发现它更轻松地更新它们通过[导出一览数据和脱机](import-and-export-store-listings.md)。）
...
此图像需要在 Windows Phone 8.1 及更早版本上才能正常显示。 如果你以前发布的应用支持 Windows Phone 8.1 或更早版本，并且你不提供此图像，这些客户将看到你的应用一览空白图标。 （如果这也适用于 Windows 10 客户你的应用仅有面向 Windows Phone 8.1 或更早版本的程序包。）
...
你可以选择防止应用商店向 Windows 10 （包括 Xbox） 上的客户显示你的一览时，你的应用包中使用的徽标图像，并改为具有应用商店使用你上传的图像。 这可以使你更好地控制应用在整个 Microsoft Store 上针对 Windows 10（包括 Xbox）客户的各种显示的外观。 （如果你以前发布的应用支持较早的操作系统版本，这些客户可能仍会看到你的程序包中的图像。）

要 For Windows 10 和 Xbox 上的客户，显示在应用商店仅使用图像你上传 （适用于使用 Windows 10 客户，包括 Xbox），并且不使用你的程序包中的任何图像，请检查框**上传的徽标图像，而不是我程序包中的图像**.

选中此框，出现了名为**应用商店显示图像**的新部分。 在这里，你可以上传 3 张图像，其中包括**1:1 应用磁贴图标 （300 x 300 像素）** 大小 （如果你选中该框，提供该图像将移动到此部分的字段）。 如果你使用此选项，我们建议提供所有三种图像大小：300 x 300、150 x 150 和 71 x 71 像素。 但是，只需要 300 x 300 大小的图像。
...
> [!TIP]
> [16:9 超级人物插图](#windows-10-and-xbox-image-169-super-hero-art)图像尤其被建议如果你打算将[视频预告片](#trailers)包含在你的应用商店一览。如果你不包括，则一览顶部不会显示预告片。


### <a name="trailers"></a>预告片

预告片是一个简短视频，可为客户提供在实际操作中查看产品的方法，以便他们能更好地了解产品内容。 它们会显示在你的应用的应用商店一览 （只要包含[16:9 超级人物插图](#windows-10-and-xbox-image-169-super-hero-art)图像） 的顶部。 
...
> [!IMPORTANT]
> 当使用预告片，还必须提供[16:9 超级人物插图](#windows-10-and-xbox-image-169-super-hero-art)图像部分，以使预告片的应用商店一览顶部显示的顺序。 预告片播放完成后将显示此图像。
...
若要从列表中删除预告片，请单击其文件名旁边的 **X**。 你可以选择是否要删除仅当前应用商店一览，你正在开发，或将其删除来自所有产品的应用商店一览 （每种语言）。
...
如果你以前提交的应用支持较早的操作系统版本 (Windows 8.x 和/或 Windows Phone 8.x)，必须提供这些图像，以便我们考虑特别推荐你在促销布局中的应用 （但他们不能保证你的应用将获得特别推荐） 中。 如果你的应用不支持这些较早的操作系统版本，则跳过此部分。 （此部分以前称为**可选促销图像**。）
