...
2. 在**通知**页上，选择**新通知**。
3. 在**选择模板**部分中，选择要发送，然后单击**确定**的[通知的类型](#notification-template-types)。
4. 在下一页中，使用下拉菜单选择要为其生成通知的**单个应用**或**多个应用**。 您只能选择已[配置为接收通知使用 Microsoft 存储服务 SDK](../monetize/configure-your-app-to-receive-dev-center-notifications.md)的应用程序。
...
2.  在现有的通知的表中，选择**正在进行**或**已完成**，，然后查找**传递率**和**应用程序启动速率**列，请参阅高级性能的每个通知。
...
1.  展开 Windows 开发人员中心仪表板中的**参与**部分，然后选择**通知**。
2.  在**通知**页上单击**分析**选项卡。此选项卡将显示以下数据：
    * 图表的 toast 和操作中心通知的各个用户操作状态的视图。
    * World 地图视图的 toast 和操作通过率单击中心通知。
3. 在页面顶部附近，可以选择希望显示数据的时间段。 默认选择为 30D（30 天），但你可以选择要显示 3、6 或 12 个月的数据或指定的自定义数据范围的数据。 也可以展开要筛选的所有应用程序和市场数据的**筛选器**。
