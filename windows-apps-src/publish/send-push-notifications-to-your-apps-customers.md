...
[合作伙伴中心](https://partner.microsoft.com/dashboard)提供以数据驱动的客户参与平台，可用于将通知发送到所有应用的客户，或仅面向符合[客户中所定义条件的应用的 Windows 10 客户子集类别](create-customer-segments.md)。 你还可以创建一条通知发送到多个应用的客户。
...
1. **注册应用以接收推送通知。** 通过在应用中添加对 Microsoft Store Services SDK 的引用，然后添加几行注册通知通道合作伙伴中心和应用之间的代码执行此操作。 我们将使用该通道向你的客户传递通知。 有关详细信息，请参阅[针对定向推送通知配置应用](../monetize/configure-your-app-to-receive-dev-center-notifications.md)。
...
> [!NOTE]
> 从合作伙伴中心，应用可以接收通知之前，你必须首先调用[RegisterNotificationChannelAsync](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicesengagementmanager.registernotificationchannelasync)在你的应用来注册应用以接收通知。 此方法在 [Microsoft Store Services SDK](http://aka.ms/store-em-sdk) 中可用。 有关如何调用此方法的详细信息以及代码示例，请参阅[针对定向推送通知配置应用](../monetize/configure-your-app-to-receive-dev-center-notifications.md)。
...
2.  在应用中，调用[ParseArgumentsAndTrackAppLaunch](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicesengagementmanager.parseargumentsandtrackapplaunch)通知合作伙伴中心，响应目标通知已启动你的应用。 此方法由 Microsoft Store Services SDK 提供。 有关如何调用此方法的详细信息，请参阅[配置应用以接收合作伙伴中心的通知](../monetize/configure-your-app-to-receive-dev-center-notifications.md)。


### <a name="to-view-notification-performance"></a>查看通知性能

如上所述配置通知和你的应用来测量通知性能，如前文所述，你可以查看通知执行的程度。
...
3.  若要查看更详细的性能细节，请选择通知名称。 在**交货统计**部分，可以查看以下通知**状态**类型的**计数**和**百分比**信息：
    ...
    * **通道过期失败**： 无法发送通知，因为合作伙伴中心与应用之间的通道过期。 这可能会在客户长时间没有打开应用的情况下发生。
    * **正在发送**：通知排在待发送队列中。
    * **已发送**：通知已发送。
...
2.  在**通知**页面上，单击**分析**选项卡。此选项卡上显示以下数据：
    * 你的 toast 和操作中心通知的各种用户操作状态的图形视图。
    * 为你的 toast 和操作通过率单击世界地图视图中心通知。
3. 在页面顶部附近，可以选择希望显示数据的时间段。 默认选择为 30D（30 天），但你可以选择要显示 3、6 或 12 个月的数据或指定的自定义数据范围的数据。 你还可以展开**筛选器**来筛选的所有数据的应用和市场。

## <a name="translate-your-notifications"></a>翻译通知

若要最大化发挥通知的影响力，请考虑将它们翻译为客户首选的语言。 合作伙伴中心，可以轻松为你的[Microsoft Translator](https://www.microsoft.com/translator/home.aspx)服务的力量自动翻译通知。
