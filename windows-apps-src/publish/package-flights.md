...
> [!NOTE]
> 如果有多个软件包外部测试版，则需要为每个版本分配排名。 有关详细信息，请参阅下面的[添加其他软件包外部测试版并为其排名]。
...
> [!IMPORTANT]
> 当在软件包外部测试版中逐步推出程序包时，不在获取新程序包比例范围内的用户将从之前的软件包外部测试版提交中获取程序包（除非他们可以获取排名更高的外部测试版）。
