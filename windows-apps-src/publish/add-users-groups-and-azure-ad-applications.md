...
ms.locfileid: "2864646"
---
# <a name="add-users-groups-and-azure-ad-applications-to-your-dev-center-account"></a>向开发人员中心帐户添加用户、组和 Azure AD 应用程序

Windows 开发人员中心 （在**帐户设置**） 下的**用户**部分中，可以使用 Azure Active Directory 将用户添加到您的开发人员中心帐户。 为每个用户分配一个角色（或自定义权限集），并且该角色定义他们的帐户访问权限。 你还可以添加[用户组](#groups)和 [Azure AD 应用程序](#azure-ad-applications)，以授予他们对你的开发人员中心帐户的访问权限。
...
1.  选择齿轮图标 （附近的仪表板右上角），然后选择**帐户设置**。 在**设置**菜单中，选择**用户**。
2.  从**用户**页面，选择**添加用户**。 
...
-   **添加现有用户**： 选择用户已经存在于组织的目录中并授予其访问您的开发人员中心帐户，使用上述方法。
-   **创建新用户**： 创建新的用户帐户添加到同时贵组织的目录和您的开发人员中心帐户
...
1.  从 （在**帐户设置**） 下的**用户**页中，选择**添加用户**，然后选择**创建新用户**。
2.  输入新用户的名字、姓氏和用户名。
...
1.  从 （在**帐户设置**） 下的**用户**页中，选择**添加用户**，然后选择**邀请用户通过电子邮件**。
...
1.  从 （在**帐户设置**） 下的**用户**页中，选择您想要编辑的用户帐户的名称。
2.  选择页面底部的**重置密码**按钮。
...
1.  1.  选择齿轮图标 （附近的仪表板右上角），然后选择**帐户设置**。 在**设置**菜单中，选择**用户**。
...
1.  从**用户**页上的 （**帐户设置**） 下，选择**添加 Azure AD 应用程序**。
2.  在下一页上，选择**新 Azure AD 应用程序**。
...
1.  从 （在**帐户设置**） 下的**用户**页中，选择 Azure AD 应用程序的名称。
    > [!TIP]
...
1.  从 （在**帐户设置**） 下的**用户**页中，选择用户、 组或想要编辑的 Azure AD 应用程序帐户的名称。
...
在**用户**页上的 （**帐户设置**） 下，选择**最后一次活动**下显示的用户想要查看其浏览历史记录的链接。 你可以查看此用户在过去 30 天内访问过的所有网页的 URL。

<span id="remove" />

## <a name="remove-users-groups-and-azure-ad-applications"></a>删除用户、组和 Azure AD 应用程序

要从您的开发人员中心帐户中删除用户、 组或 Azure AD 应用程序，请选择显示按其名称，在**用户**页上的**删除**链接。 在确认你希望删除它后，该用户、组或 Azure AD 应用程序将无法再访问你的开发人员中心帐户（除非你以后再次添加它）。
