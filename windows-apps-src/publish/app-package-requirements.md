...
| 应用包属性 | 要求                                                          |
|----------------------|----------------------------------------------------------------------|
| 程序包大小         | . .msixbundle 或 .appxbundle:每个束最多 25 GB <br>面向 Windows 10 的 .msix 或 .appx 包:每个包最多 25 GB<br>.appx 包目标 Windows 8.1:每个包的最大 8 GB <br> 面向 Windows 8 的 .appx 包:每个包最多 2 GB <br> .appx 包目标 Windows Phone 8.1:每个包的最大 4 GB <br> .xap 包:每个包的最大大小为 1 GB                                                                           |
| 块映射哈希     | SHA2-256 算法                                                   |
...
StoreManifest.xml 是一种可选的配置文件，可包含在应用包中。 该文件旨在支持程序包清单未涵盖的功能，例如将应用声明为 Microsoft Store 设备应用或声明程序包适用于某个设备所依据的要求。 如果使用了 Storemanifest.xml, 则会随应用包一起提交, 且必须位于应用主项目的根文件夹中。 有关详细信息，请参阅 [StoreManifest 架构](https://docs.microsoft.com/uwp/schemas/storemanifest/store-manifest-schema-portal)。
