...
[**XamlLight**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.xamllight) 对象用于应用 [**CompositionLight**](https://docs.microsoft.com/uwp/api/Windows.UI.Composition.CompositionLight) 以动态照明 XAML UIElement。 XamlLight 提供面向 UIElements 或 XAML 画笔，将灯应用于的 UIElements，树的方法和帮助管理 CompositionLight 的生存期基于是否是当前在资源使用。
...
本示例显示适用于目标 UIElements 和画笔的彩色的聚焦自定义 XamlLight 的定义。
...
您可以将此浅色应用于任何 XAML UIElement 或画笔浅色它们。 本示例显示不同的潜在用法。

> [!Important]
> 有关[C + + / WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt)，删除的两个实例`local:OrangeSpotLight.IsTarget="True"`从下面的标记。 代码隐藏中已设置的附加的属性。
...
![Xaml 灯亮起的元素的示例](images/orange-spot-light.png)
