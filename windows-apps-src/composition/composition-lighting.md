...
ms.locfileid: "5395665"
---
# <a name="using-lights-in-windows-ui"></a>Windows UI 中使用光

Windows.UI.Composition Api 使你可以创建实时动画和效果。 合成光线使 2D 应用程序中的 3D 照明。 在此概述中，我们将运行效果如何设置合成光、 标识视觉对象以接收每个光源，以及使用效果定义你的内容的材料的功能。
...
- 光的独立于场景实现沉浸式方案，如音乐播放场景中的其他对象的转换。
...
| 类型 | 说明 |
| --- | --- |
| [AmbientLight](/uwp/api/windows.ui.composition.ambientlight) | 发出显示无方向光的光源反映由场景中的所有内容。 |
...
当光面向视觉 （添加到[目标](/uwp/api/windows.ui.composition.compositionlight.targets)列表），视觉对象及其所有后代已注意到，对此光源响应。 这可能很简单作为根处树和下面的所有视觉对象的 PointLight 源响应点光方向的动画设置。

**ExclusionsFromTargets**使你能够与添加目标类似的方式删除或视觉效果的子树的视觉对象的光。 通过被排除的可视根的树中的子元素均未亮起作为结果。
...
请参阅完整的[文本框闪现](https://github.com/Microsoft/WindowsUIDevLabs/tree/master/SampleGallery/Samples/SDK%2014393/TextShimmer)示例在 WindowUIDevLabs 示例条样若要了解详细信息。
...
**坐标空间** | 每个 Visual 具有隐式的 3D 坐标空间。 X 方向是从左到右。 Y 方向是从顶部到底部。 Z 方向是退出平面的点。 此坐标的原始点是左上角与可视，并且在单元设备无关的像素 (DIP)。 在此坐标中定义的光的偏移量。
**内锥和外锥细胞** | 聚光发出的光锥包括两个部分：一个明亮的内锥和一个外锥。 合成允许你控制内锥和外锥角和颜色。
**Offset** | 相对于其坐标空间 Visual 光源的偏移量。

> [!NOTE]
> 当多个光命中相同视觉对象，或者每当光的颜色值获取足以超过 1.0，光的颜色可能由于固定光颜色通道的更改。
...
- 标识光对象： 面向在相关的视觉效果的光。
- [可选]定义如何单个对象对光作出响应： 使用 SceneLightingEffect 与 EffectBrush 自定义用于显示 SpriteVisual 反射光。 反射默认设置支持子级的光源的 CoordinateSpace 的照明。  可视对象与 SceneLightingEffect 绘制覆盖该视觉对象的默认光。
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect)经常用于材料创建。 SceneLightingEffect 是当你想要实现更复杂，如启用在图像上的反射属性和/或提供深度错觉与正常映射所使用的效果。 SceneLightingEffect 提供使用照明属性，例如高光和漫射金额自定义你的 UI 的功能。 你可以进一步自定义效果管道，从而允许你单独混合和撰写与你的内容不同的照明反应的其余部分的照明效果。
