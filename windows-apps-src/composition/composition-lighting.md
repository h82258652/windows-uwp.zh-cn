...
| [PointLight](/uwp/api/windows.ui.composition.pointlight) | 点光发出在所有方向的光的光源。 如灯泡。 |
...
**ExclusionsFromTargets**使你能够删除的光的视觉对象或视觉效果的子树中添加目标的类似方式。 通过被排除的可视根的树中的子元素均未亮起作为结果。
...
有几个因素，确定哪些内容会照亮 CompositionLight 时需要考虑。

概念 | 详细信息
--- | ---
**环境光** | 向场景添加非环境光将关闭所有现有的光。  不面向非环境光的项目将显示为黑色。  照亮周围的视觉对象不定向光的自然的方式，与其他光配合使用环境光。
**光的数量** | 你可以使用任意两个非环境合成光的任意组合要针对你的 UI。 环境光没有限制;发现，点和远的光。
**生命周期** | CompositionLight 可能会遇到生命周期条件 (示例： 使用之前，垃圾回收器可能回收光对象)。  我们建议通过将光照添加为成员来帮助管理生命周期的应用程序保持对你灯的引用。
...
具体取决于使用的光的类型，光可以具有衰减和空间的属性。 并非所有光类型都使用所有属性。
...
**坐标空间** | 每个 Visual 具有隐式的 3D 坐标空间。 X 方向是从左到右。 Y 方向是从顶部到底部。 Z 方向是退出平面的点。 此坐标的原点为左上角与可视，并且该单元设备无关的像素 (DIP)。 在此坐标中定义的光的偏移量。
...
- 标识光对象： 面向在相关的视觉效果的光。
- [可选]定义如何单个对象响应光： 使用 SceneLightingEffect 与 EffectBrush 自定义用于显示 SpriteVisual 反射光。 反射默认设置支持的光的光源的 CoordinateSpace 子元素。  可视对象与 SceneLightingEffect 绘制覆盖该视觉对象的默认光。
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect)经常用于材料创建。 SceneLightingEffect 是当你想要实现更复杂，如启用在图像上的反射属性和/或提供深度错觉与正常映射所使用的效果。 SceneLightingEffect 能够通过照明属性，例如高光和漫射金额自定义你的 UI。 你可以进一步自定义效果管道可以单独混合和撰写与你的内容不同的照明反应的其余部分的照明效果。
...
属性 | 说明
--- | ---
**正常映射** | NormalMaps 创建其中正常指向光线将显得更亮并正常指向立即将暗的纹理效果。 若要添加到目标 visual NormalMap 使用 LoadedImageSurface 用于加载 NormalMap 资产[CompositionSurfaceBrush](/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush) 。
**环绕** | 环境光属性主要用于控制整体颜色反射。
**反射** | 镜面反射高光对象，从而使其看上去闪光上创建 你可以控制镜面反射程度以及闪光的级别。  这些属性被其创建材料效果，例如 shinny 金属或镜面纸张。
**漫射** | 漫射的反射散布在所有方向的光。
