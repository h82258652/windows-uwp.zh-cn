...
ms.locfileid: "4024293"
---
# <a name="using-lights-in-windows-ui"></a>Windows UI 中使用光

Windows.UI.Composition Api 使你可以创建实时动画和效果。 合成光线使 2D 应用程序中的 3D 照明。 在此概述中，我们会介绍如何设置合成光、 标识接收的每个光源的视觉对象，并使用效果定义内容的材料的功能。
...
| [DistantLight](/uwp/api/windows.ui.composition.distantlight) | 大型无限远距离光源发射光的一个方向。 如太阳。 |
...
**ExclusionsFromTargets**使你能够为添加目标类似的方式删除照明的视觉对象或视觉效果的子树。 通过排除的可视根的树中的子元素不作为结果亮起。
...
概念 | 详细信息
--- | ---
**环境光** | 向场景添加非环境光将关闭所有现有的光。  不面向非环境光的项目将显示为黑色。  照亮周围的视觉对象不定向光的自然的方式，使用与其他光一起使用的环境光。
**光的数量** | 你可以使用任意两个非环境合成光的任意组合要针对你的 UI。 环境光没有限制;发现，点和远的光。
**生命周期** | CompositionLight 可能会遇到生存期条件 (示例： 使用之前，垃圾回收器可能回收光对象)。  我们建议通过将光添加为成员来帮助管理生命周期的应用程序保持在灯的引用。
...
**坐标空间** | 每个 Visual 具有隐式的 3D 坐标空间。 X 方向是从左到右。 Y 方向是从顶部到底部。 Z 方向是从平面的点。 此坐标的原点为左上角与可视，并且在单元设备无关的像素 (DIP)。 在此坐标中定义的光的偏移量。
**内锥和外锥细胞** | 聚光发出的光锥包括两个部分：一个明亮的内锥和一个外锥。 合成允许你控制内锥和外锥角和颜色。
**Offset** | 相对于其坐标空间 Visual 光源的偏移量。

> [!NOTE]
> 当多个灯命中同一个 Visual，或者每当光的颜色值获取足够大，超过 1.0，光的颜色可能由于固定光颜色通道的更改。
...
属性 | 说明
--- | ---
**正常映射** | NormalMaps 创建其中正常指向光线会显得更亮并正常指向距离将是暗的纹理效果。 若要添加到你定向 visual NormalMap，请使用使用 LoadedImageSurface 加载 NormalMap 资产[CompositionSurfaceBrush](/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush) 。
**环绕** | 环境光属性主要用于控制整体颜色反射。
**反射** | 镜面反射创建对象，使它们看起来闪光上突出显示。 你可以控制级别的镜面反射以及闪光的级别。  这些属性被其创建材料效果，例如 shinny 金属或镜面纸张。
**漫射** | 漫射的反射散布在所有方向的光。
