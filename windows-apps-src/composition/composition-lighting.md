...
Windows.UI.Composition Api 使你能够创建实时动画和效果。 合成光线使 2D 应用程序中的 3D 照明。 在此概述中，我们会介绍如何设置合成光、 标识视觉对象以接收每个光源，以及使用效果来定义内容的材料的功能。
...
**ExclusionsFromTargets**使你能够为添加目标类似的方式删除照明的视觉对象或视觉效果的子树。 通过被排除的可视根的树中的子元素均未亮起作为结果。
...
概念 | 详细信息
--- | ---
**环境光** | 向场景添加非环境光将关闭所有现有的光。  不面向非环境光的项目将显示为黑色。  若要照亮周围的视觉效果不定向光的自然的方式，与其他光配合使用环境光。
...
- 标识光对象： 面向在相关的视觉效果的光。
- [可选]定义如何单个对象对光作出响应： 使用 SceneLightingEffect 与 EffectBrush 自定义用于显示涂刷反射光。 反射默认设置支持的光的光源的 CoordinateSpace 子元素。  可视对象与 SceneLightingEffect 绘制覆盖该视觉默认照明。
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect)经常用于材料创建。 SceneLightingEffect 是当你想要实现更复杂，如启用在图像上的反射属性和/或提供深度错觉与正常映射所使用的效果。 SceneLightingEffect 提供使用照明属性，例如高光和漫射金额自定义 UI 的功能。 你可以进一步自定义效果管道可以单独混合并撰写与你的内容不同的照明反应的其余部分的照明效果。

> [!NOTE]
> 场景照明不会产生阴影;它是专注于 2D 呈现效果。  它不会考虑 3D 照明方案包含实际照明模型，包括阴影到。


属性 | 说明
--- | ---
**正常映射** | NormalMaps 创建其中正常指向光线会显得更亮并正常指向距离将是暗的纹理效果。 若要添加到目标 visual NormalMap 使用使用 LoadedImageSurface 加载 NormalMap 资产[CompositionSurfaceBrush](/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush) 。
**环绕** | 环境光属性主要用于控制整体颜色反射。
**反射** | 镜面反射创建对象，使它们看起来闪光上突出显示。 你可以控制镜面反射程度以及闪光的级别。  这些属性的操作以创建材料效果，例如 shinny 金属或镜面纸张。
**漫射** | 漫射的反射散布在所有方向的光。
**反射模型** | [反射率模型](/uwp/api/windows.ui.composition.effects.scenelightingeffectreflectancemodel)允许你[Blinn Phong](https://docs.microsoft.com/visualstudio/designers/how-to-create-a-basic-phong-shader)和物理基于 Blinn Phong 之间进行选择。  如果你想要有压缩反射高光，你将选择以物理方式基于 Blinn Phong。
