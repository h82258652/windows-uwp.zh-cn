...
ms.locfileid: "6664018"
---
# <a name="using-lights-in-windows-ui"></a>Windows UI 中使用光

Windows.UI.Composition Api 使你可以创建实时动画和效果。 合成光线使 2D 应用程序中的 3D 照明。 在此概述中，我们将通过如何设置合成光、 标识视觉对象以接收每个光源，以及使用效果定义内容的材料的功能运行。
...
**ExclusionsFromTargets**使你能够删除的光的视觉对象或视觉效果的子树中添加目标的类似方式。 通过排除的可视根的树中的子元素均未亮起作为结果。
...
概念 | 详细信息
--- | ---
**环境光** | 向场景添加非环境光将关闭所有现有的光。  不面向非环境光的项目将显示为黑色。  照亮周围的视觉对象不定向光的自然方式，使用与其他光一起使用的环境光。
...
属性 | 说明
--- | ---
**正常映射** | NormalMaps 创建其中正常指向光线会显得更亮并正常指向距离将是暗的纹理效果。 若要添加到你的目标 visual NormalMap 使用使用 LoadedImageSurface 加载 NormalMap 资产[CompositionSurfaceBrush](/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush) 。
**环绕** | 环境光属性通常用于控制整体颜色反射。
**反射** | 镜面反射高光对象，从而使其看上去闪光上创建 你可以控制镜面反射程度以及闪光的级别。  这些属性的操作以创建材料效果，例如 shinny 金属或镜面纸张。
**漫射** | 漫射的反射散布在所有方向的光。
**反射模型** | [反射模型](/uwp/api/windows.ui.composition.effects.scenelightingeffectreflectancemodel)允许你[Blinn Phong](https://docs.microsoft.com/visualstudio/designers/how-to-create-a-basic-phong-shader)和物理基于 Blinn Phong 之间进行选择。  如果你想要有压缩的反射高光，你将选择以物理方式基于 Blinn Phong。
