...
ms.locfileid: "8329933"
---
# <a name="using-lights-in-windows-ui"></a>Windows UI 中使用光

Windows.UI.Composition Api 使你能够创建实时动画和效果。 合成光线使 2D 应用程序中的 3D 照明。 在此概述中，我们将运行效果如何设置合成光、 标识接收的每个光源的视觉对象，并使用效果来定义内容的材料的功能。
...
| [PointLight](/uwp/api/windows.ui.composition.pointlight) | 点光发出在所有方向的光的光源。 如物品环绕。 |
| [聚焦](/uwp/api/windows.ui.composition.spotlight) | 光源发出内锥和外锥。 如手电筒。 |

## <a name="targets"></a>目标

当光面向视觉 （添加到[目标](/uwp/api/windows.ui.composition.compositionlight.targets)列表），视觉对象及其所有后代已注意到，对此光源进行响应。 这可能很简单作为根处树和下面的所有视觉 PointLight 源响应点光方向的动画设置。

**ExclusionsFromTargets**使你能够为添加目标类似的方式删除照明的视觉对象或视觉效果的子树。 获得 root 权限由被排除的可视树中的子元素均未亮起作为结果。
...
请参阅完整的[文本闪现](https://github.com/Microsoft/WindowsUIDevLabs/tree/master/SampleGallery/Samples/SDK%2014393/TextShimmer)示例在 WindowUIDevLabs 示例条样，若要了解详细信息。
...
概念 | 详细信息
--- | ---
**环境光** | 向场景添加非环境光将关闭所有现有的光。  不面向非环境光的项目将显示为黑色。  若要照亮周围的视觉对象不定向光的自然的方式，使用与其他光一起使用的环境光。
**光的数量** | 你可以使用任意两个非环境合成光的任意组合要针对你的 UI。 环境光没有限制;位置时，点和远的光。
**生命周期** | CompositionLight 可能会遇到生命周期条件 (示例： 垃圾回收器可能回收光对象，则在使用之前)。  我们建议通过将光照添加为成员来帮助管理生命周期的应用程序保持对你灯的引用。
...
属性 | 描述
--- | ---
**正常映射** | NormalMaps 创建其中正常指向光线会显得更亮并正常指向距离将是暗的纹理效果。 若要添加你定向的视觉 NormalMap 使用[CompositionSurfaceBrush](/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush)使用 LoadedImageSurface 加载 NormalMap 资产。
**环绕** | 环境光属性通常用于控制整体颜色反射。
**反射** | 镜面反射创建对象，从而使其看上去闪光上突出显示。 你可以控制镜面反射程度以及闪光的级别。  这些属性被其创建材料效果，例如 shinny 金属或镜面纸张。
**漫射** | 漫射的反射散布在所有方向的光。
