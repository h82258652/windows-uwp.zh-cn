...
Windows.UI.Composition Api 使你能够创建实时动画和效果。 合成光线使 2D 应用程序中的 3D 照明。 在此概述中，我们将运行效果如何设置合成光、 标识视觉对象以接收每个光源，并使用效果来定义内容的材料的功能。
...
[CompositionLight](https://docs.microsoft.com/uwp/api/windows.ui.composition.compositionlight)可以创建各种光，并将它们放置在坐标空间中。 这些光面向你想要标识为光照亮的视觉效果。
...
当光面向一种可视 （添加到[目标](/uwp/api/windows.ui.composition.compositionlight.targets)列表），视觉对象及其所有后代已注意到，对此光源响应。 这可能很简单作为一个树和下面的所有视觉效果的根处 PointLight 源响应点光方向的动画设置。

**ExclusionsFromTargets**使你能够为添加目标类似的方式删除照明的视觉对象或视觉效果的子树。 通过排除的可视根的树中的子元素均未亮起结果。
...
有几个因素 CompositionLight 将会亮起确定哪些内容时需要考虑。
...
具体取决于使用光的类型，光可以具有衰减和空间的属性。 并非所有光类型都使用所有属性。
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect)经常用于材料创建。 SceneLightingEffect 是当你想要实现更复杂，如启用在图像上的反射属性和/或提供深度错觉与正常映射所使用的效果。 SceneLightingEffect 提供使用照明属性，例如高光和漫射金额自定义你的 UI 的功能。 你可以进一步自定义效果管道可分别用于混合和撰写与你的内容的不同照明反应的其余部分的照明效果。
...
属性 | 说明
--- | ---
**正常映射** | NormalMaps 创建其中正常指向光线会显得更亮并且正常指向距离将是暗的纹理效果。 若要添加到目标 visual NormalMap 使用使用 LoadedImageSurface 加载 NormalMap 资产[CompositionSurfaceBrush](/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush) 。
**环绕** | 环境属性主要用于控制整体颜色反射。
**反射** | 镜面反射对象，使它们看起来闪光上创建高光。 你可以控制镜面反射程度以及闪光的级别。  这些属性操作以创建材料效果，例如 shinny 金属或镜面纸张。
**漫射** | 漫射的反射散布在所有方向的光。
**反射模型** | [反射模型](/uwp/api/windows.ui.composition.effects.scenelightingeffectreflectancemodel)可以[Blinn Phong](https://docs.microsoft.com/visualstudio/designers/how-to-create-a-basic-phong-shader)和物理基于 Blinn Phong 之间进行选择。  如果你想要有压缩反射高光，你将选择以物理方式基于 Blinn Phong。
