---
title: 合成光线
description: 合成照明 Api 可用于将动态 3D 照明添加到你的应用程序。
ms.date: 07/16/2018
ms.topic: article
...
Windows.UI.Composition Api 使你能够创建实时动画和效果。 合成光线使 2D 应用程序中的 3D 照明。 在此概述中，我们将运行通过如何设置合成光、 标识视觉对象以接收每个光源，并使用效果来定义内容的材料的功能。
...
- 若要启用沉浸式方案，如音乐播放场景的场景中的其他对象的光独立于转换。
- 能够配对具有光的对象，因此它们一起移动独立于场景中，从而启用[展示](/design/style/reveal.md)Fluent 突出显示之类的方案的其余部分。
...
当光面向视觉 （添加到[目标](/uwp/api/windows.ui.composition.compositionlight.targets)列表），视觉对象及其所有后代已注意到，对此光源响应。 这可能很简单作为根处树和下面的所有视觉 PointLight 源响应点光方向的动画设置。
...
**生命周期** | CompositionLight 可能会遇到生命周期条件 (示例： 垃圾回收器可能回收光对象，则在使用之前)。  我们建议通过将光照添加作为成员来帮助管理生命周期的应用程序保持对你灯的引用。
...
属性 | 描述
--- | ---
**颜色** | 光的[颜色](/uwp/api/windows.ui.color)。 照明通过[D3D](https://docs.microsoft.com/windows/uwp/graphics-concepts/light-properties)漫射、 Ambient，并定义正在发出的颜色的 Specular 定义的值的颜色。 照明的光; 使用了 RGBA 值不使用 alpha 颜色分量。
...
- 创建和放置光： 创建光，并将它们放置在指定的坐标空间。
- 标识对象与光： 面向在相关的视觉效果的光。
- [可选]定义如何单个对象对光作出响应： 使用 SceneLightingEffect 与 EffectBrush 自定义用于显示 SpriteVisual 反射光。 反射默认设置支持的光的光源的 CoordinateSpace 子元素。  可视对象与 SceneLightingEffect 绘制覆盖该视觉对象的默认光。
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect)经常用于材料创建。 SceneLightingEffect 是当你想要实现更复杂，如启用在图像上的反射属性和/或提供深度错觉与正常映射所使用的效果。 SceneLightingEffect 提供使用照明属性，例如高光和漫射金额自定义你的 UI 的功能。 你可以进一步自定义效果管道可以单独混合和撰写与你的内容不同的照明反应的其余部分的照明效果。

> [!NOTE]
> 场景照明不会产生阴影;它是专注于 2D 呈现效果。  它不会考虑 3D 照明包含的方案的真实照明模型，包括阴影到。


属性 | 描述
--- | ---
**正常映射** | NormalMaps 创建其中正常指向光线将显得更亮并正常指向立即将暗的纹理效果。 若要添加到目标 visual NormalMap 使用[CompositionSurfaceBrush](/uwp/api/Windows.UI.Composition.CompositionSurfaceBrush)使用 LoadedImageSurface 加载 NormalMap 资产。
**环绕** | 环境光属性主要用于控制整体颜色反射。
**反射** | 镜面反射创建对象，使它们看起来闪亮上突出显示。 你可以控制镜面反射程度以及闪光的级别。  These properties are manipulated to create material effects like shinny metals or glossy paper.
**漫射** | 漫射的反射散布在所有方向的光。
**反射率模型** | [反射率模型](/uwp/api/windows.ui.composition.effects.scenelightingeffectreflectancemodel)允许你[Blinn Phong](https://docs.microsoft.com/visualstudio/designers/how-to-create-a-basic-phong-shader)和物理基于 Blinn Phong 之间进行选择。  当你想要有压缩反射高光，你可以选择以物理方式基于 Blinn Phong。
