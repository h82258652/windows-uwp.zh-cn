...
Windows 运行时还支持 [{x:Null} 标记扩展](x-null-markup-extension.md)。 你可以使用它在 XAML 中将 [**Nullable**](T:System.Nullable%601) 值设置为 **null**。 例如，你可以将其用于 [**CheckBox**](https://msdn.microsoft.com/library/windows/apps/br209316) 的控制模板，这会将 **null** 解释为不确定的选中状态（从而引发“Indeterminate”视觉状态）。
