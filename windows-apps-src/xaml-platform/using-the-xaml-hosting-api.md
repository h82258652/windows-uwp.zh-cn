...
UWP XAML 托管 API 为奠定了广泛的我们提供了以使开发人员可以将 Fluent UI 带到非 UWP 桌面应用程序的控件。 这种情况有时称为*XAML 群岛*。 有关此开发人员方案的详细信息，请参阅[在桌面应用程序的 UWP 控件](xaml-host-controls.md)。

## <a name="is-the-uwp-xaml-hosting-api-right-for-your-desktop-application"></a>UWP XAML 托管 API 适合你的桌面应用程序？

UWP XAML 托管 API 提供托管 UWP 控件在桌面应用程序的低级别的基础结构。 某些类型的桌面应用程序可以选择使用备用、 更便利的 Api 来实现此目的。  

* 如果你有的 c + + Win32 桌面应用程序，并且你希望你的应用程序中托管 UWP 控件，则必须使用 UWP XAML 托管 API。 没有针对这些类型的应用程序的方法。
...
有 GitHub 上的多个示例演示了如何使用 UWP XAML 在 c + + Win32 应用程序中托管 API:
  ...
  * [XamlIslands32](https://github.com/clarkezone/cppwinrt/tree/master/Desktop/XamlIslandsWin32)。 此示例演示了如何将多个基本 UWP 控件添加到 c + + Win32 应用程序和处理 DPI 更改。
...
* 在创建**DesktopWindowXamlSource**时，此对象将自动创建本机子窗口来托管你的 UWP 控件。 此本机子窗口主要抽象你的代码，但你可以访问其句柄 (HWND) 如有必要。

* 最后，在顶层是要在桌面应用程序中托管 UWP 控件。 这可以是任何派生自[**Windows.UI.Xaml.UIElement**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement)，包括由 Windows SDK 以及自定义用户控件提供的任何 UWP 控件的 UWP 对象。
...
1. 你的应用程序创建的任何[**Windows.UI.Xaml.UIElement**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement)对象，它为将要托管在[**DesktopWindowXamlSource**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting.desktopwindowxamlsource)之前，请初始化的 UWP XAML 框架的当前线程。
    ...
    * 但是，如果你的应用程序创建**Windows.UI.Xaml.UIElement**对象创建将承载它们的**DesktopWindowXamlSource**对象之前，你的应用程序必须调用静态[**WindowsXamlManager.InitializeForCurrentThread**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting.windowsxamlmanager.initializeforcurrentthread)方法来显式初始化 UWP XAML 框架之前**Windows.UI.Xaml.UIElement**对象实例化。 你的应用程序通常应时应调用此方法进行实例化承载**DesktopWindowXamlSource**的父 UI 元素。
    ...
    > [!NOTE]
    > 此方法返回一个[**WindowsXamlManager**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting.windowsxamlmanager)对象包含对 UWP XAML 框架的引用。 你可以创建多个**WindowsXamlManager**对象根据需要在给定的线程上。 但是，由于每个对象包含到 UWP XAML 框架的引用，你应释放的对象，以确保将最终释放 XAML 资源。

2. 创建一个[**DesktopWindowXamlSource**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.hosting.desktopwindowxamlsource)对象并将其附加到在你的应用程序与窗口句柄相关联的父 UI 元素。
...
2. UWP XAML 控件的类型名称分配时调用[**GetXamlType**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.markup.ixamlmetadataprovider.getxamltype)方法根元数据提供程序 （这可能分配在代码中在运行时，或你可能会选择启用该选项可在 Visual Studio 属性窗口中分配）。
...
当用户更改父 UI 元素的大小时，你将需要处理任何必要的布局更改，以确保你的 UWP 控件按预期显示。 下面是要考虑的一些重要方案。

1. 当父 UI 元素需要获取适合你**DesktopWindowXamlSource**承载**Windows.UI.Xaml.UIElement**所需的矩形区域的大小时，调用**Windows.UI.Xaml.UIElement 的[**度量**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.measure)方法**. 例如：
...
2. 若要配置你的应用程序需要注意每个显示器的 DPI，向项目添加[程序集并排比较清单](https://docs.microsoft.com/windows/desktop/SbsCs/application-manifests)并将设置```<dpiAwareness>```元素中的```PerMonitorV2```。 有关此值的详细信息，请参阅[**DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2**](https://docs.microsoft.com/windows/desktop/hidpi/dpi-awareness-context)的描述。
    ...
    有关完整示例-并行集清单，请参阅 GitHub 上的[XamlIslands32](https://github.com/clarkezone/cppwinrt/tree/master/Desktop/XamlIslandsWin32)示例中的[XamlIslandsWin32.exe.manifest](https://github.com/clarkezone/cppwinrt/blob/master/Desktop/XamlIslandsWin32/XamlIslandsWin32.exe.manifest)文件。
