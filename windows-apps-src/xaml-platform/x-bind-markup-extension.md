...
ms.locfileid: "57658612"
---
# <a name="xbind-markup-extension"></a>{x:Bind} 标记扩展

**请注意**  有关如何使用在应用中使用数据绑定的常规信息 **{x： 绑定}** (和之间的全部比较 **{x： 绑定}** 和 **{Binding}**)，请参阅[深度中的数据绑定](https://msdn.microsoft.com/library/windows/apps/mt210946)。
...
如果数据源是字典或地图，则属性路径可以按字符串名称指定集合中的项。 例如**&lt;TextBlock 文本 ="{x： 绑定玩家\[为 John Smith 的\]"/&gt;** 将查找名为"John Smith"的字典中的项。 名称需要使用引号括起来，单引号或双引号都可以使用。 乘幂号 (^) 可用于转义字符串中的引号。 通常最简单的做法是替换使用用于 XAML 属性的引号。
...
**提示**  如果你需要指定单大括号的值，例如在[**路径**](https://msdn.microsoft.com/library/windows/apps/br209830)或者[ **ConverterParameter** ](https://msdn.microsoft.com/library/windows/apps/br209827)，它在前面加上一个反斜杠： `\{`。 此外，将包含需要转义的括号的整个字符串放在第二组引号中，例如 `ConverterParameter='{Mix}'`。
