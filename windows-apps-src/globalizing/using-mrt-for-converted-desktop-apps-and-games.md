...
description: "此白皮书介绍打包为 AppX 程序包时将 .NET 和 Win32 应用从旧的 Win32 资源迁移到 MRT 资源的技巧。"
ms.author: ptorr
ms.date: 03/09/2017
ms.topic: article
ms.prod: windows
...
---
# <a name="migrating-legacy-resources-to-the-windows-10-resource-management-platform"></a>将传统资源迁移到 Windows 10 资源管理平台

## <a name="executive-summary"></a>执行摘要

Win32 应用程序通常本地化为不同语言，从而扩展总目标市场<a name="footnote1_ref" href="#footnote1"><sup>1</sup></a>。 有多种方法可本地化传统的 Win32 应用程序，但 Windows 8 引入了[新资源管理系统](https://msdn.microsoft.com/en-us/library/windows/apps/jj552947.aspx)（本文中称为“MRT”<a name="footnote2_ref" href="#footnote2"><sup>2</sup></a>），它可以跨各种编程语言和应用程序类型进行工作，并提供超越简单本地化的功能。 

与基于 AppX 的部署（例如，来自 Windows 应用商店）相结合，MRT 可以自动为指定用户/设备提供最适用的资源，这可以最大程度地减小应用程序下载和安装的大小。 减小大小对于具有大量本地化内容的应用程序来说非常有意义，或许近似于 AAA 游戏的几*千兆字节*。 MRT 的其他好处包括 Windows Shell 和 Windows 应用商店的本地化列表，用户的首选语言与可用资源不匹配时的自动回退逻辑。
...
在许多情况下，你可以继续使用现有的本地化格式和源代码，同时与 MRT 集成以在运行时解析资源并最小化下载大小 - 这不是“全有或全无”的方法。 下表汇总了每个阶段的工作和估计成本/好处<a name="footnote3_ref" href="#footnote3"><sup>3</sup></a>：
...
<tr>
<td>本地化 AppX 清单</td>
<td>让本地化内容显示在 Windows Shell 和 Windows 应用商店中需要完成的最基本工作</td>
<td>小</td>
</tr>
<tr>
...
下面是一个在两个按钮（`openButton` 和 `saveButton`）上有文本标签并有用于徽标 (`logoImage`) 的 PNG 文件的应用程序示例。 文本标签被本地化为英语和德语，徽标针对正常桌面显示（100% 比例系数）和高分辨率手机（300% 比例系数）进行了优化<a name="footnote4_ref" href="#footnote4"><sup>4</sup></a>。 请注意，此图表呈现的是模型的高级概念视图；它不会准确映射到实现：
...
完成上述阶段 0-3 之后，你将有一个可以提交到 Windows 应用商店的应用程序“捆绑包”，它们还可以让用户通过忽略不需要的资源（例如，他们不使用的语言）最小化下载和安装大小。 可以采用一个最终步骤对应用程序的大小和功能进行进一步的改进。 
...
0. 在 XML 文件中，添加以下内容，其中<span style="background-color: yellow">突出显示文本</span>替换为你的应用的相应文本（使用你的默认语言<a name="footnote5_ref" href="#footnote5"><sup>5</sup></a>）
...
如果你直接编辑 XML，打开 `AppxManifest.xml` 文件，对<span style="background-color: lightgreen">突出显示值</span>进行以下更改 - 使用此*确切*文本，而不是特定于应用程序的文本<a name="footnote6_ref" href="#footnote6"><sup>6</sup></a>。 这些名称应与你在 `.resw` 文件中创建的 `Names` 匹配，带有前缀 `ms-resource:` 组合和 `Resources/` 命名空间。 
...
### <a name="step-12-build-pri-file-make-an-appx-package-and-verify-its-working"></a>步骤 1.2：生成 PRI 文件、进行 AppX 打包，确认是否能够正常使用
...
0. 键入以下命令，将“contoso_demo.xml”替换为适合你的项目的名称，并将“en-US”替换为你的应用的默认语言（或如果适用，保留为 en-US）。 请注意，xml 文件在父目录中创建（**不是**在项目目录中），因为它不是应用程序的一部分<a name="footnote7_ref" href="#footnote7"><sup>7</sup></a>：
...
作为一个反例，如果你的应用程序使用平面文件结构（具有包含文件 `english_strings.dll` 和 `french_strings.dll` 的单个 `Resources` 目录），它**不会**很好地映射到 MRT。 更好的结构是 `Resources`，有子目录和文件 `en\strings.dll` 和 `fr\strings.dll`
<a name="footnote8_ref" href="#footnote8"><sup>8</sup></a>。 请注意，即使你不能遵循此文件命名约定，也仍然有可能可以使用 MRT 和 AppX 打包的好处；只是需要更多工作。 未来的白皮书将介绍这种情况。 
...
每当 .NET 运行时无法找到资源 DLL 时，它都将引发 `AssemblyResolve` 事件，此时，所提供的事件处理程序将通过 MRT 找到所需文件，并返回程序集<a name="footnote9_ref" href="#footnote9"><sup>9</sup></a>。
...
<a name="footnote1" href="#footnote1_ref">1.</a> 本文不提供本地化本身的其他动机；本文假设读者已经了解此内容。

<a name="footnote2" href="#footnote2_ref">2.</a> 过去，这代表“现代资源技术”，但术语“现代”已停止使用。 资源管理器也可能被称为 MRM（现代资源管理器）或 PRI（数据包资源索引）。

<a name="footnote3" href="#footnote3_ref">3.</a> 此表不包含非本地化任务，如提供高分辨率或高对比度的应用程序图标。 有关提供多个磁贴、图标等资源的详细信息，请参阅 MSDN。

<a name="footnote4" href="#footnote4_ref">4.</a> 此处不包含具体的比例系数

<a name="footnote5" href="#footnote5_ref">5.</a> 请注意，这些字符串中有些有长度限制，请参阅 [MSDN 中的 **VisualElements** 主题](https://msdn.microsoft.com/en-us/library/windows/apps/br211471.aspx)了解详细信息 

<a name="footnote6" href="#footnote6_ref">6.</a> 对于使用这些具体的资源名称没有要求 - 你可以选择自己的名称 - 但不论你如何选择，所选名称都必须与 `.resw` 文件中的名称完全一致。

<a name="footnote7" href="#footnote7_ref">7.</a> 你可以选择需要的任何其他目录，但请务必在将来的命令中进行替换

<a name="footnote8" href="#footnote8_ref">8.</a> 也可以使用相同的基本文件名，但具有嵌入限定符，如 `strings.lang-en.dll` 和 `strings.lang-fr.dll`，不过使用具有语言代码的目录在概念上更简单，所以我们将更加关注。

<a name="footnote9" href="#footnote9_ref">9.</a> 请注意，如果你的应用已有用于其他用途的 `AssemblyResolve` 处理程序，你需要将资源解决代码与现有代码结合。
