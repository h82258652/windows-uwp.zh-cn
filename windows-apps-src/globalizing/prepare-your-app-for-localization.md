...
重复使用字符串看上去像是最佳解决方案，但如果同一字词或短语具有不同的含义或上下文，则会导致本地化问题。
...
将用于辅助功能的访问键与本地化的访问键的显示“同步”比较困难，因为这两个字符串资源被分类为两个单独的部分。 请务必为标签字符串（如 `Make sure that the emphasized shortcut key  is synchronized with the access key.`）提供注释
