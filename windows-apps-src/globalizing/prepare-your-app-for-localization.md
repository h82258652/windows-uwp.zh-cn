...
## <a name="use-resource-files-and-qualifiers"></a>使用资源文件和限定符。
...
## <a name="add-contextual-comments"></a>添加上下文注释。
...
## <a name="localize-sentences-instead-of-words"></a>本地化句子而不是字词。
...
## <a name="ensure-the-correct-parameter-order"></a>确保正确的参数顺序。
...
## <a name="do-not-use-the-same-strings-in-dissimilar-contexts"></a>不要在不同的上下文中使用相同的字符串。
...
## <a name="identify-resources-with-unique-attributes"></a>使用唯一属性标识资源。
...
## <a name="choose-an-appropriate-translation-approach"></a>选择相应的翻译方法。
...
## <a name="support-furigana-for-japanese-strings-that-can-be-sorted"></a>支持可存储的日语字符串的假名注音。
