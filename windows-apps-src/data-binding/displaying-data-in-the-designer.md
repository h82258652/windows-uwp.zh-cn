...
> [!NOTE]
> 本文中的屏幕截图取自 Visual Studio 的早期版本。 如果你使用的是 Visual Studio 2019，它们可能与你的开发体验不完全匹配。
...
第一个补救方法是尝试注释掉该 **DataContext** 分配，改为在页面标记中设置 **DataContext**。 这样，你的实时数据在设计时和运行时都会显示出来。 若要执行此操作，请首先打开你的 XAML 页面。 然后，在“文档大纲”窗口中，单击根可设计元素（通常带有标签 \[Page\]）来选择它 。 在“属性”窗口中，找到“DataContext”属性（在公共类别中），然后对其进行修改 。 从“选择对象”对话框中选择视图模型类型，然后单击“确定” 。
