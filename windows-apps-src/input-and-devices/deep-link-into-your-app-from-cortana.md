...
-   将应用激活为前台应用（请参阅 [Activate a foreground app with voice commands through Cortana（在 Cortana 中使用语音命令激活前台应用）](launch-a-foreground-app-with-voice-commands-in-cortana.md)）。
-   将特定功能公开为后台应用服务（请参阅 [Activate a background app with voice commands through Cortana（在 Cortana 中使用语音命令激活后台应用）](launch-a-background-app-with-voice-commands-in-cortana.md)）。
...
在 **AdventureWorks** 样例的 AdventureWorksVoiceCommandService.cs 示例中，我们将指定目标传递到检索所有匹配的行程并提供拒用应用深层链接的内容卡的 SendCompletionMessageForDestination 方法。
