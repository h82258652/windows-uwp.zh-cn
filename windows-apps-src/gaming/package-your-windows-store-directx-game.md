...
除了应用包模型，Windows 10 还支持应用程序捆绑包分为一组两种类型的包：
...
> **重要**  通过处理在加载和管理这些资源**Windows.ApplicationModel.Resources** \* Api。 如果你使用这些应用模型资源 API 来加载适用于区域设置、比例系数或 DirectX 功能级别的正确文件，你不需要使用显式文件路径来加载资源；相反，你可以只使用所需资源的一般化文件名来提供资源 API，并让资源管理系统获取适用于用户的当前平台和区域设置配置（同样可以使用相同的 API 直接指定）的资源的正确变体。

 

可以使用以下两种基本方式之一来指定用于资源打包的资源：

-   资产文件具有相同的文件名，并且资源包的特定版本均放置在特定的命名目录中。 系统将保留这些目录名称。 例如， \\en-我们\\缩放 140， \\dxfl dx11。
-   虽然资源文件采用任意名称存储在文件夹中，但这些文件名中都带有共同的标签，其中附加了系统保留的字符串以表示语言或其他限定符。 后下划线具体而言，对通用 filename 附限定符字符串 ("\_")。 例如，\\资产\\菜单上\_option1\_lang en us.png\\资产\\菜单\_option1\_规模-140.png，\\资产\\coolsign\_dxfl dx11.dds。 你也可以合并这些字符串。 例如，\\资产\\菜单上\_option1\_缩放 140\_lang en us.png。
    > **请注意**  语言限定符中使用时在文件名中而不是单独的目录名称，必须采用格式"lang-<tag>"，例如"lang-en-我们的"中所述[定制你的资源的语言，小数位数和其他限定符](../app-resources/tailor-resources-lang-scale-contrast.md)。

     

可以合并目录名称以在资源打包时实现额外特定性。 但是，它们不能是多余的。 例如， \\en-我们\\菜单上\_option1\_lang en us.png 是冗余的。

只要每个资源目录中的目录结构相同，你就可以在资源目录下指定所需的任何非保留的子目录名称。 例如， \\dxfl dx10\\资产\\纹理\\coolsign.dds。 当你加载或引用资源时，路径名必须一般化，从而删除任何用于语言、缩放或 DirectX 功能级别的限定符，无论它们位于文件夹节点还是位于文件名中。 例如，若要在代码中的变体的哪一个的资产，请参阅是\\dxfl dx10\\资产\\纹理\\coolsign.dds，使用\\资产\\纹理\\coolsign.dds。 同样，来指代的资产的一个变体\\映像\\背景\_规模 140.png，使用\\映像\\background.png。

下面是保留的目录名称和文件名下划线字符前缀：

| 资产类型                   | 资源包目录名称                                                                                                                  | 资源包文件名后缀                                                                                                    |
|------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| 本地化资源             | 所有可能的语言或语言和区域设置的组合，适用于 Windows 10。 （文件夹名称中不需要限定符前缀“lang-”。） | "\_"跟语言、 区域设置或语言区域设置说明符。 例如，"\_en"，"\_我们"，或"\_en-我们"分别。 |
| 比例系数资源        | scale-100、scale-140、scale-180。 它们分别用于 1.0x、1.4x 和 1.8x UI 比例系数。                                     | "\_"后接"缩放 100"、"缩放 140"或"缩放 180"。                                                                    |
| DirectX 功能级别资源 | dxfl-dx9、dxfl-dx10 和 dxfl-dx11。 它们分别用于 DirectX 9、10 和 11 功能级别。                                     | "\_"后接"dxfl dx9"、"dxfl-dx10"或"dxfl dx11"。                                                                     |
...
-   请确保每个资源或字符串资源文件 (.resw) 在每个目录中具有相同名称。 例如，菜单\_option1.png 应具有相同的名称，在这种\\en-我们和\\jp-jp 目录即使文件的内容适用于不同的语言。 在这种情况下，你会看到它们作为\\en-我们\\菜单上\_option1.png 和\\jp-jp\\菜单\_option1.png。
    > **请注意**   （可选） 可以将区域设置追加到文件名称，并将其存储在相同的目录; 例如，\\资产\\菜单\_option1\_lang en us.png、 \\资产\\菜单\_option1\_lang jp jp.png。
...
Windows 10 提供了三个用户界面缩放因素：1.0 x，1.4 x 和 1.8 x。 在安装期间基于许多组合因素为每个屏幕设置比例值： 屏幕大小、屏幕分辨率，以及假定的用户与屏幕的平均距离。 用户也可调整比例因子以提高可读性。 为了尽可能提供最佳体验，你的游戏应该可以感知 DPI 和比例因子。 此类感知的部分涵义是为三种比例系数分别创建重要可见资源的版本。 它还包含指针交互和点击测试！
...
-   请确保每个资源在每个目录中具有相同名称。 例如，菜单\_option1.png 应具有相同的名称，在这种\\缩放 100 和\\即使文件的内容是不同的缩放 180 目录。 在这种情况下，你会看到它们作为\\规模-100\\菜单上\_option1.png 和\\缩放 140\\菜单\_option1.png。
    > **请注意**  同样，可以根据需要将缩放因子后缀追加到文件名称，并将其存储在相同的目录; 例如，\\资产\\菜单\_option1\_缩放-100.png\\资产\\菜单上\_option1\_规模 140.png。
...
因为资源机制主要侧重于资源支持的纹理格式，所以它仅支持 3 种整体功能级别。 如果你需要具有 DX9 等的子级别 （圆点版本） 的单独着色器\_1 vs DX9\_3，你的资产管理和呈现代码必须处理这些显式。

在配置你的应用来支持用于不同的 DirectX 功能级别的资源包时，你应该：

-   为你要支持的每个 DirectX 功能级别（dxfl-dx9、dxfl-dx10 和 dxfl-dx11）创建应用子目录（或文件版本）。
-   在开发期间，将特定于功能级别的资源放置在各个功能级别资源目录中。 与区域设置和比例系数不同，你的游戏中的每个功能级别都可以具有不同的呈现代码分支，并且如果纹理、编译着色器或其他资源仅用于一个受支持的功能级别，或者仅用于所有受支持的功能级别的子集，则仅将相应资源放置在使用它们的功能级别的目录中。 对于在所有功能级别上加载的资源，请确保每个功能级别资源目录中都包含其具有相同名称的版本。 例如，为功能级别独立纹理名为"coolsign.dds"，将中的 BC3 压缩版本\\dxfl dx9 directory 和中的 BC7 压缩版本\\dxfl dx11 目录。
-   确保各个目录中的每个资源（如果它可用于多种功能级别）都具有相同的名称。 例如，coolsign.dds 应具有相同的名称，在这种\\dxfl dx9 和\\dxfl dx11 即使文件的内容是不同的目录。 在这种情况下，你会看到它们作为\\dxfl dx9\\coolsign.dds 和\\dxfl dx11\\coolsign.dds。
    > **请注意**  同样，可以根据需要将功能级别后缀追加到文件名称，并将其存储在相同的目录; 例如，\\纹理\\coolsign\_dxfl-dx9.dds \\纹理\\coolsign\_dxfl dx11.dds。
...
-   在 [**Windows.ApplicationModel.Resources.Core**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Resources.Core) 中使用 API 来加载资源。 资源引用应该一般化（无后缀），且不带功能级别。 然而，与语言和规模不同，系统不会自动确定哪个功能级别最适合给定屏幕；你需要根据代码逻辑自行确定。 确定后，请使用 API 通知首选功能级别的操作系统。 然后，系统可以根据该首选项检索正确资源。 下面的代码示例显示了如何将当前平台的 DirectX 功能级别通知你的应用：
    ...
    > **请注意**  在代码中，纹理加载到直接按名称 （或功能级别目录的路径）。 不要包含功能级别目录名称或后缀。 例如，加载"纹理\\coolsign.dds"，而非"dxfl dx11\\纹理\\coolsign.dds"或"纹理\\coolsign\_dxfl dx11.dds"。
