...
* 在此类中定义常量缓冲区是为了保留呈现所需的各种数据。
    ...
    * 在此游戏示例中，定义了 4 个常量缓冲区。
        ...
        2. __m\_constantBufferChangeOnResize__包含投影矩阵。 投影矩阵依赖于窗口的大小和纵横比。 它在 [__CreateWindowSizeDependentResources__](#createwindowsizedependentresource-method) 方法中设置一次，然后在使用 [__FinalizeCreateGameDeviceResources__](#finalizecreategamedeviceresources-method) 方法加载资源后进行更新。 如果在 3D 中呈现，也会每帧更改两次。
        3. __m\_constantBufferChangesEveryFrame__包含视图矩阵。 此矩阵依赖于相机位置和观看方向（投影的法线），并且使用 __Render__ 方法每帧仅更改一次。 之前在__呈现框架 I：呈现简介__中的 [__GameRenderer::Render__方法](tutorial--assembling-the-rendering-pipeline.md#gamerendererrender-method)中进行了讨论。
        4. __m\_constantBufferChangesEveryPrim__包含每个基元的模型矩阵和材料属性。 模型矩阵将顶点从本地坐标转换为世界坐标。 这些常量特定于每个基元并在每次绘图调用时更新。 之前在__呈现框架 I：呈现简介__中的[基元呈现](tutorial--assembling-the-rendering-pipeline.md#primitive-rendering)下进行了讨论。
...
* 在此类中定义的着色器对象用于计算基元和纹理。 
    * 在此游戏示例中，着色器对象为__m\_vertexShader__、 __m\_vertexShaderFlat__和__m\_pixelShader__， __m\_pixelShaderFlat__。
...
* 实例化变量（__m\_gameResourcesLoaded__ = false 和__m\_levelResourcesLoaded__ = false），用于指示在向前转换到呈现之前是否已加载资源，因为我们要以异步方式加载资源。 
...
__CreateGameDeviceResourcesAsync__是从__create\_任务__循环中的__GameMain__构造函数方法调用的，因为我们是以异步方式加载游戏资源。
...
此方法用于：
* 创建4个[常量缓冲区](tutorial--assembling-the-rendering-pipeline.md#buffer)： __m\_constantBufferNeverChanges__， __m\_constantBufferChangeOnResize__， __m\_constantBufferChangesEveryFrame__， __m\_constantBufferChangesEveryPrim__
...
* 将__m\_gameResourcesLoaded__ __布尔__全局变量设置为指示资源现在已加载到缓冲区中，以供下一步。 回想一下，我们首先通过 __GameRenderer::CreateDeviceDependentResources__ 方法在 __GameRenderer__ 的构造函数方法中将此变量初始化为 __FALSE__。 
* 如果此__m\_gameResourcesLoaded__为__TRUE__，则可能会呈现场景对象。 在__呈现框架 I：呈现简介__文章的 [__GameRenderer::Render__](tutorial--assembling-the-rendering-pipeline.md#gamerendererrender-method) 方法部分对此进行了介绍。
