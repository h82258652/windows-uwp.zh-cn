...
description: 本文档介绍大理石迷宫项目; 的基本特征例如，它如何在 Windows 运行时环境中使用 Visual c + +、 如何创建和结构化，它和生成的方式。
...
-   使用[SAL 注释](https://docs.microsoft.com/visualstudio/code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects)以及代码分析工具有助于发现您的应用程序中的错误。

## <a name="creating-the-visual-studio-project"></a>创建 Visual Studio 项目


如果您已下载并提取示例，您可以在 Visual Studio 中打开的**MarbleMaze_VS2017.sln**文件 （在**c + +** 文件夹中），并且您将拥有之前您的代码。
...
1. 在 Visual Studio 2017，选择**文件 > 新建 > 项目...**

2. 在**新建项目**窗口中左侧边栏中，选择**Installed > 模板 > Visual c + +**。

3. 在中间的列表中，选择**DirectX 11 应用程序 (通用 Windows)**。 如果您看不到此选项，您可能没有安装所需的组件&mdash;有关如何安装其他组件的信息，请参阅[通过添加或删除工作负荷和组件的修改 Visual Studio 2017](https://docs.microsoft.com/visualstudio/install/modify-visual-studio) 。

4. 为项目提供**名称**、**位置**文件要存储和一个**解决方案名称**，然后单击**确定**。
...
从 Microsoft 存储获取每个 UWP 应用程序的应用程序包的形式出现。 应用包中包含一个程序包清单，后者包含有关应用的信息。 例如，你可指定应用的功能（如需要的访问受保护的系统资源或用户数据的能力）。 如果你确定应用需要某些功能，可使用程序包清单来声明所需的功能。 清单还允许指定项目属性，例如支持的设备旋转方向、磁贴图像和初始屏幕。 你可以打开项目中的 **Package.appxmanifest** 来编辑清单。 有关应用包的详细信息，请参阅[打包应用](https://msdn.microsoft.com/library/windows/apps/mt270969)。
...
您可以使用触摸、 加速感应器、 Xbox 一个控制器或控制大理石迷宫鼠标。

-   使用控制器上的方向板更改活动菜单项。
-   使用触摸屏，A 或开始控制器或鼠标选取一个菜单项上的按钮。
-   使用触摸、加速计、左操纵杆或鼠标来倾斜迷宫。
-   使用触摸屏，A 或开始控制器或鼠标关闭如高的菜单上的按钮分数表。
-   使用开始按钮上的控制器或键盘上的 P 键暂停或继续游戏。
-   使用控制器上的“后退”按钮或键盘上的 Home 键重新启动游戏。
-   高分数表可见时，使用后退按钮上的控制器或键盘上的主页键清除所有分数。

##  <a name="code-conventions"></a>代码转换


Windows 运行时是可用于创建仅在特殊应用程序环境中运行的 UWP 应用的编程接口。 此类应用程序使用授权的函数、 数据类型和设备，并从 Microsoft 存储分布式。 在最低级别上，Windows 运行时由一个应用程序二进制接口 (ABI) 组成。 ABI 是一个低级二进制合约，它使得 Windows 运行时 API 能够访问多种编程语言，例如 JavaScript、.NET 语言和 Visual C++。
...
Marble Maze 结合使用 **^** 和 **Microsoft::WRL::ComPtr** 来管理堆分配的对象并且最大程度减少内存泄露。 我们建议您使用 ^ 管理的 Windows Runtime 变量的生存期**ComPtr**管理 （例如，当您使用 DirectX），COM 变量和**std::shared\_ptr**或**std::unique\_ptr**管理所有其他的生存期的生存期堆分配 c + + 对象。
...
-   当调用返回 **HRESULT** 的 DirectX 例程时，请使用 **DX::ThrowIfFailed** 函数。 此函数中[DirectXHelper.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/master/C%2B%2B/Shared/DirectXHelper.h)定义。 如果提供的**HRESULT**错误代码， **ThrowIfFailed**将引发异常。 例如，**E\_POINTER** 会导致 **ThrowIfFailed** 引发 [Platform::NullReferenceException](https://msdn.microsoft.com/library/windows/apps/hh755823.aspx)。
...
-   虽然我们建议您避免使用**HRESULT**意外错误，很多重要避免使用异常处理以控制流的代码。 因此，最好在必要时使用 **HRESULT** 返回值来控制代码流。
...
请考虑**BasicLoader::LoadMesh**方法，该声明中[BasicLoader.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/e62d68a85499e208d591d2caefbd9df62af86809/C%2B%2B/Shared/BasicLoader.h)方法。 此方法使用`_In_`指定的*文件名*为输入的参数 （并因此只能被从读取），`_Out_`可以指定*vertexBuffer*和*indexBuffer*输出参数 （并因此将只能写入），和`_Out_opt_`指定*vertexCount*和*indexCount*是可选输出参数 （和可能写入）。 因为 *vertexCount* 和 *indexCount* 是可选的输出参数，所以允许它们使用 **nullptr**。 C/C++ 代码分析工具检查对此方法的调用，以确保它传递的参数满足这些条件。
...
若要对您的应用程序的菜单栏上，执行代码分析选择**构建 > 解决方案上运行代码分析**。 有关代码分析的详细信息，请参阅[使用代码分析分析 C/C++ 代码质量](https://docs.microsoft.com/visualstudio/code-quality/analyzing-c-cpp-code-quality-by-using-code-analysis)。
