...
-   调用 glBindBuffer 定义布局的缓冲区，例如 GL\_元素\_数组\_缓冲区。
...
设置一个统一的通常涉及到提供某个 glUniform\*具有指向应用内存中的数据的指针以及 GPU 中的上传位置的方法。 在查看器 glUniform\*方法执行、 统一的数据位于 GPU 内存并通过着色器已声明的统一的可访问。 你应该确保采用这样的方式来打包数据，以便着色器可以基于着色器中的 uniform 声明解释该数据（使用兼容的类型）。
...
由于着色器管道可以修改顶点数据，因此，OpenGL ES 2.0 要求你将数据指定为“attribute”而不是“uniform”。 （这已更改的 OpenGL 和 GLSL 更高版本中。）此类的顶点位置的特定于顶点的数据、 normals、 切线和颜色值作为属性值提供给着色器。 这些属性值与顶点数据中每个元素的特定偏移相对应；例如，第一个属性可以指向单个顶点的位置组件，第二个属性可以指向法线等。
...
OpenGL ES 2.0:将顶点缓冲区数据上传到着色器属性
...
通过声明顶点数据元素的格式以及用于每个组件的语义来声明输入布局并将其与顶点着色器关联。 顶点元素数据布局 D3D11 中所述\_输入\_元素\_DESC 创建必须对应于相应的结构的布局。 你在此处为包含两个组件的顶点数据创建一个布局：
...
Direct3D 11:声明具有 HLSL 语义着色器输入的数据
