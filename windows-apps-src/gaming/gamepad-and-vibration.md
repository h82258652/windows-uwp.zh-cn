...
理想状态下，中心位置闲置的操纵杆每次都会在 X 和 Y 轴生成相同的中性读数。 但是，由于机械作用力以及操纵杆的敏感性，中心位置的实际读数只能近似于理想的中性值，在后续的读数之间可能会有所不同。 出于此原因，你必须始终使用小的_死区_&mdash;一系列被忽略的接近理想中心位置值&mdash;来弥补制造差异、 机械磨损或者其他游戏板问题。
...
作为 UI 导航控制器，游戏板与左的操纵杆、 方向键、**视图**、**菜单**、 **A**、 **B**按钮映射导航命令[所需的设置](ui-navigation-controller.md#required-set)。
...
[Gamepad][] 类会提供一个静态属性 [Gamepads][]，该属性是当前已连接游戏板的只读列表。 因为你可能只对某些连接的游戏板感兴趣，建议你保留自己的集合，而来进行通过访问`Gamepads`属性。

下面是将所有已连接游戏板复制到一个新集合的示例。 请注意，因为将会在后台中的其他线程访问此集合 （在[GamepadAdded][]和[GamepadRemoved][]事件），你需要放置锁定任何读取或更新集合的代码。
...
下面的示例停止跟踪已删除游戏板。 你还需要处理要跟踪它们正在删除; 时的游戏板会发生什么情况例如，此代码仅跟踪来自一个游戏板输入，并且只需将其设置为`nullptr`时删除。 你将需要检查每个帧，如果你的游戏板处于活动状态，以及哪些游戏板你正在从输入时收集控制器的连接和断开连接的更新。
...
有关详细信息，请参阅[游戏输入实践](input-practices-for-games.md)。
...
每个操纵杆会在 X 和 Y 轴提供一个介于 -1.0 和 +1.0 之间的模拟读数。 在 X 轴，值 -1.0 对应于操纵杆最左位置；值 +1.0 对应于最右的位置。 在 Y 轴，值 -1.0 对应于操纵杆最下面的位置；值 +1.0 对应于最上面的位置。 在这两个轴，值大约是 0.0，当摇杆处于中心位置，但正常的精确值有所不同，即使是在后续读数; 之间在本部分后面部分讨论减小此误差的策略。
...
有时你可能想要确定从一个按钮的过渡时按下到释放或释放到按下，是按下还是释放多个按钮，或者是否按特定方式安排一组按钮，是&mdash;按下，释放一些。 有关如何检测这些条件的详细信息，请参阅 [检测按钮转换](input-practices-for-games.md#detecting-button-transitions) 和 [检测复杂按钮安排](input-practices-for-games.md#detecting-complex-button-arrangements)。
...
切记，这两个电机不是完全相同的，所以将这些属性设置为相同的值并不会在一个电机中生成与另一个电机中相同的振动。 对于任何值，左的电机会以较低的频率比右电动机的更强振动&mdash;对于相同的值&mdash;生成更轻柔的振动更高的频率。 即使是最大值，左电机也无法生成右电机的高频率，右电机也无法生成左电机的高动力。 因为电机通过游戏板刚性连接，所以即使电机具有不同的特性并且能够以不同的强度振动，游戏玩家仍然不能完全独立地体验振动。 相比完全相同的电机，这种布置可以产生更大范围、更丰富的感觉。
