---
title: 添加控件
description: 现在，我们来看看示例游戏如何实现3-d 游戏中的移动外观控制，以及如何开发基本的触摸、鼠标和游戏控制器控件。
...
[**CoreWindow：:P ointerReleased**](/uwp/api/windows.ui.core.corewindow.pointerreleased) |释放鼠标左按钮，或者移开与触摸表面接触的物体。
[**CoreWindow::PointerExited**](/uwp/api/windows.ui.core.corewindow.pointerexited) |指针移出主窗口。
[**Windows::Devices::Input::MouseMoved**](/uwp/api/windows.devices.input.mousedevice.mousemoved) | 鼠标移动了一定距离。 注意，我们只关注鼠标移动增量值，不关注当前 X-Y 位置。
...
当收到 [**PointerPressed**](/uwp/api/windows.ui.core.corewindow.pointerpressed) 事件时，**MoveLookController** 将获取窗口创建的指针 ID 值。 指针 ID 表示特定的输入类型。 例如，在多点触控设备上，可能同时有若干不同的主动输入。 ID 用于跟踪玩家在使用哪个输入。 如果触摸屏的移动矩形中有一个事件，将指派一个指针 ID 跟踪移动矩形中的任何指针事件。 射击矩形中的其他指针事件则使用单独的指针 ID 进行单独跟踪。
...
为此，我们通过检查由 [**MouseMoved**](/uwp/api/windows.devices.input.mousedevice.mousemoved) 事件返回的 [**Windows::Device::Input::MouseEventArgs::MouseDelta**](/uwp/api/windows.devices.input.mouseeventargs.mousedelta) 参数对象上的 [**MouseDelta::X**](/uwp/api/Windows.Devices.Input.MouseDelta) 和 **MouseDelta::Y** 字段获取 X（水平运动）和 Y（垂直运动）坐标的变化。
...
-   如果在移动或射击矩形中发生了 [**PointerMoved**](/uwp/api/windows.ui.core.corewindow.pointermoved) 事件，则更新控制器的指针位置。
-   如果在屏幕的其他位置（定义为观看控制区）发生了 [**PointerMoved**](/uwp/api/windows.ui.core.corewindow.pointermoved) 事件，则计算观看方向矢量的俯仰和偏航变化。
...
为了使用键盘，示例游戏在[**MoveLookController：： InitWindow**](https://github.com/Microsoft/Windows-universal-samples/blob/ef073ed8a2007d113af1d88eddace479e3bf0e07/SharedContent/cpp/GameContent/MoveLookController.cpp#L84-L88)方法中注册了两个新的事件： [**CoreWindow：： KeyUp**](/uwp/api/windows.ui.core.corewindow.keyup)和[**CoreWindow：： KeyDown**](/uwp/api/windows.ui.core.corewindow.keydown)。 这些事件处理键的按下和释放。
...
在此方法中，我们将检查是否有与该枚举一起使用的指针设备类型 [`Windows::Devices::Input::PointerDeviceType`](/uwp/api/Windows.Devices.Input.PointerDeviceType) 。 如果游戏处于 **Active** 状态且 **PointerDeviceType** 不是 **Touch**，我们假设为鼠标输入。
...
对于此游戏，游戏板支持通过调用 [Windows.Gaming.Input](/uwp/api/windows.gaming.input) API 添加。 这组 API 提供对游戏控制器输入（如赛车方向盘和飞行杆）的访问。 
...
**MoveLookController** 实例的 [**UpdatePollingDevices**](https://github.com/Microsoft/Windows-universal-samples/blob/ef073ed8a2007d113af1d88eddace479e3bf0e07/SharedContent/cpp/GameContent/MoveLookController.cpp#L654-L782) 方法立即检查游戏板是否已连接。 如果已连接，我们将开始通过 [**Gamepad.GetCurrentReading**](/uwp/api/windows.gaming.input.gamepad.GetCurrentReading) 读取它的状态。 这将返回 [**GamepadReading**](/uwp/api/Windows.Gaming.Input.GamepadReading) 结构，让我们可以检查点击了哪些按钮或移动了什么操纵杆。
...
- 如果按下任何移动键盘输入，则 `1.0f` `-1.0f` 会在**m_moveCommand**向量的相应分量中添加或的值，以便 &mdash; `1.0f` 向前和 `-1.0f` 向后。
