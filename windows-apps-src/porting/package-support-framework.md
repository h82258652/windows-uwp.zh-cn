...
首先，创建用于你的应用程序的程序包。 然后，将它安装、 运行它，并观察其行为。 你可能收到错误消息，以帮助你识别兼容性问题。 你还可以使用[进程监视器](https://docs.microsoft.com/en-us/sysinternals/downloads/procmon)来识别问题。  常见的问题与应用程序工作目录和计划路径访问权限有关的假设。
...
如果你怀疑文件系统访问失败，搜索失败是在 System32/SysWOW64 或包文件路径下的事件。 筛选器还可帮助在这里，太。 在此列表的底部的开始菜单和向上滚动。 最近发生故障的显示在此列表的底部。 大多数重视包含诸如拒绝访问，"的字符串的错误和"未找到路径/名称"，并忽略看起来不可疑的操作。 [PSFSample](https://github.com/Microsoft/MSIX-PackageSupportFramework/blob/master/samples/PSFSample/)具有两个问题。 你可以看到在下图中出现的列表中的这些问题。
...
在此问题，应用程序无法.log 文件写入其程序包路径。 这会建议文件重定向修正可能帮助。
...
从 Windows SDK 中，并按照以下步骤，你可以应用一些简单的工具使用现有的运行时修复。
...
如果你已经有.msix （或.appx） 文件，你可以将其内容解包到布局文件夹，将用作你的程序包暂存区域。  你可以完成此操作**x64 适用于 VS 2017 的本机工具命令提示**，或手动使用的可执行的搜索路径中的 SDK bin 路径。
...
将所需的 32 位和 64 位 PSF Dll 和可执行文件添加到包目录中。 使用下表作为指南。 你还需要包含所需的任何运行时修补程序。 在我们的示例，我们需要文件重定向运行时修复。
...
默认情况下，跟踪修正的输出获取发送到附加调试程序。 对于此示例中，我们不打算连接调试器，并将改为使用[DebugView](https://docs.microsoft.com/en-us/sysinternals/downloads/debugview)计划从 SysInternals 以查看其输出。 运行应用之后, 我们可以看到相同的故障以前一样，这将为我们指出相同的运行时修复。
...
如果你尚未获得一个解决方案桌面应用程序，请在 Visual Studio 中创建新的**空白解决方案**。
...
打开项目，并在**常规**设置页的属性页将**目标名称**属性设置为``PSFLauncher32``或``PSFLauncher64``具体取决于你的应用程序的体系结构。

![PSF 启动程序参考](images/desktop-to-uwp/shim-exe-reference.png)
...
一旦你已设置此，你可以的桌面应用程序代码和运行时修复项目中设置断点旁边的代码行。 如果你没有源代码你的应用程序，你将能够在运行时修复项目中设置断点仅旁边的代码行。
...
声明``FIXUP_DEFINE_EXPORTS``宏，然后添加包含语句`fixup_framework.h`在每个顶部。想要添加的运行时修复函数 CPP 文件。
...
例如，你可能会产生的替换函数`CreateFile`函数。 你的实现可能会调用`CopyFile`函数，但的实现`CopyFile`函数可能会调用`CreateFile`函数。 这可能导致无限递归周期对的调用`CreateFile`函数。
