...
|参数 |与动词命令关联的实参参数和值的列表。 如果应用程序是完全信任打包应用程序，这些参数传递给应用程序作为事件参数时的应用程序激活。 可以自定义基于不同的激活谓词应用程序的行为。 如果变量可包含文件路径，请用引号将参数值括起来。 这将避免路径包含空格的情况下出现的任何问题。 如果你的应用程序是 UWP 应用，你无法将参数传递。 应用转而接收参数（请参阅上一项）。|
...
|名称 |协议的名称。 |
|参数 |参数和值的应用程序激活时要传递给应用程序作为事件参数的列表。 如果变量可包含文件路径，请用引号将参数值括起来。 这将避免路径包含空格的情况下出现的任何问题。 |
