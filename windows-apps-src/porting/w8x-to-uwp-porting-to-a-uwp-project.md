...
单击 "**项目** &gt; " "**添加引用** &gt; " " **Windows 通用** &gt; **扩展**"，然后选择相应的扩展 SDK。 例如，如果要调用的 API 仅在移动设备系列中可用，且它们已在版本 10.0.x.y 中引入，请选择 **“适用于 UWP 的 Windows 移动版扩展”** 。
...
**注意如果要**在一个代码文件中支持 Windows 8.1、Windows Phone 8.1 和 Windows 10，则还可以执行此操作。   如果你在 "项目属性" 页上的 "windows 10" 项目中查看，你会看到项目\_将 Windows UAP 定义为条件编译符号。 因此，您可以将它与 windows\_应用程序和 windows\_PHONE\_应用程序结合使用。 这些示例显示了从通用8.1 应用删除条件编译并替换 Windows 10 应用的等效代码的更简单的情况。
