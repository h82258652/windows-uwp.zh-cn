...
1.  启动 Microsoft Visual Studio 2015 并创建新的空白应用程序 （Windows 通用） 项目。 有关详细信息，请参阅[快速开始你的 Windows 运行时 8.x 应用中使用模板 (C#，c + +、 Visual Basic)](https://msdn.microsoft.com/library/windows/apps/hh768232)。 新建项目会构建一个可在所有设备系统上运行的应用包（appx 文件）。
...
-   有关在二进制级别（而不是源代码级别）上重复使用的信息，请参阅[使用 C# 和 Visual Basic 创建 Windows 运行时组件](https://msdn.microsoft.com/library/windows/apps/xaml/br230301.aspx)。 此外，还有可移植类库，它支持面向 Windows 8.1、 Windows Phone 8.1 和 Windows 10 应用 (.NET Core) 的.NET Framework 和完整的.NET Framework 中可用的.NET Api 的子集。 可移植类库程序集为二进制形式，可与所有这些平台兼容。 使用 Visual Studio 创建一个面向可移植类库的项目。 请参阅[使用可移植类库的跨平台开发](https://msdn.microsoft.com/library/gg597391.aspx)。
...
Windows 10 上收敛[ **PickSingleFileAsync** ](https://msdn.microsoft.com/library/windows/apps/jj635275) API，因此你的代码简化了与此：
...
Windows 10 中的后退按钮事件是一个通用的概念。 已在硬件或软件中实现的后退按钮都将引发 [**BackRequested**](https://msdn.microsoft.com/library/windows/apps/dn893596) 事件（即，为要处理的事件）。
...
* [开发适用于通用 Windows 平台应用](https://msdn.microsoft.com/library/dn975273.aspx)
* [快速开始你的 Windows 运行时 8.x 应用中使用模板 (C#，c + +、 Visual Basic)](https://msdn.microsoft.com/library/windows/apps/hh768232)
* [创建 Windows 运行时组件](https://msdn.microsoft.com/library/windows/apps/xaml/hh441572.aspx)
* [使用可移植类库的跨平台开发](https://msdn.microsoft.com/library/gg597391.aspx)
