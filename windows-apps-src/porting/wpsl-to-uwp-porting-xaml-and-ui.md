...
有时只需修复少量的强制性代码，例如更改参数类型。 其他情况下，您需要使用 UWP Api，而不是 Windows Runtime.NET Api 8.x 应用程序。 若要确定支持哪些 Api，结合使用[的 Windows.NET 运行时 8.x 应用程序概述 （英文）](https://msdn.microsoft.com/library/windows/apps/xaml/br230302.aspx)和[Windows 运行库参考 （英文）](https://msdn.microsoft.com/library/windows/apps/br211377)中使用本移植指南的其余部分。
...
| Panorama | Windows Phone 的 Silverlight 全景控件映射到[Windows Runtime 8.x 应用程序中的集线器控件的指南](https://msdn.microsoft.com/library/windows/apps/dn449149)和准则集线器控件。 <br/> 请注意，Panorama 控件从最后一部分环绕到第一部分，并且其背景图像相对于具体部分在视差中移动。 [Hub](https://msdn.microsoft.com/library/windows/apps/dn251843) 部分不会环绕，并且不使用视差。 |
...
较常规的方法（适用于位图的浅色和深色主题之间有差异的情况）是使用两个图像资源：一个带有深色前景（用于浅色主题），而另一个带有浅色前景（用于深色主题）。 有关如何命名这套位图资产的详细信息，请参阅[定制您的语言、 可伸缩性和其他限定符的资源](../app-resources/tailor-resources-lang-scale-contrast.md)。 为一组图像文件正确命名后，你可以使用它们的根名称在摘要中引用它们，如下所示：
