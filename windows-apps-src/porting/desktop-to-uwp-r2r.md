...
我们已发布的本机映像编译器的预览版本作为[NuGet 程序包](https://www.nuget.org/packages/Microsoft.DotNet.Framework.NativeImageCompiler)。 你可以将此包应用到任何针对.NET Framework 版本 4.6.2 的.NET Framework 应用或更高版本。 此包将添加 post 生成步骤，包括对你的应用程序使用的所有二进制文件的本机负载。 应用程序运行在.NET 4.7.2 和更高版本上一版本将仍可以加载的 MSIL 代码时，将加载此优化的负载。
...
> [!IMPORTANT]
> 如果你想要生成 Windows 应用程序打包项目打包你的应用程序的本机映像，请确保将该项目的目标平台最小版本设置为 Windows 周年更新。
...
若要配置你的项目面向.net 4.6.2 你将需要在.net 4.6.2 开发工具或更高版本。 这些工具已通过 Visual Studio 安装程序作为下的.NET 桌面开发工作负荷的可选组件：
...
如果你的解决方案中有多个项目，仅入口点项目 （最有可能生成可执行文件的项目） 必须进行编译为 x86 或 x64。 从主项目引用的其他二进制文件将会处理与在主项目中，指定的体系结构，即使在编译为 AnyCPU。
...
本机映像编译器 NuGet 包，你需要添加到 Visual Studio 项目生成的可执行文件的形式提供。 这通常是你的 Windows 窗体或 WPF 项目。 使用此 PowerShell 命令执行该操作。
...
> [!NOTE]
> 预览包的发布 NuGet.org 作为列出。 通过浏览 NuGet.org 或使用 Visual Studio 中的程序包管理器用户界面，将不会找到它们。 但是，从程序包管理器控制台，安装它们，当你从另一台计算机还原。 当我们发布的第一个非预览版本，我们会将程序包完全进行访问。

## <a name="create-a-release-build"></a>创建一个发行版本

NuGet 包配置要运行另一种工具的发布版本的项目。 此工具将本机代码添加到相同的二进制文件。
...
A. 通过在 Visual Studio 2017 的反馈工具报告的问题。 [详细信息](https://docs.microsoft.com/visualstudio/ide/how-to-report-a-problem-with-visual-studio-2017)。

**问:。 什么是添加到现有的二进制文件的本机映像的影响？**
