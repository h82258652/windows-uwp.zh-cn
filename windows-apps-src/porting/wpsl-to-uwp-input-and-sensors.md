...
title: I/O、 设备和应用模型移植到 UWP 的 Windows Phone Silverlight
...
ms.locfileid: "66372178"
---
#  <a name="porting-windowsphone-silverlight-to-uwp-for-io-device-and-app-model"></a>移植到 UWP 的 Windows Phone Silverlight 的 I/O、 设备和应用程序模型
...
在 Windows Phone Silverlight 应用包含用于保存和还原其应用程序状态和其视图状态，以支持正在被逻辑删除并随后重新激活代码。 通用 Windows 平台 (UWP) 应用的应用程序生命周期具有强 parallels 与 Windows Phone Silverlight 应用程序，因为它们都设计最大程度地提供的资源相同的目标是任何应用到用户已选择在在任何时间点的前景色。 你会发现你的代码可以相当轻松地适应新系统。

**请注意**  按硬件**回**按钮将自动终止一个 Windows Phone Silverlight 应用程序。 按下移动设备上的硬件 **“后退”** 按钮*不会*自动终止 UWP 应用。 但是，它将处于暂停状态，然后可能会被终止。 但是，对于对应用程序生命周期事件做出相应响应的应用，这些详细信息是透明的。
...
**请注意**  我们建议你不使用操作系统或设备系列功能的状态进行检测。 通常情况下，标识当前操作系统或设备系列并不是确定是否存在特定的操作系统或设备系列功能的最佳方式。 与其检测操作系统或设备系列（和版本号），不如自行测试功能是否存在（请参阅[条件编译和自适应代码](wpsl-to-uwp-porting-to-a-uwp-project.md)）。 如果你必须请求某个特定操作系统或设备系列，请确保将其用作受支持的最低版本，而不是针对某一版本设计相应测试。
...
Windows Phone Silverlight 应用程序可以使用**Microsoft.Phone.Info.DeviceStatus**类，以获取有关其运行应用的设备的信息。 尽管 **Microsoft.Phone.Info** 命名空间没有直接的 UWP 等效项，但下面提供了一些可在 UWP 应用中使用的属性和事件，从而无需调用 **DeviceStatus** 类的成员。

| Windows Phone Silverlight                                                               | UWP                                                                                                                                                                                                                                                                                                                                |
|-----------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **ApplicationCurrentMemoryUsage** 和 **ApplicationCurrentMemoryUsageLimit** 属性 | [**MemoryManager.AppMemoryUsage** ](https://docs.microsoft.com/uwp/api/windows.system.memorymanager.appmemoryusage)并[ **AppMemoryUsageLimit** ](https://docs.microsoft.com/uwp/api/windows.system.memorymanager.appmemoryusagelimit)属性                                                                                                                                    |
...
| **DeviceFirmwareVersion** 属性                                                      | [**EasClientDeviceInformation.SystemFirmwareVersion** ](https://docs.microsoft.com/uwp/api/windows.security.exchangeactivesyncprovisioning.easclientdeviceinformation.systemfirmwareversion)属性 （仅桌面版设备系列）                                                                                                                                                                             |
| **DeviceHardwareVersion** 属性                                                      | [**EasClientDeviceInformation.SystemHardwareVersion** ](https://docs.microsoft.com/uwp/api/windows.security.exchangeactivesyncprovisioning.easclientdeviceinformation.systemhardwareversion)属性 （仅桌面版设备系列）                                                                                                                                                                             |
| **DeviceManufacturer** 属性                                                         | [**EasClientDeviceInformation.SystemManufacturer** ](https://docs.microsoft.com/uwp/api/windows.security.exchangeactivesyncprovisioning.easclientdeviceinformation.systemmanufacturer)属性 （仅桌面版设备系列）                                                                                                                                                                                |
| **DeviceName** 属性                                                                 | [**EasClientDeviceInformation.SystemProductName** ](https://docs.microsoft.com/uwp/api/windows.security.exchangeactivesyncprovisioning.easclientdeviceinformation.systemproductname)属性 （仅桌面版设备系列）                                                                                                                                                                                 |
...
当声明位置功能在 Windows 10 上其应用程序包清单运行应用时，系统将提示最终用户同意的情况下进行。 因此，如果你的应用显示自己的自定义许可提示，或者如果它提供了一个开/关切换开关，则需要删除它以便仅提示最终用户一次。
