...
- 在清单中更新了 Project Centennial 的最低受支持桌面版本 (10.0.14316.0)。

## 系统要求

### 支持的操作系统
+ Windows 10 周年更新企业版预览（内部版本 10.0.14316.0 和更高版本）
...
1. 确保你具有最新的 Windows 10 Insider Preview 操作系统 - 企业版（内部版本 10.0.14316.0 和更高版本）。
2. 下载 DesktopAppConverter.zip 和 BaseImage-14316.wim。
...
5. 发布公共 SxS Fusion 程序集不起作用。 在安装期间，应用程序可以发布公共并行 Fusion 程序集，这些程序集可供任何其他进程访问。 在进程激活上下文创建期间，这些程序集将由名为 CSRSS.exe 的系统进程检索。 当 Centennial 进行执行此操作时，这些程序集的激活上下文创建和模块加载将失败。 内置程序集（如 ComCtl）随操作系统附带，因此从 Centennial 进程获取它们的依赖项很安全。 SxS Fusion 程序集在以下位置注册：
...
|参数|描述|
|---------|-----------|
|```-Setup [<SwitchParameter>]``` | 使用此标志在设置模式下运行 DesktopAppConverter。 设置模式支持扩展所提供的基本映像。|
...
|参数|描述|
|---------|-----------|
|```-ExpandedBaseImage <String>``` | 已扩展的基本映像的完整路径。|
