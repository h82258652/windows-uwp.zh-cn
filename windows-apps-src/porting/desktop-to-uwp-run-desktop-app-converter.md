...
title: "桌面到 UWP 桥：Desktop App Converter"
ms.author: normesta
ms.date: 03/09/2017
ms.topic: article
ms.prod: windows
...
translationtype: HT
---
# <a name="desktop-to-uwp-bridge-desktop-app-converter"></a>桌面到 UWP 桥：Desktop App Converter

[获取 Desktop App Converter](https://aka.ms/converter)

Desktop App Converter 是一款工具，允许你将为 .NET 4.6.1 或 Win32 编写的现有桌面应用发布到通用 Windows 平台 (UWP)。 你可以在无人参与（无提示）模式下通过转换器运行桌面安装程序，并获取可在开发计算机上使用 Add-AppxPackage PowerShell cmdlet 安装的 Windows 应用包。

Desktop App Converter 现已在 [Windows 应用商店](https://aka.ms/converter) 中提供。

转换器使用作为转换器下载的一部分提供的干净基本映像在隔离的 Windows 环境中运行桌面安装程序。 它捕获桌面安装程序进行的任何注册表和文件系统 I/O，并将其作为输出的一部分打包。 转换器输出一个带有程序包标识和调用各种 WinRT API 的功能的 Windows 应用包。

## <a name="whats-new"></a>新增功能

DAC 的最新版本是 v1.0.9.0。 本更新中的新增功能：

* 无需安装程序的转换：如果使用 xcopy 安装应用，或者你熟悉应用的安装程序对系统所做的更改，则可以通过将 -Installer 参数设置为应用文件的根目录来运行无需安装程序的转换。
* 应用包验证：使用新 `-Verify` 标志根据桌面桥和应用商店的要求来验证转换的应用包
...
*（这些步骤不是无需安装程序的转换所必需的步骤）*

Desktop App Converter 依靠最新的 Windows10 功能。 请确保你使用的是 Windows10 周年更新 (14393.0) 或更高版本。
...
以下示例介绍如何将 *MyPublisher* 发布的名为 *MyApp* 的桌面应用转换为 Windows 应用包。

#### <a name="no-installer-conversion"></a>无需安装程序的转换

对于无需安装程序的转换，`-Installer` 参数指向应用文件的根目录，并且 `-AppExecutable` 参数是必需的。
...
#### <a name="installer-based-conversion"></a>基于安装程序的转换

对于基于安装程序的转换，`-Installer` 指向应用的安装程序。
...
## <a name="deploy-your-converted-windows-app-package"></a>部署你转换后的 Windows 应用包
...
## <a name="sign-your-windows-app-package"></a>对 Windows 应用包签名

Add-AppxPackage cmdlet 要求必须对要部署的 Windows 应用包 (.appx) 进行签名。 使用作为转换器命令行的一部分的 ```-Sign``` 标志或 Microsoft Windows10 SDK 附带的 SignTool.exe 对 Windows 应用包进行签名。

有关如何对 Windows 应用包进行签名的其他详细信息，请参阅[对已转换的桌面应用进行签名](desktop-to-uwp-signing.md)。

注意：如果你尝试使用自动生成的证书对程序包进行签名，你将需要使用默认密码“123456”。

## <a name="modify-vfs-folder-and-registry-hive-optional"></a>修改 VFS 文件夹和注册表配置单元（可选）
...
1. 主机上的 Windows10 版本必须与作为 Desktop App Converter 下载的一部分获取的基本映像匹配。  
2. 确保桌面安装程序是独立的目录，因为转换器会将目录的所有内容复制到隔离的 Windows 环境中。  
3. 当前，Desktop App Converter 仅支持在 64 位操作系统上运行转换过程。 你仅可以将转换的 Windows 应用包部署到 64 位 (x64) 操作系统。  
...
* 如果主可执行文件或任何依赖项放置在“Program Files”或“Windows\System32”下，使用“AnyCPU”版本选项编译的 .NET 应用将无法安装。 作为解决方法，请使用特定于体系结构的桌面安装程序（32 位或 64 位）成功生成 Windows 应用包。
...
|```-Installer <String>``` | 应用程序的安装程序的路径 - 必须能够在无人参与/无提示的情况下运行。 无需安装程序的转换，这是应用文件的根目录的路径。 |
...
### <a name="windows-app-package-identity-parameters"></a>Windows 应用包 Identity 参数  

|参数|说明|
|---------|-----------|
|```-PackageName <String>``` | 通用 Windows 应用程序包的名称
|```-Publisher <String>``` | 通用 Windows 应用程序包的发布者
|```-Version <Version>``` | 通用 Windows 应用包的版本号

### <a name="optional-windows-app-package-manifest-parameters"></a>可选的 Windows 应用包清单参数  

|参数|说明|
|---------|-----------|
|```-AppExecutable <String> [optional]``` [可选] | 你的应用程序的主可执行文件的名称（“MyApp.exe”）。 无需安装程序的转换需要此参数。 |
|```-AppFileTypes <String>``` [可选] | 应用程序将与其关联的文件类型的逗号分隔列表（例如 “.txt, .doc”，不包含引号）。|
|```-AppId <String>``` [可选] | 在 Windows 应用包清单中指定要将应用程序 ID 设置为的值。 如果未指定，则将其设置为 *PackageName* 传入的值。|
|```-AppDisplayName <String>``` [可选] | 在 Windows 应用包清单中指定要将应用程序显示名称设置为的值。 如果未指定，则将其设置为 *PackageName* 传入的值。 |
|```-AppDescription <String>``` [可选] | 在 Windows 应用包清单中指定要将应用程序描述设置为的值。 如果未指定，则将其设置为 *PackageName* 传入的值。|
|```-PackageDisplayName <String>``` [可选] | 在 Windows 应用包清单中指定要将程序包显示名称设置为的值。 如果未指定，则将其设置为 *PackageName* 传入的值。 |
|```-PackagePublisherDisplayName <String>``` [可选] | 在 Windows 应用包清单中指定要将程序包发布者显示名称设置为的值。 如果未指定，则将其设置为 *Publisher* 传入的值。 |

### <a name="other-conversion-parameters"></a>其他转换参数  
...
|```-LogFile <String>``` [可选] | 指定日志文件。 如果省略，将创建一个日志文件临时位置。 |
| ```-Sign [<SwitchParameter>] [optional]``` | 告诉此脚本对输出 Windows 应用包进行签名。 此开关应该位于开关 ```-MakeAppx``` 旁边。
...
| ```-Verify [<SwitchParameter>] [optional]``` | 一个告知 DAC 根据桌面桥和 Windows 应用商店的要求验证已转换应用程序包的开关（当存在时）。 结果是一个“VerifyReport.xml”验证报告，该报告在浏览器中能够以最佳方式显示。 此开关应该位于开关 `-MakeAppx` 旁边。
...
在转换过程中，DesktopAppConverter 将自动运行 PEHeaderCertFixTool，以便修复任何损坏的 PE 标头。 但是，还可以在 UWP Windows 应用包、松散文件或指定二进制文件上运行 PEHeaderCertFixTool。 示例用法：
