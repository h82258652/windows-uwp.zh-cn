---
author: normesta
Description: "本文列出了在使用桌面桥将应用打包前你需要知道的事项。 你可能不需要执行很多操作即可使应用为打包过程做好准备。"
...
本文列出了在对桌面应用打包前你需要知道的事项。 你可能无需执行很多操作即可使应用准备好进行打包过程，但是如果以下任意一项适用于你的应用程序，则在打包前需要解决该问题。 请记住，Windows 应用商店会为你处理授权和自动更新，使你能够从基本代码中删除与这些任务相关的任何功能。

+ __你的应用使用早于 4.6.1 的 .NET 版本__。 仅 .NET 4.6.1 受支持。 在进行打包之前，需要将应用重定目标到 .NET 4.6.1。
...
+ __你的应用使用 Dev11 VCLibs 框架程序包__。 如果 VCLibs 11 库被定义为 Windows 应用包中的依赖项，则可以直接从 Windows 应用商店中安装。 若要执行此操作，请对应用包清单进行以下更改：在 `<Dependencies>` 节点下，添加：  
...
+ __应用将启动一个实用工具以执行任务__。 避免启动 PowerShell 和 Cmd.exe 等命令实用工具。 实际上，如果用户将你的应用安装在运行 Windows 10 S 的系统上，那么你的应用将完全无法启动它们。 这可能会使你的应用无法提交至 Windows 应用商店，因为提交到 Windows 应用商店的所有应用都必须与 Windows 10 S 兼容。
...
+ __应用将生成代码__。 你的应用可以生成其在内存中使用的代码，但请避免将生成的代码写入磁盘，因为 Windows 应用认证过程在应用提交之前无法验证该代码。 此外，向磁盘写入代码的应用将无法在运行 Windows 10 S 的系统中正常运行。这可能会使你的应用无法提交至 Windows 应用商店，因为提交到 Windows 应用商店的所有应用都必须与 Windows 10 S 兼容。

+ __应用使用 MAPI API__。 目前，在桌面桥应用中不支持 [Outlook MAPI API](https://msdn.microsoft.com/library/office/cc765775.aspx(d=robot))。
