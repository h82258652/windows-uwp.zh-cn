...
translationtype: HT
---
# <a name="desktop-to-uwp-bridge-sign"></a>桌面到 UWP 桥：签名

本文介绍如何对已转换为通用 Windows 平台 (UWP) 的桌面应用进行签名。 你必须先使用证书对 Windows 应用包进行签名，然后才能部署它。

## <a name="automatically-sign-using-the-desktop-app-converter-dac"></a>使用 Desktop App Converter (DAC) 自动签名

运行 DAC 自动对 Windows 应用包进行签名时，请使用 ```-Sign``` 标志。 有关更多详细信息，请参阅 [Desktop App Converter 预览](desktop-to-uwp-run-desktop-app-converter.md)。
...
```
最后，使用 SignTool.exe 通过证书对 Windows 应用包进行签名。
