...
如果应用程序中的可执行文件具有大写的 **.EXE** 扩展名，则可能会出现这种情况。 虽然此扩展名的大小写不应影响是否在你的应用程序运行，这可能导致 DAC 生成此错误。

若要解决此问题，请尝试在打包时指定 **-AppExecutable** 标志，并使用小写的“.exe”作为主要可执行文件的扩展名（例如 MYAPP.exe）。    或者你可以在应用程序小写字母从大写更改的所有可执行文件的大小写 (例如： 从。EXE.exe)。
