---
author: stevewhims
description: 通过在 Visual Studio 中创建一个新的 windows 10 项目并将文件复制到其中开始移植过程。
...
通过在 Visual Studio 中创建一个新的 windows 10 项目并将文件复制到其中开始移植过程。
...
-   对于在二进制级别，而不是源代码级别上重复使用，存在一些可移植类库，它适用于 windows 10 应用 (.NET Core) 支持在 WindowsPhone Silverlight 中可用的.NET api 子集。 可移植类库程序集为二进制形式，可与这些 .NET 平台及其他平台兼容。 使用 Visual Studio 创建一个面向可移植类库的项目。 请参阅[使用可移植类库的跨平台开发](http://msdn.microsoft.com/library/gg597391.aspx)。
