...
尽管此工具名称中出现了“Converter”一词，但它实际上并不转换你的应用。 你的应用程序保持不变。 但是，此工具将为你生成 Windows 应用包。 它可以是非常方便在情况下，其中应用程序执行大量的系统修改或有任何有关您的安装程序的作用的不确定性。

Desktop App Converter 将转换到的虚拟文件和注册表系统，将使用你的应用程序的打包的版本安装程序的操作。 Desktop App Converter 还可以为你执行一些额外的操作。 以下是其中一些例子。

:heavy_check_mark:自动注册预览控件、缩略图处理程序、属性处理程序、防火墙规则、URL 标志。

:heavy_check_mark:自动注册文件类型映射，来分组文件的用户通过使用**种类**文件资源管理器中的列。

:heavy_check_mark:注册公共的 COM 服务器。

:heavy_check_mark:生成可用于运行您的应用程序的证书。

:heavy_check_mark:验证针对打包桌面应用程序和 Microsoft Store 要求应用程序。

使用 Desktop App Converter 很好的另一个原因是如果您通过使用 Visual Studio 以外的其他开发环境维护你的应用程序。 可以使用 Desktop App Converter，即使你的应用程序没有安装程序。

请参阅[打包桌面应用程序使用 Desktop App Converter](desktop-to-uwp-run-desktop-app-converter.md)
...
请参阅[打包桌面应用程序集成，使用 Windows 10](desktop-to-uwp-extensions.md)。
...
一些 Windows 10 体验（例如：启用触摸功能的 UI 页面）必须在现代应用容器内运行。 一般情况下，首先应确定是否可以通过 UWP API [增强](desktop-to-uwp-enhance.md)现有桌面应用程序来增加你的体验。 如果您必须使用 UWP 组件，将获得的体验，你可以将 UWP 项目添加到你的解决方案和应用服务用于桌面应用程序和 UWP 组件之间进行通信。
...
中实际设置测试你的应用程序，如准备进行分发，它是应用程序进行签名，然后将其安装。 请参阅[测试你的应用](https://docs.microsoft.com/en-us/windows/uwp/porting/desktop-to-uwp-debug#test-your-app)。
...
如果你使用 Visual Studio，可以验证你的应用程序从**创建应用程序包**向导。 请参阅[创建应用包上传文件](../packaging/packaging-uwp-apps.md#create-an-app-package-upload-file)。
...
| [打包应用程序使用 Desktop App Converter](desktop-to-uwp-run-desktop-app-converter.md) | 介绍如何运行 Desktop App Converter。 |
| [手动打包桌面应用程序](desktop-to-uwp-manual-conversion.md) | 了解如何手动创建应用包和清单。 |
| [使用 Visual Studio 将桌面应用程序打包](desktop-to-uwp-packaging-dot-net.md)| 演示如何通过使用 Visual Studio 打包桌面应用程序。 |
| [将桌面应用程序与 Windows 10 集成](desktop-to-uwp-extensions.md) | 将你的应用程序与 Windows 10 集成使用通过描述打包项目的包清单文件中的任务。 |
| [增强适用于 Windows 10 桌面应用程序](desktop-to-uwp-enhance.md)| 使用 UWP API 来添加为 Windows 10 用户而设的现代化体验。 |
| [UWP Api 可用于打包桌面应用程序](desktop-to-uwp-supported-api.md) | 请参阅 UWP Api 都可用于打包桌面应用程序使用。 |
| [扩展桌面应用程序的与现代 UWP 组件](desktop-to-uwp-extend.md)| 添加必须在 UWP 应用容器内运行的高级体验。 通过使用应用服务与 UWP 进程连接桌面应用程序。|
| [运行、 调试和测试打包桌面应用程序](desktop-to-uwp-debug.md) | 说明用于调试已打包应用的选项。 |
| [分发打包桌面应用程序 ](desktop-to-uwp-distribute.md) | 请参阅如何将分发到用户转换的应用程序。  |
| [已知的 Issues(desktop-to-uwp-known-issues.md) | 有关打包桌面应用程序的已知问题的列表。 |
