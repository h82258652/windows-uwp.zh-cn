...
创建应用程序 （更准确地讲称为通用 Windows 平台 (UWP) 应用程序） 的 Windows 10 是相当类似于创建 iOS 应用程序使用演示图板。 Windows 10 应用通常构造对页面上，每个页面包含用户界面，例如网站的不同部分。 通常每个页面具有两个关联的源文件：一个源文件存储用户界面（以 [XAML 概述](https://docs.microsoft.com/windows/uwp/xaml-platform/xaml-overview)格式存储），另一个源文件包含源代码（通常为 C#）。 当用户与应用交互时，它们会在这些页面之间导航。 在本演练中，你将创建带有两个页面的应用。
