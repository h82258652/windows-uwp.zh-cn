...
基于 ARM 的 Windows 10 运行从 Microsoft Store 的所有 x86、 ARM32 和 ARM64 [UWP 应用](../get-started/universal-application-platform-guide.md)。 没有任何模拟，以本机方式运行的 ARM32 和 ARM64 应用时的 x86 应用运行在模拟环境。 如果你是 UWP 开发人员，请确保为你的应用提交 ARM 包，因为这将为设备提供最佳的用户体验。 有关详细信息，请参阅[应用包体系结构](../packaging/device-architecture.md)。
...
除了 UWP 应用中，基于 ARM 的 Windows 10 还可以运行你的 x86 Win32 应用 （如 Adobe Reader) 未被修改，提供良好的性能和无缝的用户体验，就像任何电脑。 这些 x86 Win32 应用无需为 ARM 重新编译，并且即使没有意识到它们运行在 ARM 处理器上。 请注意，不支持 64 位 x64 Win32 应用，但绝大多数应用都有 x86 版本，因此从用户角度看，只需选择 32 位 x86 安装程序，就能在基于 ARM 的 Windows 电脑上运行。
...
|[基于 ARM 的 ARM 应用疑难解答](apps-on-arm-troubleshooting-arm32.md)|ARM32 和 ARM64 应用在 ARM，以及如何解决这些问题上运行时的常见问题。 |
